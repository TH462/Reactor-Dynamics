/* comp_valve_horizontal.js — ball valve on a horizontal run, ported from
 * inbox/design_import/Valve Horizontal.dc.html.
 ...
 * viewBox '41 41 58 58' (ports at cx±R = 41 & 99 land on the tile edges).
...
(function () {
  ...
  RD.BoardComps['Valve Horizontal'] = { build: build };

  var CYAN = '#4fe3ff';
  var OPEN_ANG = 0; // bore orientation when OPEN. 0 = along the horizontal pipe.

  function ensureStyles() {
    if (document.getElementById('bd-valve-horizontal-styles')) return;
  ...
  function build(cfg, env) {
    ...
    // the ball + bore, rotating as a unit
    ...
    // connection ports (inline on a horizontal run) — read by the board to route pipes
    portEls.a = h('circle', { cx: cx - R, cy: cy, r: 0.75, fill: 'none', 'data-port': 'a', 'data-fluid': fluidKey, 'data-phase': '', 'data-temp': '', 'data-dir': 'left', 'data-size': vsize, 'data-out': flowDir < 0 ? '1' : '0', 'data-active': '1' });
    portEls.b = h('circle', { cx: cx + R, cy: cy, r: 0.75, fill: 'none', 'data-port': 'b', 'data-fluid': fluidKey, 'data-phase': '', 'data-temp': '', 'data-dir': 'right', 'data-size': vsize, 'data-out': flowDir < 0 ? '0' : '1', 'data-active': '1' });
    ...
    function rebuildConns() {
      ...
      [-1, 1].forEach(function (s) {
        var pg = K.pipe({ x1: (cx + s * Rc) * sc, y1: cy * sc, x2: (cx + s * R) * sc, y2: cy * sc, d: sizeD, fluid: { bore: '#000000', flow: '#ffffff' }, flow: true, dir: s * flowDir > 0 ? 1 : -1 });
        neckRefs.push({ bore: pg.childNodes[1], flow: pg.childNodes[2] });
        connG.appendChild(pg);
        connG.appendChild(K.flange({ x: (cx + s * R) * sc, y: cy * sc, angle: 0, d: sizeD }));
      });
      colorizeConns();
    }
