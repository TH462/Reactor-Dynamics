  ...
  var PWR = [
    {
      ...
      cautions: ['Rates are time-compressed for training — a real heatup takes many hours against brittle-fracture and pressurizer limits.', 'The heatup heat source is controlled fission: the lumped plant cannot practically heat up on pump heat alone. Keep the Startup Rate low and let the temperature defect regulate power on the ride. Measured, power sits around 6 % for most of the climb, then rises to roughly 24 % at the end — as Tavg reaches the no-load point the steam dump opens, the secondary becomes a real heat sink, Tavg stops rising and the temperature defect stops pushing back. That rise is the ride ending, not a fault; it is also why the power-range trips are blocked at steps 8–9. Secure the dilution when Tavg reaches the hot band, as step 15 says — left running, it keeps adding reactivity the plant can no longer absorb as temperature, and power climbs toward the 50 % P-9 permissive, which would trip the reactor with the turbine offline.', 'Boron moves through a mixing lag (~30 s) — dilution keeps adding reactivity briefly after you stop it. Stop early, never chase.', 'There is no live boron meter: concentration is known from CHEM SAMPLE grab results (~60 s compressed lab turnaround) and the change you have ordered. Sample to confirm a shutdown-margin boration; do not steer the ride off ppm you cannot yet see.', 'Mind the Steam Generator, and engage Feed AUTO at the start (step 3) rather than setting a feed-pump demand by hand. A standing manual demand keeps filling a generator that is not yet boiling: measured, ~30 % pump for this heatup floods the SG past 90 %, isolates main feedwater and stands a SG LVL HI HI alarm for the rest of the run. Expect level to swell above its setpoint anyway as the primary heats — warming water expands and no controller can pump water out — and to come back down once the nuclear heatup starts drawing steam.',
        'This heatup deliberately runs the reactor at ~10–30 % power as its heat source, which is straight through the startup protection net: INTERMEDIATE RANGE HIGH at ~20 % and the POWER RANGE LOW SETPOINT at 25 %. Both are blocked as explicit steps before the ascent (steps 8–9). Miss them and the ride is scrammed at about 20 % with the plant barely above 100 °C.'],
      steps: [
        ...
        { text: 'Take the generator off line before you make any steam: Disconnect Grid. A cold plant has no business following load — and the moment the heatup starts raising power, a turbine left in FOLLOW opens its governor and takes the steam you are trying to heat the plant with.',
          control: 'Turbine Load', target: 'generator disconnected, governor shut',
          note: 'Measured: leave it in FOLLOW and the governor passes ~46 % steam flow by the end of the ride — the heatup stalls at 240 °C because the turbine is carrying the heat away as fast as the core makes it. The steam dump, set to the no-load anchor two steps down, is what should be holding the secondary. You put the turbine back on line at the END of the startup checklist, not here.',
          cmd: { action: 'disconnect_grid' }, hold: 10,
          hl: ['Turbine Load', 'Main Breaker'] },
        { text: 'Put steam-generator level control in AUTO now, while level is still at its cold 65 % (Automate → "Feed pump → SG level (three-element)"). The channel captures the level it finds as its setpoint, so engaging it here — before anything moves — is what sets it to the right number.',
    ...
    },
    {
      ...
      steps: [
        ...
        { text: 'Set up the heat sink before you make any heat: put steam-generator level control in AUTO (Automate → "Feed pump → SG level (three-element)"). Level is at its nominal 65 % now, and the channel captures that as its setpoint — so engage it HERE, while the number is right.',
          control: 'Feed Pumps', target: 'Feed AUTO engaged, SG level ≈ 65 %',
        ...
        { text: 'First burst: withdraw the Control Bank at Norm, release to stop, let the count rate settle — then press "Plot point" again. Two points draw a line. Read where it crosses zero: it will predict criticality far PAST where the reactor actually goes critical, and that is normal. Down here the rods are in the flat toe of the worth curve, so the early trend is too shallow. Do not trust this number yet.',
          ...
          note: 'On the CONTROL GROUP card: set Rod Speed (S/M/F), then hold WITHDRAW; release to stop motion. Two points predict ~step 409 against a true ~224 — the error is 185 steps, and always on the far side.',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 120, speed: 'normal' }, hold: 60,
          acc: { p: 'sr_counts_cps', op: '>', v: 620 },
        ...
        { text: 'Second burst — smaller, because every point from here is worth more than the last. Withdraw, settle, plot. Three points now, and the prediction has pulled in a long way, but it is still tens of steps beyond the truth. This is the step most people stop at, and it is not close enough to withdraw against.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 50, speed: 'normal' }, hold: 60,
          acc: { p: 'sr_counts_cps', op: '>', v: 1000 },
          hl: ['Control Bank', '1/M Plot Tool', 'Source Range'] },
        { text: 'Third burst: withdraw, settle, plot. Point 4 — the fit is now working on points that sit on the steep part of the curve, and the prediction drops inside about twenty steps of actual. Keep the bursts shrinking as the estimate tightens.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 30, speed: 'normal' }, hold: 60,
          acc: { p: 'sr_counts_cps', op: '>', v: 1800 },
        ...
        { text: 'Fourth burst — small now. Withdraw, settle, plot. Point 5 lands the prediction within roughly ten steps, and the count rate is climbing visibly between plots. You are close.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 15, speed: 'normal' }, hold: 60,
          acc: { p: 'sr_counts_cps', op: '>', v: 3300 },
          hl: ['Control Bank', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Last plotted burst — a nudge. Withdraw, settle, plot. Point 6 is your working number: the prediction is now within a handful of steps and still reads slightly HIGH, which is the safe side. Stop here and take the rest by creeping — never withdraw straight to the predicted position.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 8, speed: 'normal' }, hold: 60,
          acc: { p: 'sr_counts_cps', op: '>', v: 6200 },
          hl: ['Control Bank', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        ...
        { text: 'Creep up on criticality: withdraw at Slow in single steps. The reactor goes critical and power begins to climb — watch the Startup Rate (SUR) and keep the reactor period long.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 11, speed: 'slow' }, hold: 600,
