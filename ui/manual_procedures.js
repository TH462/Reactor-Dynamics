...
;(function (RD) {
  ...
  var PWR = [
    {
      ...
      steps: [
        ...
        { text: 'On the Primary view: Rod Speed → Slow, then hold Control Bank → Withdraw in small bursts toward criticality. Power begins to climb once you pass critical; keep the Startup Rate (SUR) low and the reactor period long.',
          control: 'Control Bank', target: 'SUR ≤ 1 DPM, reactor period ≥ 30 s',
          note: 'Set Rod Speed to Slow first; release Withdraw to stop motion. Watch SUR on Tools → Training (Reactivity Computer) or Primary → Reactor Core. From fully inserted rods, slow withdrawal takes several minutes. If the period drops below 30 s, stop or insert — the reactor is accelerating.',
  ...
  var RBMK = [
    {
      ...
      steps: [
        ...
        { text: 'Primary view: Rod Speed → Slow, then hold Control Bank → Withdraw toward criticality; watch the Startup Rate (SUR) and the Operating Reactivity Margin (ORM).',
          control: 'Control Bank', target: 'SUR low, ORM above minimum',
          note: 'Set Rod Speed to Slow first.',
  ...
  var BWR = [
    ...
    },
    {
      ...
      steps: [
        { text: 'Increase the Recirculation (recirc) flow setpoint. Primary view → Recirc Drive → Set %. Fewer voids → positive reactivity → power rises and self-limits.',
          control: 'Recirc Drive', target: 'power rises to a new balance',
