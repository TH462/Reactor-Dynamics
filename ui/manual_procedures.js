  ...
  var PWR2 = [
    {
      ...
      steps: [
        ...
        { text: 'Withdraw the SHUTDOWN BANK to fully out (627 / 627). Drive it in manual bank control at Fast — full travel takes about 10 plant-minutes. It stays out for every mode above this one.',
          ...
          control: 'Shutdown Bank', target: 'bank fully withdrawn, 627 / 627 — ρ ≈ −2,130 pcm',
          cmd: { action: 'rod_nudge', group_id: 'shutdown_rods', steps: 627, speed: 'fast' }, hold: 660,
          /* CHECKED OFF ON THE BANK, NOT ON REACTIVITY *(OWNER, 2026-09-02 playtest, M5->3 item 1:
           * "Step 3 should be based on rod position not reactivity")*. The step's instruction is
           * "withdraw the bank to 627 / 627", so the bank is what the check should read — and
           * reactivity is the wrong proxy for it in both directions: it also answers to boron and
           * to moderator temperature, both of which move on their own during this heatup, so the
           * -3000 pcm threshold could be satisfied with the bank part-way out and could be missed
           * with it fully out. The pcm figures stay in the target and the `why` — they are the
           * teaching, they are just not the evidence. */
          acc: { p: 'shutdown_bank_pct', op: '>=', v: 100 },
