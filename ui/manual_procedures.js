...
;(function (RD) {
   ...
   *   · the control bank is 0..200 steps (differential ~31 pcm/step near critical, ~4.6x
   *     the retired fine-step bank), so the whole 1/M ladder re-derives;
  ...
  var PWR2 = [
    {
      ...
      steps: [
        obs('Confirm the cold plant: Mode 5, Tavg ≈ 122 °F (50 °C), ~363 psi (2.5 MPa), RHR in service, reactor coolant pumps secured, both rod banks fully inserted.',
        ...
        { text: 'Start the Reactor Coolant Pumps (RCP → Run). Forced flow is the heat source and couples the steam generator.',
          ...
          acc: { p: 'pump_flow_pct', op: '>', v: 90 },
          hl: ['Reactor Coolant Pumps (RCP)', 'RCP Run/Stop'] },
        { text: 'Withdraw the SHUTDOWN BANK to fully out (200 / 200). Drive it in manual bank control at Fast — full travel takes about 4 plant-minutes. It stays out for every mode above this one.',
          why: 'Mode 5 holds both banks on the bottom: measured on this plant, ρ = −5,809 pcm on 918 ppm with both banks in, and the shutdown bank alone is worth 3,676 pcm of that. Withdrawing it is not a step toward criticality — it is the prerequisite for one (real practice withdraws the shutdown banks prior to criticality, WTSM 8.1.1), and what you are spending is the TIME the margin was buying you against an unattended dilution. Withdraw it deliberately and do not leave a dilution running.',
          control: 'Shutdown Bank', target: 'bank fully withdrawn, 200 / 200 — ρ ≈ −2,130 pcm',
          cmd: { action: 'rod_nudge', group_id: 'shutdown_rods', steps: 200, speed: 'fast' }, hold: 300,
          acc: { p: 'reactivity_pcm', op: '>', v: -3000 },
          hl: ['Shutdown Bank — Withdraw', 'Reactivity'] },
        { text: 'Confirm the generator is off line (Disconnect Grid if it is not). Leave the turbine off until the end of the startup path.',
          ...
          cmd: { action: 'disconnect_grid' }, hold: 10,
          hl: ['Turbine Load', 'Main Breaker'] },
        { text: 'Put steam-generator level control in AUTO now, while level sits at its normal band (STEAM GEN FEED → AUTO). The three-element controller holds level from here.',
          why: 'On pump heat the generator barely boils, so AUTO simply holds. What it prevents is a standing manual feed demand quietly filling a generator nobody is boiling.',
          control: 'Feed Pumps', target: 'Feed AUTO engaged',
        ...
        { text: 'Set the Steam Dump Setpoint to the no-load anchor — 1020 psi (7.03 MPa) — so the secondary bottles up with the heatup instead of dumping it. Leave the dump shut.',
          ...
          cmd: { action: 'set_steam_dump_setpoint', mpa: 7.03 }, hold: 5,
          hl: ['Dump SP', 'Steam Dump'] },
        { text: 'Start the pressurization — dial the Pressurizer Pressure Setpoint DOWN to its 1700 psig floor (the bottom of the dial). Not 2235 psi yet: the first stage stops below the 1972 psia P-11 permissive, and the last stretch waits for a hot secondary (the step after the ride). The accumulator window in the next step opens ~33 plant-minutes from this command. Watch RHR auto-isolate on the way past its 585 psig autoclosure.',
          why: 'Pressure is the subcooling guarantee — but completing it from cold is the trap on this plant. The cold lineup ships the safety-injection actuation signals BLOCKED (taken on the last cooldown, permitted only below the P-11 permissive at 1972 psia), and climbing past P-11 auto-reinstates them. Dial 2235 now and the primary crosses P-11 while the steam generator is still cold — below the 327.7 psia low-steam-pressure SI setpoint — so the reinstate exposes a STANDING safety-injection signal on a healthy plant: SI actuates, the heaters shed to the emergency-bus lineup, and the pressurization dies ~300 psi short (measured full-stack 2026-08-31: SI at the P-11 crossing, cause low steam pressure, heaters 157.8 kW → 0). Staging the setpoint below P-11 until the secondary is hot is exactly the coordination a real heatup curve enforces. Measured pace at full heaters: 600 psig at ~33 min, 1000 psig at ~72 min from this command.',
        ...
        { text: 'Re-align the Safety Injection accumulators NOW, inside the window: pressure is past their 600 psi cover gas and must still be below the 1600 psig valve-power lock. Measured on this plant the window is ~33 to ~104 plant-minutes from the Pressure SP command — and LCO 3.5.1 wants them OPERABLE by 1000 psig (~72 min). Nothing opens them for you.',
          why: 'The accumulators are the passive half of emergency injection — big tanks of borated water behind a check valve, pushed by nitrogen. Cold lineups isolate them (they would dump into a depressurized plant); at power they must be armed. Above 1600 psig the plant removes power from the valve operator (an administrative lock, TS Bases B 3.5.1), so miss the window and you cannot arm them until the next cooldown.',
          control: 'Accumulator valve', target: 'accumulators armed',
          cmd: { action: 'open_accumulator_valve' }, hold: 10,
          acc: { p: 'accumulator_valve_open', op: '>', v: 0 },
          hl: ['Accumulator valve', 'ECCS'] },
        { text: 'Ride the heatup to Hot Standby with pressure holding at the 1700 psig stage. Monitor Tavg and its rate (trim the RHR heat exchanger if the rate crowds 100 °F/hr), secondary pressure bottling up toward the 1020 psi anchor, pressurizer level swelling on expansion — and the reactor staying exactly where you left it. Do not pull rods. Do not dilute. Do not complete the pressurization yet.',
          why: 'The staged setpoint holds the primary at ~1715 psia — 60 °F of subcooling even at the hot end, and safely below the 1972 psia P-11 permissive — while the secondary does the thing the final pressurization is waiting for: bottling up past the 327.7 psia low-steam-pressure SI setpoint on its way to the 1020 psi no-load anchor.',
          control: '(observe)', target: 'Tavg ≥ 541.4 °F (283 °C), still subcritical',
          wait_hint: true,
          hold: 40000,
          saw: { p: 'tavg_c', op: '>', v: 150 },
        ...
        obs('Confirm Mode 3, Hot Standby: hot at the no-load band, pressurized, SUBCRITICAL with the control bank never moved. Ready for the approach to criticality (the startup checklist).',
          { p: 'plant_mode', op: '~', v: 3, tol: 0.1 }),
        obs('Confirm the reactor stayed shut down: reactivity deeply negative (measured arrival ρ ≈ −3,400 pcm on this plant), power still source-range.',
          { p: 'reactivity_pcm', op: '<', v: -300 }),
    ...
    },
    {
      ...
      cautions: [
        'Withdraw in small bursts, letting the count rate settle between them — target Startup Rate ≤ 1 decade per minute. ONE step of this bank is worth ~31 pcm near the critical band (measured — a much coarser bank than fine-step plants), so the final approach is single steps at Slow.',
        'Plot ENOUGH 1/M points. The prediction always reads high early and walks down as you add points — never withdraw straight to the predicted position; creep up on it. Measured on this plant: criticality at ~74–75 of 200 steps from the 719 ppm start.',
      ...
      steps: [
        ...
        { text: 'First burst: withdraw the Control Bank to 30 steps at Norm, let the count rate settle, then press "Plot point". Two points draw a line — it will predict criticality far PAST where the reactor actually goes critical, and that is normal.',
          control: 'Control Bank', target: '30 / 200 — point 2 (over-estimate)',
          note: 'Let the Startup Rate settle back toward zero before you plot — a point taken mid-rise reads low.',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 30, speed: 'normal' }, hold: 90,
          accs: [{ p: 'sr_counts_cps', op: '>', v: 560, label: 'Counts settled above 560 cps' },
        ...
        { text: 'Second burst — smaller: withdraw to 50 steps, settle, plot. The prediction pulls in a long way but is still tens of steps beyond the truth.',
          control: 'Control Bank', target: '50 / 200 — point 3',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 20, speed: 'normal' }, hold: 90,
          accs: [{ p: 'sr_counts_cps', op: '>', v: 820, label: 'Counts settled above 820 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Third burst: withdraw to 60 steps, settle, plot. The fit is now on the steep part of the worth curve and the prediction lands within a dozen steps of actual.',
          control: 'Control Bank', target: '60 / 200 — point 4',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 10, speed: 'normal' }, hold: 90,
          accs: [{ p: 'sr_counts_cps', op: '>', v: 1250, label: 'Counts settled above 1,250 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Fourth burst — small now: withdraw to 66 steps, settle, plot. Keep each burst small enough that the Startup Rate stays under 1 DPM.',
          control: 'Control Bank', target: '66 / 200 — point 5',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 6, speed: 'normal' }, hold: 90,
          accs: [{ p: 'sr_counts_cps', op: '>', v: 2000, label: 'Counts settled above 2,000 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Last plotted burst — a nudge to 70 steps, settle, plot. Point 6 is your working number: within a handful of steps, still reading slightly HIGH, which is the safe side. Take the rest by creeping.',
          control: 'Control Bank', target: '70 / 200 — the working prediction (true critical ≈ 74–75)',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 4, speed: 'normal' }, hold: 90,
          accs: [{ p: 'sr_counts_cps', op: '>', v: 3300, label: 'Counts settled above 3,300 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Creep up on criticality: withdraw at SLOW to 75 steps. The slow drive paces the insertion — measured, the Startup Rate peaks ~0.7 DPM and the reactor arrives just past critical (ρ ≈ +20 pcm).',
          why: 'One step of this bank is ~31 pcm — a third of a dollar. The Slow speed is what makes a multi-step drive safe here: the drive rate spreads the insertion so the rate meter can answer while you still have your finger on the button.',
          control: 'Control Bank', target: '75 / 200 — critical, SUR ≤ 1 DPM',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 5, speed: 'slow' }, hold: 120,
          acc: { p: 'sr_counts_cps', op: '>', v: 9000 },
          hl: ['Withdraw', 'Rod Speed — Slow', 'Startup Rate', 'Source Range'] },
        { text: 'Establish the rise: two more steps at Slow (to 77) puts ~80 pcm of excess in, and power climbs through the decades at ~0.4 DPM. Watch the Startup Rate and the Intermediate Range; the Source Range hands off and de-energizes by itself on the way up.',
          control: 'Control Bank', target: 'power rising to the point of adding heat',
          wait_hint: 'The rise through the decades takes ~13 plant-minutes at this rate — Fast time is fine; watch the SUR.',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 2, speed: 'slow' }, hold: 780,
          saw: { p: 'startup_rate_dpm', op: '>', v: 0 },
          acc: { p: 'power_pct', op: '>', v: 0.5 },
          hl: ['Withdraw', 'Startup Rate', 'Intermediate Range'] },
        obs('Confirm the SR→IR handoff happened: the Source Range de-energized itself as the Intermediate Range took over (no operator action on this plant — the channel auto-secures at the P-6 class point).',
          { p: 'sr_energized', op: '<', v: 1 },
          null, ['Source Range', 'Intermediate Range']),
        { text: 'Level off at the point of adding heat: ONE decisive inward drive of 2 steps at Norm, released as the Startup Rate crosses zero. Power settles near 1 %.',
          ...
          cmd: { action: 'rod_nudge', group_id: 'control', steps: -2, speed: 'normal' }, hold: 150,
          acc: { p: 'power_pct', op: '<', v: 5 },
          hl: ['Insert', 'Startup Rate', 'Intermediate Range'] },
        { text: 'Cross the 5 % boundary deliberately: withdraw 4 steps at Slow. Power rises into the high single digits — entering Mode 1, At Power is a decision you make, not something the ascent does to you.',
          control: 'Control Bank', target: '≈ 8 %, Mode 1, At Power',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 4, speed: 'slow' }, hold: 300,
          acc: { p: 'power_pct', op: '>', v: 5 },
          hl: ['Withdraw', 'Startup Rate', 'Intermediate Range'] },
        { text: 'Put the turbine on line — two real actions on this plant: LATCH the turbine (un-trip it), then set a load target of 10 MWe. The generator picks up the load and the reactor follows it up to ~10 %.',
          ...
          hl: ['Turbine Load', 'Main Breaker'] },
        { text: 'Block the 25 % INTERMEDIATE RANGE high flux trip — now that you are above P-10 (10 % power), the permissive lets you. On the TRIP BLOCKS panel, press IR HIGH FLUX. This is the first of two blocks and it comes first, because its setpoint is the one you would reach next.',
    ...
    },
    {
      ...
      from: '50_percent',
      prereq: ['Reactor critical and stable at power, above P-10.', 'Turbine on line.', 'BOTH startup-net blocks taken above P-10 — the intermediate range and the 35 % low-flux setting (the startup checklist\'s last two acts).'],
      precond: [
        { p: 'power_pct', op: '>', v: 10, text: 'Reactor critical and at power (above P-10)' },
        { p: 'mwe_output', op: '>', v: 5, text: 'Turbine on line, carrying load' },
      ],
      cautions: [
    ...
    },
    {
      ...
      steps: [
        ...
        { text: 'ALIGN RHR while the spray still holds pressure low: open the hot-leg suction (RHR card → ALIGN) and set the heat-exchanger split to 7 %. The permissive refuses the align above 425 psig (sourced, WTSM 5.1) — and shutting the spray first lets pressure bounce back over it, which is why the spray stays on until this is done.',
