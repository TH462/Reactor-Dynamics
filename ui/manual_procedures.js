  ...
  var PWR2 = [
    {
      ...
      title: 'Mode 5, Cold Shutdown → Mode 3, Hot Standby — plant heatup (pump heat)',
      purpose: 'Take the plant from Mode 5, Cold Shutdown to Mode 3, Hot Standby on reactor-coolant-pump heat alone: start the RCPs, withdraw the shutdown bank, pressurize to normal operating pressure, re-align the SI accumulators inside their window, and ride temperature up with the reactor never critical.',
      from: 'cold_shutdown',
      prereq: ['Plant in Mode 5, Cold Shutdown: cold (~122 °F / 50 °C), depressurized (~363 psi / 2.5 MPa), subcritical, RHR in service.', 'RCPs available to start (heat source).'],
      precond: [
        { p: 'tavg_c', op: '<', v: 95, text: 'Plant cold — Mode 5 (Tavg ≈ 122 °F / 50 °C)' },
      ...
      cautions: [
        'Measured on this plant (2026-08-31, full stack): the pumps alone warm the Mode 5 plant at ~87 °F/hr with the pressurization running alongside, and the pump-heat class runs up to 113.7 °F/hr at the rated rotor — ABOVE the 100 °F/hr administrative limit. Rate compliance is yours: trim the RHR heat exchanger to bleed the excess, and secure an RCP to stop the climb entirely.',
        'Keep the steam generator bottled (turbine off, dumps shut at the no-load anchor). On pump heat the dump is a coarse lever: a small manual demand removes heat faster than the pumps put it in.',
        'Do NOT pull the control bank and do not dilute — Hot Standby is hot AND subcritical. The shutdown bank is the exception and has its own step.',
      ],
      steps: [
        obs('Confirm the cold plant: Mode 5, Tavg ≈ 122 °F (50 °C), ~363 psi (2.5 MPa), RHR in service, reactor coolant pumps secured, turbine tripped, main feed pumps secured, both rod banks fully inserted.',
          { p: 'plant_mode', op: '~', v: 5, tol: 0.1 },
          null, ['Tavg', 'Plant Pressure', 'Residual Heat Removal (RHR)'],
          'This is a picture of the starting plant, not an action. Cold Shutdown means the coolant is well below boiling, pressure is down, the residual-heat-removal loop is the heat sink, and both rod banks are fully in. If you already started the pumps or pulled the shutdown bank, this picture is behind you — the checklist will skip it.',
          [{ p: 'pump_flow_pct', op: '>', v: 90 }, { p: 'shutdown_bank_pct', op: '>=', v: 98 }, { p: 'plant_mode', op: '<', v: 5 }]),
        { text: 'Start the Reactor Coolant Pumps (RCP → Run). Forced flow is the heat source and couples the steam generator.',
          why: 'A shut-down reactor makes almost no heat, but the pumps themselves dump about half a percent of rated power into the water as friction. That is enough to warm the whole plant — real crews heat up exactly this way, with the reactor never critical.',
          control: 'RCP Run/Stop', target: 'pump flow ~100 %',
          cmd: { action: 'set_rcp', running: true }, hold: 30,
          acc: { p: 'pump_flow_pct', op: '>', v: 90 },
          hl: ['RCP Run/Stop'] },
        { text: 'Withdraw the SHUTDOWN BANK to fully out (627 / 627). Drive it in manual bank control at Fast — full travel takes about 10 plant-minutes. It stays out for every mode above this one.',
          why: 'The shutdown bank is the plant\'s trip margin — the rods that drop on a scram to hold the core subcritical. Mode 5 keeps both banks on the bottom; every mode above this one wants the shutdown bank parked fully out, because a trip only works if those rods have somewhere to fall. Withdrawing it is not a step toward criticality. It is the prerequisite for one (real practice withdraws the shutdown banks prior to criticality, WTSM 8.1.1), and what you are spending is the time that extra margin was buying you against an unattended dilution. Withdraw it deliberately and do not leave a dilution running. The check is the bank position on the card, 627 / 627 — not a reactivity number, which also answers to boron and temperature.',
          control: 'Shutdown Bank', target: 'bank fully withdrawn, 627 / 627',
        ...
        { text: 'Confirm the turbine is TRIPPED and the generator off line, and leave it that way until the end of the startup path. Press UNLOAD on the TURBINE-GENERATOR card to walk the load target to zero if anything has been dialled into it.',
          why: 'The cold lineup ships the machine tripped — stop valves shut, nothing admitted — which is where a Mode 5 plant leaves it, and this step is a confirmation rather than an action. It is worth making anyway: a cold plant has no business carrying load, and a governor cracked open on pump heat drains the very heatup you are trying to make. Note the difference you will need later: UNLOAD is not TRIP. Unloading walks the load target down and leaves the machine LATCHED; re-latching a tripped turbine is its own step, on the startup path.',
          control: 'Turbine Load', target: 'turbine tripped, generator disconnected, 0 MWe',
          cmd: { action: 'disconnect_grid' }, hold: 10,
          acc: { p: 'turbine_tripped', op: '>', v: 0 },
          hl: ['Turbine Load', 'Main Breaker'] },
        { text: 'Start the main feed pumps and put steam-generator level control in AUTO, while level still sits in its normal band (STEAM GEN FEED → AUTO). Selecting AUTO starts the pumps; the three-element controller holds level from here.',
          why: 'The cold lineup has the main feed pumps SECURED — nothing is boiling, so nothing needs feeding, and a standing feed demand on a shut-down generator is a way to fill it by accident. Placing level control in service before the heatup starts is the ordinary order: you want the controller holding the band while the plant is quiet, not chasing it once the generator begins to boil. On pump heat AUTO will barely do anything, which is the point — it is there for the swell that comes later.',
        ...
        { text: 'Set the Steam Dump Setpoint to the no-load anchor — 1020 psi (7.03 MPa) — so the secondary bottles up with the heatup instead of dumping it. Leave the dump shut.',
          why: 'The dump is the plant\'s steam-to-condenser relief. On pump heat you want the steam generator bottled: pressure rises with temperature instead of venting the heat you are trying to make. 1020 psi is this plant\'s no-load anchor — the pressure a hot, unloaded generator sits at. Leave the valve shut; a small manual demand removes heat faster than the pumps put it in.',
          control: 'Dump SP', target: '1020 psi (7.03 MPa)',
          cmd: { action: 'set_steam_dump_setpoint', mpa: 7.03 }, hold: 5,
          hl: ['Dump SP', 'Steam Dump'] },
        ...
        { text: 'Re-align the Safety Injection accumulators NOW, inside the window: pressure is past their 600 psi cover gas and must still be below the 1600 psig valve-power lock. Nothing opens them for you.',
          ...
          acc: { p: 'accumulator_valve_open', op: '>', v: 0 },
          hl: ['Accumulator valve'] },
        { text: 'Ride the heatup to Hot Standby, pressure holding at the 1700 psig stage. Watch Tavg and its RATE — trim the RHR heat exchanger if it crowds 100 °F/hr. Do not pull rods. Do not dilute. Do not complete the pressurization yet.',
          why: 'WHAT ELSE TO WATCH ON THE RIDE: secondary pressure bottling up toward the 1020 psi anchor, pressurizer level swelling as the water expands, and the reactor staying exactly where you left it.\n\nThe staged setpoint holds the primary at ~1715 psia — 60 °F of subcooling even at the hot end, and safely below the 1972 psia P-11 permissive — while the secondary does the thing the final pressurization is waiting for: bottling up past the 327.7 psia low-steam-pressure SI setpoint on its way to the 1020 psi no-load anchor.',
          control: '(observe)', target: 'Tavg ≥ 541.4 °F (283 °C), still subcritical',
          wait_hint: true,
          hold: 40000,
          saw: { p: 'tavg_c', op: '>', v: 150 },
        ...
        { text: 'Complete the pressurization: raise the Pressurizer Pressure Setpoint to 2235 psi (15.41 MPa). The secondary is hot and bottled at the 1020 psi anchor, so the P-11 crossing ahead reinstates the SI signals onto a CLEAR plant. Full heaters take about an hour over the last 520 psi.',
          why: 'This is the other half of the staged pressurization: climbing past the 1972 psia P-11 permissive auto-reinstates the SI actuation signals the cold lineup had blocked — and every one of them now reads clear (steam pressure 1020 psia against the 327.7 psia setpoint, primary already above the 1715 psia low-pressure setpoint). The same crossing made with a cold secondary actuates safety injection on a healthy plant and sheds the heaters — which is why this dial waited for the ride to finish.',
          ...
          acc: { p: 'pressure_mpa', op: '>', v: 15.0 },
          hl: ['Pressure SP', 'Primary Pressure'] },
        obs('Confirm Mode 3, Hot Standby: hot at the no-load band, pressurized, SUBCRITICAL with the control bank never moved. Ready for the approach to criticality (the startup checklist).',
          { p: 'plant_mode', op: '~', v: 3, tol: 0.1 }, null, ['Tavg', 'Primary Pressure'],
          'Hot Standby is hot and pressurized with the reactor still shut down. The control bank should still be on the bottom — this heatup never needed it. The next checklist is the approach to criticality, and it starts with a dilution: you are still at cold-shutdown boron.'),
        obs('Confirm the reactor stayed shut down: the core is still deeply subcritical, power still source-range.',
          { p: 'reactivity_pcm', op: '<', v: -300 }, null, ['Source Range'],
          'The shutdown bank is out and the control bank never moved, so the core is held subcritical on boron and the remaining control-bank worth. Source-range counts should be a steady background, not climbing.'),
        obs('Confirm no fission heat was ever made — this heatup was pump work, start to finish.',
      ...
      outcome: 'Plant at Mode 3, Hot Standby: hot, pressurized, subcritical with zero control-bank motion. NOT yet ready to pull rods: the heatup dilutes nothing, so you are still at cold-shutdown boron (~918 ppm) — the startup checklist begins with the dilution to the estimated critical concentration.',
    },
    {
      id: 'pwr_startup', category: 'startup', manual_ref: 'PWR-T03', next: 'pwr_raise_power',
      title: 'Mode 3, Hot Standby → Mode 1, At Power — startup to power',
      purpose: 'Take the reactor from Mode 3, Hot Standby through criticality (Mode 2, Startup), across the 5 % boundary into Mode 1, At Power, and put the turbine on line. You will dilute to the estimated critical boron, use the 1/M (inverse-count) plot to predict criticality, and watch the Startup Rate on the way up.',
      from: 'hot_zero_power',
      prereq: ['Plant at Mode 3, Hot Standby: subcritical, hot, at operating temperature/pressure.', 'Reactor Coolant Pumps running — forced flow established.', 'Control bank inserted; shutdown bank parked withdrawn.'],
      ...
      cautions: [
        'Withdraw in small bursts, letting the count rate settle between them — target Startup Rate ≤ 1 decade per minute. ONE step of this bank is worth about 8 pcm near the critical band (measured; the bank spans 627 steps, the sourced four-bank overlap scale), so the approach is a matter of patience rather than precision — but the steps add up, and the rate is what tells you.',
        'Plot ENOUGH 1/M points. The prediction always reads high early and walks down as you add points — never withdraw straight to the predicted position; creep up on it. Measured on this plant: criticality near 226–238 of 627 steps from the 719 ppm start, depending on how patiently you approach it.',
        'The Source Range needs no securing on this plant — the channel de-energizes itself as the Intermediate Range takes over during the rise (there is no operator lever, #529 class).',
        'Mind the Steam Generator: from the point of adding heat the SG boils down with the turbine still offline. Feed AUTO (step 3) is what holds level.',
      ],
      steps: [
        obs('Confirm the plant is ready: subcritical with a steady Source Range count rate, hot (Tavg ≈ 546.8 °F / 286 °C), pressurized, pumps running.',
          { p: 'tavg_c', op: '~', v: 286, tol: 8 },
          null, ['Source Range', 'Tavg', 'Primary Pressure', 'Reactor Coolant Pumps (RCP)'],
          'This is the plant the heatup hands you: hot, at pressure, pumps running, still shut down. A steady source-range count means nothing is drifting toward critical yet. If you arrived from the Hot Standby preset instead of the heatup, the shutdown bank should already be out and boron should already be near 719 ppm.',
          { p: 'power_pct', op: '>', v: 1 }),
        { text: 'Dilute to the ESTIMATED CRITICAL CONCENTRATION — 719 ppm on this lineup. Engage the boron controller (BORON card → AUTO) and set the target to 719. A plant fresh from the pump-heat heatup sits near 918 ppm; the make-up panel meters the dilution at about 3 ppm/min.',
          why: 'Work out where criticality should be BEFORE you move a rod. At 918 ppm the bank cannot make the core critical inside its travel; at 719 ppm criticality sits near 230 of 627 steps — about 37 % withdrawn, comfortably inside the insertion limit. Diluting first is the difference between a predicted criticality and a surprise one. (Starting from the Hot Standby preset you are already at 719 and this step simply verifies.)',
          control: 'Boron control', target: '719 ppm',
          wait_hint: 'From the heatup\'s 918 ppm this takes ~65 plant-minutes at the make-up rate — run at Fast time.',
        ...
        { text: 'Set up the heat sink before you make any heat: put steam-generator level control in AUTO (STEAM GEN FEED → AUTO).',
          why: 'Skip it and nothing happens until the point of adding heat — then the generator boils down with no regulator and auxiliary feedwater parks the level in the low amber band. AFW is an emergency sink, not a level control system.',
          control: 'Feed Pumps', target: 'Feed AUTO engaged, SG level ≈ 65 %',
          cmd: { action: 'set_feed_coupled', active: true }, hold: 5,
          hl: ['SG Feed AUTO', 'SG Level'] },
        { text: 'Set the 1/M baseline BEFORE moving any rods: open the 1/M PLOT tool and press "Plot point". This captures the shutdown count rate as the 1.0 reference.',
          why: '1/M = C₀/C. As you approach criticality the count rate C climbs, so 1/M falls toward zero — where the trend crosses zero is the predicted critical rod position. The plot fits the latest three points, so the estimate sharpens as you add more.',
          control: '1/M Plot', target: 'baseline captured (point 1)',
          accs: [{ cmd: 'plot_1m_point', label: 'Baseline point plotted' }],
          hl: ['1/M Plot Tool', 'Source Range'] },
        { text: 'First burst: withdraw the Control Bank to 30 steps at Norm, let the count rate settle, then press "Plot point". Two points draw a line — it will predict criticality far PAST where the reactor actually goes critical, and that is normal.',
          why: 'The first two 1/M points always over-predict. Early in the bank the rods are worth less per step, so the line they draw is shallow and crosses zero far past the real critical position. That is expected, not a mistake — you plot more points so the prediction walks in.',
          control: 'Control Bank', target: '94 / 627 — point 2 (over-estimate)',
        ...
        { text: 'Second burst — smaller: withdraw to 50 steps, settle, plot. The prediction pulls in a long way but is still tens of steps beyond the truth.',
          why: 'Each new point is taken closer to critical, where a step is worth more, so the fitted line steepens and the predicted crossing walks toward you. Still treat it as an over-estimate: never withdraw straight to the number it prints.',
          control: 'Control Bank', target: '157 / 627 — point 3',
          ...
          accs: [{ p: 'sr_counts_cps', op: '>', v: 1400, label: 'Counts settled above 1,400 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Third burst: withdraw to 60 steps, settle, plot. The fit is now on the steep part of the worth curve and the prediction lands within a dozen steps of actual.',
          ...
          control: 'Control Bank', target: '188 / 627 — point 4',
          ...
          accs: [{ p: 'sr_counts_cps', op: '>', v: 3000, label: 'Counts settled above 3,000 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Fourth burst — small now: withdraw to 66 steps, settle, plot. Keep each burst small enough that the Startup Rate stays under 1 DPM.',
          ...
          control: 'Control Bank', target: '202 / 627 — point 5',
          ...
          accs: [{ p: 'sr_counts_cps', op: '>', v: 7000, label: 'Counts settled above 7,000 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Last plotted burst — a nudge to 211 steps, settle, plot. Point 6 is your working number: still reading slightly HIGH, which is the safe side. Take the rest by creeping.',
          why: 'This is the last plotted point on purpose. The remaining distance to critical is short enough that creeping on the Startup Rate is safer than trusting one more fitted number. A prediction that still reads a little high is the side you want.',
          control: 'Control Bank', target: '211 / 627 — the working prediction (true critical ≈ 226–238)',
          ...
          accs: [{ p: 'sr_counts_cps', op: '>', v: 20000, label: 'Counts settled above 20,000 cps' },
                 { cmd: 'plot_1m_point', label: 'Point plotted' }],
          hl: ['Withdraw', '1/M Plot Tool', 'Source Range', 'Startup Rate'] },
        { text: 'Creep up on criticality: withdraw at SLOW to 226 steps. The slow drive paces the insertion — measured, the Startup Rate peaks ~0.9 DPM and the reactor arrives just past critical (ρ ≈ +145 pcm).',
          why: 'One step of this bank is about 8 pcm, so no single step is dramatic — but fifteen of them are, and the rate is what tells you. The Slow speed spreads the insertion so the rate meter can answer while you still have your finger on the button; the discipline here is patience, not precision.',
          control: 'Control Bank', target: '226 / 627 — critical, SUR ≤ 1 DPM',
        ...
        { text: 'Establish the rise: two more steps at Slow (to 228) is all it takes, and power climbs through the decades. Watch the Startup Rate and the Intermediate Range; the Source Range hands off and de-energizes by itself on the way up.',
          why: 'You are just critical. Two more steps put excess reactivity in so power actually climbs. Below the point of adding heat there is no temperature feedback to stop you — the Startup Rate is the only speedometer. The source range will hand off to the intermediate range on its own; you do not switch it.',
          control: 'Control Bank', target: 'power rising to the point of adding heat',
          wait_hint: 'The rise through the decades takes ~15 plant-minutes at this rate — Fast time is fine; watch the SUR.',
        ...
        obs('Confirm the SR→IR handoff happened: the Source Range de-energized itself as the Intermediate Range took over (no operator action on this plant — the channel auto-secures at the P-6 point, where the intermediate range comes on scale at 5e-11 A).',
          { p: 'sr_energized', op: '<', v: 1 },
          null, ['Source Range', 'Intermediate Range'],
          'The source-range detectors would saturate and wear out if they stayed on through power. At the P-6 point the intermediate range is on scale, so this plant secures the source range by itself — there is no operator lever. You are confirming it happened, not doing it.'),
        { text: 'Level off at the point of adding heat: ONE decisive inward drive of 14 steps at Norm, released as the Startup Rate crosses zero. Power settles low single digits.',
          ...
          control: 'Control Bank', target: 'power steady, ~1 % (still Mode 2, Startup)',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: -14, speed: 'normal' }, hold: 240,
          acc: { p: 'power_pct', op: '<', v: 5 },
          hl: ['Insert', 'Startup Rate', 'Intermediate Range'] },
        { text: 'Cross the 5 % boundary deliberately: withdraw 13 steps at Slow. Power rises into the high single digits — entering Mode 1, At Power is a decision you make, not something the ascent does to you.',
          why: 'Mode 1 begins at 5 % power. Crossing it on purpose — a slow pull you chose — is the difference between a controlled entry and an ascent that walked you there. The turbine is still offline; the next step is putting it on the grid so the reactor has somewhere to send the heat.',
          control: 'Control Bank', target: '≈ 8 %, Mode 1, At Power',
          cmd: { action: 'rod_nudge', group_id: 'control', steps: 13, speed: 'slow' }, hold: 400,
          acc: { p: 'power_pct', op: '>', v: 5 },
          hl: ['Withdraw', 'Startup Rate', 'Intermediate Range'] },
        { text: 'Put the turbine on line — two real actions on this plant: LATCH the turbine (un-trip it), then set a load target of 10 MWe. The generator picks up the load and the reactor follows it up to ~10 %.',
          why: 'There is no one-button "connect grid" here: reconnection is the real sequence — protection reset if a trip stands, turbine latched, load dialed. The reactor following the turbine is the plant\'s central coupling, and this is the first time you feel it.',
          ...
          hl: ['Turbine Load', 'Main Breaker'] },
        { text: 'Block the 25 % INTERMEDIATE RANGE high flux trip — now that you are above P-10 — the power permissive at 8 % rated — the permissive lets you. On the TRIP BLOCKS panel, press IR HIGH FLUX. This is the first of two blocks and it comes first, because its setpoint is the one you would reach next.',
          why: 'The startup net has two rungs and you take them in order: the intermediate range at 25 %, then the power range low setting at 35 %. Below P-10 the plant will not let you block either — that is what makes the net a net rather than a switch. THIS button also clears the 20 % rod stop that is about to freeze your withdrawal: one operator action takes the trip and the stop together. Below P-10 the request auto-revokes, so a shutdown re-arms the whole net without anybody remembering to.',
          control: 'Trip Blocks', target: 'intermediate-range trip blocked, rod stop clear',
          cmd: { action: 'set_trip_block', trip_id: 'ir_high', blocked: true }, hold: 10,
          hl: ['Trip Blocks'] },
        { text: 'Now block the 35 % LOW-FLUX trip setting — the second rung, same panel, same permissive. Press the block beside PR HIGH (LOW SETPT).',
          ...
          hl: ['Trip Blocks'] },
        obs('Confirm Mode 1, At Power: critical, generator on line, power ~10 %. Continue with the power ascension checklist.',
          { p: 'plant_mode', op: '~', v: 1, tol: 0.1 },
          null, ['Tavg', 'Turbine Load', 'SG Level'],
          'The reactor is critical, the generator is carrying load, and both startup-net blocks should be standing. The next checklist is the climb to full power — rods lead, turbine follows.'),
      ],
      guard: { never_melted: true, never: [{ p: 'fuel_temp_c', op: '>=', v: 1200 }] },
      outcome: 'Reactor critical and in Mode 1, At Power with the generator carrying ~10 MWe and BOTH startup-net blocks standing (intermediate range and low flux); ready for the power ascension.',
    },
    {
      id: 'pwr_raise_power', category: 'power', manual_ref: 'PWR-N07', next: 'pwr_lower_power',
      title: 'Mode 1, At Power — power ascension to 100 %',
      purpose: 'Take the plant from low power to full power in legs: RODS LEAD, TURBINE FOLLOWS. Withdraw a few steps, raise the load target to match, and keep Tavg on its program. Measured on this plant end to end: ~10 % to 101 % with Tavg landing exactly on program.',
      from: '50_percent',
      prereq: ['Reactor critical and stable at power, above the 8 % P-10 permissive.', 'Turbine on line.', 'BOTH startup-net blocks taken above P-10 — the intermediate range and the 35 % low-flux setting (the startup checklist\'s last two acts).'],
      precond: [
        { p: 'power_pct', op: '>', v: 10, text: 'Reactor critical and at power (clear of the 8 % P-10 permissive)' },
        { p: 'mwe_output', op: '>', v: 5, text: 'Turbine on line, carrying load' },
      ],
      cautions: [
        'Rods lead going UP. Raising load first drags Tavg below program and, un-blocked, runs you into the 20 % intermediate-range rod stop and then the 25 % trip behind it; the measured discipline is pull, load, trim.',
        'Approach 100 % gently: the POWER RANGE high-flux rod stop stands above 103 %, and the 118 % trip is behind it. Overshoot is real on this plant — 100 MWe of load lands power at ~101 %.',
        'Watch xenon: after each leg the burnable poisons move for hours. The boron controller is the long-term trim; rods are the minutes-scale one.',
      ],
      steps: [
        obs('Confirm the at-power lineup: critical, generator loaded, Feed AUTO holding SG level, both startup-net blocks standing.',
          { p: 'mwe_output', op: '>', v: 5 },
          null, ['Turbine Load', 'SG Feed AUTO', 'Trip Blocks'],
          'This is the plant the startup hands you: critical, on the grid, feed holding level. The two trip blocks (intermediate-range and low-flux) must still be standing — without them the climb will trip at 25 % then 35 %.',
          { p: 'power_pct', op: '>', v: 40 }),
        { text: 'Start the long-term reactivity source: set the boron controller target to 705 ppm. The make-up panel dilutes at ~3 ppm/min underneath the whole ascension; rods stay your minutes-scale trim, and the last stretch toward the full-power ~626 ppm is xenon’s business over the following day.',
          why: 'Every percent of power costs reactivity (the fuel heats and the moderator thins). Rods could pay for all of it, but they would end deep in the core distorting the power shape — real plants dilute boron for the bulk and keep the bank in its manoeuvring band. Measured on this plant: diluting to ~705 ppm carries a load-led ascension to full power with Tavg landing on its program; the equilibrium full-power boron (~626 ppm) arrives later as xenon builds in.',
          ...
          hl: ['Boron', 'Boron control'] },
        { text: 'Leg to 30 MWe: withdraw ~8 steps at Norm (power rises ahead of the load), then set the load target to 30. Trim a step either way until Tavg sits on its program.',
          why: 'The reactor follows the turbine on this plant — but going UP you lead with rods so the temperature program is approached from above, not dragged from below. The program runs 546.8 °F at no load to 577.7 °F at 100 %: Tavg on program is how you know reactor and turbine agree.',
          control: 'Control Bank', target: '30 MWe, Tavg on program',
          cmd: { action: 'set_load_target', mwe: 30 }, hold: 480,
          accs: [{ p: 'mwe_output', op: '>', v: 28, label: 'Generator at 30 MWe' },
                 { p: 'power_pct', op: '>', v: 28, label: 'Reactor following, ~30 %' }],
          hl: ['Withdraw', 'Turbine Load', 'Tavg'] },
        { text: 'Leg to 50 MWe: pull ~10 steps, load target 50, trim to program.',
          why: 'Same discipline as the 30 MWe leg: rods first so Tavg is approached from above, then the load target, then a trim. Halfway up, xenon is starting to build — the boron controller is the long-term answer, rods are still the minutes-scale one.',
          control: 'Control Bank', target: '50 MWe, Tavg on program',
          cmd: { action: 'set_load_target', mwe: 50 }, hold: 480,
          accs: [{ p: 'mwe_output', op: '>', v: 48, label: 'Generator at 50 MWe' },
                 { p: 'power_pct', op: '>', v: 47, label: 'Reactor following, ~50 %' }],
          hl: ['Withdraw', 'Turbine Load', 'Tavg'] },
        { text: 'Leg to 75 MWe: pull ~12 steps, load target 75, trim to program.',
          why: 'Three-quarter power. Tavg program is well up the line now (toward 577.7 °F at 100 %). If Tavg is low, you led with load instead of rods — pull a few more steps before you add more megawatts.',
          control: 'Control Bank', target: '75 MWe, Tavg on program',
          cmd: { action: 'set_load_target', mwe: 75 }, hold: 480,
          accs: [{ p: 'mwe_output', op: '>', v: 72, label: 'Generator at 75 MWe' },
                 { p: 'power_pct', op: '>', v: 70, label: 'Reactor following, ~75 %' }],
          hl: ['Withdraw', 'Turbine Load', 'Tavg'] },
        { text: 'Leg to 90 MWe: pull ~12 steps, load target 90, trim to program. The rod stop above 103 % is getting close — smaller pulls from here.',
          why: 'The power-range high-flux rod stop sits above 103 %, with the 118 % trip behind it. Overshoot is real on this plant — 100 MWe of load lands power at ~101 %. Smaller pulls from here so you do not walk into the stop.',
          ...
          hl: ['Withdraw', 'Turbine Load', 'Tavg'] },
        { text: 'Final leg to 100 MWe: pull ~8 steps, load target 100, trim. Measured: power settles ~101 %, Tavg lands on the 577.7 °F program point, bank near 114 of 200.',
          why: 'Full power is a landing, not a lunge. A small pull, then the last 10 MWe of load, then trim Tavg onto 577.7 °F. The bank will sit part-way out in its manoeuvring band — boron carried the bulk of the reactivity the climb cost.',
          ...
          accs: [{ p: 'mwe_output', op: '>', v: 97, label: 'Generator at 100 MWe' },
                 { p: 'tavg_c', op: '~', v: 303.2, tol: 8, label: 'Tavg near the full-power program point (rod trims land it exactly)' }],
          hl: ['Withdraw', 'Turbine Load', 'Tavg'] },
        obs('Confirm Hot Full Power: ~100 % reactor power, 100 MWe, Tavg on program. This is the plant the hot_full_power preset hands you.',
          { p: 'power_pct', op: '>', v: 96 },
          null, ['Tavg', 'Turbine Load', 'SG Level'],
          'You are at the design point: ~100 % / 100 MWe, Tavg on the 577.7 °F program. The round trip back down starts with the load rampdown checklist.'),
      ],
      guard: { never_melted: true, never: [{ p: 'fuel_temp_c', op: '>=', v: 1200 }] },
      outcome: 'Hot Full Power: ~100 % / 100 MWe with Tavg on the program. The round trip back down starts with the load rampdown checklist.',
    },
    {
      id: 'pwr_lower_power', category: 'power', manual_ref: 'PWR-N08', next: 'pwr_shutdown',
      title: 'Mode 1, At Power — load rampdown to ~15 %',
      purpose: 'Bring the plant down from full power to low power in legs: TURBINE LEADS DOWN, RODS TRIM. Reduce the load target, let the reactor follow, and insert rods to keep Tavg from riding above its program. Measured end to end: 100 % to ~14 % with Tavg within ~1 °F of program at every leg.',
      from: 'hot_full_power',
      prereq: ['Reactor at power, turbine on line.'],
      precond: [
        { p: 'power_pct', op: '>', v: 10, text: 'Reactor critical and at power' },
      ...
      cautions: [
        'Turbine leads going DOWN. Dropping load leaves the reactor hot — Tavg rides ABOVE program until you insert rods to trim it back. Insertion is never blocked; take it in small bites.',
        'Watch SG level on each drop — shrink-and-swell moves the indication the wrong way first.',
      ],
      steps: [
        { text: 'Start the long-term reactivity sink: set the boron controller target back toward the no-load concentration (719 ppm). Boration removes the reactivity the plant no longer needs as load falls; rods are the minutes-scale trim on top.',
          why: 'Coming down is the ascension in reverse: every percent shed hands reactivity back (the fuel cools, the moderator thickens), and it has to go somewhere. Boration carries the bulk out; without it the reactor hangs high above the falling load and the steam dump quietly carries the difference.',
          control: 'Boron control', target: 'boration running toward 719 ppm',
          wait_hint: 'The boration runs ~half a plant-hour — start it first and take the legs while it works.',
          cmd: { action: 'set_auto_setpoint', channel_id: 'boron_conc', value: 719 }, hold: 30,
          hl: ['Boron', 'Boron control'] },
        { text: 'Leg to 75 MWe: reduce the load target, let power follow down, then insert rods (measured ~40 steps over this leg) until Tavg comes back to program.',
          why: 'The reactor follows the turbine: less steam drawn means the core\'s heat has nowhere to go, Tavg rises, and moderator feedback walks power down after it — but the equilibrium sits hot until rods take the excess reactivity out. The program is the same line you climbed on the way up.',
        ...
        { text: 'Leg to 50 MWe: reduce, follow, trim (~20 steps in).',
          why: 'Same reverse discipline: turbine first, then insert rods so Tavg does not sit hot above program. Shrink-and-swell will dip indicated SG level the wrong way first — do not chase it; Feed AUTO holds the band.',
          ...
          hl: ['Turbine Load', 'Insert', 'Tavg'] },
        { text: 'Leg to 30 MWe: reduce, follow, trim (~10 steps in).',
          why: 'Low-power legs need smaller rod bites. The program is walking back toward 546.8 °F. Keep Tavg from riding high — a hot plant at low load dumps the difference to the condenser.',
          ...
          hl: ['Turbine Load', 'Insert', 'Tavg'] },
        { text: 'Final leg to 15 MWe: reduce, follow, trim (~6 steps in). This is the handoff point to the shutdown checklist — low enough to scram from without a thermal slam, high enough that the plant is still making its own steam.',
          why: 'Scramming from full power slams the primary thermally. ~15 % is low enough that the trip is gentle and high enough that the steam generators still have steam to dump. The shutdown checklist takes the generator off and drops the rods from here.',
          control: 'Turbine Load', target: '15 MWe, power ~14 %',
          cmd: { action: 'set_load_target', mwe: 15 }, hold: 900,
          accs: [{ p: 'power_pct', op: '<', v: 30, label: 'Reactor below 30 % and falling as the boration finishes (rod trims take it to ~15 %)' }],
          hl: ['Turbine Load', 'Insert', 'Tavg'] },
      ],
      guard: { never_melted: true, never: [{ p: 'fuel_temp_c', op: '>=', v: 1200 }] },
      outcome: 'Plant stable at ~15 % / 15 MWe, Tavg on program. The shutdown checklist takes it to Mode 3.',
    },
    {
      id: 'pwr_shutdown', category: 'shutdown', manual_ref: 'PWR-N14', next: 'pwr_cooldown',
      title: 'Mode 1, At Power → Mode 3, Hot Standby — normal shutdown',
      purpose: 'Shut the reactor down: take the generator off load, trip the reactor, confirm decay heat is being removed. Measured from ~15 %: scram to source range in seconds, Mode 3 with ~2 % decay heat settling out on the dumps.',
      from: 'hot_full_power',
      prereq: ['Reactor at power.'],
      precond: [
        { p: 'power_pct', op: '>', v: 10, text: 'Reactor at power — there is something to shut down' },
      ],
      cautions: ['Decay heat persists after shutdown and decays over hours — the steam dump at the no-load anchor is the heat sink until the cooldown begins.'],
      steps: [
        { text: 'Reduce the load target to zero — the generator unloads and the reactor follows down.',
          ...
          hl: ['Turbine Load'] },
        { text: 'Trip the reactor (SCRAM). Both banks drop; power collapses to the source range.',
          ...
          acc: { p: 'power_pct', op: '<', v: 5 },
          hl: ['SCRAM'] },
        obs('Confirm the chain reaction has stopped and decay heat remains (measured ~1.9 % settling) — the dumps carry it at the no-load anchor. This is Mode 3, Hot Standby.',
          { p: 'decay_heat_pct', op: '>', v: 1 },
          null, ['Steam Dump', 'Tavg'],
          'Fission is gone; decay heat is not. The steam dump at the 1020 psi no-load anchor is the heat sink until the cooldown checklist takes over. Tavg should be sitting near 546.8 °F, subcritical, pressurized — Hot Standby.'),
      ],
      guard: { never_melted: true },
    ...
    },
    {
      ...
      title: 'Mode 3, Hot Standby → Mode 5, Cold Shutdown — controlled cooldown',
      purpose: 'Take a hot, subcritical plant all the way to Mode 5, Cold Shutdown: borate for cold shutdown margin, block the protection that would trip you on the way down, walk the secondary down the saturation curve, depressurize on spray once the Pressure SP reaches its 1700 psig floor, isolate the accumulators inside their window, align Residual Heat Removal and secure the pumps so RHR carries the plant cold. Measured end to end on this plant: Mode 3 to Mode 5 in ~6.7 plant-hours, the RHR leg at −92 °F/hr, subcooling never below 75 °F.',
      from: 'hot_zero_power',
      prereq: [
        'Plant at Mode 3, Hot Standby: hot (546.8 °F / 286 °C), at normal operating pressure, subcritical with the control bank in.',
        'Reactor coolant pumps running; steam generator level on the three-element controller.',
        'Condenser available — the steam dump is the heat sink until RHR takes over.',
      ],
      precond: [
        { p: 'tavg_c', op: '~', v: 286, tol: 8, text: 'Hot Standby at the no-load temperature (≈ 546.8 °F / 286 °C)' },
        { p: 'power_pct', op: '<', v: 1, text: 'Reactor shut down — this is a cooldown, not a power maneuver' },
      ],
      cautions: [
        'BORATE BEFORE YOU COOL. Cooling a core makes it more reactive (the cold moderator is denser) — the margin you have hot is not the margin you will have cold.',
        'Hold the cooldown under 100 °F/hr. The dump-setpoint walk sets the secondary-led rate; the RHR heat-exchanger split sets the cold-end rate (measured: the authored walk runs ~−60 °F/hr, the RHR leg ~−92 °F/hr).',
        'The accumulator window on the way DOWN: the isolation valve regains power below 1600 psig and the tanks would discharge below their ~650 psig cover pressure — isolate INSIDE that band, early.',
        'Below the Pressure SP\'s 1700 psig floor the heaters cannot follow you down — the depressurization tool is the SPRAY with the heaters off, and subcooling margin is what you are guarding while it runs.',
      ],
      auto_channels: ['boron_conc'],
      steps: [
        { text: 'BORATE FIRST — nothing cools until this is done. Engage the boron controller and set the target to 920 ppm, the cold-shutdown concentration this plant\'s own Mode 5 lineup carries.',
          why: 'At 546.8 °F the plant is comfortably subcritical on ~719 ppm — but cold water moderates better, and the same core at 122 °F needs ~920 ppm for the same safety. Borating first means the margin arrives before the cold does. The batch dose runs ~3 ppm/min, so this is the hour the secondary walk (next steps) rides on.',
          control: 'Boron control', target: '920 ppm',
          wait_hint: 'The boration takes ~60 plant-minutes — start it and continue; the cooldown legs run while it doses.',
          cmd: { action: 'set_auto_setpoint', channel_id: 'boron_conc', value: 920 }, hold: 3900,
          acc: { p: 'boron_ppm', op: '>', v: 880 },
          hl: ['Boron', 'Boron control'] },
        { text: 'Lower the Pressurizer Pressure Setpoint to 1900 psi (13.1 MPa) — under the P-11 permissive — so the protection blocks in the next step become available.',
          why: 'The low-pressure trip and the safety-injection actuation both need the P-11 permissive before you can block them. P-11 is 1,972 psia — so the setpoint has to come under it first. This is not the depressurization; it is unlocking the next step.',
          ...
          acc: { p: 'pressure_mpa', op: '<', v: 13.6 },
          hl: ['Pressure SP', 'Primary Pressure'] },
        { text: 'Block the low-pressure reactor trip and the Safety Injection actuation — both P-11 blocks — and take HPI and LPI to OFF. The cooldown walks the primary through setpoints that read, to an armed protection system, exactly like a LOCA.',
          why: '"Block SI" is why the setpoint came down first: both blocks need the P-11 permissive. Leave them armed and the first cooling leg trips the plant and starts an injection you did not ask for — measured on the retired plant, cold injection cools ~ten times faster than the programme you are trying to hold.',
          control: 'Trip Blocks', target: 'LO PRESS and SI blocked; HPI/LPI off',
          ...
          hl: ['Trip Blocks'] },
        { text: 'Walk the Steam Dump Setpoint down — 1020 → 400 → 120 psi over about two and a half plant-hours. The secondary rides its saturation curve down and pulls the primary with it through the steam generator.',
          why: 'The secondary cannot pull the primary below its own saturation temperature — that is why the walk continues all the way to 120 psi (saturation ≈ 341 °F): it is what lets the primary reach the RHR entry class. The walk rate IS the cooldown rate: measured, this leg runs ≈ −60 °F/hr, inside the 100 °F/hr limit.',
        ...
        { text: 'Bring the Pressurizer Pressure Setpoint to its 1700 psig floor. The board\'s dial goes no lower — from here down, pressure control changes hands.',
          why: 'The setpoint span is the at-power pressure-control dial (sourced, 1700–2500 psig). A real cooldown leaves that dial\'s world exactly here: below it the heaters have nothing to hold and the operator depressurizes by hand.',
          ...
          hl: ['Pressure SP', 'Primary Pressure'] },
        { text: 'Depressurize on the SPRAY: pressurizer heaters to MANUAL-0, then open the spray. Pressure falls fast — watch the subcooling margin, and isolate the accumulators at the next step as you pass ~1500 psi.',
          why: 'With the pumps still running the normal spray condenses pressurizer steam with recirculated loop water — no net mass added, so the level holds while pressure walks down. Measured: ~1700 → 400 psi in about ten plant-minutes with subcooling never under 100 °F. The heaters must be OFF first or they fight the spray with the whole ladder.',
        ...
        { text: 'Isolate the Safety Injection accumulators NOW, inside the band: below 1600 psig the valve operator has power again, and the tanks must be shut before pressure reaches their ~650 psig cover gas — or they discharge into the plant.',
          ...
          acc: { p: 'accumulator_valve_open', op: '<', v: 0.5 },
          hl: ['Accumulator valve'] },
        { text: 'Ride the spray-down past the RHR permissive: let pressure fall through 425 psig indicated. Measured, the spray walks ~1500 → 400 psi in about eight plant-minutes with subcooling never under 100 °F.',
          why: 'RHR suction will not open above 425 psig — that interlock is sourced (WTSM 5.1). Leave the spray on while you wait: shutting it now lets pressure bounce back over the permissive. Subcooling is what you are guarding; it should stay well above 100 °F on this spray.',
        ...
        { text: 'ALIGN RHR while the spray still holds pressure low: open the hot-leg suction (RHR card → ALIGN) and set the heat-exchanger split to 7 %.',
          ...
          control: 'Residual Heat Removal (RHR)', target: 'RHR aligned, HX 7 %',
          cmd: { action: 'set_rhr', active: true }, hold: 60,
          acc: { p: 'rhr_valve_open', op: '>', v: 0 },
          hl: ['Residual Heat Removal (RHR)', 'Primary Pressure'] },
        { text: 'Secure the Reactor Coolant Pumps, then shut the spray. RHR provides the circulation from here; with the pumps stopped the steam generator decoupled and the loop spray lost its head anyway.',
          why: 'RHR is aligned and circulating. The reactor coolant pumps are now waste heat and the spray has no driving head without them. Stop the pumps first, then shut the spray — the order the heatup ran in reverse.',
        ...
        { text: 'RHR-LED COOLDOWN TO MODE 5: walk the heat-exchanger split from 7 % up to 25 % and ride the plant cold. Mode 5, Cold Shutdown arrives at 199.4 °F (93 °C) — measured, this leg runs ≈ −92 °F/hr and takes about two plant-hours.',
          why: 'The heat-exchanger split is the cooldown rate from here. 25 % is enough to make Mode 5 in about two plant-hours without crowding the 100 °F/hr limit (measured ≈ −92 °F/hr). Watch Tavg and its rate; trim the split down if it runs away.',
          control: 'Residual Heat Removal (RHR)', target: 'Tavg below 199.4 °F (93 °C) — Mode 5',
          wait_hint: true,
        ...
        obs('Confirm Mode 5, Cold Shutdown: coolant below 199.4 °F (93 °C), pressure settled in the 250–550 psi band on the cold bubble, RHR carrying the decay heat, pumps off.',
          { p: 'plant_mode', op: '~', v: 5, tol: 0.1 }, null, ['Tavg', 'Primary Pressure'],
          'This is the cold-shutdown picture: coolant below 199.4 °F, a small steam bubble still in the pressurizer, pumps off, RHR carrying the heat. It is the same state the cold_shutdown preset loads, and the heatup checklist takes it back up.'),
        obs('Confirm the accumulators are still FULL and isolated — they stay that way until the next heatup re-arms them inside its window.',
          { p: 'accumulator_volume_pct', op: '>', v: 99 }, null, ['Accumulator valve'],
          'You isolated them on the way down so they would not discharge into a depressurized plant. They must still be full — the next heatup re-opens them inside its 600–1600 psig window, and empty tanks then are a missed safety function.'),
        obs('Confirm RHR is the heat sink and the suction valve is open — this is the lineup the plant sits in until it is brought back up. The round trip is complete.',
          { p: 'rhr_valve_open', op: '>', v: 0 }, null, ['Residual Heat Removal (RHR)'],
