 ...
 *                 (with --shard, only that shard's block — paste all N to refresh)
 *   --quiet       suppress per-runner output (failures still dump their tail)
 *   --shard=i/N   run only partition i of N (1-based, N ≤ 16). CI runs the shards as a
 *                 matrix; see partitionShards() below and .github/workflows/gates.yml.
 *                 The no-baseline guard still runs over the FULL discovered set.
 *   --list        print the runners this invocation WOULD run, and exit without
 *                 running them (still exits 1 on an unbaselined runner — --list is a
 *                 dry form of the gate, not a way past its one sacred check)
 *
...
var BASELINES = {
  ...
  'run_hr3.js':            { code: 0, score: '32checks 0failed' },   // 31 -> 32 (#654, 2026-09-08 merge): the boron channel's deliveredRate hook is one more DECLARED control_state read. // 30 -> 31 (#386 stage 3): set_ctmt_recombiners declared. 28 -> 30 (#386 stage 2): set_containment_spray/set_ctmt_fans declared in the valueFieldFor allow-list
  ...
  /* 442 -> 445 (#591/#564/#578, 2026-08-31): three more HR11 citation SITES, not three more
   * rules. The count is `sites`, so every new dated-and-quoted ruling reference raises it —
   * this change carries the AFW ruling into CHANGELOG.md, BUILD_DECISIONS.md and
   * PWR2_VALIDATION.md. The gate caught the first draft of the CHANGELOG one, which had the
   * date and no verbatim quote: HR11 wants both, and a bare date is the shape that lets a
   * paraphrase pass as a ruling. */
  // 461 -> 471 (#616, 2026-09-03): +10 HR11 citations, no new rule and no new leak. The check
  // counts every OWNER RULING / OWNER DIRECTIVE in tracked Markdown, and Blueprint/STYLE_GUIDE.md
  // quotes ten of them — the units directive, the codes directive, the voice directive, the
  // naming directive, the checklist auto-tick, warn-never-block, the doc-budget ruling and the
  // binding-list ruling among them. WORTH KNOWING: the runner reports "0 failed" whether or not
  // the count moved, so a doc that quotes the owner moves a baselined score while every gate you
  // are likely to run reads green. Two commits landed before this was noticed.
  'run_hardrules.js':      { code: 0, score: '531checks 0failed' },   // 528 -> 531 (#686, 2026-09-11): +3 HR11 ruling-citation SITES, 0 undeclared, no simulator code moved -- the TUNING_LOG entry for this session cites all three #686 rulings in the `(OWNER RULING, DATE: "quote")` form. // 525 -> 528 (#698/#700/#705 close-out, 2026-09-11): +3 HR11 ruling-citation SITES, 0 undeclared, and no simulator code moved -- the session's TUNING_LOG entry and the new Manuals/12 row each cite the 2026-09-10 ruling. TWO OF THE THREE LANDED UNDECLARED and the gate caught them: they were written `(OWNER RULING, 2026-09-10, option B.)`, a date with no verbatim words, which is precisely the half of HR11 that makes a citation advisory. Both now carry the owner's actual sentence, "All decisions as recommended." A ruling given as a blanket ratification is the easy one to paraphrase, because the words do not mention the issue you are citing them for. // 524 -> 525 (#685/#693/#692/#653 close-out, 2026-09-11): +1 HR11 ruling-citation SITE in CHANGELOG.md, 0 undeclared, and NO simulator code moved -- the sweep's CHANGELOG entry quotes the 2026-09-09 `(OWNER RULING, ...: "TMI does not go open until I can playtest it.")` that is the reason the TMI-2 work gets no `changelog.html` entry and no version bump. The Diagnostic/TUNING_LOG.md entry written in the same commit adds NONE: its owner quotes are playtest-sheet `(OWNER, ...)` forms, which MARKER (/OWNER (RULING|DIRECTIVE)/) does not match. // 523 -> 524 (#693, 2026-09-11): one more dated+quoted ruling citation -- Manuals/08 §6.0's condensate-polisher row now records that narrating it was RULED (OWNER RULING, 2026-09-10, "All decisions as recommended") rather than merely observed, which is what HR11 asks of a row that reads as standing law.   // MERGED 2026-09-11 (develop+backshop+workbench): measured 523 checks on the merged tree (two independent HR11-citation sites, #679 and #696/#697, both landed) -- 521 -> 522 (#696/#697, 2026-09-10): +1 HR11 ruling-citation SITE, 0 undeclared, no simulator code moved -- Diagnostic/TUNING_LOG.md's session entry quotes the 2026-08-06 "Warn, never block" precond ruling that #696's fix relies on. Verified: the only tracked-markdown edit this session touched is that one entry.   // 520 -> 521 (2026-09-10, pre-existing at this lane's tip, not #691): +1 HR11 ruling-citation SITE, 0 undeclared, no simulator code moved. 793f4c02 (the #684/#701/#699 close-out) added a well-formed `(OWNER RULING, 2026-08-04: "A")` citation to Diagnostic/TUNING_LOG.md's run_ops writeup without moving this baseline in the same commit -- caught here because #691's own `run_all` came back DRIFT on a runner #691 never touches (run_hardrules scans only Blueprint/, Diagnostic/, Manuals/, CLAUDE.md, CHANGELOG.md, README.md, .claude/ -- never ui/ or test/, which is everything #691 changed). Verified by reading 793f4c02's diff directly, not inferred.   // 518 -> 520 (#670, 2026-09-09): +2 HR11 ruling-citation SITES, 0 undeclared, and NO simulator code moved. The owner's "A." ruling on the operator pass 2's finding S-4 -- a walkthrough acceptance grades the UNDAMPED transmitter, not the damped tile -- recorded at BOTH places that had been left saying the question was open: beside S-4's own Measured/Verdict pair in CHECKLIST_PLAYTEST_2026-09-09_OPERATOR_TMI_PASS2.md, and in the TUNING_LOG paragraph that ends "Left for the owner's ruling rather than patched". Editing only one would have left the record contradicting the issue. Same acknowledged class as every prose move on this line.   // 515 -> 518 (#670 operator pass 2 + #675, 2026-09-09): +5 HR11 ruling-citation SITES in one wave, and no simulator behaviour behind any of them. TWO are the owner's 2026-09-06 no-SI ruling quoted where it is now ENFORCED rather than only where it was recorded -- in fmtPredValue (ui/app.js), the function that was violating it, and in the browser check that pins the rendered line. THREE are the 2026-09-09 warp-line ruling ('Warp line as you recommend.') recorded in the pass-2 report and TUNING_LOG, which is what marks S-6 and S-7 superseded rather than open. A ruling cited at the code it governs, and at the finding it closes, is the point of this count moving. // 513 -> 515: +2 HR11 ruling-citation SITES and no simulator behaviour behind them -- the owner's 2026-09-06 no-SI ruling is now quoted where it is ENFORCED rather than only where it was recorded: in fmtPredValue (ui/app.js), the function that was violating it, and in the browser check that pins the rendered line (verify_ckl_relevance). A ruling cited at the code it governs is the point of the HR11 count moving. // 511 -> 513 (#670, 2026-09-09): +2 HR11 ruling-citation SITES, 0 undeclared, and NO simulator code moved -- both are in Blueprint/TMI_WALKTHROUGH_PLAN.md, which cites the 2026-09-09 "TMI does not go open until I can playtest it." ruling twice: once in the status block and once in the §10 Phase 4 row, because that row is the thing an agent picking up Phase 4 reads first and a pointer to where the ruling is written is not the ruling. Same acknowledged class as every prose move on this line.   // 499 -> 511 (#642 +3, #659 +1, #661 +1, #662 +2, #668 +4, #670 +1, 2026-09-08). The #670 site arrived in the TMI walkthrough PLAN document -- the guard counts ruling citations wherever they are written, docs included, so a planning doc that quotes an owner ruling moves this number.: HR11 ruling-citation sites. //   // 496 -> 499 (2026-09-08 merge): three more dated ruling citations from the develop side (#654 A, #655 x2). // 492 -> 496 (#647): four more dated ruling citations, HR11 480 sites 0 undeclared. // 487 -> 492 (#633/#508/#645, 2026-09-06): +1 HR11 SITE, not a new check written by hand — the guard counts RULING CITATIONS, and this batch added one (the 2026-09-06 'Move the pressurizer copy too' ruling, cited with its date and the owner's verbatim words at the pressurizer's no-load anchor). HR11 now 476 sites, 0 undeclared — the +5 are the two 2026-09-06 rulings cited at the code that implements them AND in CHANGELOG/TUNING_LOG/BUILD_DECISIONS, which the guard also scans. A ruling cited WITHOUT its date and quote would not raise this count, it would red the gate — which is the whole design. // 485 -> 487 at the merge, and this line is why the merge skill says COMBINE rather than pick a side: BOTH lanes independently moved it 485 -> 486, for DIFFERENT +1s, so either comment taken whole would have shipped the other lane's citation uncounted and a 486 baseline against a 487 tree. develop: 485 -> 486 (#637, 2026-09-05): +1 Hard Rule 11 citation SITE, 0 undeclared — the dated owner quote for the sharding ruling ("Do 1. Plan the fix then have opus agents perform the work.") in CHANGELOG.md; the count is the number of dated quotes in the tree, so it moves with every ruling recorded.   // 481 -> 485 (2026-09-05, the #629 x #631 merge: develop's +4 sites (#624 item 24) and workbench's +1 (#631) add; measured on the merged tree). Before: 477 -> 481 (#624/#619 item 24, 2026-09-04): +4 HR11 citation SITES, 0 undeclared -- the RATE decision departs from an owner-ruled number, so the 2026-08-08 ruling is quoted verbatim in CHANGELOG, TUNING_LOG and BUILD_DECISIONS (TUNING_LOG twice: once for the rate, once for the raises-only direction it RESTORES). Caught by run_all as a DRIFT: the first draft cited the ruling by DATE ONLY ("an OWNER RULING of 2026-08-08"), which is exactly what HR11 exists to red -- a marker with no verbatim words is not a citation.   // 474 -> 477 (#624/#619 items 14/25, 2026-09-04): +3 HR11 citation SITES, 0 undeclared -- the letdown-split ruling is quoted in CHANGELOG, TUNING_LOG and BUILD_DECISIONS, one site each. Previously 473 -> 474 (#623/#619 item 23, 2026-09-04): +1 HR11 citation SITE, not a new rule -- the 1/M help panel is built to the Scanner's in-place idiom and the TUNING_LOG entry quotes the 2026-08-11 directive that settled that shape, with its date and the owner's verbatim words. This baseline moves when you write PROSE, not when you change code.   // 472 -> 473 (#621/#619 wave 1, 2026-09-03): +1 HR11 citation SITE. The gate CAUGHT the half-citation form first -- a CHANGELOG line read '(OWNER RULING, 2026-09-03, on that exact distinction)', a date with no verbatim words, which HR11 refuses. A selection from written options is cited by quoting the OPTION.   // 471 -> 472 (#618, 2026-09-03): +1 HR11 citation SITE -- the owner's "A" ruling on repointing the ECC/creep gates, cited with its date and verbatim word. Prose, not code, exactly as the note further along this line warns.   // 460 -> 461 (#611, 2026-09-03): +1 HR11 citation, not a new rule. The check counts every OWNER RULING / OWNER DIRECTIVE in tracked markdown and asserts each carries a DATE and the owner's VERBATIM WORDS; the develop-push policy added one to CLAUDE.md. This number moves whenever a ruling is recorded, which is the intended behaviour -- if it moves and you did NOT add a citation, something removed one.   // MERGED 2026-09-02 (develop + workbench): BOTH lanes bumped this for DIFFERENT HR11 citation sites, and the merged tree carries all of them — develop reached 454, workbench 452, and the true number is NEITHER and is not recoverable from the diff. RUN THE GATE, do not choose a side. // 451 -> 452 (#603, 2026-09-02): the owner's "no extra UI elements" directive cited at its site. The gate ALSO caught a citation carrying a date and no verbatim quote -- HR11 wants both, and half a citation is the shape that launders a remembered ruling into a fresh assertion.   // 449 -> 451 (workbench<-develop merge, 2026-09-02): MEASURED, not resolved by picking a side. Both lanes bumped this baseline for DIFFERENT HR11 citation sites — workbench 446->448 (+2: the #598 owner-directive dating, then the item 13 ruling), develop 446->449 (+3: #602 phase 1) — so the merged tree carries all five and the answer is neither branch's number. This is the BASELINES conflict CLAUDE.md names: "a mechanical resolution can take the wrong number silently", and here both sides looked equally plausible in the diff.   // 447 -> 448 (#598 item 13, 2026-09-02): the owner's ruling to promote the control pill back out of the Details fold, cited at its site in ui/app.js with the date and his verbatim words.   // 446 -> 447 (#598, 2026-09-01): one more HR11 site. The gate CAUGHT this one for real — eight owner-directive citations went in carrying only an issue number ("OWNER DIRECTIVE, #598 item 8"), and HR11 wants a DATE with the verbatim quote or the ruling is advisory. Dated, and the site count moved with the one that lives in a scanned file.   // 445 -> 446 (#596, 2026-08-31): the aux-spray removal directive cited in CHANGELOG/TUNING_LOG/BUILD_DECISIONS — one more HR11 citation site.   // 441 -> 442 (#532 phase 4, 2026-08-30): the CHANGELOG entry cites the whole-set ruling in the HR11 quoted-option form, which is one more citation SITE for the guard to check. // 438 -> 441 (#528, 2026-08-30): the manual-rod ruling and the keep-it-dark selection, cited across DESIGN_COMPANION §8.36, the manual revision row, the changelog and the tuning log.   // 436 -> 438 (#528, 2026-08-30): the manual-rod-control ruling, cited at its two binding sites — DESIGN_COMPANION §8.36 and the manual revision row.   // 434 -> 436 (#590, 2026-08-29): the steady-state criterion ruling, cited in the validation section and the tuning log.   // 432 -> 434 (#516 items 6-8, 2026-08-29): the item-8 close-out ruling, cited in the validation section and the tuning log.   // 429 -> 432 (#516 item 9, 2026-08-29): the melt-path ruling cited at its three sites — the constant it moves in pwr2_water, the validation section, and the changelog/tuning-log pair.   // 427 -> 429 (#516 Group A, 2026-08-29): the same ruling cited in CHANGELOG.md and TUNING_LOG.md as well as the validation section — three sites for one decision, which is the intended shape.   // 426 -> 427 (#516 Group A, 2026-08-29): one new HR11 citation site — PWR2_VALIDATION §120 records the owner's scope ruling on the playtest list. Written first WITHOUT a quoted string and the gate caught it: a selection from written options still has to quote the option, the CLAUDE.md "a selection, not verbatim words" idiom.   // 423 -> 426 (#580, 2026-08-29): the stage-2 ruling recorded where it binds — the catalog row in pwr_control, PWR2_VALIDATION §119, and the status line in CLAUDE.md. Three citation sites for one ruling is the intended shape: a ruling that lands only in a Blueprint doc has not landed.   // 421 -> 423 (#500/#577, 2026-08-29): two new HR11 citation sites — the 2026-08-28 bundle ruling in pwr_control's alarm rows and the 2026-07-28 status-auto-ack ruling in PWR2_VALIDATION §119. The gate caught one of them UNDECLARED first: a dated OWNER RULING with no verbatim quote is not a citation, and §119 had one.   // 411 -> 414 (#574 DOCS, 2026-08-28): the code half measured 411 and the DOC half of the same change adds three more HR11 citation sites — the wall-scoping ruling and the shrink-the-claims ruling, each cited again in TUNING_LOG and BUILD_DECISIONS. Recorded 411 first, from a gate run BEFORE the write-ups: the file's own trap, and run_all caught it. 0 undeclared.   // 409 -> 411 (#574, 2026-08-28): two new HR11 citation sites, both declared with date and verbatim words — the ruling that scoped the walls to all eleven nodes, and the one that shrank the core-damage claims. 0 undeclared.   // 405 -> 409 (#573/#473, 2026-08-28): four new HR11 citation sites, all declared with date + verbatim words — the 2026-08-12 heater-elevation ruling (built on the retired engine, never on this one) and the 2026-08-28 scoping ruling that took the board half with it. 0 undeclared, as before.   // 402 -> 405 (#536, 2026-08-28): three new HR11 citation sites, all declared with date + verbatim words — the ruling that re-scaled the source-range gauge ("Re-scale the gauges too", from three options put to the owner). 0 undeclared, as before.   // 398 -> 400 (#572, 2026-08-28): the dated ruling that scoped this change ("A" — build the block, from two costed options) lands as new HR11 citation sites. Same acknowledged class.   // 399 -> 398 (2026-08-28): DOWN by one, and drift is symmetric so it is recorded rather than absorbed. CLAUDE.md's 'Built, waiting on review or a close' bullet was cut — all seven issues it listed measured CLOSED — and it carried a dated ruling citation (#458, OWNER RULING 2026-08-12). The rulings themselves are untouched in BUILD_DECISIONS and the issues; what left is one CITATION SITE in the file that is paid for every turn.   // 395 -> 399 (#545, 2026-08-28): the two rulings that scoped the rod-drive fix (#545 + the two one-liners; refuse both directions), both menu selections cited in that form, land as new dated HR11 citation sites — FOUR of them, because each ruling is cited in both PWR2_VALIDATION and the BUILD_DECISIONS/TUNING_LOG pair. Recorded 397 first: that number was measured BEFORE the doc pass, which is the file's own 'do not copy a number written mid-change' trap, caught here by the gate rather than by me.   // 393 -> 395 (#551 + #559 + #567 + #558 + #560, 2026-08-27): the two rulings that scoped the turbine cluster (latch = back on the line, roll stays out under #307; replace the dispatch pair with LATCH / TRIP), both menu selections cited in that form, land as new dated HR11 citation sites. Same acknowledged class.   // 387 -> 393 (#540 + #549 + #541 + #562, 2026-08-27): the two rulings that scoped this cluster (model the overfill wall; the level hold is a control-layer channel), both menu selections cited in that form, land as new dated HR11 citation sites across PWR2_VALIDATION §99, CHANGELOG, TUNING_LOG, BUILD_DECISIONS and CLAUDE.md. Same acknowledged class.   // 385 -> 387 (#539, 2026-08-27): the two rulings that scoped this change (fix the class; adjudicate the reds one at a time), both given as menu selections and cited in that form.  // 381 -> 382 (#523, 2026-08-26): the four cutover rulings land as new dated HR11 citation sites (PWR2_VALIDATION §94, CHANGELOG, TUNING_LOG, BUILD_DECISIONS, CLAUDE.md). Same acknowledged class.   // 380 -> 381 (#518, 2026-08-26): the dated owner ruling "Canary + sub-step" in PWR2_VALIDATION §89 and CHANGELOG   // 379 -> 380 (#517, 2026-08-26): PWR2_VALIDATION §88's and CHANGELOG's dated owner ruling "Build the superheat wing anyway."   // 378 -> 379 (#515 Build 3, 2026-08-26): PWR2_VALIDATION §87's dated owner-ruling citation. Earlier: 371 -> 374 (#513 follow-up, 2026-08-25): the pwr-campaign-split ruling's citation sites (CLAUDE.md, TUNING_LOG, BUILD_DECISIONS) — counted BEFORE the push this time, in the same change. Same acknowledged class.   // 367 -> 371 (#513, 2026-08-25): the session's four new dated ruling citations (the two plan-question rulings in BUILD_DECISIONS/TUNING_LOG, and their echoes in the runner comments) are HR11 sites — CAUGHT BY CI, not locally: the full local gate ran before the doc entries were written, and the doc-gates spot check ran the runner solo, whose own exit code cannot see a count move. Same acknowledged class.   // 366 -> 367 (#514, 2026-08-25): the load-cut owner ruling cited in BUILD_DECISIONS is an HR11 site. Same acknowledged class.   // 365 -> 366 (#511, 2026-08-24): the owner's model-these-valves directive lands as a new dated HR11 citation site (350 sites scanned).   // 364 -> 365 (2026-08-23): the acceptance-windows owner directive ("Let's fix your acceptance windows first.") cited in TUNING_LOG and the endurance runner is an HR11 site. Same acknowledged class.   // 363 -> 364 (2026-08-22c): the #507 batch's dated ruling citations (the SGTR UNVERIFIED-area declaration in pwr2_shell.js, the LOOP row's "Full LOOP + clear") are HR11 sites. Same acknowledged class.   // 362 -> 363 (2026-08-22b): the #458 ruling citation the pwr2 shell's RHR refusal now carries is an HR11 site. Same acknowledged class.   // 360 -> 362 (2026-08-22a): the #506 bundle's two new dated ruling citations (the shutdown-bank directive, the boron-panel ruling) are HR11 sites. Same acknowledged class.   // 357 -> 360 (2026-08-21d): the #501-#504 bundle's three new dated ruling citations (the flat-seed reversal in BUILD_DECISIONS/CLAUDE.md/CHANGELOG) are HR11 sites. Same acknowledged class.   // 356 -> 357 (2026-08-21c): the merge session record's own directive citation is an HR11 site — and it went to origin ungated, because a 'docs-only' push skipped the static scan. The gate-the-exact-commit rule has no docs-only exemption; CI caught it in 40m11s. Same acknowledged class.   // MERGED TREE 2026-08-21 (develop 307 x backshop 334 — a STATIC SCAN carries the union; measured below, not picked — the 2026-08-17 trap).   // 332 -> 334 (2026-08-21b): the CVCS balance-point ruling cited in PWR2_VALIDATION.md sec 64 and TUNING_LOG. Same acknowledged class.   // 330 -> 332 (2026-08-21a): the feed-train ruling ("Due next as recommended") cited in PWR2_VALIDATION.md sec 63 and TUNING_LOG. Same acknowledged class.   // 328 -> 330 (2026-08-20i): the two AFAS rulings ("AFW auto-start" + "Include the trip rung too") cited in PWR2_VALIDATION.md sec 62 and TUNING_LOG. Same acknowledged class.   // 327 -> 328 (2026-08-20g): the A/B-pass ruling citation in PWR2_VALIDATION.md sec 60. Same acknowledged class.   // 325 -> 327 (2026-08-20e): the 'Next: option B' ruling cited in PWR2_VALIDATION.md secs 57-58 and in pwr2_shell.js, the class built under it. Same acknowledged class.   // 323 -> 325 (2026-08-20a): the 'Do option 1' ruling cited in PWR2_VALIDATION.md sec 54 AND in pwr2_instruments.js, the module built under it. Same acknowledged class.   // 322 -> 323 (2026-08-19g): PWR2_VALIDATION.md §49 carries the facade ruling citation. Same acknowledged class.   // 321 -> 322 (2026-08-19f): the ADV section in PWR2_VALIDATION.md §48 carries a ruling citation. Same acknowledged class.   // 319 -> 321 (2026-08-19e): the 'Defer. A.' ruling cited in PWR2_VALIDATION.md §47 and in pwr2_dumpctl.js, the module built under it. Same acknowledged class.   // 317 -> 319 (2026-08-18b): two more HR11 sites -- the "Option 1" pressurizer ruling cited in PWR2_VALIDATION.md §43 and in the module that was built under it. Same acknowledged class as every prior move.   // 316 -> 317 (2026-08-18): PWR2_VALIDATION.md §42 carries the #489 handoff's owner-ruling citation, and the HR11 provenance scan counts it — the turnover's warned-about baseline move, acknowledged same-change.   // 316 MEASURED ON THE MERGED TREE (2026-08-17, backshop<-develop). The merge conflicted here with backshop at 311 and develop at 286, and the answer is NEITHER: this runner is a STATIC SCAN of source files, so the merged tree carries the union of both lanes' HR11 sites. Taking either side would have silently certified a wrong count against a green gate -- the exact failure CLAUDE.md names for a mechanical BASELINES resolution. Run the runner; do not pick a number.   // 382 -> 385 (2026-08-27): three ruling citations added by the #534 save-path cluster. Trap: the guard cannot tell PROSE ABOUT a citation from a citation — a TUNING_LOG bullet explaining the red reproduced it, 386/1. // 446 -> 454 across #602 (2026-09-01): +8 HR11 citation SITES, not assertions. The runner counts every ruling citation it scans, and #602 carries two rulings quoted in PWR2_VALIDATION.md, TUNING_LOG.md and CHANGELOG.md. All carry the date and the owner's verbatim words, so the tally is 0 undeclared. ⚠ THIS BASELINE DRIFTS WHEN YOU WRITE PROSE, NOT WHEN YOU CHANGE CODE — it moved four times in one effort purely from writing the record, and each move looks like a gate change until you read what the runner counts.   // 446 -> 449 (#602 phase 1, 2026-09-01): +3 HR11 SITES, not +3 assertions — the runner counts every ruling citation it scans, and the #602 ruling is cited once each in PWR2_VALIDATION.md §128, TUNING_LOG.md and CHANGELOG.md. All three carry the date and the owner's verbatim words, so the tally is 433 sites / 0 undeclared. A score that moves when you write PROSE is worth knowing about: this baseline drifts on documentation, not on code.   // 445 -> 446 (#596, 2026-08-31): the aux-spray removal directive cited in CHANGELOG/TUNING_LOG/BUILD_DECISIONS — one more HR11 citation site.   // 441 -> 442 (#532 phase 4, 2026-08-30): the CHANGELOG entry cites the whole-set ruling in the HR11 quoted-option form, which is one more citation SITE for the guard to check. // 438 -> 441 (#528, 2026-08-30): the manual-rod ruling and the keep-it-dark selection, cited across DESIGN_COMPANION §8.36, the manual revision row, the changelog and the tuning log.   // 436 -> 438 (#528, 2026-08-30): the manual-rod-control ruling, cited at its two binding sites — DESIGN_COMPANION §8.36 and the manual revision row.   // 434 -> 436 (#590, 2026-08-29): the steady-state criterion ruling, cited in the validation section and the tuning log.   // 432 -> 434 (#516 items 6-8, 2026-08-29): the item-8 close-out ruling, cited in the validation section and the tuning log.   // 429 -> 432 (#516 item 9, 2026-08-29): the melt-path ruling cited at its three sites — the constant it moves in pwr2_water, the validation section, and the changelog/tuning-log pair.   // 427 -> 429 (#516 Group A, 2026-08-29): the same ruling cited in CHANGELOG.md and TUNING_LOG.md as well as the validation section — three sites for one decision, which is the intended shape.   // 426 -> 427 (#516 Group A, 2026-08-29): one new HR11 citation site — PWR2_VALIDATION §120 records the owner's scope ruling on the playtest list. Written first WITHOUT a quoted string and the gate caught it: a selection from written options still has to quote the option, the CLAUDE.md "a selection, not verbatim words" idiom.   // 423 -> 426 (#580, 2026-08-29): the stage-2 ruling recorded where it binds — the catalog row in pwr_control, PWR2_VALIDATION §119, and the status line in CLAUDE.md. Three citation sites for one ruling is the intended shape: a ruling that lands only in a Blueprint doc has not landed.   // 421 -> 423 (#500/#577, 2026-08-29): two new HR11 citation sites — the 2026-08-28 bundle ruling in pwr_control's alarm rows and the 2026-07-28 status-auto-ack ruling in PWR2_VALIDATION §119. The gate caught one of them UNDECLARED first: a dated OWNER RULING with no verbatim quote is not a citation, and §119 had one.   // 411 -> 414 (#574 DOCS, 2026-08-28): the code half measured 411 and the DOC half of the same change adds three more HR11 citation sites — the wall-scoping ruling and the shrink-the-claims ruling, each cited again in TUNING_LOG and BUILD_DECISIONS. Recorded 411 first, from a gate run BEFORE the write-ups: the file's own trap, and run_all caught it. 0 undeclared.   // 409 -> 411 (#574, 2026-08-28): two new HR11 citation sites, both declared with date and verbatim words — the ruling that scoped the walls to all eleven nodes, and the one that shrank the core-damage claims. 0 undeclared.   // 405 -> 409 (#573/#473, 2026-08-28): four new HR11 citation sites, all declared with date + verbatim words — the 2026-08-12 heater-elevation ruling (built on the retired engine, never on this one) and the 2026-08-28 scoping ruling that took the board half with it. 0 undeclared, as before.   // 402 -> 405 (#536, 2026-08-28): three new HR11 citation sites, all declared with date + verbatim words — the ruling that re-scaled the source-range gauge ("Re-scale the gauges too", from three options put to the owner). 0 undeclared, as before.   // 398 -> 400 (#572, 2026-08-28): the dated ruling that scoped this change ("A" — build the block, from two costed options) lands as new HR11 citation sites. Same acknowledged class.   // 399 -> 398 (2026-08-28): DOWN by one, and drift is symmetric so it is recorded rather than absorbed. CLAUDE.md's 'Built, waiting on review or a close' bullet was cut — all seven issues it listed measured CLOSED — and it carried a dated ruling citation (#458, OWNER RULING 2026-08-12). The rulings themselves are untouched in BUILD_DECISIONS and the issues; what left is one CITATION SITE in the file that is paid for every turn.   // 395 -> 399 (#545, 2026-08-28): the two rulings that scoped the rod-drive fix (#545 + the two one-liners; refuse both directions), both menu selections cited in that form, land as new dated HR11 citation sites — FOUR of them, because each ruling is cited in both PWR2_VALIDATION and the BUILD_DECISIONS/TUNING_LOG pair. Recorded 397 first: that number was measured BEFORE the doc pass, which is the file's own 'do not copy a number written mid-change' trap, caught here by the gate rather than by me.   // 393 -> 395 (#551 + #559 + #567 + #558 + #560, 2026-08-27): the two rulings that scoped the turbine cluster (latch = back on the line, roll stays out under #307; replace the dispatch pair with LATCH / TRIP), both menu selections cited in that form, land as new dated HR11 citation sites. Same acknowledged class.   // 387 -> 393 (#540 + #549 + #541 + #562, 2026-08-27): the two rulings that scoped this cluster (model the overfill wall; the level hold is a control-layer channel), both menu selections cited in that form, land as new dated HR11 citation sites across PWR2_VALIDATION §99, CHANGELOG, TUNING_LOG, BUILD_DECISIONS and CLAUDE.md. Same acknowledged class.   // 385 -> 387 (#539, 2026-08-27): the two rulings that scoped this change (fix the class; adjudicate the reds one at a time), both given as menu selections and cited in that form.  // 381 -> 382 (#523, 2026-08-26): the four cutover rulings land as new dated HR11 citation sites (PWR2_VALIDATION §94, CHANGELOG, TUNING_LOG, BUILD_DECISIONS, CLAUDE.md). Same acknowledged class.   // 380 -> 381 (#518, 2026-08-26): the dated owner ruling "Canary + sub-step" in PWR2_VALIDATION §89 and CHANGELOG   // 379 -> 380 (#517, 2026-08-26): PWR2_VALIDATION §88's and CHANGELOG's dated owner ruling "Build the superheat wing anyway."   // 378 -> 379 (#515 Build 3, 2026-08-26): PWR2_VALIDATION §87's dated owner-ruling citation. Earlier: 371 -> 374 (#513 follow-up, 2026-08-25): the pwr-campaign-split ruling's citation sites (CLAUDE.md, TUNING_LOG, BUILD_DECISIONS) — counted BEFORE the push this time, in the same change. Same acknowledged class.   // 367 -> 371 (#513, 2026-08-25): the session's four new dated ruling citations (the two plan-question rulings in BUILD_DECISIONS/TUNING_LOG, and their echoes in the runner comments) are HR11 sites — CAUGHT BY CI, not locally: the full local gate ran before the doc entries were written, and the doc-gates spot check ran the runner solo, whose own exit code cannot see a count move. Same acknowledged class.   // 366 -> 367 (#514, 2026-08-25): the load-cut owner ruling cited in BUILD_DECISIONS is an HR11 site. Same acknowledged class.   // 365 -> 366 (#511, 2026-08-24): the owner's model-these-valves directive lands as a new dated HR11 citation site (350 sites scanned).   // 364 -> 365 (2026-08-23): the acceptance-windows owner directive ("Let's fix your acceptance windows first.") cited in TUNING_LOG and the endurance runner is an HR11 site. Same acknowledged class.   // 363 -> 364 (2026-08-22c): the #507 batch's dated ruling citations (the SGTR UNVERIFIED-area declaration in pwr2_shell.js, the LOOP row's "Full LOOP + clear") are HR11 sites. Same acknowledged class.   // 362 -> 363 (2026-08-22b): the #458 ruling citation the pwr2 shell's RHR refusal now carries is an HR11 site. Same acknowledged class.   // 360 -> 362 (2026-08-22a): the #506 bundle's two new dated ruling citations (the shutdown-bank directive, the boron-panel ruling) are HR11 sites. Same acknowledged class.   // 357 -> 360 (2026-08-21d): the #501-#504 bundle's three new dated ruling citations (the flat-seed reversal in BUILD_DECISIONS/CLAUDE.md/CHANGELOG) are HR11 sites. Same acknowledged class.   // 356 -> 357 (2026-08-21c): the merge session record's own directive citation is an HR11 site — and it went to origin ungated, because a 'docs-only' push skipped the static scan. The gate-the-exact-commit rule has no docs-only exemption; CI caught it in 40m11s. Same acknowledged class.   // MERGED TREE 2026-08-21 (develop 307 x backshop 334 — a STATIC SCAN carries the union; measured below, not picked — the 2026-08-17 trap).   // 332 -> 334 (2026-08-21b): the CVCS balance-point ruling cited in PWR2_VALIDATION.md sec 64 and TUNING_LOG. Same acknowledged class.   // 330 -> 332 (2026-08-21a): the feed-train ruling ("Due next as recommended") cited in PWR2_VALIDATION.md sec 63 and TUNING_LOG. Same acknowledged class.   // 328 -> 330 (2026-08-20i): the two AFAS rulings ("AFW auto-start" + "Include the trip rung too") cited in PWR2_VALIDATION.md sec 62 and TUNING_LOG. Same acknowledged class.   // 327 -> 328 (2026-08-20g): the A/B-pass ruling citation in PWR2_VALIDATION.md sec 60. Same acknowledged class.   // 325 -> 327 (2026-08-20e): the 'Next: option B' ruling cited in PWR2_VALIDATION.md secs 57-58 and in pwr2_shell.js, the class built under it. Same acknowledged class.   // 323 -> 325 (2026-08-20a): the 'Do option 1' ruling cited in PWR2_VALIDATION.md sec 54 AND in pwr2_instruments.js, the module built under it. Same acknowledged class.   // 322 -> 323 (2026-08-19g): PWR2_VALIDATION.md §49 carries the facade ruling citation. Same acknowledged class.   // 321 -> 322 (2026-08-19f): the ADV section in PWR2_VALIDATION.md §48 carries a ruling citation. Same acknowledged class.   // 319 -> 321 (2026-08-19e): the 'Defer. A.' ruling cited in PWR2_VALIDATION.md §47 and in pwr2_dumpctl.js, the module built under it. Same acknowledged class.   // 317 -> 319 (2026-08-18b): two more HR11 sites -- the "Option 1" pressurizer ruling cited in PWR2_VALIDATION.md §43 and in the module that was built under it. Same acknowledged class as every prior move.   // 316 -> 317 (2026-08-18): PWR2_VALIDATION.md §42 carries the #489 handoff's owner-ruling citation, and the HR11 provenance scan counts it — the turnover's warned-about baseline move, acknowledged same-change.   // 316 MEASURED ON THE MERGED TREE (2026-08-17, backshop<-develop). The merge conflicted here with backshop at 311 and develop at 286, and the answer is NEITHER: this runner is a STATIC SCAN of source files, so the merged tree carries the union of both lanes' HR11 sites. Taking either side would have silently certified a wrong count against a green gate -- the exact failure CLAUDE.md names for a mechanical BASELINES resolution. Run the runner; do not pick a number.   // 382 -> 385 (2026-08-27): three ruling citations added by the #534 save-path cluster. Trap: the guard cannot tell PROSE ABOUT a citation from a citation — a TUNING_LOG bullet explaining the red reproduced it, 386/1.  // 457 -> 460 (#605, 2026-09-02): three more sites for the HR1/HR5/HR11 scanners to read — the loss-of-main-feed arming, its ruling citations, and the pwr2 feed-pump commands. Better than baseline is still drift: acknowledged, 0 failed, 1 declared HR1 debt unchanged. || workbench: 485 -> 486 (#628 follow-up, 2026-09-05): ONE more HR11 site, and it is in CHANGELOG.md -- which trackedMd() scans alongside Blueprint/Diagnostic/Manuals/.claude, a fact easy to forget because the other four are directories. The Alpha 1.7.3-rc1 entry cites the #631 verdict-colour ruling, carries its date and its quoted words, and passes. VERIFIED rather than absorbed: HR11 markers in Blueprint+Diagnostic+Manuals are 382 before and after, and CHANGELOG.md goes 78 -> 79, which is the whole delta.   // 481 -> 485 (2026-09-05, the #629 x #631 merge: develop's +4 sites (#624 item 24) and workbench's +1 (#631) add; measured on the merged tree). Before: 477 -> 481 (#624/#619 item 24, 2026-09-04): +4 HR11 citation SITES, 0 undeclared -- the RATE decision departs from an owner-ruled number, so the 2026-08-08 ruling is quoted verbatim in CHANGELOG, TUNING_LOG and BUILD_DECISIONS (TUNING_LOG twice: once for the rate, once for the raises-only direction it RESTORES). Caught by run_all as a DRIFT: the first draft cited the ruling by DATE ONLY ("an OWNER RULING of 2026-08-08"), which is exactly what HR11 exists to red -- a marker with no verbatim words is not a citation.   // 474 -> 477 (#624/#619 items 14/25, 2026-09-04): +3 HR11 citation SITES, 0 undeclared -- the letdown-split ruling is quoted in CHANGELOG, TUNING_LOG and BUILD_DECISIONS, one site each. Previously 473 -> 474 (#623/#619 item 23, 2026-09-04): +1 HR11 citation SITE, not a new rule -- the 1/M help panel is built to the Scanner's in-place idiom and the TUNING_LOG entry quotes the 2026-08-11 directive that settled that shape, with its date and the owner's verbatim words. This baseline moves when you write PROSE, not when you change code.   // 472 -> 473 (#621/#619 wave 1, 2026-09-03): +1 HR11 citation SITE. The gate CAUGHT the half-citation form first -- a CHANGELOG line read '(OWNER RULING, 2026-09-03, on that exact distinction)', a date with no verbatim words, which HR11 refuses. A selection from written options is cited by quoting the OPTION.   // 471 -> 472 (#618, 2026-09-03): +1 HR11 citation SITE -- the owner's "A" ruling on repointing the ECC/creep gates, cited with its date and verbatim word. Prose, not code, exactly as the note further along this line warns.   // 460 -> 461 (#611, 2026-09-03): +1 HR11 citation, not a new rule. The check counts every OWNER RULING / OWNER DIRECTIVE in tracked markdown and asserts each carries a DATE and the owner's VERBATIM WORDS; the develop-push policy added one to CLAUDE.md. This number moves whenever a ruling is recorded, which is the intended behaviour -- if it moves and you did NOT add a citation, something removed one.   // MERGED 2026-09-02 (develop + workbench): BOTH lanes bumped this for DIFFERENT HR11 citation sites, and the merged tree carries all of them — develop reached 454, workbench 452, and the true number is NEITHER and is not recoverable from the diff. RUN THE GATE, do not choose a side. // 451 -> 452 (#603, 2026-09-02): the owner's "no extra UI elements" directive cited at its site. The gate ALSO caught a citation carrying a date and no verbatim quote -- HR11 wants both, and half a citation is the shape that launders a remembered ruling into a fresh assertion.   // 449 -> 451 (workbench<-develop merge, 2026-09-02): MEASURED, not resolved by picking a side. Both lanes bumped this baseline for DIFFERENT HR11 citation sites — workbench 446->448 (+2: the #598 owner-directive dating, then the item 13 ruling), develop 446->449 (+3: #602 phase 1) — so the merged tree carries all five and the answer is neither branch's number. This is the BASELINES conflict CLAUDE.md names: "a mechanical resolution can take the wrong number silently", and here both sides looked equally plausible in the diff.   // 447 -> 448 (#598 item 13, 2026-09-02): the owner's ruling to promote the control pill back out of the Details fold, cited at its site in ui/app.js with the date and his verbatim words.   // 446 -> 447 (#598, 2026-09-01): one more HR11 site. The gate CAUGHT this one for real — eight owner-directive citations went in carrying only an issue number ("OWNER DIRECTIVE, #598 item 8"), and HR11 wants a DATE with the verbatim quote or the ruling is advisory. Dated, and the site count moved with the one that lives in a scanned file.   // 445 -> 446 (#596, 2026-08-31): the aux-spray removal directive cited in CHANGELOG/TUNING_LOG/BUILD_DECISIONS — one more HR11 citation site.   // 441 -> 442 (#532 phase 4, 2026-08-30): the CHANGELOG entry cites the whole-set ruling in the HR11 quoted-option form, which is one more citation SITE for the guard to check. // 438 -> 441 (#528, 2026-08-30): the manual-rod ruling and the keep-it-dark selection, cited across DESIGN_COMPANION §8.36, the manual revision row, the changelog and the tuning log.   // 436 -> 438 (#528, 2026-08-30): the manual-rod-control ruling, cited at its two binding sites — DESIGN_COMPANION §8.36 and the manual revision row.   // 434 -> 436 (#590, 2026-08-29): the steady-state criterion ruling, cited in the validation section and the tuning log.   // 432 -> 434 (#516 items 6-8, 2026-08-29): the item-8 close-out ruling, cited in the validation section and the tuning log.   // 429 -> 432 (#516 item 9, 2026-08-29): the melt-path ruling cited at its three sites — the constant it moves in pwr2_water, the validation section, and the changelog/tuning-log pair.   // 427 -> 429 (#516 Group A, 2026-08-29): the same ruling cited in CHANGELOG.md and TUNING_LOG.md as well as the validation section — three sites for one decision, which is the intended shape.   // 426 -> 427 (#516 Group A, 2026-08-29): one new HR11 citation site — PWR2_VALIDATION §120 records the owner's scope ruling on the playtest list. Written first WITHOUT a quoted string and the gate caught it: a selection from written options still has to quote the option, the CLAUDE.md "a selection, not verbatim words" idiom.   // 423 -> 426 (#580, 2026-08-29): the stage-2 ruling recorded where it binds — the catalog row in pwr_control, PWR2_VALIDATION §119, and the status line in CLAUDE.md. Three citation sites for one ruling is the intended shape: a ruling that lands only in a Blueprint doc has not landed.   // 421 -> 423 (#500/#577, 2026-08-29): two new HR11 citation sites — the 2026-08-28 bundle ruling in pwr_control's alarm rows and the 2026-07-28 status-auto-ack ruling in PWR2_VALIDATION §119. The gate caught one of them UNDECLARED first: a dated OWNER RULING with no verbatim quote is not a citation, and §119 had one.   // 411 -> 414 (#574 DOCS, 2026-08-28): the code half measured 411 and the DOC half of the same change adds three more HR11 citation sites — the wall-scoping ruling and the shrink-the-claims ruling, each cited again in TUNING_LOG and BUILD_DECISIONS. Recorded 411 first, from a gate run BEFORE the write-ups: the file's own trap, and run_all caught it. 0 undeclared.   // 409 -> 411 (#574, 2026-08-28): two new HR11 citation sites, both declared with date and verbatim words — the ruling that scoped the walls to all eleven nodes, and the one that shrank the core-damage claims. 0 undeclared.   // 405 -> 409 (#573/#473, 2026-08-28): four new HR11 citation sites, all declared with date + verbatim words — the 2026-08-12 heater-elevation ruling (built on the retired engine, never on this one) and the 2026-08-28 scoping ruling that took the board half with it. 0 undeclared, as before.   // 402 -> 405 (#536, 2026-08-28): three new HR11 citation sites, all declared with date + verbatim words — the ruling that re-scaled the source-range gauge ("Re-scale the gauges too", from three options put to the owner). 0 undeclared, as before.   // 398 -> 400 (#572, 2026-08-28): the dated ruling that scoped this change ("A" — build the block, from two costed options) lands as new HR11 citation sites. Same acknowledged class.   // 399 -> 398 (2026-08-28): DOWN by one, and drift is symmetric so it is recorded rather than absorbed. CLAUDE.md's 'Built, waiting on review or a close' bullet was cut — all seven issues it listed measured CLOSED — and it carried a dated ruling citation (#458, OWNER RULING 2026-08-12). The rulings themselves are untouched in BUILD_DECISIONS and the issues; what left is one CITATION SITE in the file that is paid for every turn.   // 395 -> 399 (#545, 2026-08-28): the two rulings that scoped the rod-drive fix (#545 + the two one-liners; refuse both directions), both menu selections cited in that form, land as new dated HR11 citation sites — FOUR of them, because each ruling is cited in both PWR2_VALIDATION and the BUILD_DECISIONS/TUNING_LOG pair. Recorded 397 first: that number was measured BEFORE the doc pass, which is the file's own 'do not copy a number written mid-change' trap, caught here by the gate rather than by me.   // 393 -> 395 (#551 + #559 + #567 + #558 + #560, 2026-08-27): the two rulings that scoped the turbine cluster (latch = back on the line, roll stays out under #307; replace the dispatch pair with LATCH / TRIP), both menu selections cited in that form, land as new dated HR11 citation sites. Same acknowledged class.   // 387 -> 393 (#540 + #549 + #541 + #562, 2026-08-27): the two rulings that scoped this cluster (model the overfill wall; the level hold is a control-layer channel), both menu selections cited in that form, land as new dated HR11 citation sites across PWR2_VALIDATION §99, CHANGELOG, TUNING_LOG, BUILD_DECISIONS and CLAUDE.md. Same acknowledged class.   // 385 -> 387 (#539, 2026-08-27): the two rulings that scoped this change (fix the class; adjudicate the reds one at a time), both given as menu selections and cited in that form.  // 381 -> 382 (#523, 2026-08-26): the four cutover rulings land as new dated HR11 citation sites (PWR2_VALIDATION §94, CHANGELOG, TUNING_LOG, BUILD_DECISIONS, CLAUDE.md). Same acknowledged class.   // 380 -> 381 (#518, 2026-08-26): the dated owner ruling "Canary + sub-step" in PWR2_VALIDATION §89 and CHANGELOG   // 379 -> 380 (#517, 2026-08-26): PWR2_VALIDATION §88's and CHANGELOG's dated owner ruling "Build the superheat wing anyway."   // 378 -> 379 (#515 Build 3, 2026-08-26): PWR2_VALIDATION §87's dated owner-ruling citation. Earlier: 371 -> 374 (#513 follow-up, 2026-08-25): the pwr-campaign-split ruling's citation sites (CLAUDE.md, TUNING_LOG, BUILD_DECISIONS) — counted BEFORE the push this time, in the same change. Same acknowledged class.   // 367 -> 371 (#513, 2026-08-25): the session's four new dated ruling citations (the two plan-question rulings in BUILD_DECISIONS/TUNING_LOG, and their echoes in the runner comments) are HR11 sites — CAUGHT BY CI, not locally: the full local gate ran before the doc entries were written, and the doc-gates spot check ran the runner solo, whose own exit code cannot see a count move. Same acknowledged class.   // 366 -> 367 (#514, 2026-08-25): the load-cut owner ruling cited in BUILD_DECISIONS is an HR11 site. Same acknowledged class.   // 365 -> 366 (#511, 2026-08-24): the owner's model-these-valves directive lands as a new dated HR11 citation site (350 sites scanned).   // 364 -> 365 (2026-08-23): the acceptance-windows owner directive ("Let's fix your acceptance windows first.") cited in TUNING_LOG and the endurance runner is an HR11 site. Same acknowledged class.   // 363 -> 364 (2026-08-22c): the #507 batch's dated ruling citations (the SGTR UNVERIFIED-area declaration in pwr2_shell.js, the LOOP row's "Full LOOP + clear") are HR11 sites. Same acknowledged class.   // 362 -> 363 (2026-08-22b): the #458 ruling citation the pwr2 shell's RHR refusal now carries is an HR11 site. Same acknowledged class.   // 360 -> 362 (2026-08-22a): the #506 bundle's two new dated ruling citations (the shutdown-bank directive, the boron-panel ruling) are HR11 sites. Same acknowledged class.   // 357 -> 360 (2026-08-21d): the #501-#504 bundle's three new dated ruling citations (the flat-seed reversal in BUILD_DECISIONS/CLAUDE.md/CHANGELOG) are HR11 sites. Same acknowledged class.   // 356 -> 357 (2026-08-21c): the merge session record's own directive citation is an HR11 site — and it went to origin ungated, because a 'docs-only' push skipped the static scan. The gate-the-exact-commit rule has no docs-only exemption; CI caught it in 40m11s. Same acknowledged class.   // MERGED TREE 2026-08-21 (develop 307 x backshop 334 — a STATIC SCAN carries the union; measured below, not picked — the 2026-08-17 trap).   // 332 -> 334 (2026-08-21b): the CVCS balance-point ruling cited in PWR2_VALIDATION.md sec 64 and TUNING_LOG. Same acknowledged class.   // 330 -> 332 (2026-08-21a): the feed-train ruling ("Due next as recommended") cited in PWR2_VALIDATION.md sec 63 and TUNING_LOG. Same acknowledged class.   // 328 -> 330 (2026-08-20i): the two AFAS rulings ("AFW auto-start" + "Include the trip rung too") cited in PWR2_VALIDATION.md sec 62 and TUNING_LOG. Same acknowledged class.   // 327 -> 328 (2026-08-20g): the A/B-pass ruling citation in PWR2_VALIDATION.md sec 60. Same acknowledged class.   // 325 -> 327 (2026-08-20e): the 'Next: option B' ruling cited in PWR2_VALIDATION.md secs 57-58 and in pwr2_shell.js, the class built under it. Same acknowledged class.   // 323 -> 325 (2026-08-20a): the 'Do option 1' ruling cited in PWR2_VALIDATION.md sec 54 AND in pwr2_instruments.js, the module built under it. Same acknowledged class.   // 322 -> 323 (2026-08-19g): PWR2_VALIDATION.md §49 carries the facade ruling citation. Same acknowledged class.   // 321 -> 322 (2026-08-19f): the ADV section in PWR2_VALIDATION.md §48 carries a ruling citation. Same acknowledged class.   // 319 -> 321 (2026-08-19e): the 'Defer. A.' ruling cited in PWR2_VALIDATION.md §47 and in pwr2_dumpctl.js, the module built under it. Same acknowledged class.   // 317 -> 319 (2026-08-18b): two more HR11 sites -- the "Option 1" pressurizer ruling cited in PWR2_VALIDATION.md §43 and in the module that was built under it. Same acknowledged class as every prior move.   // 316 -> 317 (2026-08-18): PWR2_VALIDATION.md §42 carries the #489 handoff's owner-ruling citation, and the HR11 provenance scan counts it — the turnover's warned-about baseline move, acknowledged same-change.   // 316 MEASURED ON THE MERGED TREE (2026-08-17, backshop<-develop). The merge conflicted here with backshop at 311 and develop at 286, and the answer is NEITHER: this runner is a STATIC SCAN of source files, so the merged tree carries the union of both lanes' HR11 sites. Taking either side would have silently certified a wrong count against a green gate -- the exact failure CLAUDE.md names for a mechanical BASELINES resolution. Run the runner; do not pick a number.   // 382 -> 385 (2026-08-27): three ruling citations added by the #534 save-path cluster. Trap: the guard cannot tell PROSE ABOUT a citation from a citation — a TUNING_LOG bullet explaining the red reproduced it, 386/1. // 446 -> 454 across #602 (2026-09-01): +8 HR11 citation SITES, not assertions. The runner counts every ruling citation it scans, and #602 carries two rulings quoted in PWR2_VALIDATION.md, TUNING_LOG.md and CHANGELOG.md. All carry the date and the owner's verbatim words, so the tally is 0 undeclared. ⚠ THIS BASELINE DRIFTS WHEN YOU WRITE PROSE, NOT WHEN YOU CHANGE CODE — it moved four times in one effort purely from writing the record, and each move looks like a gate change until you read what the runner counts.   // 446 -> 449 (#602 phase 1, 2026-09-01): +3 HR11 SITES, not +3 assertions — the runner counts every ruling citation it scans, and the #602 ruling is cited once each in PWR2_VALIDATION.md §128, TUNING_LOG.md and CHANGELOG.md. All three carry the date and the owner's verbatim words, so the tally is 433 sites / 0 undeclared. A score that moves when you write PROSE is worth knowing about: this baseline drifts on documentation, not on code.   // 445 -> 446 (#596, 2026-08-31): the aux-spray removal directive cited in CHANGELOG/TUNING_LOG/BUILD_DECISIONS — one more HR11 citation site.   // 441 -> 442 (#532 phase 4, 2026-08-30): the CHANGELOG entry cites the whole-set ruling in the HR11 quoted-option form, which is one more citation SITE for the guard to check. // 438 -> 441 (#528, 2026-08-30): the manual-rod ruling and the keep-it-dark selection, cited across DESIGN_COMPANION §8.36, the manual revision row, the changelog and the tuning log.   // 436 -> 438 (#528, 2026-08-30): the manual-rod-control ruling, cited at its two binding sites — DESIGN_COMPANION §8.36 and the manual revision row.   // 434 -> 436 (#590, 2026-08-29): the steady-state criterion ruling, cited in the validation section and the tuning log.   // 432 -> 434 (#516 items 6-8, 2026-08-29): the item-8 close-out ruling, cited in the validation section and the tuning log.   // 429 -> 432 (#516 item 9, 2026-08-29): the melt-path ruling cited at its three sites — the constant it moves in pwr2_water, the validation section, and the changelog/tuning-log pair.   // 427 -> 429 (#516 Group A, 2026-08-29): the same ruling cited in CHANGELOG.md and TUNING_LOG.md as well as the validation section — three sites for one decision, which is the intended shape.   // 426 -> 427 (#516 Group A, 2026-08-29): one new HR11 citation site — PWR2_VALIDATION §120 records the owner's scope ruling on the playtest list. Written first WITHOUT a quoted string and the gate caught it: a selection from written options still has to quote the option, the CLAUDE.md "a selection, not verbatim words" idiom.   // 423 -> 426 (#580, 2026-08-29): the stage-2 ruling recorded where it binds — the catalog row in pwr_control, PWR2_VALIDATION §119, and the status line in CLAUDE.md. Three citation sites for one ruling is the intended shape: a ruling that lands only in a Blueprint doc has not landed.   // 421 -> 423 (#500/#577, 2026-08-29): two new HR11 citation sites — the 2026-08-28 bundle ruling in pwr_control's alarm rows and the 2026-07-28 status-auto-ack ruling in PWR2_VALIDATION §119. The gate caught one of them UNDECLARED first: a dated OWNER RULING with no verbatim quote is not a citation, and §119 had one.   // 411 -> 414 (#574 DOCS, 2026-08-28): the code half measured 411 and the DOC half of the same change adds three more HR11 citation sites — the wall-scoping ruling and the shrink-the-claims ruling, each cited again in TUNING_LOG and BUILD_DECISIONS. Recorded 411 first, from a gate run BEFORE the write-ups: the file's own trap, and run_all caught it. 0 undeclared.   // 409 -> 411 (#574, 2026-08-28): two new HR11 citation sites, both declared with date and verbatim words — the ruling that scoped the walls to all eleven nodes, and the one that shrank the core-damage claims. 0 undeclared.   // 405 -> 409 (#573/#473, 2026-08-28): four new HR11 citation sites, all declared with date + verbatim words — the 2026-08-12 heater-elevation ruling (built on the retired engine, never on this one) and the 2026-08-28 scoping ruling that took the board half with it. 0 undeclared, as before.   // 402 -> 405 (#536, 2026-08-28): three new HR11 citation sites, all declared with date + verbatim words — the ruling that re-scaled the source-range gauge ("Re-scale the gauges too", from three options put to the owner). 0 undeclared, as before.   // 398 -> 400 (#572, 2026-08-28): the dated ruling that scoped this change ("A" — build the block, from two costed options) lands as new HR11 citation sites. Same acknowledged class.   // 399 -> 398 (2026-08-28): DOWN by one, and drift is symmetric so it is recorded rather than absorbed. CLAUDE.md's 'Built, waiting on review or a close' bullet was cut — all seven issues it listed measured CLOSED — and it carried a dated ruling citation (#458, OWNER RULING 2026-08-12). The rulings themselves are untouched in BUILD_DECISIONS and the issues; what left is one CITATION SITE in the file that is paid for every turn.   // 395 -> 399 (#545, 2026-08-28): the two rulings that scoped the rod-drive fix (#545 + the two one-liners; refuse both directions), both menu selections cited in that form, land as new dated HR11 citation sites — FOUR of them, because each ruling is cited in both PWR2_VALIDATION and the BUILD_DECISIONS/TUNING_LOG pair. Recorded 397 first: that number was measured BEFORE the doc pass, which is the file's own 'do not copy a number written mid-change' trap, caught here by the gate rather than by me.   // 393 -> 395 (#551 + #559 + #567 + #558 + #560, 2026-08-27): the two rulings that scoped the turbine cluster (latch = back on the line, roll stays out under #307; replace the dispatch pair with LATCH / TRIP), both menu selections cited in that form, land as new dated HR11 citation sites. Same acknowledged class.   // 387 -> 393 (#540 + #549 + #541 + #562, 2026-08-27): the two rulings that scoped this cluster (model the overfill wall; the level hold is a control-layer channel), both menu selections cited in that form, land as new dated HR11 citation sites across PWR2_VALIDATION §99, CHANGELOG, TUNING_LOG, BUILD_DECISIONS and CLAUDE.md. Same acknowledged class.   // 385 -> 387 (#539, 2026-08-27): the two rulings that scoped this change (fix the class; adjudicate the reds one at a time), both given as menu selections and cited in that form.  // 381 -> 382 (#523, 2026-08-26): the four cutover rulings land as new dated HR11 citation sites (PWR2_VALIDATION §94, CHANGELOG, TUNING_LOG, BUILD_DECISIONS, CLAUDE.md). Same acknowledged class.   // 380 -> 381 (#518, 2026-08-26): the dated owner ruling "Canary + sub-step" in PWR2_VALIDATION §89 and CHANGELOG   // 379 -> 380 (#517, 2026-08-26): PWR2_VALIDATION §88's and CHANGELOG's dated owner ruling "Build the superheat wing anyway."   // 378 -> 379 (#515 Build 3, 2026-08-26): PWR2_VALIDATION §87's dated owner-ruling citation. Earlier: 371 -> 374 (#513 follow-up, 2026-08-25): the pwr-campaign-split ruling's citation sites (CLAUDE.md, TUNING_LOG, BUILD_DECISIONS) — counted BEFORE the push this time, in the same change. Same acknowledged class.   // 367 -> 371 (#513, 2026-08-25): the session's four new dated ruling citations (the two plan-question rulings in BUILD_DECISIONS/TUNING_LOG, and their echoes in the runner comments) are HR11 sites — CAUGHT BY CI, not locally: the full local gate ran before the doc entries were written, and the doc-gates spot check ran the runner solo, whose own exit code cannot see a count move. Same acknowledged class.   // 366 -> 367 (#514, 2026-08-25): the load-cut owner ruling cited in BUILD_DECISIONS is an HR11 site. Same acknowledged class.   // 365 -> 366 (#511, 2026-08-24): the owner's model-these-valves directive lands as a new dated HR11 citation site (350 sites scanned).   // 364 -> 365 (2026-08-23): the acceptance-windows owner directive ("Let's fix your acceptance windows first.") cited in TUNING_LOG and the endurance runner is an HR11 site. Same acknowledged class.   // 363 -> 364 (2026-08-22c): the #507 batch's dated ruling citations (the SGTR UNVERIFIED-area declaration in pwr2_shell.js, the LOOP row's "Full LOOP + clear") are HR11 sites. Same acknowledged class.   // 362 -> 363 (2026-08-22b): the #458 ruling citation the pwr2 shell's RHR refusal now carries is an HR11 site. Same acknowledged class.   // 360 -> 362 (2026-08-22a): the #506 bundle's two new dated ruling citations (the shutdown-bank directive, the boron-panel ruling) are HR11 sites. Same acknowledged class.   // 357 -> 360 (2026-08-21d): the #501-#504 bundle's three new dated ruling citations (the flat-seed reversal in BUILD_DECISIONS/CLAUDE.md/CHANGELOG) are HR11 sites. Same acknowledged class.   // 356 -> 357 (2026-08-21c): the merge session record's own directive citation is an HR11 site — and it went to origin ungated, because a 'docs-only' push skipped the static scan. The gate-the-exact-commit rule has no docs-only exemption; CI caught it in 40m11s. Same acknowledged class.   // MERGED TREE 2026-08-21 (develop 307 x backshop 334 — a STATIC SCAN carries the union; measured below, not picked — the 2026-08-17 trap).   // 332 -> 334 (2026-08-21b): the CVCS balance-point ruling cited in PWR2_VALIDATION.md sec 64 and TUNING_LOG. Same acknowledged class.   // 330 -> 332 (2026-08-21a): the feed-train ruling ("Due next as recommended") cited in PWR2_VALIDATION.md sec 63 and TUNING_LOG. Same acknowledged class.   // 328 -> 330 (2026-08-20i): the two AFAS rulings ("AFW auto-start" + "Include the trip rung too") cited in PWR2_VALIDATION.md sec 62 and TUNING_LOG. Same acknowledged class.   // 327 -> 328 (2026-08-20g): the A/B-pass ruling citation in PWR2_VALIDATION.md sec 60. Same acknowledged class.   // 325 -> 327 (2026-08-20e): the 'Next: option B' ruling cited in PWR2_VALIDATION.md secs 57-58 and in pwr2_shell.js, the class built under it. Same acknowledged class.   // 323 -> 325 (2026-08-20a): the 'Do option 1' ruling cited in PWR2_VALIDATION.md sec 54 AND in pwr2_instruments.js, the module built under it. Same acknowledged class.   // 322 -> 323 (2026-08-19g): PWR2_VALIDATION.md §49 carries the facade ruling citation. Same acknowledged class.   // 321 -> 322 (2026-08-19f): the ADV section in PWR2_VALIDATION.md §48 carries a ruling citation. Same acknowledged class.   // 319 -> 321 (2026-08-19e): the 'Defer. A.' ruling cited in PWR2_VALIDATION.md §47 and in pwr2_dumpctl.js, the module built under it. Same acknowledged class.   // 317 -> 319 (2026-08-18b): two more HR11 sites -- the "Option 1" pressurizer ruling cited in PWR2_VALIDATION.md §43 and in the module that was built under it. Same acknowledged class as every prior move.   // 316 -> 317 (2026-08-18): PWR2_VALIDATION.md §42 carries the #489 handoff's owner-ruling citation, and the HR11 provenance scan counts it — the turnover's warned-about baseline move, acknowledged same-change.   // 316 MEASURED ON THE MERGED TREE (2026-08-17, backshop<-develop). The merge conflicted here with backshop at 311 and develop at 286, and the answer is NEITHER: this runner is a STATIC SCAN of source files, so the merged tree carries the union of both lanes' HR11 sites. Taking either side would have silently certified a wrong count against a green gate -- the exact failure CLAUDE.md names for a mechanical BASELINES resolution. Run the runner; do not pick a number.   // 382 -> 385 (2026-08-27): three ruling citations added by the #534 save-path cluster. Trap: the guard cannot tell PROSE ABOUT a citation from a citation — a TUNING_LOG bullet explaining the red reproduced it, 386/1.  // 457 -> 460 (#605, 2026-09-02): three more sites for the HR1/HR5/HR11 scanners to read — the loss-of-main-feed arming, its ruling citations, and the pwr2 feed-pump commands. Better than baseline is still drift: acknowledged, 0 failed, 1 declared HR1 debt unchanged.
  ...
  /* 52 -> 67 (#591 items 1-3 + #564 items 1-2 + #578, 2026-08-31): the owner's playtest and the board half of the #534 sweep. FOUR groups. THE CW INLET BOX (#591.1/#592) was dark behind a refusal carrying the RETIRED plant's reason while pwr2_condenser had computed vacuum from that temperature all along — and the discriminator is not the number (the band is the same sourced 35-85 degF) but the PROVENANCE: the check moves the plant's published band and the box must follow, which a board reading _TB at script load cannot. THE PZR SPRAY BOX (#564.1) read DELIVERED flow as both the operator's demand and the `asked` half of its own amber, so the one state that indication exists for was an identity that is always zero; measured both ways on this plant (a standing 60 % demand read 0.0 once the pressurizer went solid; a stuck valve read back 100 % the player never asked for). THE TRIP BLOCKS PANEL (#564.2): five rows drawn, three published, so lo_flow/rcp_breaker/ir_high rendered ENABLED in every state and could only throw - and si_trip, a block this plant DOES carry, had no row at all. THE DELTA-T TILE (#578) now reads ts.rod_stop / ts.runback_active, which nothing in ui/diagram/board/ had ever read. Mutations 17 -> 22. */
  'run_pwr2_board.js':     { code: 0, score: '82passed 0failed 82checks', secs: 101 },   // 80 -> 82 (#699, 2026-09-10): the ECCS pump on the RHR branch. RHR has no pump of its own -- it is a suction ALIGNMENT on the shared ECCS train -- and the impeller keyed on injection flow, which is ZERO on a cooldown, so the only pump art on the mimic was drawn STOPPED through the whole of Mode 4 and Mode 5. One check rides a cold_shutdown RHR cooldown and asserts it turns; the SECOND is the load-bearing one and asserts it STOPS on a dead bus WITH `rhr_valve_open` still true, because gating on the alignment BUTTON would satisfy the first and re-create the spinning-rotor-on-a-dead-bus defect #350 items 7/13/15 removed from three other pumps. Two mutations, both caught (29 total): the board reverts to injection-only, and the SHELL stops publishing the delivered flag.   // 77 -> 80 (#624/#619 item 24, 2026-09-04): THE 17 % LETDOWN ISOLATE REACHES THE BOARD. `control_state.letdown_isolated` was published correctly and had ZERO consumers anywhere in ui/, so while the cut stood the four orifice buttons kept lighting the operator's last SELECTION over two shut valves. +3: all four lamps go dark and CLOSED carries the amber warn with the card's new status word reading ISOLATED; the positive half (a selected lineup still lights) so the check cannot pass on a dead board; and the dark lamps are not a dead panel — pressing an orifice re-lines and re-lights. +2 mutations. The lamp mutation has to revert ALL FOUR `active` functions: with only CLOSED reverted the standing selection is A, so CLOSED stays dark for the right reason by accident and the replay came back BLIND (measured).   // 71 -> 77 (#598 items 11 and 15, 2026-09-01): THE LEVEL TILE'S NORMAL BAND IS THE PLANT'S PROGRAM, not the authored flat 40-70 — a FULL-POWER band drawn on every mode, so a Mode 3 or Mode 5 plant whose level program is 25 % had its perfectly-on-program level painted 15 points BELOW normal. That is what the owner filed as "CHARGING in AUTO doesn't try to hold a decent PZR level": measured, charging in AUTO tracks the program to within 0.11 % in all four initial conditions with NO hunt, so the controller was right and the band was wrong. The pair is deliberate — centring at full power, and the Mode 5 case as the DISCRIMINATOR, because at full power the program sits inside 40-70 and the centring check alone still passes on the shipped defect. +1 mutation (the band reverts to the authored literals). The harness also gained bindWorld(): the Mode 5 check needs a second plant, and the driver's single ctxRef re-pointed at it reddens the no-orphan sweep with five "silent" buttons — the trap this file's own section note records, hit a second time.   // 67 -> 71 (#600, 2026-09-01): the TRIP BLOCKS panel's presence guard has no live subject any more — every drawn row is a published one, because the two rows offering an AUTOMATIC P-7 block were deleted and the intermediate-range trip the third drew was built. The guard is kept for the preview channel's retired engine and PROVEN BY INJECTION instead: one check deletes a row's trip_block_status entry from a snapshot and asserts that row darks with its neighbours untouched; one asserts the new IR row is live at the plant's own 25 %. +3 more for the CAPTIONS, which could not be checked at all before because they existed only as a DOM write: the flux rows carried a literal "25%" over a 35 % plant and the pressure row read the static pwr1 table's 1800 psi over a published 1775, while the power TILE was already drawing the right number from the same snapshot. The caption is a field on the row now. The third pins the NO-FALLBACK rule — a plant publishing no setpoint prints no number, because hard-coding this plant's 35/25 there is the #557 defect wearing the fix's clothes, and the retired engine the preview channel boots publishes entries with no `setpoint` field. All three proven by injection.   // 50 -> 52 (#516 item 7, 2026-08-29): THE PIPES SAY WHAT IS IN THEM. The primary runs declared `contents: 'water'` unconditionally, so a voided loop drew as coolant — and temperature could not say otherwise, because once the loop saturates BOTH legs sit at T_sat(P). That equality is CORRECT physics (measured equal to the decimal through a whole LOCA while the legs differed 0.35 in quality) and was left alone; the phase now comes from the per-leg void instead. The first draft of the check was a SOURCE SCAN — the hollow shape the standing trap list names — and was replaced with a behavioural one driving compProps at void 0.00 and 0.95. Mutations 16 -> 17.   // 48 -> 50 (#516 item 6, 2026-08-29): THE VESSEL DRAWS A LEVEL, NOT AN INVENTORY. Its water column was fed `core_inventory_pct` = M_total/M_nominal RCS-wide, which is not a level — early in a blowdown the mass collapses because the water FLASHES while the column does not. Measured on a 20 cm2 break with injection secured: 17.3 % mass against a core 69 % uncovered at 90 s, and 0.74 % (an essentially dry vessel) against 9 % still covered at 630 s. Fed from `core_uncovered_frac` below the half-way split and the hot-leg void above it. The discriminating check holds the mass fraction CONSTANT across a covered and a dry core, which the old wiring answered with one identical number. +1 mutation.   // 43 -> 48 (#516 items 1+11, 2026-08-29): the owner's own playtest. THE CHARGING BOX'S CEILING was `RD.PWR_CONFIG.reactivity.charging_max` captured at script load — the RETIRED engine's 60 gpm — while the shell clamps the demand against THIS plant's 30.14, so the top half of the range was one indistinguishable value under a caption reading "0-60 gpm"; three checks (bound, the not-60 discriminator, and the caption, which is authored in GENERATED board data and so had to become derived). THE SG FEED BOX read back `feed_pump_speed_pct` — the DELIVERED fraction, behind the pump lag — so each arrow click re-anchored the demand onto a lagging number: measured, eight +1 gpm clicks moved it +0.5 gpm. Two checks drive the renderer's own numberFor -> onNumber path. +2 mutations (the ceiling publication severed; the box reads delivery again).   // 41 -> 43 (#576c, 2026-08-29): the PRIMARY PRESSURE tile, which this file had never asserted against a PWR2 plant — its own note said the power tile was the only band check here. The pair is deliberate: the first says the band equals the running plant's setpoint plus its OWN published ladder (computed from pwr2_pressurizer.CONTROL, never retyped), the second says it is SYMMETRIC at +-25 psi, which is the discriminator — a band centred right and 30/50 wide passes the first and fails the second, and 30/50 is exactly the retired plant's. +2 mutations (the plant stops publishing its band; the plant stops publishing its live level program) and the #500 level-tile check rewritten to assert the edge MOVES with the program rather than pinning the number 17.   // 39 -> 41 (#571, 2026-08-28): a check here was PINNING the defect — 'reset under a standing LOCA is accepted and the protection RE-LATCHES within 3 s'. Its real concern (not a WEDGE — a reset that can never be satisfied is the #509 dead-button class) is kept and now actually asserted, via the sourced P-11 release; the accepted-reset half is replaced by the refusal plus the reset_block reason the board's caption maps to. The pair fails on the old build for the right reason, so it is a strengthening, not a refit (HR10).   // 38 -> 39 (#570, 2026-08-27): the sweep also fails an enabled button that THROWS FROM INSIDE A MAPPED HANDLER, not just one whose action is in the REFUSED registry. STEAM DUMP OPEN was exactly that and both gates written for the class were blind to it. Refusals that are CONDITIONAL (RHR's sourced permissive, the turbine latch's six holds, the AFW reset window) are declared WITH the condition — an entry without one is a control that can never work.   // 37 -> 38 (#567, 2026-08-27): the no-orphan sweep stops counting a THROWN REFUSAL as a live control. It accepted ok / blocked / error-WITH-A-MESSAGE, and a button whose action is in PWR2's REFUSED registry satisfies that third clause with developer jargon — five did. The plant half of the claim (the capability flag exists) is run_pwr2_kernel band 4's; this is the board half (the control reads it and renders dark).   // 36 -> 37 (#540 + #549 + #541 + #562, 2026-08-27): the AFW-scale fixture gained a DISCRIMINATING-fixture precondition. It reddened on this change for an honest reason and the FIXTURE was stale, not the board: it derived 'true' flow by summing the RATED flows of whichever pumps were running — a second copy of the delivery law — and delivery gained a THROTTLE term, which the shipped `afw_level` channel holds shut above its band. It now reads the plant's own afw_flow_normalized, opens the valve so the ratio is not vacuous, and asserts the one-pump fixture is still 0.333 rather than 1.0.   // 28 -> 36 (#557 + #556 + #561, 2026-08-27): THE BOARD READS THIS PLANT, not a second copy of a constant — the AFW gauge scales on the plant's own published 86.2 gpm rating (was a 640 gpm literal, 7.40x out), the pressurizer tile arms at the engine-carried 87 % high-level scram and collapses the low band the plant has no trip for (the 17 % alarm edge came with it, unfiled), and the delta-T margin gauge takes the plant's own sourced Ginna Table 15.0-7 equation instead of the retired fitted DNB surface. Mutations 6 -> 11; each new check has a paired revert, and both ENDS of the publish are mutated separately so a board agreeing with a constant by luck still reds. Harness trap recorded in the file: a second mkWorld() rebinds the driver's single ctxRef and sends the FIRST world's presses into the second's log — five working buttons read as silent.   // 26 -> 28 (#512, 2026-08-25, owner design): PER-SYSTEM UNLATCH — a securing click inside the sourced 45-60 s reset window refuses with the time remaining (WTSM 12.3.2.3's time-delay relay + P-4); after the relay ONE click resets the function and secures the pump SIGNAL PRESENT OR NOT (what keeps a deliberate TMI-style termination reachable — owner requirement), with automatic re-actuation blocked until the signal clears; the per-system ACTUATED lamps publish (si/afas/fwi/heater-shed); RPS RESET is trip-only. Mutations 5 -> 7 (delay-0, reset-click severed; the payload anchor re-pointed into the shared guard; the timer-accrual mutation deliberately ABSENT — the engine captures the protection module at load so a stepProtection mutation cannot reach the running plant, and the relay-met check's own fixture defends the timer wire).   // 24 -> 26 (#511, 2026-08-24): the item-11 check FLIPPED — all four clickable valves read OPERABLE (the disabled statics retired); the MSIV round-trips from the diagram and closing it at power trips the turbine (the mirror then reports cause "turbine_trip" — P-9); the accumulator valve refuses AT POWER with the sourced >1600 psig power-removed lock (B 3.5.1).   // 13 -> 24 (#509, 2026-08-24): THE TRIP RIDE — the kernel's rps latch LEARNS an engine-owned automatic trip (the evaluate mirror; before it resetRps returned null for ever and every SI/FWI seal-in was unreleasable), ECCS STOP / AFW STOP / MFW RESTORE refuse OUT LOUD against a standing seal-in (WTSM 12.3.2.3), a reset with rods seated is ACCEPTED (the stale-snapshot "rods not inserted" lie retired), reset under a standing LOCA re-latches rather than wedging, the AFW block valve round-trips BOTH ways off the board's own {open} payload with a live lamp, charging OFF zeroes the FLOW (not just the lamp), and MSIV/accumulator read disabled art (registered statics) while AFW stays operable. Mutations 2 -> 5 (scram mirror severed, reset snapshot patch severed, set_afw_block payload revert), all caught.   // 11 -> 13 (#507 wave 7): the engine-owned block surface merged through the kernel (standing at-power block, setpoint 35 on the snapshot) + the WHOLE button path in one check — unblocking the low-flux trip at 100 % scrams on the spot, cause hi_flux_lo, on a FRESH world (the sweep's own world is already lo_flow-scrammed and would satisfy it vacuously). The tile-presence mutation RETIRED as superseded (the merge makes it invisible on a live pwr2 ride; the class rides verify_board_check's TRIP-35 fixture + the shell setpoint mutation) and replaced by the merge-severed mutation.   // NEW (#506, 2026-08-22): the pwr board driver over a REAL pwr2 SimulationService, headless — the seam the playtest broke and no gate stood on. Pump props finite (the RCP froze on a NaN spin speed) · power tile authored bands (was "TRIP 25%" at 99.8 %) · the payload round trips (HPI STOP secures, heater MANUAL lands, letdown lamps latch, dump CLOSED reads, charging OFF reads, S/F rod speeds measurably differ) · THE NO-ORPHAN SWEEP: every button item handled+acknowledged, momentary, UI-local or disabled (DESIGN_CRITERIA Q4 as a check). 2 mutations (tile presence condition, set_hpi payload revert), both caught.
  'run_diag_bundle.js':    { code: 0, score: '70checks 0failed', secs: 12 },   // 52 -> 70 (#702, 2026-09-10): TR-11, PWR2's own recorder field list. fieldsFor('pwr2') was falling back to FIELDS.pwr -- ten channels, none reactivity -- so a boron/xenon/rod-driven defect (report mtsmvirv-yav1uix2) could not be diagnosed from its own recording. PWR2 now gets boron_ppm, rod_steps, xenon_pct_eq, reactivity_pcm on top of the ten pwr channels, injection-verified on a real full-stack tick()-driven PWR2 recording (all four finite and plausible, not NaN/placeholder) and wiring-checked that ui/app.js keys the recorder off engId() rather than ui.plant (which stays 'pwr' for both engines). Payload cost measured on a synthetic full 14,400-row ring: 1,595 KB (78 % of the 2 MB cap) against 1,133 KB (55 %) at 10 fields, +41 %, matching the issue's own ~+40 % estimate -- still under budget, so option (a) (add and re-measure) stands with no field dropped.
  // 35 -> 52 (#681, 2026-09-10): TR-10, the wire size of the bundle. The ring was bounded in ROWS and never in BYTES, and the bytes were not history -- they were digits: 432,000 doubles written to 17 significant figures put a 4-plant-hour report at 2,939 KB against the Worker's 2 MB cap, so every report from 2 h 45 min of plant time onward was answered 413 and the player was told to email. +17: the rounding rule asserted on the SERIALISED digits rather than on the constant, a sub-unity value keeping 6 significant digits (a flat 4 dp quantises a decay-heat steam flow by 6 %), the recorder still holding full precision in memory and build() handing out COPIES, a full ring gzipping under the wire budget, and the oldest-first byte trim -- exactly what it was asked for, on every column, the manifest saying so, stopping at the floor and then reporting 0 so a caller loop terminates. The load-bearing one is the FIXTURE CANARY: the same rows UNROUNDED must still be over the cap, or a smoother fixture quietly stops exercising the defect and the green means nothing. Injection-verified four ways: rounding removed -> 2 red, a flat 4 dp -> 2, trimOldest a no-op -> 5, trimOldest dropping the NEWEST -> 1.
  // #613 wave 3, stream 2: the bug-bundle performance blocks. `ui/perf.js` used to report
  // only the app's OWN paint cadence, which cannot tell "the compositor is back-pressured"
  // (expensive board) from "the browser is withholding frames" (backgrounded/software-
  // rendered tab) — env/visibility/raf/loaf split that apart. Hand-rolled DOM stubs (no
  // jsdom in this tree), same `Object.defineProperty` technique as run_telemetry.js and the
  // same indirect-`eval` mutation harness as run_warp_tier.js. Found ITS OWN bug on first
  // run: `hiddenSince`/`blurredSince` used 0 as a "not active" sentinel, which silently ate
  // the whole hidden interval when a tab went hidden at t=0 — fixed to a null sentinel.
  // 72 -> 81 (same-day review): three fixes. (1) The WebGL renderer probe — creating a
  // context spins up GPU-process state, tens of ms, for a value only a bug report reads — now
  // runs lazily from the FIRST summary() call only (never broadcast()), is cached, and
  // releases the context via WEBGL_lose_context; +5 checks proving the timing/caching/release
  // with a call-counting canvas stub. (2) The BACKGROUNDED verdict branch keys on hidden_ms
  // ALONE, not hidden+blurred — Chrome/Edge throttle rAF for hidden/occluded tabs, not a
  // merely-unfocused window, and the owner types in another window while the sim runs; a
  // blur-inclusive share would misattribute a real raster problem to "the tab". +2 checks (a
  // long blur alone must NOT read as backgrounded) +1 mutation (the share re-widened to
  // include blurred_ms). (3) `loaf.style_layout_*` renamed nowhere (key unchanged) but
  // documented honestly as style+layout+PAINT RECORDING, since the LoAF window it measures
  // (styleAndLayoutStart -> frame end) contains all three and the spec draws no finer line.
  // 81 -> 95 (#631, 2026-09-04): the verdict learns the PACING TIER. At the new 70 ms WARP step
  // budget the physics is MEANT to use ~70 % of the broadcast, which tripped the COMPUTE-BOUND
  // threshold and told the player to slow down on a plant behaving exactly as designed — the
  // same "amber on every machine at the top rung" failure the achieved-rate ruling names. The
  // branch keys on the budget, not the tier alone, so WARP well past what it was allowed still
  // reads as the fault. +2 mutations. TRAP recorded in the fixture: the first cut's PLAY
  ...
  'run_events.js':         { code: 0, score: '41checks 0failed', secs: 1 },   // 40 -> 41 (#605, 2026-09-02): the CONTROL_LABEL_MAP scan was a fixed 4,000-BYTE WINDOW off the literal's start, so a label past it read as missing and a COMMENT could red the gate without any label moving (it did). Bounded on the literal's closing brace now, plus a check that the literal parsed at all.
  ...
  'run_pwr2_kinetics.js':  { code: 0, score: '94passed 0failed 94checks', secs: 10 },   // 82 -> 94 (#536, 2026-08-28): THE NEUTRON SOURCE, which did not exist — a subcritical core was a pure decaying exponential and hot standby fell 3.6031e-5 % -> 6.3798e-8 % untouched in 300 s while the board read -0.341 dpm / -76 s. The retired engine's constant could NOT be ported (it is tied to that engine's 500x-inflated Lambda), so it is derived from this plant's rated neutron population; the integration is augmented 8x8 because a source makes the system affine. 45 -> 54 mutations. The critical-hold check was SPLIT rather than widened: the integrator claim runs at S = 0, the plant's creep is its own check.   // 71 -> 82 (#515 Build 3, 2026-08-26): the void/boron form — a LIQUID moderator reference (the old one was two-phase below 9.145 MPa), the boron factor floored at the sourced +5 pcm/degF envelope, the void term capped by the boron worth it can remove; theorem void + boron <= -K*deficit at 45 states; 45 mutations (was 40). The identity check is a declared REFIT (its expectation path WAS the artefact path). Earlier: 50 -> 63 (2026-08-16): the direct BORON worth term was missing entirely and the gate scored 50/50 with 25/25 mutations against it -- mutation testing perturbs code that exists and is structurally blind to a term nobody wrote. Added the term, the rho_excess solve pinned to the 975 ppm BEAVRS anchor, and a cross-module tie to pwr2_fuel's derived Doppler reference.   // +8 (#479, 2026-08-17): THE VOID HALF of the density coupling, which did not exist. moderatorReactivity takes a TEMPERATURE and rebuilds a density off the LIQUID branch, so a boiling core was invisible to it: on a depressurising plant the coolant follows saturation down and the term read 'colder, therefore denser' and inserted +3433 pcm at t=60 s of a 50 cm2 break -- five times prompt critical, power 0.8 % -> 4.7e+12 in ONE step. A PWR is undermoderated; voiding shuts it down. No new constant: the deficit is measured against saturated liquid with the same calibrated modCoeff, EXACTLY zero on a subcooled core, and measures -89.4 pcm per % void by volume.
  /* 37 -> 36 (#591 item 1 / #592, 2026-08-31): A CHECK CORRECTLY LOST. Band 4 asserts one row per action in PWR2's REFUSED registry; `set_condenser_cw_temp` left that registry for MAPPED, so there is no longer a refusal to darken. Its DARKENED map entry was deleted with it rather than left as a row nothing can reach. Replaced by 3 checks in run_pwr2_condenser and 4 in run_pwr2_board. */
  'run_pwr2_kernel.js':    { code: 0, score: '36passed 0xfail 0failed 36checks', secs: 22,   // 34/5xfail/39 -> 37/0/37 (#551 + #559 + #567, 2026-08-27): ALL FIVE board-vocab xfails PROMOTED. The grid FOLLOW/MAN dispatch pair is the turbine LATCH/TRIP pair (there is one dispatch mode, and MAN threw twice per press); SR DET and the condenser CW temp box darken off capability flags the plant now publishes. Band 4's claim was also RE-EXPRESSED: a REFUSED action is only a defect if the player can REACH it, so it asserts the darkening FLAG rather than the action's absence from the registry — the board half of that claim is run_pwr2_board's.   // NEW (#546 + #547, 2026-08-27): DOES THE CONTROL KERNEL PASS ON WHAT THE OPERATOR SENT? PWR2 kept the retired plant's failure table BY REFERENCE and seven rows are `command_override` — the kernel's licence to drop or rewrite a command, in the old plant's action names. Measured before the fix: 18 divergences over 1,408 (casualty row x board action) pairs; after, 6, every one of them the ENGINE refusing out loud with the arrived command byte-identical. run_pwr2_forwarding asks this question of the engine's internal layers; nothing asked it of the kernel, which is why the ATWS pushbutton and the stuck-PORV close were both dead through the shipped stack.
    note: 'THE 5 XFAILS ARE BAND 4 AND ARE NOT THIS ISSUE — board controls that send a command PWR2 REFUSES, so the press can only throw (Grid MANUAL sends connect_grid AND set_load_mode, two throws per press; SR DET; the condenser CW temp box; the ADV setpoint box). They are the BOARD still speaking the retired plant\'s vocabulary, tracked separately, and they are born failing here because run_pwr2_board\'s no-orphan sweep is HOLLOW for this case — it accepts a press whose result is "an error WITH a message". Strict: promoting a fix without promoting its xfail reds this runner.' },
  ...
  'run_pwr2_reactor.js':   { code: 0, score: '42passed 0failed 42checks', secs: 15 },   // 4th re-measure (#515 Build 3, 2026-08-26): the climb sample 20 -> 51 s and its bound 5 -> 1 dpm — the +12 dpm / 477 % ring was the two-phase reference's spurious insertion below 1,600 psia; count unchanged. Earlier: SUR settle sample 38 -> 60 s (stage 2a, 2026-08-19): the sourced 17 % heater cut removes pressure support mid-outsurge and the unprotected adversarial scram-recovery fixture now RINGS (second excursion to 477 % at ~32 s, settled ~100 % by 56 s -- a real RPS would have tripped at the first spike); mechanism checks unchanged.   // FIXTURE PRESSURIZED (#479 pzr, 2026-08-18): the default fixture carries pwr2_pressurizer in the extraMass seat -- the saturation-riding check RE-POINTED A THIRD TIME, back to its ORIGINAL subcooled-outlet form, now true for the right reason (~2226 psia, 45 degF subcooling); the void-half check keeps a DELIBERATE rigid fixture (a boiling core is its subject); the SUR scram-recovery samples re-measured (the vessel fights the depressurisation, recovery arrives ~8 s later -- 20 s/38 s samples, mechanism unchanged).   // +1 (#490, 2026-08-18): coreRegime hands true HOMOGENEOUS VOID FRACTION onward, not quality -- the 40 % fixture cannot tell them apart (0.40 vs 0.80, both clear 0.3), so a 1.53 % quality fixture pins the 8.37 % alpha against independent volume algebra; a new mutation replays the shipped defect and reds 10 checks.   // +7 (#479, 2026-08-17): reactor period + startup rate, derived from the fission signal, no new sourcing needed -- SUR*period recovers 60/ln(10) exactly as a pure identity, and the over-cooling recovery scenario already in this file shows -3.37 dpm falling / +12.28 dpm climbing back through criticality / 0.08 dpm re-settled.   // +1 (#479, 2026-08-17): the 'core node above the loop reference' check was reading a SATURATION temperature and calling it a core outlet -- measured with the void term disabled, the fixture settles at 11.098 MPa with the core node exactly at T_sat and 3.28 % void. Re-pointed to assert saturation (passes on BOTH plants) plus a void-coefficient sizing check. The cause is the fixture having no pressure control; #472 owns it. +5 more (2026-08-17): coreRegime -- the function that tells pwr2_fuel whether the rods are being cooled -- landed with NO direct coverage and could not have got any by accident, because every fixture here runs at rated flow on a subcooled core, so a version returning 'fully cooled, no void' unconditionally agrees with all of them. Three mutations now pin it, plus a cross-module check tying MDOT_RATED to pwr2_sources.PUMP.mdot_rated (the same number written down twice, the protection-cadence failure mode). // 41 -> 42 (#583, 2026-08-28): the void-half check's SUBJECT was manufactured by the defect -- the rigid fixture rode saturation at 8.55 MPa only because the phantom pressurizer node ballasted it, and runs to the 0.1 MPa floor without it. The void state is constructed and NAMED now, and a second check pins that it SCALES with the void
  'run_pwr2_turbine.js':   { code: 0, score: '29passed 0failed 29checks', secs: 2 },
    'run_pwr2_dumpctl.js':   { code: 0, score: '25passed 0failed 25checks', secs: 1 },   // 23 -> 25 (#647, 2026-09-06): THE TURBINE-TRIP BAND IS DERIVED FROM THIS PLANT'S OWN SPAN. tt_full_c = tavg_full_c - tavg_noload_c = 33.10 degF (18.39 degC), was the typed WAT-plant 27.7 *(OWNER RULING, 2026-09-06: 'A' - keep 580.1 degF, re-derive the dependents)*. Full demand now arrives AT full-power Tavg instead of 5.4 degF short, the structure the source itself uses (27.7 = 584.7 - 557 on the WAT plant). Cost, reported not tuned: post-trip margin to the 1085 psig pop 63.4 -> 52.3 psi, 9.5 of it the band's; safeties do not lift, ADV stays shut, 0 lbm vented. // 22 -> 23 (#508, 2026-09-05): THE NO-LOAD Tavg ANCHOR IS THE PLANT'S OWN STEAM SIDE. tavg_noload_c moved 291.67 -> 286.11 degC (557 -> 547 degF) *(OWNER RULING, 2026-09-05: '547 degF - re-anchor to Ginna')*. The +1 is the check that had never existed: the anchor must AGREE with the temperature the engine boots every no-load IC at (T_sat of the module's own 7.03 MPa = 286.113 degC, band 0.2) -- the 10.01 degF disagreement WAS the defect and nothing asserted it. Both Tavg-mode dump controllers were dead at the plant's own no-load point, so a routine turbine trip parked on the ATMOSPHERIC dump valve at 39.9 % venting 18,813 lbm in 30 min with the condenser dumps shut; after, ADV 0.0 %, dumps 8.75 %, 0 lbm, and the margin to the 1085 psig MSSV pop goes 7.7 -> 51.2 psi (63.1 with #633's pressure-dependent relief). Two fixtures were BUILT AT THE ANCHOR and re-expressed rather than re-valued (Tref-at-zero-load re-pointed at the constant; the C-8 half-band written as tavg_noload_c + tt_full_c/2, verified 0.500 at BOTH anchors). // NEW (ruled §42 criterion A, 2026-08-19): the steam dump control system, WTSM 11.2 (ML11223A294, fetched into this lane's corpus) + WAT 05 bands -- three controllers, three arming signals, the 5 degF deadband, and the C-7 rate unit's 120 s lag DERIVED from the two sourced thresholds' mutual consistency (a 30 s guess read a clean 10 % step as 20 %/min). 8 mutations incl. 'C-7 always armed', the old engine's hidden-parallel-sink defect, none blind.
  'run_pwr2_pressurizer.js':{ code: 0, score: '103passed 0failed 103checks', secs: 130 },   // 101 -> 103 (#647, 2026-09-06): THE PRECONDITION WAS A DEFECT IN IC CONSTRUCTION. The plant settled 2.3 degF (1.3 degC) below its own 580.1 degF design point and the level program never reached 61.5 %. Decomposed: Tref exactly 580.10, load fraction exactly 1.0000, so the program was right and the PLANT was under it -- the whole gap was the SG 16.8 psi under design, pinned by criticality (0.003 degF spread across an 80 psi boot sweep). Root cause: createReactor seeded the fuel against the LEG AVERAGE while stepReactor drives it against the CORE node, 18.1 degC hotter -- 15.5 degC of Doppler the boron trim never paid for, returned as 1.3 degC of moderator cooling. Fixed at the seed. All three at-power ICs now land inside 0.15 % of program (hot_full_power 580.23 degF / 61.55 % / 825.9 psia). +2: the two files' FULL-POWER knot equality (nothing watched this half; #645 gated only no-load) and the settled plant reaching its knot. // 100 -> 101 (#645, 2026-09-06): NET +1 from +2 -1. THE LEVEL PROGRAM'S NO-LOAD KNOT NOW MATCHES THE Tavg PROGRAM'S. tavg_noload_c moved 291.67 -> 286.11 degC (557 -> 547 degF) *(OWNER RULING, 2026-09-06: 'Move the pressurizer copy too')*, completing what #508 started in pwr2_dumpctl. The two copies described DIFFERENT coolant expansions and the error peaked at -10.95 points at 0.30 dispatch, zero at both ends -- invisible to two investigations because the program CLAMPS at 25 % across the whole 547-557 degF band, so the no-load point reads an identical 25.00 % under either anchor. +2: the bare literal (the decision) and the INVARIANT the defect violated -- PZ.LEVEL.tavg_noload_c === DC.DUMP.tavg_noload_c, one plant quantity in two files, and the only check that can see a split. Injection-discriminated: reverting the pzr copy reds BOTH, moving only the dumpctl copy reds ONLY the agreement. -1: the low-level-cut check was on one physical line TWICE, so the tally read one above the coverage. A THIRD check was HOLLOW and only the re-anchor exposed it -- 'the level controller MOVES charging when letdown drains the vessel' set cv.letdownOpen 1 to 1 (it DEFAULTS to 1), drained nothing, and was really measuring the level PI unwinding a CONSTRUCTION offset that the re-anchor removed: it went red on a change that made the plant more correct. Replaced with an operator-reachable stimulus (isolate letdown, vessel fills, controller must CUT charging), asserted on demand AND delivered flow, and measured at BOTH anchors so it passes on the old plant too. // 97 -> 100 (#516 item 10, 2026-08-29): "charging in auto doesnt hold the pzr level". The level PROGRAM is a function of Tavg and the caller wires it to the INDICATED channel, correctly (HR1) — but it read that channel RAW, and the program's 2.845 %/degC slope amplifies noise onto the SETPOINT, where the level PI structurally cannot reject it. Measured at steady full power: TRUE Tavg spanned 0.022 degC, INDICATED 0.63, the published program swung 1.77 % and charging hunted 0 to 17 gpm. A 25 s lag on the reference (the pwr2_feedwater level_lag_s precedent; the program IS coolant thermal expansion and cannot move at RTD-noise bandwidth) takes program noise 2.472 -> 1.018 % and the hunt 9.99 -> 7.80 gpm. Three checks: noise rejection under a deterministic square wave, the ramp-tracking trade measured at 1.097 % (tau*rate) with a LOWER bound so it cannot go hollow — the first draft ramped past the program's clamp and reported a triumphant 0.000 % — and +1 mutation reverting the reference to raw Tavg.   // 86 -> 92 (#573, 2026-08-28): HEATER ELEVATION — the ruled progressive authority loss the retired engine had and PWR2 never inherited. A wetted-fraction ramp on TRUE level (the 17 % bistable keeps the INSTRUMENT, HR1), the ordering pinned rather than the literals, and the ENERGIZED/DELIVERED split that keeps the readback an identity. 41 -> 47 mutations. // 79 -> 85 (#515 Build 2, 2026-08-26): CHOKED RELIEF — each valve an effective area from its sourced rating, the flow = area x the homogeneous critical flux of what the vessel offers (steam, or WATER from a solid vessel — 95 % of the TMI ride); 6 new checks (rated mass at the rating pressure for PORV and safeties, steam within 25 % of ideal-gas choked, flux ~ P, liquid 2-4x steam and far below the orifice law, a solid vessel relieves liquid), mutations 37 -> 40 (pressure-blind flux, solid relieves at the steam flux, the throat search never chokes); the stuck-PORV check tests the LAW at the ride's own pressure; the WATER SOLID rate window ends at the first relief step.   // 69 -> 79 (#515, 2026-08-25, owner ruling "A"): THE TWO-REGION VESSEL — steam region compressed along dh = v dP, a stratified bottom layer, a saturated pool; the seat's fill terms frozen at P_ref and the outsurge saturating (liquid first, then steam); 10 new checks (the P-only insurge ceiling +197 psi for 3.6 pts at tau -> inf, the adopted tau below it, >= 10 psi/pt, the outsurge rains out, single-phase regions, manual heaters 0.36 psi/s, manual spray -157 psi/60 s, the old-save migration); mutations 30 -> 37 (rigid seat, no steam compression, insurge into the pool, interface deleted, rain-out deleted, flash deleted, spray condenses nothing, migration skipped). REFITS, declared in the check text: the CONSTRUCTION h_bar check (two regions now), the solid poke (m_stm = 0), and WATER SOLID's "> 8x the bubbled rate" — that bubbled 0.21 psi/s WAS the softness §84 measured; replaced by the solid rate in class [4, 25] psi/s (7.5) and bubbled < solid (6.15).   // 68 -> 69 (#515, 2026-08-25, owner design): the stick ARMS and waits for a lift (pinned: armed alone = shut, cold, stuck false); mutations 28 -> 30 (arm-opens-the-valve i.e. the pre-latch build, and clear-never-releases — both red).   // 66 -> 68 (#510 H-4 + H-6, batch 2): a blackout kills AUX SPRAY (the charging pumps drive it — commanded 1.0 on a dead bus delivers 0.00 kg/s; was 29 gpm through a blackout), and each shed-arming signal EDGES INDEPENDENTLY (a LOOP arriving after an SI with the heaters re-loaded between sheds AGAIN — the old OR'd single edge never fired on the second signal). Mutations 26 -> 29 (the aux vital-bus gate severed, the OR'd-edge revert, the SI-shed anchor re-pointed to the split signal).   // 63 -> 66 (#507 waves 4+6): the NUREG-0737 shed LATCH (armed by SI/LOOP/dead-bus, cleared only by the operator's heater command — a LOOP now sheds the banks with the vital bus alive) + the two wave-6 failure seats: heaters_failed (0 kW under a standing demand, nothing shed — a third seat beside manual and the shed) and spray_stick (the porv_stick twin: full spray against a manual-zero demand, still needing RCP head). Mutations 24 -> 26; the SI-shed mutation re-anchored onto the ARMING signal.   // +3 (stage 2c, 2026-08-19): AUXILIARY SPRAY -- the CVCS path that condenses with the RCPs STOPPED (WTSM 3.2 verbatim; the capability #472 measured the old engine lacking), operator-commanded, never automatic; capacity is the CVCS charging max written down twice, TIED BY THE GATE (the MDOT_RATED pattern). Mutations 19 -> 21 (command-dead, gated-on-RCPs both red).   // +5 (stage 2b, 2026-08-19): THE TMI LEVERS -- drivers.porv_stick (one valve of two, PWR2's first failure-injection machinery), the block valve, and the tailpipe with its [open] fast-heat/slow-cool asymmetry. THE DECEPTION EMERGES UNSCRIPTED: stuck 3-11 min the level reads 100 % (hi alarm in) while inventory falls 96 -> 84 % through the valve; closing the block valve freezes the loss instantly and the tailpipe stays hot for minutes. Mutations 14 -> 19; quiet rides trimmed (the replay was >600 s).   // +12 (stage 2a, 2026-08-19): THE LEVEL CONTROL SYSTEM, WTSM 10.3 -- PI charging demand about the source's own 46/180 balance point (anti-windup capped at half the demand range; the first closed-loop probe measured the integral railing the demand with the level ABOVE program), the 25..61.5 % program over this plant's own Tavg span (the source's 557 degF no-load IS the HZP anchor), the +5 % anticipatory backup-heater signal, and the 17 % letdown-isolate/heater-cut latch with a 20 % restore differential. Closed-loop with the real CVCS: holds program, answers a 6 kg/s drain with full charging. Mutations 10 -> 14.   // NEW (#479 stage 1, owner ruling 2026-08-18 "Option 1"): the pressurizer -- WTSM Fig 10.2-3's delta ladder retyped as independent literals, the Ginna-derived per-MWt volume, compliance monotone with the water-solid collapse expressed, the sourced -17/-25 backup hysteresis and 5 % safety blowdown pinned, a balanced plant settling INSIDE the proportional band at 2238 psia (#486's 490-psi defect, gone), and the vessel drivable SOLID at 10 psi/s against 0.07 with the bubble. Three formulation failures are recorded in the module header; 10 mutations, no blind spots.   // 59 -> 63 (control switchover, 2026-08-20b): THE HR1 SPLIT -- ladder/level-PI/17%-cut on drivers.indicated_* (absent = truth, fixtures unchanged), code safeties on TRUE P; both halves proven on LIES (+120 psi lie opens the PORV with the safety shut; true 2510 psia lifts the safety over a lying-low channel). +3 mutations; one stale anchor (level_ctl rename) caught by ANCHOR MISS and repointed. // 92 -> 94 (#583, 2026-08-28): ONE VESSEL, ONE NUMBER -- the ledger's pressurizer row must BE this module's GEOM (they were 125.2 vs 147.5 ft3), and the pressurizer must not ALSO be a ring node
  'run_pwr2_true_state.js':{ code: 0, score: '81passed 0failed 81checks', secs: 2 },   // 80 -> 81 (#642, 2026-09-08): THE [sourced] COPY OF P-6 WAS ON THE WRONG SENTENCE. The engine carried 5e-11 A quoting Ginna TS Bases 'range channels are < 5E-11 amps (below the P-6 setpoint)' -- a channel BELOW the setpoint, not the setpoint; ~600 lines on, the same Bases give 'the P-6 permissive is actuated when any NIS intermediate range channel goes approximately one decade (1 E-10 amps) above the minimum channel reading', and 5E-11 is the SOURCE-RANGE TRIP that P-6 re-enables on decreasing power. So the manual's and control layer's unsourced 1e-10 were right and the sourced engine was wrong. P-6 now lives ONCE, in pwr2_protection beside P-7/P-9/P-10/P-11, exported; true_state reads it. The +1 pins the single copy. Injection also settled the filed question: the control-layer block is DEAD for PWR2 (interlocks in config: 0; set_sr_detector is refused by name; mutating the setpoint seven decades changes nothing) -- the permissive permits nothing here. Second finding filed as #661: PWR2 has no source-range high-flux reactor trip at all. // 79 -> 80 (#603, 2026-09-02): the ECCS word's THIRD state gets a check -- actuated, pumps running, above both shutoff heads so delivering nothing, which read as the quiet plant's 'standby' until now. The two existing ECCS fixtures also gained a protection context: `hpi_active` is the SAFETY INJECTION SIGNAL again (a ruling PWR2 had diverged from by publishing delivered flow), so a fixture that runs the pumps must say what started them. Both were validated against the OLD behaviour too -- the low-pressure fixture injects, so the flag was true there under the flow reading as well -- which is what makes it a strengthening rather than a refit.   // 77 -> 79 (#516 item 7, 2026-08-29): the COLD leg publishes its own void. The hot leg always had one and the cold leg did not, so nothing could tell the legs apart once the loop saturated and temperature stopped separating them. The discriminator voids ONE leg, so a copy-paste publishing the hot value twice reddens. Mutations 29 -> 30.   // 75 -> 77 (#516 item 9, 2026-08-29): `water_regime` PUBLISHED. Layer 0 gained a sourced ideal-gas branch above IAPWS-95's 1000 degC limit and that branch is DECLARED, not validated — so which side of it the fluid is on has to reach the player, the same argument model_held rests on one step further out. A regime reporter defined and not published would be `wallLumps` again. +1 mutation, and it models the defect the check caught on its first outing: the scan iterating the KEYED node lookup, whose .length is undefined, so it ran zero times and every plant read 'ok'.   // 71 -> 75 (#536, 2026-08-28): the SOURCE-RANGE scale is this plant's now, not the retired plant's — k_sr 5.0e8 -> 2.6e11, re-anchored so hot standby reads the ~500 cps `Manuals/09` §9.0 already documents (the inherited scale read this plant 0.5 cps). k_ir does NOT move: pwr2_protection derives the sourced 20 %-current-equivalent rod stop through it. Both 1e-9 display floors removed — they existed only because a sourceless core decayed to zero. 24 -> 28 mutations.   // #511 (2026-08-24): SIX statics retired to LIVE fields (the five accumulator rows + msiv_open — the machinery exists now); two mutations re-targeted onto surviving statics (the drift and the reason-blank moved to sg_imbalance_active/'secondary').   // 63 -> 64 (#507 wave 10): the plant-mode LADDER's cold rungs — Mode 4 at 248 degF, Mode 5 at 176 degF (the rung waits on Layer 0 extending below its 0.1 MPa floor); Mode 2 folded into 3, declared. Mutations 17 -> 18 (the Mode-4 rung deleted).   // count unmoved by #507 wave 6 either: spray_stuck and afw_blocked went LIVE (their statics retired — the spray lever and the AFW discharge block are real state now) and both checks TURNED AROUND to guard the repair (the field follows the lever, and a blocked system reads RUNNING/not-delivering/blocked — three facts separated).   // count unmoved by #507 wave 4 (registry-driven checks): the 'electrical' STATIC retired — ac_available/station_blackout are LIVE fields from the facade's two-bus state; the thin-reason mutation re-anchored onto the surviving MSIV static.   // 61 -> 63 (the CVCS currency, 2026-08-21b): the contract's CVCS flow fields are the #408 fraction currency (gpm/450,000) -- the B1 shim published kg/s, which read as ~343,000 gpm and stood the CHG FLOW HI annunciator (the finish list's '120 gpm balance' was THIS, not physics; the module's own sourced-scaled 7.5+5=12.5 gpm balance was right all along). +the-450,000-literal-recovers-true-gpm and below-the-36-gpm-annunciator checks, +a kg/s revert mutation (16 -> 17).   // 60 -> 61 (the feed train, 2026-08-21): fw_flow_normalized reads the FEED MODULE, not the steam side -- pinned on a half-flow feedwater result reading 0.5 while steam_out_total stays whole (the retired feed-≡-steam identity cannot sneak back).   // 59 -> 60 (AFAS, 2026-08-20i): the demand/delivery split (#200/#329/#332 house idiom) -- afw_pump_running now reads the pumps' DEMAND, afw_active the delivery; pinned on a demanded pump with avail 0, which the old total_kgs>0 keying reported SECURED (the self-healing shape).   // 47 -> 59 (Option B stage B1, 2026-08-20d): THE CONTRACT COMPLETED -- 109/109 emitted (85 derived/translated + 19 registered STATICS + 0 missing/unaccounted). The STATIC third class: a constant stating the model TRUTH about an unmodeled system, triple-gated; the every-static-IS-a-contract-field check caught an invented name on its first run. 22 new derivations (SG levels via the adopted sourced sg_mass_map over REAL mass; H2 from the damage model own oxidation; the sump on a declared ruler; the HEM uncovery proxy D4 upheld; SR de-energizes above the P-6 class point). RHR wired into the facade (existed, gated, never instantiated). 16 mutations: 2 RETIRED as proven no-ops (empty MISSING registry), 1 replaced, 1 repointed, 4 new.   // +1 (stage 2b, 2026-08-19): porv_stuck, block_valve_open and porv_tailpipe_temp_c move to SUPPLIED (64 -> 67 of 109); spray_stuck is the failure-injection block's one survivor and its reason says why.   // +1 (#479 pzr, 2026-08-18): seven pressurizer-block fields move declared-missing -> SUPPLIED (level, mass share, heater shed, PORV, spray, both subcooling margins -- 57 -> 64 of 109); the '#472 owns it' reason check and the 'ABSENT not zero' check both TURNED AROUND to guard the repair; the four fields still missing each name their OWN blocker; trace pinned on an OFF-default 40 % vessel because the program-point fixture agrees with a fabricated 61.5 exactly.   // +1 (#491, 2026-08-18): steam_dump_valve_pct is pwr2_relief's own reported dump_demand, not 100*flow/(rated*0.28) with the current engine's capacity constant retyped untagged -- pinned on a commanded-open dump with the condenser unavailable, where the flow form reads 0 % on a 40 %-open valve.   // +1 (#490, 2026-08-18): core_void_fraction/primary_void_fraction publish HOMOGENEOUS VOID FRACTION, not quality -- pinned on a 1.53 %-quality core reading 8.37 % against independent algebra, where the shipped shim read 1.53.   // THE SHIM. 51 of 109 contract fields supplied, 58 DECLARED missing with a reason and an owning system, 0 unaccounted. Rose from 37/72 across three passes 2026-08-17 (#479): break/containment/condenser/ECCS/AFW landed as gated systems but the shim still declared them fully missing -- the SAME defect this file exists to prevent, in reverse -- plus reactor_period_s/startup_rate_dpm, newly derived. The fraction is NOT a score to raise by writing mappings -- it measures how much plant exists, and any other way of raising it is the fabrication the gate forbids.   // +8 (#479, 2026-08-17): core damage wired -- clad_temp_c, fuel_damaged, melted, destruction_cause, zirc_heat_pct. Coverage 51 -> 56 of 109 because five SYSTEMS landed, which is the only legitimate way it moves. core_uncovered_frac stays declared-missing with a REWRITTEN reason: not 'no damage model' (there is one now) but that a level needs PHASE SEPARATION Layer 2's homogeneous-equilibrium water does not have, and that machinery is #472's. Trap pinned by a new mutation: the damage block first landed INSIDE the AFW guard, so a caller with damage and no auxiliary feedwater got all five fields silently dropped. +1 (protection): `scrammed` moves from declared-missing to SUPPLIED, and the check that pinned its absence was TURNED AROUND rather than deleted -- it now guards the repair, asserting false-on-healthy AND true-past-a-setpoint so the false is earned. One mutation RETIRED as a proven no-op: it injected ts.scrammed=false before the real assignment, which now overwrites it.
  'run_pwr2_relief.js':    { code: 0, score: '73passed 0failed 73checks', secs: 1 },   // 71 -> 73 (#643, 2026-09-08): safety_flow_frac 0.84 WORE A [sourced] MARKER AND NO DOCUMENT CONTAINS IT. It is Ginna's ratio AFTER its 1775 MWt uprate (one line's bank 3,310,489 lb/hr over half an ENVELOPE flow of 7.92e6 = 0.836), inherited by reference from the retired engine's sg_safety_flow_max. The source sizes the bank at 100 % of DESIGN flow in words (ch10 §10.3.2.4 'do not relief 100% steam capacity at 1775 MWt' names its own uprate departure; B 3.7.1 'passing 100% of design steam flow'), and three independent routes give 1.0025-1.0062. The old check compared the engine's 0.84 against a 0.84 RETYPED in the gate -- the number agreeing with itself, which is how #542 verdicted the arrangement and inherited the figure. Replaced by TWO provenance checks, each reddening alone under injection. The CONSTANT is unchanged pending an owner ruling (status-needs-ruling on #643); the marker now reads [UNVERIFIED]. // 56 -> 71 (#633, 2026-09-05): RELIEF FLOW NOW DEPENDS ON UPSTREAM PRESSURE. Every path was a flat fraction of rated, so a 100 % ADV passed its full 8.18 kg/s at MINUS 14 psig and cooled the plant to 68 degF in an hour. Model [derived] to IEC 60534-2-1 / ISA-75.01 (find_source: 0 plant-specific hits across 39 docs in 3 lanes) -- Y*sqrt(x*P1*rho1), choked at x_T, and rho1 ~ P1 for saturated steam MEASURED against Layer 0 (P/rho 191k at 7.27 MPa vs 169k at 0.1 MPa), so at choke it IS Napier. The evidence pass found something better than a guessed reference: Ginna UFSAR ch10 ML20339A040's equipment table quotes all three paths WITH their pressures ('329,000 at 1005 psig', '302,500 at 695 psig', safeties at '+3% accumulation'). TEN of the twelve reds sampled at an ARBITRARY pressure (6.0 MPa for every dump check), so the whole gate could not distinguish a pressure-honouring model from the defect -- HR10 exactly. The #542 RATCHET SURVIVES, measured not asserted: the bottled-SG park sweep agrees before and after to within 0.5 psi, 0 sub-setpoint parks either way, because the ratchet is on LIFT and the density rides through it. Two checks were a valve's POSITION asserted by reading its FLOW; split. Mutations 33 -> 45. // 43 -> 56 (#542, 2026-08-27): THE MAIN STEAM SAFETY VALVE BANK IS THE STAGGERED ONE THE SOURCE DESCRIBES. The lift ramp was anchored at the RESEAT pressure, so 71.5 % of it lay BELOW the 1085 psig pop: the bank stepped shut -> 60.05 % of rated in ONE 0.02 s step at first lift, then parked for an hour at 24.2 % of rated and 1063.3 psig — 21 psi UNDER its own setpoint, 0 reseats, 750,078 lbm vented — while the board annunciated "SG Safeties Lifting" over help text promising it would "reseat when it falls back". The evidence pass found the arrangement in this plant's own anchor (Ginna UFSAR ch10 §10.3.2.4 + the ch10 equipment table, ML20339A040): ONE valve at 1085 psig / 797,689 lb/hr and THREE at 1140 psig / 837,600 lb/hr each, "+3% accumulation", so the [derived] 0.35 MPa lump is RETIRED and every constant is now sourced — both of the source's own cross-checks land (6,620,978 vs the stated 6.58e6 lb/hr; full lift 1174.2 psig = 98.4 % of B 3.7.1's 110 %-of-design ceiling). ⚠ THE RATCHET, not the re-anchor, is what fixes it: re-anchoring alone MOVES the park, but holding a popped valve's lift while latched makes its flow independent of pressure below the setpoint, so no sub-setpoint equilibrium exists at all. Measured after: first lift 0.07 % of rated, parks at 1094-1114 psig ABOVE the pop, and the board-reachable isolation-valve path RESEATS at t=109.0 s (it stayed cracked past 600 s). Mutations 25 -> 33. TWO TRAPS the injection self-test found and no reading would have: the old ramp check sampled at reseat+0.05 and reseat+0.20 MPa, BOTH below the pop, so the check forbidding a step could never see the step (HR10); and a mutation flipping the bank flag to `stages[0].open` is a NO-OP, because stage 1's setpoint AND reseat both sit under stage 2's, so on any continuous pressure path the two expressions are identical — it read as a blind spot and was really a mutation that was not one.   // 42 -> 43 (#539, 2026-08-27): the rated-flow guard was `=== undefined`, so it refused to invent a MISSING plant and then silently accepted one of size NOUGHT — the same fabrication, differently spelled, and the case that actually shipped. Now `> 0`. Mutations 24 -> 25.  // 38 -> 42 (#511, 2026-08-24): THE MSIV — sourced placement (Ginna TS Bases B 3.7.2): a shut valve zeroes the DUMP (downstream) and touches neither the safeties nor the ADV (upstream); mid-stroke passes proportionally; absent msiv_frac means open (the pre-#511 caller). Mutations 23 -> 24 (the msiv gate severed).   // +8 (the ADV, 2026-08-19): Ginna TS Bases B 3.7.4 verbatim -- 329,000 lb/hr per-MWt scaled (one valve, one loop; the source's own ~4%-of-RTP cross-check lands at 4.1 %), auto setpoint [derived] 1040 psig (the WAT-05 45-psi margin below the 1085 pop), modulating not popping, operator cooldown lever, block valve, and NOT gated on the condenser -- which is function (b)'s whole point. Mutations 19 -> 23 (auto-dead, condenser-gated, rung-inverted, block-ignored), incl. two anchors the ADV's own arrival broke, re-pointed.
  ...
  'run_pwr2_eccs.js':      { code: 0, score: '39passed 0failed 39checks', secs: 1 },   // 38 -> 39 (#585, 2026-08-29): a beyond_model plant gets zero flow and the TANK keeps its water — the guard lives here because the loca fixture's tank is empty by the time its ride latches, so the joint gate structurally cannot see it. +1 mutation (guard removed), 24/24 caught.   // 30 -> 38 (#511, 2026-08-24): THE ACCUMULATOR — a tank under nitrogen, not a curve: sourced 650 psig cover (WTSM T5.2-2), two-thirds water, volume = 0.435 x this plant's own RCS volume (the #408 Ginna identity), passive (discharges through a blackout with NO lineup — sourced), check valves hold at 2235 psia, the driving head FALLS as the cover gas expands, and it empties in the sourced ~36 s class then stops (N2 injection declared unmodeled). The isolation valve is the one lever. The old pump-lineup check REFIT, declared (total_kgs now includes the passive tank). Mutations 18 -> 23 (+5: passivity severed, constant-head, check valves removed, infinite inventory, valve stops isolating); the zero-flow-source anchor re-pointed.   // 28 -> 30 (#507 wave 4): the vital-bus wire — a dead bus stops BOTH SI trains with run flags standing; absent drivers mean powered. +1 mutation (gate severed).
  'run_pwr2_cvcs.js':      { code: 0, score: '53passed 0failed 53checks', secs: 132 },   // 52 -> 53 (#679, 2026-09-10): +1 check pinning Ginna's sourced pressurizer volume (650/0.87 = 747 ft3, ML20339A221) — the denominator fix for the CVCS volume-scale basis mismatch (#679); charging max 30.14 -> 26.31 gpm.   // 49 -> 52 (#654, 2026-09-07): boron_rate_delivered is the realised d(ppm)/dt, sits under the command where the pure-water clamp binds, reads 0 at 'match',   // 45 -> 49 (#624/#619 items 14/25, 2026-09-04): THE LETDOWN SPLIT. One check was INVERTED and three added. The inverted one asserted "the operator's letdown fraction still owns the path ... letdownOpen 0 passes nothing through either path" — it PINNED THE DEFECT (HR10: a test written from observed behaviour confirms the wrong parts too), and its new form asserts the sourced arrangement instead: with the orifices out and the RHR suction open, the cross-connect passes the full normal magnitude, because HCV-128 is a different valve and WTSM ch.19 has it wide open in that regime. Added: the protective isolate stops the RHR path; it stops the orifice path; and it survives construction (`createCVCS({letdownIsolated:true})`). Mutations 26 -> 32, all caught (the isolate ignored on each path, the RHR path scaled by letdownOpen again — the old coupling — and the construction seam dropped).   // 44 -> 45 (#574, 2026-08-28): its envelope guard was a PROXY — measured, the 300 MWt fixture railed at the 18.0 MPa table ceiling from 5 s on the PRE-#574 plant and the divergence band read it as healthy. Halved to 150 MWt it stays inside on both plants and the divergence falls to 0.0007 %; the envelope is asserted directly now.   // 43 -> 44 (#510 M-1, batch 3): SI CARRIES RWST BORON — the sourced 2,500 ppm was defined and read by NOTHING; the balance now takes si_kgs/si_ppm (SI raises boron toward the RWST, and SI at the RCS's own ppm is NEUTRAL — the discriminator). Mutations 26 -> 27 (the SI term dropped: injection dilutes instead of borating).   // 38 -> 43 (#510 batch 1): the RHR low-pressure letdown path (sourced WTSM ch.19/§4.1.4.5 HCV-128 + NUREG-1431 Bases; flows the NORMAL magnitude below the orifice backpressure ONLY while the RHR suction is open, behind the operator's letdown fraction, a tie at power) + the regen HX recovery (the return arrives near the cold leg when letdown is in service, cold when isolated). Mutations 24 -> 26 (path severed — the Mode 4 water-solid defect re-armed; recovery deleted — ~200 kW parasitic cooling).   // 35 -> 38 (#507 wave 4): the vital-bus wire — charging AND seal injection die on a dead bus at full demand while LETDOWN keeps flowing (an orifice, declared); absent drivers mean powered. Mutations 23 -> 24 (gate severed).   // 30 -> 35 (#507 wave 1, 2026-08-22): THE RATE ACTUATOR — rate 0 bit-identical to untouched; an unclamped command METERED exactly; a firehose clamped to the tank-and-lineup ceiling (the closed form, not a remembered number); dilution stays PROPORTIONAL-to-C through the actuator (fast at high boron — the old "slow-at-high" wording was inverted, #510 M-9); the lab sample posts/re-arms. Mutations 20 -> 23 (inversion deleted, clamp deleted, sample never posts), all caught.
  'run_pwr2_loop.js':      { code: 0, score: '51passed 0failed 51checks', secs: 5 },   // 45 -> 51 (#574, 2026-08-28): the WIRING — Layer 1's masses reach Layer 2, a missing one is REFUSED not defaulted, the film follows loop flow, and a fixed-duty cooldown takes 1.26x longer with the metal in it. Also repaired a negative control that was passing by 0.024 MPa of an 18.0 MPa envelope. 20 -> 23 mutations.   // 37 -> 45 (#518, 2026-08-26): THE PER-JUNCTION COURANT CANARY AND THE RING SUB-STEP. courantLimit divided by sys.mdot_loop -- the HEAD flow -- while donor-cell moves the DERIVED junctionFlow; identical on a healthy plant, four orders apart late in a blowdown, so a ride whose true Courant number reached 2,745 reported ZERO violations. D2 sec 17.5 ADOPTED sub-stepping and it had never been built: stepLoop now subdivides when the limit binds (measured 0.09 % cost, worst N = 3), flows re-derived every sub-step, sum exactly dt per D2 sec 24.2. New checks: the canary binds on the junction flow, courantOK can now fire, N = 1 when healthy, N calls of dt/N == one call sub-stepping N times, exactly-dt to 1e-9, convergence at dt vs dt/4. Mutations 15 -> 20.
  'run_pwr2_break.js':     { code: 0, score: '31passed 0failed 31checks', secs: 4 },   // 29 -> 31 (#585, 2026-08-29): THE HOLD — a held plant proposes zero flow and books nothing, and the ledger moves only through book() on the plant's acceptance (deferred booking; discharged_kg means 'mass the plant actually lost' by construction). +3 mutations (ledger re-anchored to book(), step_kg zeroed, held guard removed), 20/20 caught.
  /* 30 -> 35 (#591 item 1 / #592, 2026-08-31): the circulating-water inlet becomes an operator lever, so the module gains its clamp and the band gains checks. THE ROW WORTH READING is the one that asserts the band STOPS SHORT of the C-9 removal point: a first draft asserted the opposite - that the ceiling must reach the turbine trip so the casualty is dialable - which would have widened a SOURCED bound (Ginna TS Bases B 3.7.8, service water OPERABLE at <= 85 degF) to suit a lesson. Mutations 19 -> 21. */
  'run_pwr2_condenser.js': { code: 0, score: '35passed 0failed 35checks', secs: 2 },
  'run_pwr2_containment.js':{ code: 0, score: '27passed 0failed 27checks', secs: 5 },
  'run_pwr2_instruments.js':{ code: 0, score: '23passed 0failed 23checks', secs: 1 },   // 22 -> 23 (#590, 2026-08-29): the band-limit check was PINNING THE CONSTANT — it asserted autocorrelation at a hard-coded 8 s, the old global NOISE_TAU_S retyped into the fixture, and failed at 0.041 on a correct model once the correlation went per-channel. Re-expressed at the channel's OWN correlation time read from the module, and PAIRED with a far-lag reading, which is the discriminator: white noise fails the near lag, and noise correlated far too slowly (the #590 defect) passes the near one and fails the far one. +1 mutation reverting the correlation to a flat 8 s.   // 21 -> 22 (#540, 2026-08-27): the sg_steam_flow channel asserted BY ID AND SOURCE. pwr2_engine's element-2 wire reads it and FALLS BACK SILENTLY to the turbine-only channel when it is missing — which it did for six days, so no engine gate could tell the fallback from the real thing. The src half is load-bearing: `steam_flow` would satisfy an id-only check and reintroduce the defect exactly.   // 20 -> 21 (#555, 2026-08-27): a RESTORED null is not a reading. The save round-trips through JSON, which writes a non-finite reading out as null, and isFinite(null) is TRUE — so a channel with no true driver came back a hard ZERO every guard in the tree accepted (it drove the Mode 4 feed regulating valve shut). Mutations 11 -> 12. The `starved channel` anchor was ALSO repointed: it quoted the guard's COMMENT, so adding a comment line sent it to ANCHOR MISS under a green 20/20 — it now anchors on the two CODE lines only.   // 17 -> 20 (#507 wave 6): DRIFT (offset += rate*dt in sim time, HR6; default the adopted 0.5/s, explicit signed rates honored) and DEAD (rails range[0], the current engine's semantic). Mutations 9 -> 11 (drift never accumulates, dead rails high).   // 16 -> 17 (AFAS, 2026-08-20i): +sg_level channel roster check (narrow range on purpose -- the lo-lo LSSS is "a percent of narrow range instrument span", TS Bases B 3.3.2) and a channel-deletion mutation; 15 channels now.   // THE INSTRUMENT LAYER (owner ruling 2026-08-20 'Do option 1'): what the plant SAYS between truth and every reader (HR1). Sourced taus pinned by MEASURED step response (RTD 2.0 s; the hot-leg 3.5 s filter proven as a CASCADE -- 63.2 % at ~6.5 s, where either single lag reads 2.0/3.5); AR(1) noise stationary-sigma + band-limit (autocorr at 8 s ~ 1/e); per-channel INDEPENDENT PRNG streams -- the pwr_instruments append-order trap engineered away and proven by injection (starving one channel leaves a sibling's noise BIT-IDENTICAL); four failure modes incl. restore-heals-to-NOW; 8 mutations all caught.
  ...
  /* NEW (#570, 2026-08-27): DOES A COMMAND MOVE ANYTHING THE PLAYER CAN READ BACK? Two engines
   * from the same IC stepped identically, one given the command — the DIFFERENCE is the effect.
   * Built because #562 shipped a published `afw_throttle_pct` hard-coded to the run lamp, which
   * every gate we had read as a working throttle. Its own first draft asked "did anything move"
   * on ONE engine and passed 28/28 with 0 possible failures (the plant is running); the control
   * leg is the whole test. Self-tests on the #562 defect restored verbatim. */
  'run_pwr2_roundtrip.js': { code: 0, score: '20passed 0failed 20checks', secs: 55 },
  /* NEW (#570, 2026-08-27): the operator manual's §18 command table vs the shipped registries.
   * Caught 4 of 46 documented actions REFUSED by the plant the site runs on its first run —
   * open_porv, set_steam_demand, set_sr_detector, set_condenser_cw_temp — plus a fifth row
   * documenting a payload the shell swallowed. The DOC is the thing under test, which is the one
   * case where iterating a hand-maintained table is right rather than the trap. */
  'run_manual_commands.js': { code: 0, score: '8passed 0failed 8checks', secs: 2 },   // 5 -> 8 (#532 phase 3, 2026-08-30): THE ESF ARM PAYLOAD. set_esf_auto is KERNEL_OWNED, so the two checks either side passed it without asking WHICH systems it accepts — and PWR2 declares exactly one (afw) while 03 taught an HPI arm the plant refuses as an 'unknown esf system'. Both directions: an arm the manual names must exist, an arm the plant declares must be documented. // 3 -> 5 (#532, 2026-08-30): THE CASUALTY MENU, the same doc-vs-plant seam one door over. `07_ABNORMAL_EMERGENCY` names a failure id per event and the plant either has that lever or it does not — THREE events documented casualties the player cannot cause (`steam_line_break`, `steam_line_break_upstream`, `stuck_rod_on_scram`, #530). Not deleted: a procedure is real operator knowledge and the reasoning in it is true of the steam path as modelled, so they carry a canonical NOT INJECTABLE note and this holds it in place. BOTH directions, for the same reason the setpoint gate gives — a marker that can be applied to anything is a hiding place.
  /* 140 -> 145 (#591 item 1 / #592 + #564 item 1, 2026-08-31): the CW sink door and the spray demand publish. The door's checks assert the EFFECT (the vacuum moves 93.20 -> 76.12 kPa between 50 and 90 degF) rather than a landed field, which is the dark-wire shape #540 shipped. Mutations 46 -> 48. */
  'run_pwr2_shell.js':     { code: 0, score: '169passed 0failed 169checks', secs: 420 },   // 165 -> 169 (#662, 2026-09-08): GROUP O -- THE RUNAWAY-WITHDRAWAL CASUALTY DRIVES AT THE PLANT'S OWN SPEEDS. It drove 495 steps/min at severity 0.5 (990 at 1.0): the retired engine's 24 fine-steps/s on a 912-step bank, re-expressed by fraction of travel on this 627-step bank, 6.9x the sourced 72 steps/min the accident's own initiating event states (HRTD Advanced Transients 5.22/5.23, ML11216A094: 'withdraws bank D rods at 72 steps/min'). Now linear slow -> fast off ROD_SPEEDS, nothing retyped; severity 1.0 reproduces a plain FAST rod_start to the sample. Trip cause P-9-at-211 % -> intermediate-range high flux at 26-33 %. THE FIXTURE THAT HID IT: the wave-6 row nudged 25 steps off a RAILED bank, so the old rate hit max_steps in 3 s of a 10 s ride and the check sampled a saturated observable -- identical at 8.25 and 495 steps/s. Rate-independent claims stay in group G (proved green on the old rate); the rate moved to group O (old rate planted: 4 red). The slider row said '0-24 steps/s, default 12' -- the #580 Break Size trap -- now 7-63 steps/min read off ROD_SPEEDS. Mutations 60 -> 62. // 162 -> 165 (#661, 2026-09-08): GROUP N -- THE 1 DPM STARTUP-RATE ALARM REACHES PWR2. sur_high rode along by reference from the pwr alarm table and nothing asserted it arrived; #661's own measurement pass logged engine flags, never annunciators, and reported an '81-second unwarned window' that did not exist -- the alarm comes in 0.00 s behind its own instrument (1.82 s behind truth, the meter's lag). Three checks on one hot_zero_power ride: the row's channel is one this plant PUBLISHES (typeof, because isFinite(null) is true) and is a caution (never drops WARP, #655); clear at settle; active within 5 s of the rate crossing row.setpoint READ OFF THE TABLE, never typed. Dark-wire injection (instrument -> an unpublished channel) reds 2 of 3 -- and check 2 correctly stays green, because a dark wire IS clear at settle, which is why check 3 exists. New mutation: the annunciator table emptied, 3 of 3 red -- its first draft was BLIND (a duplicate object key placed EARLIER than the real one; the later key wins). Mutations 59 -> 60. // 158 -> 162 (#629, 2026-09-05): +4, GROUP M — which controller STEAM DUMP AUTO selects. `set_steam_dump {mode:'auto'}` mapped to Tavg mode unconditionally, so STEAM PRESSURE mode (WTSM 11.2's heatup/cooldown/hot-standby mode) was unreachable from any plant not booted in it — and (as the comment read until #646 -- the ordering INVERTED at #508, and #646 measured Tavg mode parking the plant 0.2 degF from pressure mode with the ADV shut; the real reason pressure mode is selected is WTSM 11.2, not the old ordering) pressing AUTO during a heatup changed nothing (measured: byte-identical trace, that valve at 7.6 %, dumps 0.0 %). Three mutations, all caught: AUTO always Tavg (the defect), AUTO always pressure (the mirror defect — the at-power plant loses the loss-of-load controller C-7 arms), and the published `steam_dump_mode` reading the controller's one-step-old copy instead of the commanded driver. THE BOOLEAN IS NOT THE CLAIM: `steam_dump_auto` is `mode !== 'off'` and reads true in BOTH modes, which is why a check written on it would have passed on the defect — the checks read the published word and the controller's own field, and both branches are asserted so a hard-coded either way reds. // 151 -> 158 (#624/#619 item 24, 2026-09-04): +7, GROUP L — the operator's load dial is rate-limited on the control side at the SOURCED 5 %/min (Ginna UFSAR ch. 10 s10.1.2.1), where `set_load_target` used to be one assignment. The checks: the pressurizer survives a 20 MWe dial step (20.36 % indicated against 16.11 % and a latched 17 % isolate on the old code); the dial lands instantly while the reference arrives at 4.000 min; a REDUCTION is IMMEDIATE — the limiter is RAISES ONLY, and 50 -> 30 MWe reads 30.000 one step later where a limited reduction would read 49.998; PROVENANCE — halve ENVELOPE.ramp_pct_per_min and the walk must double (8.000/2.000 min, ratio 4.0000), the only check that can see a retyped constant; BOTH free-play routes to the C-7 graded ride-out are live (a dial cut arms at 0.02 s on a 49.99 %/min signal; UNLOAD arms while zeroing both numbers in the SAME step) — this is the check that DECIDED raises-only rather than recording it, because the symmetric build parked the dial's reduction rate on C-7's own sourced 5 %/min with a 0.00023 %/min structural margin, which is a BIFURCATION, not a tolerance (#543/#588); the runback is unlimited and carries the dial down with it; the dial rides the save mid-walk. +4 mutations, all caught, one of them THE SYMMETRIC BUILD — this item's own first draft, caught TWICE (the immediate-reduction check AND the C-7 arming half). NO fixture moved: every check the symmetric build reddened was a load REDUCTION, so raises-only put all three back at their original values — ui/manual_procedures.js ends byte-identical to HEAD and the group-A ride is the original 40/60 s, now written as a DERIVED expression that regrows if that fixture ever becomes a raise.   // 150 -> 151 (#624/#619 item 14, 2026-09-04): the Mode 5 board opens with PZR HEATERS off and PZR SPRAY in hand (0.00 % on the bus), asserted against Mode 4 in the same check because #624's scope is Mode 5 only. +1 mutation: the AUTO lamp pinned true, which is the board that predates the lineup and which nothing but a typeof guard read. // 149 -> 150 (#624/#619 item 14, 2026-09-04): the board must show the FLOW, not the lineup. On cold_shutdown both orifice lamps are dark and `letdown_flow_normalized` is still > 0 — the plant is letting down through the RHR cross-connect — and `letdown_isolated` is published as a boolean, because a shut lineup and an isolated one are indistinguishable on the two lamps. The old form was `letdownOpen x rated`, which reads 0 on a cold plant passing the full magnitude and normal on a plant under the isolate passing nothing.   // 147 -> 149 (#602 phase 1, 2026-09-01): the surface half of the bank-scale hoist. `max_steps` AND `position_pct` both carried their own literal 200, and the second is the dangerous one — a stale divisor does not throw and does not blank the readout, it draws a fully-withdrawn 313-step bank as 156.5 % withdrawn. Mutations 48 -> 49 (the stale divisor restored); two anchors were orphaned by the hoist and re-aimed, and one of those re-aims first shipped with a literal backslash-n where the source has a real newline, which the harness reported as ANCHOR MISS rather than passing.   // 146 -> 147 (#601, 2026-09-01): the intermediate-range block is routed and published. The REFUSAL check had to move too — it used `ir_high` as "a trip this RPS does not carry", true when written and false the moment the trip was built; `lo_flow` is the honest example now and a better one (the plant HAS that trip, the operator simply has no block for it), and the +1 is the flip side, that the IR block reaches the engine.   // 135 -> 140 (#563 items 1+2, 2026-08-30): the ADVANCED INSTRUMENT-FAILURE PANEL refused 35 of its own 50 rows with a raw module string, and AUXILIARY SPRAY had no door at all. Measured both engines over the shell's own published list: RETIRED 85 channels / 85 accept / 0 throw, PWR2 86 / 16 / 70 THROW — a regression, in the Hard Rule 1 teaching tool physics.html sells, because the mirror-only path built at #507 wave 6 was hard-coded to ONE id instead of testing the condition (no internal twin). Generalised; unknown ids still throw and status booleans still throw, which is BETTER than the retired engine — it accepted all 35 of those and did nothing with any (measured: _applyFailure runs only over the numeric SOURCE loop and _copyStatus overwrites afterwards). The checks assert the EFFECT: 50/50 accept, a typo still refused, and the board reading MOVES for >= the 28 of 50 the retired engine managed (PWR2 does 29). Aux spray was in NONE of MAPPED/REHOMED/REFUSED, so the partition this file asserts was satisfied by an action that did not exist — the partition is checked against the RETIRED engine's action list and it never had aux spray. Its check rides a real 600 s pump-less ride (2235 -> 1353 psia against 2245 shut). +2 mutations, 46/46: the porv_indicator hard-coding restored, and the aux door accepting while dropping the demand — a door onto a wall, which is the shape a check asserting only acceptance would have passed.   // 133 -> 135 (#580 stage 1, 2026-08-29): the Break Size label. Nothing in test/ had ever asserted it — grep for 'pipe shear' and 'Break Size' returned zero — so the unit could promise a full pipe shear while severity 1.0 opened 0.75 % of one. Both checks assert the EFFECT: the area is parsed out of the unit string and compared against what REHOMED.primary_leak actually opens on the live break object, and the shear reference is computed from pwr2_geometry rather than retyped. The #500 mutation was INVERTED in the same change (it used to delete the pzr_level_low override; it now ADDS one back) and the rod-limit mutation's anchor re-cut — it had chained off the deleted override and went BLIND, which the runner's own ANCHOR MISS report caught.   // 130 -> 133 (#582, 2026-08-29): group T, the AFW throttle on a REAL post-trip drain under the control kernel — valve shut above the 33±5 % NR band with the pumps running, taper across it, settle where the kp 20 / ff 100 line meets decay-heat boil-off (measured 0.41-0.46 of rated at ~36 % NR: the floor is DERIVED, #391 answered). +2 mutations (program moved, kp zeroed), 44/44.   // 128 -> 130 (#573, 2026-08-28): the #538 readback round trip re-asserted with the bank PART UNCOVERED — the existing pair run at program level, where the elevation derate is inert, so they would pass against a plant that published delivered power and walked the demand down. // 112 -> 120 (#571, 2026-08-28): the reset's OTHER permissive. control_kernel refuses a reset against a live trip signal by iterating config.trips — which getProtectionConfig hands PWR2 EMPTY and correctly (protection lives in the engine, §98), so TRIP_SIGNAL_PRESENT could never fire while Manuals/03 §3.5.1 documented it as one of TWO live permissives with its own board caption. Measured: a LOCA holding lo_pzr_press at 1074 psia against 1775, reset ACCEPTED, latch cleared, re-latched one 0.1 s step later. Both paths checked (kernel permissive AND facade refusal) with three mutations that discriminate them — the row-drop reds ONE check, the kernel's, which is the whole argument for checking both. // 101 -> 112 (#545, 2026-08-28): the board's OWN rod verbs against a latched trip — rod_start/rod_nudge refuse by name on both group_ids while rod_stop/rod_stop_all still take (the board sends rod_stop on every button RELEASE, so the door refuses MOTION, not the press). Plus the two one-liners the same latch made reachable: rods_fully_in is BOTH banks (the retired engine's .every(), lost in the second copy — 0/200 published true), and the FACADE reset_rps had NO rods-in guard at all, only the kernel permissive did. // 91 -> 101 (#551 + #559 + #567 + #558 + #560, 2026-08-27): THE TURBINE LATCH. Nothing in the shipped command surface un-latched the turbine — 896 command/payload combinations were fired at a tripped plant and none cleared it, so one scram ended electrical generation for the session. The checks assert the REFUSAL by name as hard as the success: a bare un-latch is accepted and then overwritten next step by whichever of the six level-holds is standing (the #509 §79 defect), so each hold must name itself. Plus the casualty seat — an injected turbine trip was invisible in the failures list AND unclearable.   // 82 -> 91 (#541 + #562, 2026-08-27): THE AUX FEED COMMAND SURFACE, asserted where the registry is. run_pwr2_engine had proved the per-pump switch through the FACADE doors the shell did not expose — `afw_tdafw` was in no registry and threw — so a green check stood over a control the player did not have. Adds: one STOP secures both pumps, the per-pump discriminator, an unknown pump name refusing out loud, set_afw_flow honouring the DECLARED `pct` key (it read only `normalized`, so {pct:0} re-STARTED the pump), the valve-position readback, and the afw_level channel being in the config with a command PWR2 has.   // 77 -> 82 (#534 cluster: #555 + #548 + #563 item 3, 2026-08-27): THE RESTORE AS THE SERVICE DOES IT. Everything group S had reloaded into the SAME instance, at hot_full_power, on a settled fixture — and SimulationService._restore does none of those three, which is where five defects lived under a green 77. New: a non-finite reading is NAMED in the save and comes back NaN (JSON writes it null, isFinite(null) is TRUE, and the restored zero drove the Mode 4 feed regulating valve shut); rated_steam and M_nominal ride the save, so a Mode 4 restore no longer wears Hot Full Power's constants (core inventory 100.0 -> 123.1 % across a rewind, measured); Mode 4 -> a FRESH shell is bit-exact over 500 steps with the feed valve in the sample; and the shell's own shrink-and-swell driver survives a save taken 1.0 s into a scram (7.4771 points of narrow range HIGH before, 0.000000 after). Mutations 29 -> 32, one revert per fix. Also gated: the MIGRATION — strip all four fields this cluster added and load, which must land on the CONSTRUCTOR's values (the #511 and #515 migrations above have no such check, and nothing else in the tree reads an old payload). Trap, hit TWICE: a check that reads a restored scalar AFTER its own ride is measuring the ride, not the restore — read it at the load. Both times the SCOPE preflight caught it, not the check.   // 71 -> 76 (#510 batch 4): set_boron_adjust finally lands through the shell (M-8) · clear_all_failures sweeps EVERY active row through its own per-id clear via the shared engineActiveFailures detector, levers measurably reset (M-3) · sg_overfeed is its own SEAT — MANUAL 0.5 stands through inject AND clear, no forced AUTO (M-12) · fail_low rails the BOARD at ch.spec.range (M-5) · the RHR permissive reads the INSTRUMENT and the refusal quotes "indicates" (M-2, HR1 — gated on a rail-high lie over a depressurized Mode 4 plant).   // 69 -> 71 (#510 M-13, batch 2): THE LAYERED CLEAR — an SBO injected ON a standing LOOP restores the DIESELS ONLY on clear (grid stays down, ac_available true, the LOOP row still reports — the kernel ledger and the engine-derived list finally agree), and clearing the LOOP afterwards restores the grid. The old unconditional offsite=true made the LOOP vanish from the engine while the Failures tab kept drawing it.   // 67 -> 69 (#507 wave 10): the Mode 4 preset through the class (held, SECURED, both cooldown blocks on the kernel snapshot with can_clear) and the P-11 pair round-tripping the kernel forward (un-block clears the engine request, re-block below P-11 lands). Mutations 28 -> 29 (the pair's mapping severed — the cooldown blocks would be board-unreachable).   // 64 -> 67 (#507 wave 9): THE RCP HANDSWITCH — OFF trips AND latches SECURED, ON is a real restart (flow >80 % through the class), and a CASUALTY trip reads LOST never SECURED (the who-stopped-it split; the pre-wave-9 declared LOST-only note retired with its cause). Mutations 27 -> 28 (the secured latch dropped).   // 61 -> 64 (#507 §B wave 8): the rod-limit SURFACES — live control-state fields (shutdown group stays exempt), the board layer's rod_limit_margin channel tracks the engine (was pinned at its 912 default), and ROD LIMIT LO alarms at the sourced RIL+10 in THIS bank's currency (the shared row's 40 is the same number in pwr1 fine steps, 4/step — fired 4x early without the override). The alarms check turned around to the TWO-override shape (#500's + this one; a third silent divergence still reds). Mutations 24 -> 27 (fields reverted, channel severed, override dropped).   // 56 -> 61 (#507 §F wave 7): initial_state rides the service's constructor path (the 50 % preset through the class; an unknown preset throws — the #502 rule); the block button's whole path (kernel empty-trips FORWARD -> shell -> engine request; P-10 revokes a source-level block next step; a trip this RPS lacks comes back a REASONED error; the merged snapshot carries the engine's own 35 % setpoint). Mutations 22 -> 24 (block mapping severed, setpoint dropped).   // 44 -> 56 (#507 wave 6): the menu reaches 21 — afw_failure (pumps RUNNING, delivery dead, afw_blocked live), failure_to_scram (trip latched + turbine tripped, rods at 200, power feedback-limited to 76 % — the ATWS), failed_pzr_heaters (36 kW -> 0, demand standing), stuck_open_spray (247 psi down in 120 s against fighting heaters), continuous_rod_withdrawal (drives OUT at the adopted fraction-of-travel rate, rod levers REFUSED; the shipped IC parks the bank at 200/200 so the row needs inserted rods — declared), tavg_sensor_failure (BOTH layers walk +30/60 s while the lying channel drags the TRUE plant down 25 degC through the dumps), porv_indicator_stuck_closed (mirror-only board lamp, declared). THE THREE LATENT FIXES: degraded_hpi wrote ec.avail which NOTHING reads (inert through two green runs — now hhsiAvail, flow asserted), the seal-leak slider was rendered-and-discarded (now linear, 0.45/1.21/1.81 kg/s at sev 0.25/0.67/1.0), the advanced panel's `value` key was dropped (typed freezes landed at the current reading). Mutations 19 -> 22.   // 42 -> 44 (#507 wave 5): the sgtr row — a break at the tube node at the slider's 40 % of the [UNVERIFIED] double-ended area, reported by NAME (never primary_leak), SG receiving, clear shuts the tube. Mutations 18 -> 19 (row routed to the cold leg).   // 36 -> 42 (#507 wave 4): the electrical pair through the class — LOOP row is the REAL sourced LOOP *(OWNER RULING, 2026-08-22: "Full LOOP + clear" — a selection from options I wrote)* with a working clear (grid back, RCPs stay tripped, heater shed stays latched until the operator's set_heater); station_blackout row lands (menu 12 -> 13); the TDAFW-only 0.667 delivery is the SBO's own gauge; probe trap recorded: at 5 s the feed's 8 s pump tau still reads 0.535 of rated — ride 30 s before asserting the feed dead. Mutations 17 -> 18 (LOOP regressed to pump-trip-only).   // 29 -> 32 (#506, 2026-08-22): the two-bank scram ramps sampled MID-RAMP (where the old snap and the ramp disagree most) + group routing (a shutdown nudge leaves the control bank alone) + orifice/flow_pct fields; the alarms check is copied-with-ONE-override (#500, pzr_level_low 25 -> 17). Mutations 14 -> 17 (alarm-override revert, shutdown-snap revert, flow_pct drop), all caught.   // 27 -> 29 (the CVCS currency, 2026-08-21b): +the-annunciator-input-CLEAR check and the 20-gpm setter ROUND TRIP (the B2 mapper read the board's gpm/450,000 as a 0..1 demand -- any dialed setpoint became ~zero flow, a dead control wearing a working control's face). Mutations 12 -> 14 (both currency reverts).   // 25 -> 27 (the feed train, 2026-08-21): the seven feed refusals RETIRED to MAPPED (set_feed_pump_speed/set_feedwater_flow/feed_pump_nudge/set_feed_coupled/isolate_feedwater/loss_of_feedwater/sg_overfeed), landing proven with a manual-50%-then-recouple round trip; the failures menu gains loss_of_feedwater (3 -> 4); fw + the swell scalars join the bit-exact save.   // 23 -> 25 (AFAS, 2026-08-20i): +the ONE esf_systems entry -- the afw arm the AUX FEED tile needs for STANDBY, UNDISARMABLE (commands: [], so the kernel's manual scan can never flip a word the engine's non-defeatable AFAS would make a lie) -- and +the SEAM the headless pass caught: control_kernel's channel-less getAutomationState fast path dropped the esf dict, a path ONLY PWR2's config reaches (esf_systems, no channels), so the tile read SECURED over an armed AFAS; fixed in the kernel (_attachEsf on both paths) and pinned here at the exact stack seam. Mutations 10 -> 12: the arm dropped (SECURED regression) and the arm grown a command list.   // 21 -> 23 (2026-08-20h, the indications hookup): +all-35-status-passthroughs (the {} extras defect had every board status word defaulting) and +pzr_level_dev-consistency (the gauge measured the OLD plant's program: +6.4 % standing on an on-program plant; asserted as dev == level - own-program(indicated tavg) because the fixture sits inside the known boot settle, measured -9.5 % @30 s -> 0.15 % @900 s). The 16-key control-state check tightened same change: TWO rod groups in the consumer's shape (steps/max_steps 200) + the dump controller's live 7.03 MPa setpoint. Mutations 8 -> 10: the exact shipped extras-{} defect and the one-entry rod-group revert, both replayed red.   // OPTION B STAGE B2 (2026-08-20e): PWR2Engine, the RD.PWREngine-shaped class over the facade. THE COMMAND PARTITION: all 71 actions parsed from the CURRENT engine's own switch land in exactly one of MAPPED/REHOMED/REFUSED, refusals reasoned and THROWING (a silent swallow reads like survival). getProtectionConfig SUPERSEDES the courier (B3: the pwr automation channels would issue REFUSED-throwing commands; acting parts emptied, annunciators adopted, menu = 3 hostable defs; +1 leak mutation). REUSED pwr_instruments member (D4) fed by the B1-completed contract -- reset() must PRIME the lag buffers (measured: NaN across all 45 channels without it). SAVE pwr2-1.0 only, pwr-1.0 REFUSED with the D4 sec 5 fabrication reason; round trip BIT-EXACT over physics AND readings. Two fixture lessons the mutations forced: the frozen-gauge mutant needs a MANEUVER (a 2-degC band on a steady plant cannot see a frozen needle), and the readings-save mutant needs the 700 s settle -- at 400 s the prop ladder is RAILED full on truth and indication alike and the mutant is invisible (a fixture's operating point is part of what a check asserts).
  'run_pwr2_endurance.js': { code: 0, score: '25passed 0xfail 0failed', secs: 130,   // 20 -> 22 (#582, 2026-08-29): the SMALL-BREAK PLATEAU, CA-20b's two mechanism legs re-derived and MEASURED on PWR2 (worst departure below the heat sink -31.6 psi against a 50.8 psi band; SG inventory never below its initial 12,796 kg, ends +34 % with AFW at 1/3) — the gate the shipping plant never had.   // 18 -> 19 (#515 Build 3, 2026-08-26): loca-sev1-eccs-30min — reactivity never positive after the scram (born failing at 572 %); the inner guard's latch at 161 s is REPORTED, not asserted (#517)
  ...
  /* THE FACADE GATE IS THREE RUNNERS (#637, 2026-09-05) — a SCHEDULING split on the run_campaign
   * A/B/C precedent (#513), not a test change: 156 checks and 86 mutations still run, each in
   * exactly one part, and the union of check names was diffed against the unsplit file (156 = 156,
   * name for name). MEASURED per group with MUTTIME=1: the whole file is 1551 s, of which the
   * REPLAYS are 1312 s (85 %) and group I alone — the failure levers, 9 mutations x 85.4 s — is
   * 863 s, 56 % of the runner. Nothing balances that, so it sits in its own part (C) where its
   * cost stays visible in a baseline instead of buried in a mixed one; the ~700 s per-part target
   * the plan set is therefore UNMET at 830 s and cannot be met by re-partitioning — it needs
   * group I subdivided, which is coverage scoping and a separate decision (#637). A new grp()
   * block lands in part A by default and moves part A's count, so the partition cannot drift
   * silently. The OLD single entry's secs hint was 420 against a measured 1551 — 3.7x low, which
   * is part of why the CI tail was invisible to the scheduler. */
  'run_pwr2_engine.js':    { code: 0, score: '80passed 0failed 80checks', secs: 325 },     // 79 -> 80 (#650, 2026-09-06): RATED-POINT IDENTITY. Both delta-T trips normalised against DESIGN.dt_c = 31.1 degC (56 degF), a constant SOURCED TO NOTHING (find_source '606' -> 6 hits, none a temperature) while the loop flow was derived from a 33 degC split -- two rated splits in one build since PWR2's flow was derived. Settled rated plant read delta_t_frac 1.049: a 4.9 % head start into both bands, invisible for three weeks because it cancelled an equal opposite K3 credit (#647). dt_c now 32.71 degC (58.88 degF), converged as a fixed point through designHmap AND rhoRated (32.7100 in -> 32.7101 settled). The +1 asserts the identity the defect violated -- settled hot_full_power reads delta_t_frac within 0.5 % of 1.000, TRUE and INDICATED, the indicated half meaned over 60 s because it is a DIFFERENCE of two noisy RTDs and ripples +/-2.9 % instantaneously (larger than the bias that shipped). Margins GREW: OTdT 13.86 -> 17.43 degF, OPdT 5.77 -> 9.00 degF. // part A (groups A E F G L M N O P), 323.5 / 321.4 s solo
  'run_pwr2_engine_b.js':  { code: 0, score: '66passed 0failed 66checks', secs: 330 },     // 61 -> 66 (#668, 2026-09-08): THE ROD DRIVE RUNS AT THE SOURCED SPEEDS. ROD_SPEEDS was pwr1's 8/48/72 steps/min re-expressed as a FRACTION OF TRAVEL onto this bank -- 7.02 / 42.12 / 63.18, 12 % under a band whose ends are both sourced *(OWNER RULING, 2026-09-08: 'A - adopt the sourced 8 and 72; keep 48 as normal, marked [UNVERIFIED]')*. WTSM 8.1 (ML11223A252): 'a minimum speed of eight steps per minute' and 'a maximum rod speed of 72 steps/min ... the physical limitations of the rod drive mechanism, with the latter being the limiting factor'. 48 has no source of its own and says so. The #662 casualty derives its rate from ROD_SPEEDS, so severity 1.0 became the sourced accident's OWN initiating rate (HRTD Advanced Transients 5.22/5.23, ML11216A094: 'withdraws bank D rods at 72 steps/min') with no second edit -- which is the argument for deriving it there rather than typing it. The +5 land in part B's groups; part A stayed at 80 and the shell's group O followed the constant without a check moving, because it reads ROD_SPEEDS. // part B (groups B C D H K Q R), 329.0 / 326.5 s solo
  'run_pwr2_engine_c.js':  { code: 0, score: '16passed 0failed 16checks', secs: 835 },     // part C (group I alone), 839.2 / 830.2 s solo — THE TAIL
  // history of the single entry, before the split:   // 155 -> 156 (#624/#619 item 14, 2026-09-04, hours later): MODE 4 FOLLOWS. R1b flipped from 'Mode 4 stays AUTO, the scope boundary' to 'BOTH cold ICs follow, at-power does not', and R2b added — Mode 4's own 15-minute survival window, asserted SEPARATELY because 128 degF of temperature means two saturation points and therefore two measurements (364.04 -> 364.05 psia off, 373.10 in AUTO). The 'spreads to Mode 4' mutation became its inverse, 'Mode 4 drops back to AUTO', which reds both. Mutation count unchanged at 86. // 150 -> 155 (#624/#619 item 14, 2026-09-04): GROUP R, the Mode 5 pressure-control lineup — heaters OFF and spray in hand at the cold boot, the scope boundary (Mode 4 and at-power stay AUTO), the bubble surviving the 15-minute alignment window (362.59 -> 362.61 psia; the old AUTO boot reads 371.62 and fails the 3 psi band), and the Pressure SP being INERT until AUTO is pressed (0.048 psi at 0 kW against +133.4 psi at 157.8 kW). 83 -> 86 mutations. NOTE the group asserts the bubble SURVIVES, not that it falls: the note's '-68 psi/hr surge-line bleed' was never measured on this engine and is false here (+0.3 psi/hr over 60 min). // 142 -> 150 (#624/#619 items 14/25, 2026-09-04): a new group for the letdown split — the cold boot holds its inventory with the orifices OUT (pre-split, the same lineup climbed the pressurizer 25.02 -> 35.27 % in 20 plant-minutes, the #510 H-2 water-solid path); the 17 % cut isolates WITHOUT touching the selection; no automatic restoration when the latch clears; the operator's re-line restores flow; and the re-line is REFUSED while the cut still stands. THE FIXTURE FINDING IS THE VALUE HERE: scaling the pressurizer masses toward 12 % never latches the cut — the loop refills the vessel to a true 58.28 % before the channel lag moves — so the check is built on a sustained drain (a 2 cm2 break, cut at 36.1 s). Mutations 77 -> 83, all caught (the old convention restored, the cut never isolating, the command-side clear deleted, that clear ignoring the latch, the cold boot opening the orifices).   // 138 -> 140 (#602 phase 2, 2026-09-01): the bank moved to the sourced 627 steps and curve_flatten to 0.36, and SIXTEEN checks in this file reddened at once — every one a FRACTION of travel that had been spelled as an absolute while the two were the same number. +2 net: the held-press check SPLIT, because the answer now depends on the pull rate (slow: the stop holds, no trip; normal: the withdrawal outruns it and the 25 % rung ends it — on the old bank one step spanned the whole 20->25 % window so no rate could sit inside it), and AT-LIMIT gained a direct check after the mutation harness caught the RIL re-aim silently dropping the only ride that ever drove `_rodAtLimit` true.   // 130 -> 138 (#602 phase 1, 2026-09-01): THE BANK SCALE block. `200` was ten bare literals in this file alone and the gate that pins the hoist is FUNCTIONAL, not a grep — it moves RODS.max_steps to 313 and rides the plant, asserting the initial condition, the rod groups the reactivity solve reads, the target clamp, the scram ramp for both banks, the insertion-limit percent map and the restore all follow. Mutations 73 -> 75: two put a stale 200 back at the clamp and the scram ceiling, the two sites whose failure mode is SILENCE. The scram one was BLIND twice before it bit — sampled at 20 s where both builds converge to zero, then at tick 1 where both still read 313.0; the divergence is tick 2 (clean 310.5, stale 198.4) and the measured trace is in the comment.   // 129 -> 130 (#601, 2026-09-01): the 8c rod-stop block split in two. It rode ONE PRESS HELD TO 200 and asserted the stop parked the bank "short of the trip" — which passed only because the next trip above the 20 % stop was the 35 % setting, a 15-point gap. The sourced IR trip at 25 % makes it 5 points and the standing reactivity carries the flux through: measured, the bank holds 88.1 -> 88.2 for 1200 s while the flux peaks 28.24 % and settles 27.13 %. Now pinned as two claims — the stop's OWN regime (an 86-step target settles 22.59 % and does NOT trip) and the held press (the stop holds the bank, the 25 % trip ends the ascent). The startup-accident check's cause moves hi_flux_lo -> ir_high_flux, peak 100.8 % -> 91.9 %.   // 125 -> 128 (#590, 2026-08-29): STEADY STATE, and nothing in this tree asserted it on any variable. The feed loop wandered SG level 1.975 % and reactor power 0.567 % peak-to-peak at constant full power with nothing touched; the retired engine carried a whole family of such checks for its own limit cycles (#378/#394/#418) and PWR2 inherited none, which is why it shipped. Three checks, DETRENDED (a plain span reports the manual case's drift as oscillation), with a MANUAL control that proves the plant is live rather than dead. Owner-ruled criterion 0.5 % level / 0.25 % power; now 0.361 % and 0.141 %. Group E is SKIPPED IN MUTATION REPLAY unless targeted — two 35-min rides, and untagged mutations replay every group, which took this gate 478 s -> 983 s before the guard.   // 124 -> 125 (#585, 2026-08-29): the hold is the WHOLE plant — a latched engine steps 500 more times and no ledger moves (break, containment, AFW delivered_kg, accumulator, M_total; AFW was the one only the facade guard protects — measured +18 kg/10 s with it reverted). +1 mutation (short-circuit reverted), 72/72 caught.   // 122 -> 124 (#574, 2026-08-28): the break's backpressure wiring asserted DIRECTLY. Two #543 mutations went blind when the walls moved the ride — they had only ever been caught incidentally by trajectory checks. A third was mis-tagged grp C when its checks live in grp I, so the scoped replay ran a group with no opinion about it. 71/71 again.   // 118 -> 122 (#573, 2026-08-28): group P, the heater seam — the plant publishes the ENERGIZED bank and the vessel receives the WETTED heat, proven at a state where they differ and at a shed where the published one must reach zero. 69 -> 71 mutations. // 111 -> 118 (#536, 2026-08-28): group O, the neutron source ON THE PLANT — both subcritical ICs are CONSTRUCTED at their own source equilibrium instead of the retired engine's 1e-6 literal (Mode 4 sits deeper and indicates lower, which a seeded constant cannot do), the approach to criticality never runs backwards (it fell for 60 s and 42 of its first 200 steps), and a tripped plant falls at the sourced -1/3 dpm and then LEVELS — it used to read -0.322 dpm / -81 s for ever. 66 -> 69 mutations. `secs` re-measured: the 165 hint was stale long before this.   // 104 -> 107 (#572, 2026-08-28): the two SOURCED flux rod stops the plant lacked (power range 103 %, intermediate range 20 % current equivalent) — and what the plant DOES with them, which the protection suite cannot see. Measured: a controlled withdrawal from Hot Standby now parks the bank at 89.1 of a demanded 200 steps at ~28 % power with NO trip; before, the same withdrawal ran to the 35 % trip. The #545 door mutation went ANCHOR MISS here (rodDriveDoor grew a branch) and is re-pointed.   // 97 -> 104 (#545, 2026-08-28): THE TRIP BREAKERS TAKE THE ROD DRIVE'S POWER. The rods were the one reactor-trip consumer wired to the latch's rising EDGE, so after a scram both board WITHDRAW controls were ACCEPTED and the core came back to 61.18 % at 200/200 with SCRAMMED lit on the true state, the instrument and the kernel at once. Level-held now, both banks both directions [sourced, Ginna TS Bases B 3.3.1 ML20339A221], the doors refusing by name. THREE new mutations (60/60, no blind spots) — and the level-hold one went BLIND on the first draft, because the hold and the door are each sufficient for the operator's own sequence (#295): the demand is planted past the door now. // #539 (2026-08-27): THE RATED SCALE IS FROZEN — one rated_steam for all four presets (164.2471 kg/s), computed at the RATED dispatch AND the DESIGN steam pressure. It was frozen on NEITHER axis: the literal read each preset's own sg.P (50 %% and Hot Standby drifted +0.57 / +0.88 %%) and a second recompute in the cold branch ran AFTER the dispatch was zeroed, so Mode 4 booted at 0.0000 — an inert feed train behind a gauge reading back what was dialled, and code safeties indicating OPEN while passing nothing. Measured after: Mode 4 main feed puts +3,934 kg (+8,672 lbm) in the SG on a 50 %% demand, and the safeties pass 137.9676 kg/s = 0.84 x rated, bit-identical to the Hot Standby control arm (was 0.0000). Checks 94 -> 97, mutations 55 -> 57 (one revert per axis). The frozen-scale check asserts the INVARIANT, not a consequence — every prior check measured a consequence at ONE preset, which is how a denominator wrong at every preset in a different way survived 93 green runners. Trap: hoisting `sgDesign` orphaned the `50 % secondary` anchor, which quoted the whole two-line sg expression — re-pointed to the one line carrying the claim.  // 91 -> 94 (#515, 2026-08-25, owner design): THE STICK IS A LATCH — arming lifts nothing (pinned), the operator's porv_manual lift latches it, a manual close does not move a latched valve, the clear is the only release.   // 90 -> 91 (#510 batch 4): a SCRAM exempts the rod insertion limit — mid-decay (rods driving in, power still above the 5 % floor) the limit reads null, so the ROD LIMIT annunciators no longer fire on every trip; the RHR-align fixture RETRIES its command (the M-2 indicated door's 0.5 s lag silently refused a one-shot at the exact crossing); the M-2 door/autoclose anchors re-pointed.   // 84 -> 90 (#507 wave 10, group N): THE SHUTDOWN IC — Mode 4 opens ON its point (121.1 degC / 364 psia / level 30 / boron 999, blocks taken, RHR held-throttled) and holds; THE #468 INVERSION IS A MUTATION (bank-before-trim pays the margin in boron and the cold plant reads LESS than hot standby — the check pins 999 > 719); the heatup is real (87.9 degF/hr on pump heat, untripped); clearing the P-11 blocks at 350 psig CASCADES; engaging either above P-11 is REFUSED. Mutations 51 -> 55 (the inversion, blocks forgotten at boot, the hold throttle dropped, the refusal severed).   // 82 -> 84 (#507 wave 9, group M): THE RCP RESTART — a coasted pump motors flow back above 90 % inside the start class on the operator command; the start is REFUSED out loud on a dead nonvital bus (WTSM 3.2) and LANDS after the grid returns (recovery hands back a stopped pump — nothing auto-restarts). Mutations 50 -> 51 (the electrical gate severed).   // 77 -> 82 (#507 §B wave 8, group L): THE ROD INSERTION LIMIT — the adopted pwr1 curve (null at/below 5 %, 140 steps at rated, control bank only); Hot Standby's inserted bank stands NO alarm (the applicability floor's whole point); the curve RECEDES with power so a plain insertion never closes the margin (measured: insert to 145 -> power 85.8, limit 121, margin OPENS to 24) — the honest approach is rods-in + dilution (margin <10 at +35 s; at-limit at +55 s as the recovering power lifts the limit to the bank). FIXTURE LESSON kept in the comment: securing the dilution before the final insert reddened the gate on its own fixture, not the plant. Mutations 46 -> 50 (curve deleted, floor deleted, at-limit pinned, margin pinned).   // 68 -> 77 (#507 §F wave 7, group K): THE INITIAL CONDITIONS — 50 % opens ON its point (50.0 % / 298.08 degC / SG 958 psia / level 43.2) and rides to min 48.79; Hot Standby anchors to the plant's OWN no-load (Tsat of the sourced 1005 psig = 547.9 degF — the 557 degF program anchor saturates ABOVE the 1085 psig MSSV pop, measured, #508) with the sourced lineup (control bank IN, shutdown OUT, dumps in PRESSURE mode) and holds +0.05 degC/120 s; a continuous fast pull IS the startup accident (1 % -> 100.8 % inside the trip delay, hi_flux_lo answers); the CONTROLLED startup passes 35 % on the block taken at 18.2 % (42.6 % untripped); P-10 auto-revokes a source-level block; an unknown preset THROWS. Mutations 41 -> 46 (SG at the full-power literal, kinetics seeded full regardless, subcritical margin dropped, the 557 anchor, the dump lineup dropped).   // 64 -> 68 (#507 wave 6, group I): THE FAILURE LEVERS — a blocked scram LATCHES the trip (annunciated, turbine tripped) with the rods standing at 200 (the ATWS is the DROP failing, not the logic); a rod runaway drives OUT at its own rate with the rod levers REFUSED, and a WORKING scram beats the drive. The probe FOUND A REAL DEFECT: reset_protection never re-armed the trip-edge detector (_lastTrip lagged true), so reset-then-scram missed the edge — latch on, annunciators on, rods standing; fixed in the reset. Fixture lesson kept in the comment: a 50-step insert at full load lets the RPS terminate the excursion mid-probe (UFSAR 15.4's own story — a different claim than "the drive moves"). Mutations 38 -> 41.   // 58 -> 64 (#507 wave 5, group H): THE SGTR — a break at sg_primary discharges into the SECONDARY against the SG's own pressure: initial 20.9 kg/s at the default 40 % rupture (full DER ~52; the "1982 Ginna ~48" comparison is RECALLED/UNVERIFIED (#510 M-15)), the SG OVERFILLS (mass frac 1.23 at 300 s — the sourced §15.6.3 hazard the old engine never landed), OTdT trip + SI arrive unscripted, containment NEVER moves through 3,037 kg discharged (the bypass signature), and the leak tapers on sqrt(dP) (ratio 0.301 vs 0.335 — the EOP's physics). A new break REPLACES the tube (one-break slot, declared). Mutations 34 -> 38 (backpressure cut, stash severed, SG never consumes, exclusion dropped); the group-E AFW-stream anchor re-pointed (the SG drivers grew the tube-leak pair).   // 47 -> 58 (#507 wave 4, group G): THE ELECTRICAL PAIR, one probe per wire — LOOP kills the nonvital loads with selectors standing (feed 0.000, condenser lost, RCPs tripped, ac_available TRUE) and starts both AFW pumps (afw 1.000; md cause loss_of_main_feed is the same-step race AND the feed wire's gauge); the heater re-load pair is the vital bus's own gauge (LOOP re-load >100 kW, SBO re-load 0 kW); SBO reads afw 0.667 = the TDAFW-only fraction; a blackout mid-LOCA STOPS SI and restore resumes it; manual charging 6.5e-5 frac/s in LOOP vs 0 in SBO (the PLCS wants 0 in both — level 40 % over the post-trip 25 % program — so the probes force manual demand to stay non-vacuous). Mutations 28 -> 34 (six wire cuts; a seventh — blackout-forgets-offsite — was REJECTED as unkillable: offsiteOk already ANDs !blackout).   // 45 -> 47 (#507 wave 2, 2026-08-22): THE RHR ALIGN through the plant — a 20 cm2 LOCA below the 425 psig permissive, aligned tavg 133.9 degC at t=80 vs 150.3 secured (the heats merge is the subject; the merge-dropped mutant's removed_kJ ledger still climbs, only the PLANT tells the truth) + the door refuses an at-power align and the 585 psig autoclose shuts a forced valve. Mutations 26 -> 28 (merge dropped, permissive ignored), all caught.   // 41 -> 45 (the settled IC #502, 2026-08-21, group A): SETTLED IC — a fresh engine ridden 60 s untouched holds power >= 97 % / legs within 4.5 degF of settled (reds at 76.6 % on the isothermal boot); THE QUIET WIRES — three probes for the mutations the settled IC sent blind (the startup ring had been doing their sensing): a stuck PORV takes real mass out (dM -136 vs -11 kg mutated), letdown moves charging demand (0.074 vs 0.000), a turbine trip opens the dumps (75.7 vs 0.0 %). The runback fixture re-scripted quasi-static (-1 ppm/block — the old -2 overshot the band within the detection block and its standing OTdT condition matured the trip delay during the rod-stop test, dt 0.02 only).   // 33 -> 41 (the feed train, 2026-08-21, group F): the controller holds the ruled 65 % at power; the A/B load swing moves TRUE level and comes home; ONE pump = a REAL boil-down to the lo-lo trip + both AFW starts (97.6 s measured) with the anti-windup recovery pinned (the pre-fix refill hit 17,033 kg); BOTH pumps = the whole ch10 sentence (turbine trip, P-9 reactor trip, MDAFW on the loss); a manual overfeed reaches hi-hi (fwi + isolation + turbine trip); a scram SHRINKS indicated level 19.5 points below true (the adopted instrument-side downcomer term). +1 in group E: the held SI isolates main feed through the facade wire. Mutations 21 -> 26.   // 26 -> 33 (AFAS, 2026-08-20i, group E): the caller's half of the AFW starts -- a failed-LOW sg_level channel starts BOTH pumps and trips the reactor on the lie; the water is REAL (SG mass +326 kg in 60 s against ~326 rated -- the merge, not the report); secure-while-latched refused; reset clears latches without securing pumps; each pump's own switch works after; SI starts the MDAFW only. Mutations 15 -> 21 (both consumption wire-cuts, the inert-stream revert, the driver cut, reset leaving latches, the TDAFW switch cut), all caught.   // 21 -> 26 (runback, 2026-08-20c): the full sourced loop -- a quasi-static dilution (-2 ppm/2.5 s; ANY step prompt-jumps power toward the hi-flux trip) walks OTdT into the 3 % band; the ROD STOP refuses outward and allows inward; the runback nibbles 5 MWe per 30 s window; rods-in recovers the margin, the signal clears, NO trip. Measured identity: without the operator this rods-MANUAL plant trips ~51 s after onset (the load cut RAISES Tavg ~1.1 degF/MWe and K3 erodes the setpoint faster than delta-T recovers -- the runback buys TIME, not equilibrium). MUTATION REPLAYS NOW SECTION-SCOPED (groups A-D): 17 x the whole suite measured 1074 s of contention and a mutation only needs the checks built to see it. The scoping EXPOSED four latent self-test defects, all fixed: a turbine-flag anchor that had been cutting the DUMP CONTROLLER's identical line since birth; a delta-T mutation that left the ternary's truth branch live; a ladder probe vacuous under the ~330 s startup pressure dip (now a HIGH lie -- spray+PORV answer only that); an oxidation mutation caught only by chaos (now a designed eng._Qox observable). The stuck-Tavg fixture: C-8 chases the stuck 578 degF reading and drags the TRUE plant to 406 degF -- an instrument-driven overcooling casualty.   // 19 -> 21 (control switchover, 2026-08-20b): the heater ladder drives FULL on a lying-low pressure channel at +1 s (the RPS answers the same lie at +2 s and sheds -- read EARLY on purpose); a lying-high Tavg opens the dumps to 100 % and OTdT trips on the same railed channel (lumped-channel common-mode, DECLARED). +2 wire-cut mutations (pressurizer ladder, dump controller).   // 18 -> 19 (instruments, 2026-08-20): THE LYING CHANNEL -- fail primary_pressure LOW on a fresh healthy plant and the RPS trips + injects on it (lo_pzr_press + SI, true P 2224 psia); the reads-truth mutation is exactly the wiring this check reds. All RPS analog drivers now come from ins.reading (one step old, the house lag convention); truth fills the FIRST step only, before channels energize. All 18 prior checks held through the switchover unmoved.   // 17 -> 18 (OTdT wiring, 2026-08-19k): delta_t_frac (loop split / DT0_C 31.1, [derived] from the sec-43 design point) and tavg_c reach the RPS; the settled OT margin reads ~0.3 and the check needed the SETTLED plant (at t = 5 s the startup transient's delta-T overshoot reads margin 0.013 -- measured, and the comment says so). +1 mutation (the delta-T wire cut). eng.rpsReport exposes the function report.   // 16 -> 17 (P-9, 2026-08-19i): the wiring half of the turbine-trip reactor trip -- turbine_tripped and dump availability reach the RPS from the facade; the setpoint logic lives in run_pwr2_protection. +1 mutation (the turbine flag never connected).   // 14 -> 16 (#499 FIXED, 2026-08-19h): two guards in pwr2_core, both thresholds MEASURED -- a root-tracking limit (P_JUMP_MAX 2.0 MPa/step; worst legit move 0.67 on a 500 cm2 first step, the defect jumped 6.1) and a both-walls latch (nodes on BOTH envelope walls at once; the benign 50 cm2 episode has 45,087 clamped steps and ZERO two-sided). The 40 cm2 ride now latches at 46.9 s instead of NaN at 68.5; the drain fixture forces the pre-fix turbine wiring to keep the trajectory reachable and pins maxStep < 2.0 so deleting the limit reds on the teleport itself. +2 core mutations via loadAll's coreSource substitution.   // THE FACADE (owner ruling 2026-08-19 'A', the preview-page route): pwr2_engine assembles the gates' hand wiring ONCE; the central claim is EQUIVALENCE against a hand-retyped copy of run_pwr2_loadfollow's ride (bands admit the declared differences: the facade runs CVCS and a slewed rod bank). Every command lands with an observable effect; the caller-half of Hard Rule 5 is pinned (a 40 cm2 break trips and scrams UNCOMMANDED, SI starts the ECCS; 30 s window -- riding deeper reaches #499, the near-floor h-oscillation the beyond-model latch cannot see). THE PUSHBUTTON section pinned a real facade defect on arrival (measured 2026-08-19): 'scram' bypassed the RPS -- scrammed stayed false, the turbine kept pulling 100 MWe from a 2 % core, and the -240 F/min cooldown drained the pressurizer into an unlatched solver root-jump at 1724 psia (#499 thread, second instance). Now: manual trip is an RPS input (Ginna TS Bases B 3.3.1 Fn 1), the turbine trips with the reactor (UFSAR ch15, 'zero delay is assumed'), and the scram ramp is monotone-down (a second trip edge can never move rods OUT). 8 mutations, all caught.  // 140 -> 142 (#605, 2026-09-02): the loss-of-main-feed chain gained BOTH directions of its mode arming — a cold plant on RHR with its pumps secured must NOT fire it, and securing RHR must re-arm it on the same standing loss. Either half alone is satisfiable by a chain that never fires.
  'run_pwr2_loca.js':     { code: 0, score: '22passed 0failed 22checks', secs: 30 },   // 18 -> 19 (#585, 2026-08-29): the interim last-running-step snapshot is RETIRED WITH THE DEFECT — the ride now steps PAST the latch and asserts the invariant the defect violated at every held step (638 of them, drift EXACTLY 0 kg across plant/break/containment/ECCS/tank), and both closure identities hold at the FINAL state (1e-6 relative). dt_accepted closes the latching step: the Courant sub-stepping integrates part of it, and the ledgers book exactly the plant's own report (0.966 kg — one of two substeps — was the gap all-or-nothing booking left).   // 17 -> 18 (#574, 2026-08-28): the mass identities are asserted at the last RUNNING step. A held plant freezes while the break keeps discharging into containment — 222 held steps, 69.4 kg of mass from nowhere, filed #585. The frozen step is a named measured number, not a widened tolerance.   // 14 -> 17 (#511, 2026-08-24): the ACCUMULATOR joins the joint ride — pumps OFF, a 0.004 m2 break, and the passive tank empties itself (10,187 kg) with the mass closure EXACT (primary net = discharged - injected to 1e-6) and containment blind to the injection.   // FIXTURES PRESSURIZED (#486 close-out, 2026-08-19): the mass-ledger claims are identities and held untouched; ONE band re-measured -- HHSI start 3.0 -> 30 s, because the vessel now outsurges against the blowdown and holds the plant above the shutoff head for a measured 12.0 s, the SI-delay role a real pressurizer plays in a medium break.   // JOINT: break -> containment -> ECCS, added 2026-08-17 (#479). Mass bookkeeping closes to 1e-6 relative in both directions; ECCS starts injecting at t=1.10s, the moment P crosses the 9.58 MPa HHSI shutoff head -- physics-timed, not scripted.
   ...
   * SHIPPED WITH 3 XFAILS ON 2026-08-28 AND ALL THREE CLOSED ON 2026-08-29, for three different
   * reasons worth keeping apart: SI-2 by the fix (`tick()`'s `sinceEval` now carries on the
   * instance and the post-loop evaluation obeys the cadence — 10.85 evaluations/sim-s at 1x in a
   * transient became 10.00, matching every other speed); SI-6 by the same fix, after the check
   * itself was repaired (its first form scanned raw source and kept failing because the FIX'S
   * OWN COMMENT quotes the line it replaced); and SI-4 because IT WAS NEVER A REAL FAILURE —
   * it compared the two legs at their STOPPING POINTS, 200.02 s against 200.00 s, which on a
   * blowdown moving ~128 psi/s reads as 267.31 vs 269.87 psi. At matched instants the
   * trajectories are identical to 0.000e+0, before AND after the fix. Every check compares at
   * matched sim instants now and asserts the intersection is non-empty. */
'run_service_invariance.js': { code: 0, score: '8passed 0xfail 0failed 8checks', secs: 90 },
  /* NEW (#625, 2026-09-04): THE TWO PACING TIERS. WARP (600x+) steps the same physics at 0.5 s
   * — a declared departure the owner ruled in — and this is the band that bounds it (one sim
   * hour in five regimes vs the PLAY reference, 2x the worst measured deviation), plus the proof
   * the band can fail (a 1.0 s step trips the quiet plant), the in-loop lockout (a large break
   * stops the loop on step 1 of 720 and credits 0.5 s), the step budget (bit-identical on and
   * off at matched instants), and the cadence fix. Six mutations. The five PLAY reference hours
   * are ~85 % of its wall. */
  /* 17 -> 21 (#631, 2026-09-04): THE BUDGET IS PER TIER. One `stepBudgetMs` served both, and
   * 40 ms of a 100 ms broadcast is the right reserve for PLAY (a transient is when the player
   * is clicking) and the wrong one for WARP, which runs only on a quiet plant — it capped the
   * top rung at what 40 ms buys, ~1,050x against a requested 3600x on every machine. Four
   * checks: WARP spends 70 (375 of 720 steps) where PLAY spends 40 (225 of 300) on the SAME
   * service one `set_speed` apart; an unset warp budget falls back to the PLAY one; and a
   * broadcast CUT at 375 steps is bit-identical to one that REQUESTED 375, which is the
   * "only the instants move" claim at the seam where the two loop paths differ (a budget break
   * at i=374 of 720 against a natural exit that routes the final evaluation post-loop).
   * Mutations 6 -> 9 (WARP capped at PLAY's; PLAY handed WARP's; the snapshot publishing the
   * configured budget instead of the effective one). */
  'run_warp_tier.js':      { code: 0, score: '23passed 0failed 23checks', secs: 240 },   // 21 -> 23 (#655, 2026-09-08): WT-3h a caution does not drop WARP, WT-3i a warning drops it and names the alarm
  ...
  'run_dashboard_time.js': { code: 0, score: '14/14 88/88' },   // 87 -> 88 (#674, 2026-09-09): the in-sim block left analytics.js for usage.js and took every Analytics Engine query with it, so 'the SQL window is still relative and zone-free' was re-pointed at the file that now runs one -- left where it was it would have gone green over a page with nothing to assert about. The +1 is usage.js's own tripwire: it is EXEMPT from the ET-import sweep because it renders no instant, so what is pinned instead is that it still renders none. An AE timestamp is already shaped "YYYY-MM-DD HH:MM:SS" and the first one printed there would look right and be four hours out, with no ET helper in the file to have been forgotten.
  /* NEW (#674, 2026-09-09): THE FEATURE USAGE VIEW. `worker/src/usage.js` is where the
   * walkthrough telemetry becomes an ANSWER -- a completion rate, a drop-off percentage,
   * a p90 -- and all of that arithmetic is ours, not Cloudflare's. Nothing else looks at
   * a dashboard NUMBER: run_telemetry reads the ingest as text, run_rollup drives the
   * scheduled job, run_dashboard_time covers render.js's clock. A wrong denominator or a
   * quantile index off by one renders a confident, plausible, wrong page. Runs offline --
   * cfapi.js is stubbed wholesale and no query leaves the process. `--inject` flips the
   * three defects it exists for and must red 6 checks. */
  'run_usage_page.js':     { code: 0, score: '47passed 0failed', secs: 1 },
  ...
  'run_manual_controls.js': { code: 0, score: '646checks 0failed' },   // 649 -> 646 (#698, 2026-09-11): -3, the boron SAMPLE step deleted from `pwr2:pwr_raise_power`. The step is gone because its CONTROL is gone (the board's SAMPLE button was removed by owner ruling), which is the #641 shape — a command-kind check-off outliving the control that produces the command. The three are its coverage row, its `control` mapping and its one `hl` label; `test/manual_ui_map.js` is POSITIONAL and its rows below the deleted step were moved up one by hand, not re-derived. // 605 -> 649 (#692 + #653, 2026-09-11): +44 `hl_watch` LABEL checks, one per label, from the authoring pass that #685 deliberately left for the rewrites touching the same steps. Steps carrying a watch target went 8 -> 56 of 89; every one of the 44 new labels is validated against the board's own vocabulary through the same revealControl lookup `hl` uses, and the DISJOINT check between the two lists still scores zero. No check was added or removed by the runner itself -- the count IS the label count, so it moves whenever the pool's highlight authoring moves. // 601 -> 605 (#693, 2026-09-11): net +4 from the TMI-2 cascade split. The lump step's two `hl` labels go away with it; the five narrated steps that replace it carry six `hl_watch` labels between them (feed flow + primary pressure, turbine, auxiliary feed + steam generator level, the relief valve). The STEP_UI rows in manual_ui_map.js all MOVED +4 -- this table is POSITIONAL and re-deriving it is how it has been broken four times -- and none of the five new steps carries a `control`, so no coverage row is added or lost. // 593 -> 601 (#685, 2026-09-11): +8 hl_watch LABEL checks -- the new "watch this indication" list is validated against the board vocabulary exactly as `hl` is, because it resolves through the same revealControl lookup and fails the same silent way. The 8 are the four steps that measured as glowing NOTHING at all (pwr_heatup 17 three labels, pwr_tmi2_incident 9/12/13 two/two/one). A DISJOINT check between the two lists also ships and scores ZERO today, on purpose: no step names a label in both, and a label that is in both draws two rings on one element. Both injection-verified -- a typo'd label reddens 1 naming the step, and adding 'SG Level' to TMI-2 step 2's hl_watch beside its hl reddens the disjoint check. // MERGED 2026-09-11: measured 593 on the merged tree (#683's +4 and #684's -1 both landed, net +3 from 590) -- 590 -> 594 (#683, 2026-09-10): pwr_raise_power steps 10 and 11 (the verify step and the closing boron trim) gained a `control: 'Boron control'` field for the first time -- both were bare `obs()` narrative steps before #683's Part 1/3 fix and carried none. STEP_UI is POSITIONAL, so the two new rows (`i: 9`, `i: 10`) are APPENDED after the existing `i: 8` row, not inserted -- confirmed by spot-checking steps 8-11's own `control` field in ui/manual_procedures.js against the map rows now at those indices, all four agree. Injection-verified: pointing `i: 9` at a nonexistent control name reddens naming exactly "pwr_raise_power step 10", restored. // 559 -> 590 (#670 Phase 2, 2026-09-09): the TMI-2 incident walkthrough's own STEP_UI block -- 7 of its 16 steps carry a `control` (7 forward + 7 reverse checks) and 17 `hl` labels across the leg. The block is APPENDED; not one existing row moved, which is the whole discipline this table's header is about. // 552 -> 555 (#653 pass 3, 2026-09-07): the cooldown's heater-off is its own mapped step (Pressurizer Heaters (PZR) at i:5), the rest shift +1. // 547 -> 552 (#629, 2026-09-05): +5 for the heatup's STEAM DUMP AUTO step. ⚠ IT DRIFTED RED, NOT GREEN, AND THAT IS THE POINT: `STEP_UI` is indexed by ARRAY POSITION, so inserting a step at i:12 left the row below it pointing at the wrong step — reported as *pill "Steam Dump" != STEP_UI "Pressure SP"* at step 13 plus one UNMAPPED step at 14. That two-line shape is the INSERTION signature (the fourth time this table has been broken this exact way); the fix is to RENUMBER 12->13 and add the new row, never to re-derive the array. This is the gate that catches a checklist insertion nothing else sees — it is not in #629's work order's gate list and it found the omission. // 543 -> 547 (#624/#619 item 14, 2026-09-04): +4 for the heatup's pressure-control step (its control row, its two acceptance lamps and its highlight pair). // 538 -> 543 (#624/#619 items 14/25, 2026-09-04): +5 for the heatup's two new steps (the letdown line-up and the post-ride transfer confirmation) — STEP_UI rows are POSITIONAL, so the same off-by-one the note below records was re-run here and the moved rows were moved rather than the numbering re-derived.   // 535 -> 538 (#624/#619 item 27, 2026-09-04): +3 for the ascension's new boron SAMPLE step (one STEP_UI mapping check, one coverage check, one control-label check -- the same 1-step-equals-N arithmetic the notes below record). THE GATE CAUGHT THE INSERTION'S OFF-BY-ONE: STEP_UI rows are POSITIONAL, so a step added at i:2 shifted every row after it and the runner reported both symptoms -- a pill/row mismatch at the insertion point and an UNVERIFIED tail step. Fixed by moving the rows that moved, not by re-deriving the numbering, which is how this map has been broken three times.   // 532 -> 535 (#624/#619 item 28, 2026-09-04): +3. The five power-ascension legs now carry the rod withdrawal as the step's own `cmd` and the load target as an accs cmd-kind entry, so each leg commands BOTH of the actions its text describes. Before this the withdrawals were prose only -- and, from the leg's old `50_percent` start, no-ops against the bank's top stop.   // 530 -> 532 at the workbench merge (2026-09-03): MEASURED, not computed. The two lanes moved this in opposite directions and the net is zero -- #608 took a STEP_UI row OUT (heatup step 6 became an observation, and an obs step owns no `control`), #607 put two back. Both comments below are kept because each names a change the other does not.   // 532 -> 530 (#608, 2026-09-02): -2, and it is ONE deletion, not two regressions. pwr_heatup step 6 became an observation (it commanded the steam-dump setpoint the Mode 5 initial condition already sits at), and an obs step carries no `control`, so its STEP_UI row in manual_ui_map.js goes with it -- one mapping check plus one coverage check, the same 1-step-equals-2-checks arithmetic #468 recorded when it ADDED a step. The row was DELETED and the tail indices left alone: the step keeps its array position, so i:6/i:7/i:9 are unchanged. Re-deriving the numbering instead of deleting the one row is how this map has been broken three times.   // 538 -> 532 (#605, 2026-09-02): six fewer LABELS to check, not six fewer covered steps — 'ECCS' left the six accumulator steps' `hl` lists. It resolves to the enclosing ECCS panel (ims3l6k3mb0, 1055,600), which does not contain the accumulator valve at all, so those steps were lighting an unrelated card beside the valve the owner asked them to point at.   // 268 -> 538 (#598 item 14, 2026-09-01): EVERY STEP'S `hl` HIGHLIGHT LABELS ARE NOW CHECKED against the board's own vocabulary. `st.control` has been validated this way since #304; `hl` — a strictly larger vocabulary, carried by all 60 pwr2 steps where `control` is carried by 47 — had NO check anywhere in test/. app.js resolves each label through revealControl, which returns null for an unknown one, so a bad label glows NOTHING and looks exactly like a step that asked for no highlight. BORN RED ON TWELVE REAL SITES, which is its own injection proof: 'ECCS' (5 steps, both pools) and 'SG Pressure' (1) were never in CONTROL_LABEL_MAP and are added, pointing at the ECCS panel and the SG pressure readout; 'Reactivity' (2) is DELETED from the steps instead — the board's reactivity readout was removed at #453 and pointing the label at the period or the boron card would be a highlight that lies about what it is showing. Resolved through the DRIVER's map, not a copy, so deleting a key from CONTROL_LABEL_MAP reddens this too.: the pwr2 startup checklist gains the INTERMEDIATE RANGE block step ahead of the low-flux one, so `pwr2:pwr_startup` carries two Trip Blocks rows. THE GATE CAUGHT IT — the new step was UNVERIFIED until its STEP_UI row was added, which is the hand-maintained-map trap being caught by the check written for it rather than by nobody.    // 176 -> 266 (#244/#254/#526, 2026-08-31): the gate discovered the new pwr2 pool and scored 52 FAILURES against it — no VIEW_CONTROLS row for pwr2 and STEP_UI keyed by bare procedure id while the two pools share ids with different steps. manual_ui_map gains a pwr2 vocabulary row (the same board's CONTROL_LABEL_MAP) and profile-PREFIXED 'pwr2:<id>' coverage rows; an OWN_POOL_PROFILES profile never falls back to another pool's rows, so a coincidental pill match cannot read as coverage. verify_manual_follow stays pwr-only; the pwr2 rows are the static gate's coverage.   // 532 -> 534 (#607, 2026-09-03): two heatup confirm steps gained `hl` lists (Mode 3 arrival, reactor stayed shut down). RCP start lost the pump-graphic label (card only). Net +2.   // 538 -> 532 (#605, 2026-09-02): six fewer LABELS to check, not six fewer covered steps — 'ECCS' left the six accumulator steps' `hl` lists. It resolves to the enclosing ECCS panel (ims3l6k3mb0, 1055,600), which does not contain the accumulator valve at all, so those steps were lighting an unrelated card beside the valve the owner asked them to point at.   // 268 -> 538 (#598 item 14, 2026-09-01): EVERY STEP'S `hl` HIGHLIGHT LABELS ARE NOW CHECKED against the board's own vocabulary. `st.control` has been validated this way since #304; `hl` — a strictly larger vocabulary, carried by all 60 pwr2 steps where `control` is carried by 47 — had NO check anywhere in test/. app.js resolves each label through revealControl, which returns null for an unknown one, so a bad label glows NOTHING and looks exactly like a step that asked for no highlight. BORN RED ON TWELVE REAL SITES, which is its own injection proof: 'ECCS' (5 steps, both pools) and 'SG Pressure' (1) were never in CONTROL_LABEL_MAP and are added, pointing at the ECCS panel and the SG pressure readout; 'Reactivity' (2) is DELETED from the steps instead — the board's reactivity readout was removed at #453 and pointing the label at the period or the boron card would be a highlight that lies about what it is showing. Resolved through the DRIVER's map, not a copy, so deleting a key from CONTROL_LABEL_MAP reddens this too.: the pwr2 startup checklist gains the INTERMEDIATE RANGE block step ahead of the low-flux one, so `pwr2:pwr_startup` carries two Trip Blocks rows. THE GATE CAUGHT IT — the new step was UNVERIFIED until its STEP_UI row was added, which is the hand-maintained-map trap being caught by the check written for it rather than by nobody.    // 176 -> 266 (#244/#254/#526, 2026-08-31): the gate discovered the new pwr2 pool and scored 52 FAILURES against it — no VIEW_CONTROLS row for pwr2 and STEP_UI keyed by bare procedure id while the two pools share ids with different steps. manual_ui_map gains a pwr2 vocabulary row (the same board's CONTROL_LABEL_MAP) and profile-PREFIXED 'pwr2:<id>' coverage rows; an OWN_POOL_PROFILES profile never falls back to another pool's rows, so a coincidental pill match cannot read as coverage. verify_manual_follow stays pwr-only; the pwr2 rows are the static gate's coverage.   // ALSO: // 590 -> 589 (#684 section D, 2026-09-10): ONE FEWER LABEL, not one fewer covered step. The pwr2 cooldown's RCP-stop step named BOTH 'RCP Run/Stop' (the card) and 'Reactor Coolant Pumps (RCP)' (the pump art on the loop), whose rects overlap -- which is the owner's "currently both get highlighted", reported for the SECOND time after #607 item 1. One label lights one element, so the fix is deleting the pump label from that step; the label STAYS in the vocabulary, because the wiring reserves it for watch-the-flow steps. Every other pwr2 RCP step already named the card alone.   // 559 -> 590 (#670 Phase 2, 2026-09-09): the TMI-2 incident walkthrough's own STEP_UI block -- 7 of its 16 steps carry a `control` (7 forward + 7 reverse checks) and 17 `hl` labels across the leg. The block is APPENDED; not one existing row moved, which is the whole discipline this table's header is about. // 552 -> 555 (#653 pass 3, 2026-09-07): the cooldown's heater-off is its own mapped step (Pressurizer Heaters (PZR) at i:5), the rest shift +1. // 547 -> 552 (#629, 2026-09-05): +5 for the heatup's STEAM DUMP AUTO step. ⚠ IT DRIFTED RED, NOT GREEN, AND THAT IS THE POINT: `STEP_UI` is indexed by ARRAY POSITION, so inserting a step at i:12 left the row below it pointing at the wrong step — reported as *pill "Steam Dump" != STEP_UI "Pressure SP"* at step 13 plus one UNMAPPED step at 14. That two-line shape is the INSERTION signature (the fourth time this table has been broken this exact way); the fix is to RENUMBER 12->13 and add the new row, never to re-derive the array. This is the gate that catches a checklist insertion nothing else sees — it is not in #629's work order's gate list and it found the omission. // 543 -> 547 (#624/#619 item 14, 2026-09-04): +4 for the heatup's pressure-control step (its control row, its two acceptance lamps and its highlight pair). // 538 -> 543 (#624/#619 items 14/25, 2026-09-04): +5 for the heatup's two new steps (the letdown line-up and the post-ride transfer confirmation) — STEP_UI rows are POSITIONAL, so the same off-by-one the note below records was re-run here and the moved rows were moved rather than the numbering re-derived.   // 535 -> 538 (#624/#619 item 27, 2026-09-04): +3 for the ascension's new boron SAMPLE step (one STEP_UI mapping check, one coverage check, one control-label check -- the same 1-step-equals-N arithmetic the notes below record). THE GATE CAUGHT THE INSERTION'S OFF-BY-ONE: STEP_UI rows are POSITIONAL, so a step added at i:2 shifted every row after it and the runner reported both symptoms -- a pill/row mismatch at the insertion point and an UNVERIFIED tail step. Fixed by moving the rows that moved, not by re-deriving the numbering, which is how this map has been broken three times.   // 532 -> 535 (#624/#619 item 28, 2026-09-04): +3. The five power-ascension legs now carry the rod withdrawal as the step's own `cmd` and the load target as an accs cmd-kind entry, so each leg commands BOTH of the actions its text describes. Before this the withdrawals were prose only -- and, from the leg's old `50_percent` start, no-ops against the bank's top stop.   // 530 -> 532 at the workbench merge (2026-09-03): MEASURED, not computed. The two lanes moved this in opposite directions and the net is zero -- #608 took a STEP_UI row OUT (heatup step 6 became an observation, and an obs step owns no `control`), #607 put two back. Both comments below are kept because each names a change the other does not.   // 532 -> 530 (#608, 2026-09-02): -2, and it is ONE deletion, not two regressions. pwr_heatup step 6 became an observation (it commanded the steam-dump setpoint the Mode 5 initial condition already sits at), and an obs step carries no `control`, so its STEP_UI row in manual_ui_map.js goes with it -- one mapping check plus one coverage check, the same 1-step-equals-2-checks arithmetic #468 recorded when it ADDED a step. The row was DELETED and the tail indices left alone: the step keeps its array position, so i:6/i:7/i:9 are unchanged. Re-deriving the numbering instead of deleting the one row is how this map has been broken three times.   // 538 -> 532 (#605, 2026-09-02): six fewer LABELS to check, not six fewer covered steps — 'ECCS' left the six accumulator steps' `hl` lists. It resolves to the enclosing ECCS panel (ims3l6k3mb0, 1055,600), which does not contain the accumulator valve at all, so those steps were lighting an unrelated card beside the valve the owner asked them to point at.   // 268 -> 538 (#598 item 14, 2026-09-01): EVERY STEP'S `hl` HIGHLIGHT LABELS ARE NOW CHECKED against the board's own vocabulary. `st.control` has been validated this way since #304; `hl` — a strictly larger vocabulary, carried by all 60 pwr2 steps where `control` is carried by 47 — had NO check anywhere in test/. app.js resolves each label through revealControl, which returns null for an unknown one, so a bad label glows NOTHING and looks exactly like a step that asked for no highlight. BORN RED ON TWELVE REAL SITES, which is its own injection proof: 'ECCS' (5 steps, both pools) and 'SG Pressure' (1) were never in CONTROL_LABEL_MAP and are added, pointing at the ECCS panel and the SG pressure readout; 'Reactivity' (2) is DELETED from the steps instead — the board's reactivity readout was removed at #453 and pointing the label at the period or the boron card would be a highlight that lies about what it is showing. Resolved through the DRIVER's map, not a copy, so deleting a key from CONTROL_LABEL_MAP reddens this too.: the pwr2 startup checklist gains the INTERMEDIATE RANGE block step ahead of the low-flux one, so `pwr2:pwr_startup` carries two Trip Blocks rows. THE GATE CAUGHT IT — the new step was UNVERIFIED until its STEP_UI row was added, which is the hand-maintained-map trap being caught by the check written for it rather than by nobody.    // 176 -> 266 (#244/#254/#526, 2026-08-31): the gate discovered the new pwr2 pool and scored 52 FAILURES against it — no VIEW_CONTROLS row for pwr2 and STEP_UI keyed by bare procedure id while the two pools share ids with different steps. manual_ui_map gains a pwr2 vocabulary row (the same board's CONTROL_LABEL_MAP) and profile-PREFIXED 'pwr2:<id>' coverage rows; an OWN_POOL_PROFILES profile never falls back to another pool's rows, so a coincidental pill match cannot read as coverage. verify_manual_follow stays pwr-only; the pwr2 rows are the static gate's coverage.   // 532 -> 534 (#607, 2026-09-03): two heatup confirm steps gained `hl` lists (Mode 3 arrival, reactor stayed shut down). RCP start lost the pump-graphic label (card only). Net +2.   // 538 -> 532 (#605, 2026-09-02): six fewer LABELS to check, not six fewer covered steps — 'ECCS' left the six accumulator steps' `hl` lists. It resolves to the enclosing ECCS panel (ims3l6k3mb0, 1055,600), which does not contain the accumulator valve at all, so those steps were lighting an unrelated card beside the valve the owner asked them to point at.   // 268 -> 538 (#598 item 14, 2026-09-01): EVERY STEP'S `hl` HIGHLIGHT LABELS ARE NOW CHECKED against the board's own vocabulary. `st.control` has been validated this way since #304; `hl` — a strictly larger vocabulary, carried by all 60 pwr2 steps where `control` is carried by 47 — had NO check anywhere in test/. app.js resolves each label through revealControl, which returns null for an unknown one, so a bad label glows NOTHING and looks exactly like a step that asked for no highlight. BORN RED ON TWELVE REAL SITES, which is its own injection proof: 'ECCS' (5 steps, both pools) and 'SG Pressure' (1) were never in CONTROL_LABEL_MAP and are added, pointing at the ECCS panel and the SG pressure readout; 'Reactivity' (2) is DELETED from the steps instead — the board's reactivity readout was removed at #453 and pointing the label at the period or the boron card would be a highlight that lies about what it is showing. Resolved through the DRIVER's map, not a copy, so deleting a key from CONTROL_LABEL_MAP reddens this too.: the pwr2 startup checklist gains the INTERMEDIATE RANGE block step ahead of the low-flux one, so `pwr2:pwr_startup` carries two Trip Blocks rows. THE GATE CAUGHT IT — the new step was UNVERIFIED until its STEP_UI row was added, which is the hand-maintained-map trap being caught by the check written for it rather than by nobody.    // 176 -> 266 (#244/#254/#526, 2026-08-31): the gate discovered the new pwr2 pool and scored 52 FAILURES against it — no VIEW_CONTROLS row for pwr2 and STEP_UI keyed by bare procedure id while the two pools share ids with different steps. manual_ui_map gains a pwr2 vocabulary row (the same board's CONTROL_LABEL_MAP) and profile-PREFIXED 'pwr2:<id>' coverage rows; an OWN_POOL_PROFILES profile never falls back to another pool's rows, so a coincidental pill match cannot read as coverage. verify_manual_follow stays pwr-only; the pwr2 rows are the static gate's coverage.
  ...
  'run_release.js':        { code: 0, score: '29checks 0failed' },   // 28 -> 29 (2026-09-05-develop-e): the Alpha 1.7.4-rc1 entry opened by 8399d6cd added its per-entry CROSS check and the baseline was not moved with it; acknowledged here, no new assertion. // 27 -> 28 (#630, 2026-09-05): NOT a new assertion — the CROSS band runs one check per published entry, so opening the Alpha 1.7.3-rc1 entry adds one. The count is the entry tally and moves with every release.   // 25 -> 27 (#611, 2026-09-03): the gate learned a THIRD state. The release presentation is now assembled at the first push to develop, not at the merge to main, and the pending version wears -rc -- which landed in the WRONG state: RELEASED was a strict /^Alpha X.Y.Z$/ on RD_RELEASE, and failing it flips the file into PRE-LAUNCH mode, whose rules assert changelog.html has ZERO entries. Adopting the policy without touching this reddens the gate on the first push, and the obvious way to quieten that -- relaxing the pre-launch assertion -- DISARMS every cross-file consistency check at exactly the moment they do the most work. So -rc counts as RELEASED (VER_RE takes the optional suffix) and PENDING only adds information to the report. +1 for the PENDING banner on the VERSION line, +1 for the new all-or-nothing check: the suffix is on all three files or none, because half-applied ships a build calling itself 1.7.2-rc against a changelog headed 1.7.2. That check proved itself within minutes -- a failed CHANGELOG.md edit left two files of three carrying it and the gate named the half-finished bump rather than reporting it as a version disagreement.   // 24 -> 25 (Alpha 1.7.1, 2026-08-31): one more published entry -> one more CROSS row.   // 22 -> 23 (Alpha 1.6.1): one more CROSS pairing, as every release adds. | 21 -> 22 (Alpha 1.6.0): one more CROSS pairing, as every release adds. | 20 -> 21 (Alpha 1.5.2): one more CROSS pairing, as every release adds.   // 19 -> 20 (2026-08-09, Alpha 1.5.1): same mechanism as the row below — one more CROSS pairing. | 18 -> 19 (2026-08-09, Alpha 1.5.0): one more CROSS row — the check pairs each changelog.html entry with its CHANGELOG.md heading, so it grows by one per release. 9 published entries, 9 version headings.   || workbench: 27 -> 28 (#628 follow-up, 2026-09-05): the Alpha 1.7.3-rc1 entry -- one more CROSS pairing, as every release and every pending entry adds. It opened a NEW entry rather than extending 1.7.2's because 1.7.2 had SHIPPED hours earlier; the first draft edited the released section in place, which this gate cannot see (it checks that the three version strings AGREE, and they agree just as well after a released section is rewritten).   // 25 -> 27 (#611, 2026-09-03): the gate learned a THIRD state. The release presentation is now assembled at the first push to develop, not at the merge to main, and the pending version wears -rc -- which landed in the WRONG state: RELEASED was a strict /^Alpha X.Y.Z$/ on RD_RELEASE, and failing it flips the file into PRE-LAUNCH mode, whose rules assert changelog.html has ZERO entries. Adopting the policy without touching this reddens the gate on the first push, and the obvious way to quieten that -- relaxing the pre-launch assertion -- DISARMS every cross-file consistency check at exactly the moment they do the most work. So -rc counts as RELEASED (VER_RE takes the optional suffix) and PENDING only adds information to the report. +1 for the PENDING banner on the VERSION line, +1 for the new all-or-nothing check: the suffix is on all three files or none, because half-applied ships a build calling itself 1.7.2-rc against a changelog headed 1.7.2. That check proved itself within minutes -- a failed CHANGELOG.md edit left two files of three carrying it and the gate named the half-finished bump rather than reporting it as a version disagreement.   // 24 -> 25 (Alpha 1.7.1, 2026-08-31): one more published entry -> one more CROSS row.   // 22 -> 23 (Alpha 1.6.1): one more CROSS pairing, as every release adds. | 21 -> 22 (Alpha 1.6.0): one more CROSS pairing, as every release adds. | 20 -> 21 (Alpha 1.5.2): one more CROSS pairing, as every release adds.   // 19 -> 20 (2026-08-09, Alpha 1.5.1): same mechanism as the row below — one more CROSS pairing. | 18 -> 19 (2026-08-09, Alpha 1.5.0): one more CROSS row — the check pairs each changelog.html entry with its CHANGELOG.md heading, so it grows by one per release. 9 published entries, 9 version headings.  BOTH lanes moved it 27 -> 28 and the merged answer is still 28, not 29: each added the SAME pending 1.7.3-rc1 entry, and the score counts CROSS pairings, one per entry.
  // NEW 2026-09-05 (#637) — the SHARDED CI workflow, which is now the only shape in which the
  // aggregate gate runs on GitHub. Three static bands read out of .github/workflows/gates.yml
  // with a hand parser (no package.json here, so no YAML library) plus one FUNCTIONAL band that
  // drives `run_all --shard=i/N --list` for real. It exists because a workflow file can only
  // otherwise be proven by pushing it — this repo gates main and develop only, which is exactly
  // how the --fast version shipped broken and ran red for 32 consecutive runs across a release.
  // THE TRAP IT IS BUILT AROUND: main's branch ruleset (id 20067220) requires a status check
  // whose context is EXACTLY `aggregate-gate`, and a matrix job reports as `aggregate-gate (1)`
  // — so a matrix under that id leaves the required check unreported for ever and blocks every
  // release pull request behind three green checks. Hence shards under `gate-shard` and a
  // fan-in job holding the reserved id, all of it asserted here.
  // Its 12-check SHARDS band includes the sacred one, proved BY INJECTION on every run rather
  // than scanned for: a probe runner is written into test/, every shard's --list must exit 1
  // and name it, and it is removed in a finally and on exit. A runner falling between shards is
  // the failure this repo has already had twice with no sharding at all.
  // INJECTION-VERIFIED BEFORE BASELINING — every one of the 34 reddened at least once, mostly
  // one at a time, and these are the observed tallies (34 checks, n failed): the shard job
  // holding the reserved id 6; a second matrix on the fan-in 2; `fail-fast: true` 1; matrix
  // [1,2,4] 1; N mismatched between the matrix and --shard 1; a second run_all step 4; equal
  // step/job budgets 1; the fan-in deleted 7, given a display name 1, its `needs` removed 1,
  // its `if: always()` removed 1, its script cut to a bare echo 4; a plain `diagnostic-reports`
  // artifact name 1; the upload step removed 1 (the non-vacuity check); an unparseable line 18.
  ...
  'run_site_build.js':     { code: 0, score: '41checks 0failed' },   // 31 -> 41 (#523, 2026-08-26): the new RETIRED rule, ten checks, proving BOTH DIRECTIONS of the retired-engine strip — a public build carries neither the six files nor a tag naming them, a preview build carries all six, and the two files PWR2 REUSES (pwr_config, pwr_instruments) survive both. It builds twice, via the new RD_SITE_CHANNEL hook, because site/channel.js is tracked and a gate must not rewrite the tree it measures. Injection-verified in both directions: strip unconditionally -> 41/2 naming the preview rows; disable the strip -> 41/2 naming the public rows.
  ...
  // The prose gate for player-facing text (2026-09-03) — the rules in
  // Blueprint/STYLE_GUIDE.md that a regex can decide, over the live PWR2 checklist
  // pool, the PWR alarm labels and Manuals/*.md. Every check was at ZERO when written,
  ...
  // its own mutation and they all go red. That mode FORCES a non-zero corpus and is
  // NEVER a baseline. The runner also prints a BACKLOG (over-cap step texts, the
  // details-paragraph word counts, the Manuals banned-word tallies) which is
  // deliberately kept off this tally line, the same split run_manual_units makes: a
  // count that moves on every ordinary prose edit teaches people to update the number
  // without reading it.
  // 7 -> 8 on 2026-09-03 (#619 item 12): `checklist_why_length`, the details paragraph
  // capped at four sentences. The owner's rule is "no more than 2 or 3 sentences"; the
  // check counts SENTENCES rather than words on purpose, because a word cap fought the
  // units rule — a step carrying three US/SI pressure pairs runs long in three
  ...
  // the pass, worst 246).
  'run_style.js':          { code: 0, score: '11checks 0failed' },   // 10 -> 11 (#692, 2026-09-11): `checklist_text_words` -- the STYLE_GUIDE W2 twenty-word cap on a step's instruction line, SCORED for the first time. It had been printed in the unscored BACKLOG block since the runner was written, on the argument that a moving number teaches the next agent to update it without reading it; the measurement refutes that here -- the backlog figure went 0 of 61 -> 8 of 67 -> 46 of 67 -> 55 of 89 across four authoring passes while the guide's prose still claimed "0 of 61". 55 of 89 step lines were over cap at the start of the pass and 0 are now. Proven red by injection: `run_style --self-test` is 11/11 CAN FAIL. // 9 -> 10 (#670 Phase 1, 2026-09-08): checklist_story_length -- an incident step's narrative fields (clock/saw/knew/did) are drawn ABOVE the instruction and are never behind the details fold, so their length is paid on every step; 2 sentences per field, half the `why` cap because there are four of them. `story` also joins the vague, tight-percent, bare-megawatt and no-SI scans, which is a corpus change rather than a check (the pool authors none yet, so all four stay at zero). NOT the modal or reversal scans: those are `['text']`-only and state a rule about IMPERATIVES, and a story field is past-tense narrative -- "the crew believed the valve should have shut" is correct English and a category error to forbid. // 8 -> 9 on 2026-09-06: checklist_no_si (owner ruling, #653)
  ...
  'run_m5.js':             { code: 0, secs: 6, score: '26/26 122passed' },   // 120 -> 122 (#655, 2026-09-08): a caution on a quiet board does not snap fast-forward; a warning does and names the alarm
  // 23 -> 26 suites, 104 -> 120 checks 2026-08-27 (#534 cluster: #554 + #553 + #563 item 3):
  // the restore is ALL-OR-NOTHING. _restore assigned this.engine/this.layer BEFORE
  // engine.loadState could throw, so a file the UI called "Not a valid save file" had
  // already swapped the running plant for a fresh one with every failure and latch wiped;
  // loadState's error RETURN is now pinned as a return (ui/app.js discarded it, #553); and
  // the save records the initial condition it was built from, which _restore hard-coded to
  // hot_full_power. Injection-verified: putting the old assign order back reds 4 of the new
  // checks (engine identity, layer identity, power, the active-failure list).
  ...
  'run_autoctl.js':        { code: 0, secs: 22, score: '31/31' },   // 30 -> 31 (#653, 2026-09-07): a second ON press mid-dose stops the makeup panel; injection = remove the engage-branch `set_boron_adjust rate 0` in control_kernel._toggleChannel
  ...
  'run_glow_stacking.js':  { code: 0, score: '20passed 0failed 20checks', secs: 1 },   // 18 -> 20 (#685, 2026-09-11): .ckl-watch-glow, the "watch this indication" ring beside the "press this control" pulse -- +1 pin check, +1 dead-entry check. It is in the pwr_board.css override list from the day it was written, the first time that has been true; injection-verified by deleting the .bd-tile line (1 red, naming .ckl-watch-glow UNPINNED) and restoring. // NEW (#598 item 12, 2026-09-01): HIGHLIGHT GLOWS MUST NOT RESTACK THE BOARD, and this is the THIRD time the same one-line omission shipped. Board box tiles are opaque and sit at base z while their own buttons sit at 1 and the vessel at 2, so any shared rule that lifts a tile's z-index paints an opaque panel over its own contents. pwr_board.css pins those classes back; .ckl-glow/.instr-glow were added to it by #202, .hl-glow/.hl-pin by #509, and #244's PERSISTENT .ckl-step-glow never was — the worst of the three, because it is not a hover and stands for minutes, and 13 of the pwr2 checklists' highlight labels resolve to opaque BOX panels (the owner reported the BORON panel "ends up on top of everything"). A comment in the CSS saying "add new classes here" did not stop it happening twice. This reads both stylesheets and requires the two lists to agree. The DECORATOR narrowing is the load-bearing part: a floating window carrying its own z-index in its own className (.oom-win, .app-toast, .tour-tip) is a component, never applied to a tile — and the test must NOT be `classList.add('literal')`, because the highlight bus hoists its names and hands them down two call hops as a parameter, which is exactly the pair #509 was about.
  'run_oneoverm.js':       { code: 0, score: '19passed 0failed 19checks', secs: 3 },   // 13 -> 19 (#623/#619 item 23, 2026-09-04): the panel gains an in-place HELP expander (the Scanner's idiom, chosen by directive over a modal because a modal covers the plot you are asking about). Six checks in two halves, split because the DOM stub does NOT parse HTML and its querySelector hands out a fresh element with hidden:false for any selector -- so a naive toggle test would be testing the stub. Four assert the BUILT MARKUP (the button, a panel that starts hidden, and the two things the copy must carry); two drive the panel's own click delegation with the starting state set explicitly. Injection-verified: deleting the toggle line reddens the open check and nothing else.   // NEW (#598 item 2, 2026-09-01): THE 1/M STARTUP PLOT, which NOTHING gated — it is pure UI below the board, so run_pwr2_board never sees it, verify_flags_ui never opens it, and no runner require()'d the file. It gated itself on `plant_id !== 'pwr'` in two places and PWR2 publishes 'pwr2', so the tile opened the window and the NEXT broadcast hid it again; a player fast enough to press "Plot point" read "PWR only". Six steps of pwr_startup — the approach to criticality, the whole reason the tool exists — could not be performed on the plant the site runs. The fix is a CAPABILITY test (a source-range count and a control group), not a bigger plant list, so the checks assert the capability and section 4 proves a plant without a source-range channel is still refused. Driven against a real pwr2 SimulationService through the panel's own surface with a DOM stub; sections 3 and 4 use plant-id-relabelled snapshots rather than booting the retired engine, which #523 strips from a public build.
  'run_checklist.js':      { code: 0, score: '90/90' },   // 78 -> 90 (#694, 2026-09-10): +12, section 11 -- the walkthrough pause. A synthetic `zz_pause_probe` (an `inject`+`pause` step, category `incident`) drives `tick()` directly with NO UI loaded, proving the SERVICE stops itself: `svc.running` false, `tick()` itself returns null while paused, 20 further attempted broadcasts move sim_time by exactly zero, and a bare `checklist_check` (no `releaseHold`) advances the step WITHOUT resuming the clock -- that half is `ui/app.js`'s job, verified by an explicit `svc.start()`. Also asserts the ordering guarantee: the event lands on a LIVE, non-lagged read AND on the same broadcast the pause requested, which only holds because the pause forces the same reassembly the rewind path already uses. Proven red two ways (both restored): neutering the instructor's early-return reds 7 checks (the event still fires, but nothing else does); removing `_serviceInstructorRequests`'s `this.stop()` reds 4 (a harness with no UI cannot fall back on `ui/app.js`'s own compensating `pauseSim` the way `verify_e2e_ui`'s browser gate can -- see that runner's note, and TUNING_LOG 2026-09-10-workbench-d for why both gates are needed). // 77 -> 78 (#670 Phase 2, 2026-09-09): the Phase-1 check "the shipped pools author no inject/clear/story yet" MOVED rather than being deleted -- it now asserts (a) the four narrative/injection fields appear ONLY in a leg whose category is `incident`, so a cycle leg cannot quietly grow a behind-the-scenes failure, and (b) every `crew` tag sits on a step that carries a command, because the tag on a verification teaches the opposite of what the step wants. One check became two. Both proven red by injection: a `story` block moved onto pwr_shutdown reds (a); a `crew` tag on the incident's step 12 reds (b). // 60 -> 77 (#670 Phase 1, 2026-09-08): section 10, the incident-walkthrough runtime -- a step's `inject`/`clear` fires failures behind the scenes and `story`/`crew` carry the history. TWO THINGS THE BRIEF DID NOT ASSUME. (1) THE INJECTION CANNOT FIRE ON THE STEP'S ENTRY TICK: `_checklistCheckOff` requests the step-boundary checkpoint and M5 services it AFTER instructor.step() in the SAME _assembleWithInstructor, so an entry-tick injection lands inside the checkpoint Rewind restores -- injection-measured, three reds, one of them the plant reading back "active [stuck_porv_open]" where the probe wants it gone. It fires one broadcast later, by which time the clean checkpoint is on the ring. (2) THE `fired`-SET IN THE CHECKPOINT WAS A DARK WIRE until probe (g) existed: a rewind in this probe lands on step-boundary checkpoints, and those are laid immediately after the set is cleared, so they are empty either way -- deleting `fired`/`injected` from getState reddened NOTHING. The consequence that CAN see them is a SAVE taken mid-step, which must not let the step break the plant a second time on load; asserted on the inject_failure COMMAND COUNT, because an idempotent failure table makes the plant look identical either way (#542's shape). Seven injections, every probe red under at least one: entry-tick fire (3 red), fired-set guard dropped (3), `when` ignored (1), per-step reset dropped (2), story/crew dropped from the snapshot (2), the fire call removed (7), getState fields dropped (1).   // 47 -> 60 (#660 rewind guard, 2026-09-08): section 9, the walkthrough's Rewind button. Two measurements, and the FIRST ONE KILLED THE BRIEFED DEFECT: there is no one-broadcast gap after Continue, because `checklist_check` runs _assembleWithInstructor in the same call and the instructor's checkpoint request is serviced there -- ring 3 -> 4 with no tick, and a rewind issued immediately after landed at step_index 2 / simTime 2.00, identical to the same rewind one tick later. The real gap is the LOAD: a save restores step_index 2 and clears the ring, so the button drawn on `step_index > 0` sat lit over a command the service refuses ("no checkpoint to rewind to", nothing moved). rewind_ready is derived from the ring's newest checkpoint, never book-kept -- _rewind TRUNCATES, so a remembered index would read stale and the second press would go dark. Injection-verified four ways: always-true reds 2, always-false reds 3, dropping the idx>0 clause reds the step-0 probe (the ring holds free play's own checkpoint underneath), and deleting loadState's ring clear reds 2 -- that last one FIRST PASSED UNCHANGED, because the loading service was freshly constructed and its ring was empty anyway; it now takes three free-play ticks first. // 46 -> 47 (#660 C, 2026-09-08): every step waits for Continue; 38 -> 46 (#622/#619 wave 2, 2026-09-04): +8 across two items. Item 3 (the precondition comment is ENTRY-ONLY) needed a fixture that is UNDERWAY but not complete, which a one-step probe cannot be -- and the existing 're-raises the comment' check could not see the fix either way, because the probe's steps never advanced. Item 6 (a step check-off drops the clock to 1x) added the warp checks. THE PROBE ALSO STOPPED BEING NON-DETERMINISTIC: its steps were bare observations, which complete on OBSERVE_DWELL_S of SIM time -- invisible at 1x across a few ticks, but one tick at 600x covers 60 s and the step checked itself off. They now carry an unmeetable `acc`, so only checklist_check advances them.
  'run_checklist_pwr2.js': { code: 0, score: '195passed 0failed 195checks', secs: 1600 },   // 194 -> 195 (#693, 2026-09-11): TMI-2 step 2 split into five narrated steps, and the leg goes 25 -> 26 checks. MEASURED, not computed: the split ADDS four steps but only ONE check, because the lump step's two acceptances (turbine tripped, steam generator level) simply move onto the two steps that now narrate them, and what is genuinely new is step 3's `saw` on the pressure spike -- the one assertion the old step could not make, since an `acc` is read at the step's END and the spike is over by then. The leg's monotonic story-clock check (#670) caught the first build of this split running the clock backwards, step 6 04:00:52 -> step 7 04:00:45.   // MERGED 2026-09-11 (develop+backshop+workbench): 187 -> 194, measured directly on the merged tree, not reconciled from any lane's map (#679's boron/CVCS moves, #696/#697's checklist grading, and #683's pre-existing boron no-state entry all present together; the merge also taught #697's NO_STATE_EXPECTED allowlist about pwr_raise_power step 3, which #683 had already added to develop before this merge and workbench's lane never had -- see that check's own comment). // 179 -> 182 (#683, 2026-09-10): THE ASCENSION COULD HAND BACK A PLANT THAT COOLED ITSELF TO A TRIP. Boron was set to 660 ppm at step 2 with NO acceptance of any kind; the four stage average-coolant-temperature checks were ceiling-only (`op: '<'`) so a cold plant passed every one, the failure mode inverted from the overshoot they were written for; and the closing step's trim to 626 ppm carried neither command nor acceptance. Measured on the continuous Mode 5 -> Mode 1 chain: 105 degF (58 degC) down over 16 plant-hours, reactor trip, pressurizer level pinned on its 25 % program floor from hour six. +1 step 2 (boron setpoint, command-kind acc), +2 step 9/verify (boron bound below the 670 ppm pin point, plus a two-sided Tavg band replacing the one-sided power check), +1 step 10 (boron command to 626 ppm plus acceptance). The four stage bands also went two-sided in the same change (ceiling unchanged, floor added at the stage's program band minus 6 degC) -- no new check count, the same four checks now re-graded rather than latched, per the instructor_layer.js `~` re-grade fix (a two-sided acceptance is a *hold it here* claim and must not be satisfied by passing through and leaving). // 178 -> 179 (#670 operator pass 2, 2026-09-09): EVERY PWR2 REACTOR-TRIP CAUSE HAS A TRIP_CAUSE ENTRY. ui/app.js's TRIP_CAUSE names the cause on the Reactor Trip tile, and every key it shipped with was the RETIRED engine's "<instrument> <direction>" form -- PWR2's control_kernel takes the cause from engine.getTripCause(), a pwr2_protection table ID, so ALL TWELVE of this plant's causes fell through the title-case fallback and the board printed "Reactor Trip -- Ot Delta T" to a licensed operator, who filed it as a typo. Same inherited-table trap as #546/#557. A source scan is the right instrument and a replay is not: producing all twelve trips takes twelve casualties, while the defect is a map with a hole in it -- the same shape as the PRED_DISPLAY check beside it. Proven red by deleting the 'ot_delta_t' key. // 177 -> 178 (#670 Phase 3, 2026-09-09): AN INCIDENT LEG'S STORY CLOCK NEVER RUNS BACKWARDS. The TMI-2 leg went 04:10:37 -> 04:08:37 between steps 9 and 10 -- each clock correct for the event it names (the first reactor-coolant-pump vibration alarm at 10 minutes, the auxiliary-feed discovery at 8), and the pair inverted once the steps were sequenced for teaching. The layman playthrough noticed, assumed it had mis-read, and said it lost confidence in the clock for the rest of the run. NOTHING ELSE COULD CATCH IT: the clock is decoration to the replay, so all sixteen steps passed their acceptances with it inverted. Non-decreasing, not strictly increasing -- two verifications of the same instant may share a clock, which is what steps 8 and 9 now do. Injection-verified (swap the two clocks back: red, naming the pair). One check, per incident leg. // 154 -> 177 (#670 Phase 2, 2026-09-09): the TMI-2 incident walkthrough, replayed end to end -- 22 checks for its 16 steps plus 1 for the chain check splitting in two (the six cycle legs still chain through `next`; incident legs come after them and chain to nothing). `secs` 1180 -> 1600 is the plan's budget from the Phase 0 measurement: the leg is 16,000 ticks of authored hold, 267 plant-minutes, measured at 248-437 s of wall on its own. THE HINT IS SCHEDULING ONLY -- it is not the cost and is never maintained. // 147 -> 154 AT THE MERGE (#660 C +1 and #664 +6, both from 147; the arithmetic combine, confirmed by the gate below). #664: 147 -> 153 (#664, 2026-09-08): the PWR2 pool re-latched the turbine ONCE (the startup leg's 8 % step) and no leg told a recovering player to; a turbine trip at power then a climb scrammed again on P-9 at 49.5 % *(OWNER RULING, 2026-09-08 on #663: 'C - leave the logic as sourced; fix the checklist gap')*. pwr_raise_power gains a step-2 'turbine on line' with EFFECT acceptances (turbine_tripped < 1, mwe_output > 8 -- a cmd-kind acc would soft-lock a plant already on line, the #641 shape), a P-9 caution, and its opening confirm now grades plant_mode ~ 1 instead of mwe_output > 5 (an OBSERVATION step a tripped-turbine player could never satisfy -- the actual soft lock). +2 replay accs, +4 group 2m: the step exists ahead of the first rod pull; a 40 % turbine trip leaves the reactor up; the artifact's own command puts 40 MWe back; the climb crosses 50 % with no trip (peak 70.9 %). Step deleted OR cmd blanked: 3 red, 'turbine_trip at t=2903 s, peak 49.12 %'.   ||  #660 C: 147 -> 148 (#660 C, 2026-09-08): every step now waits for Continue, so 2c's cmd-kind entry splits in two — the entry latches and the step HOLDS with awaiting_ack at index 0, then an explicit `checklist_check` advances it. // 146 -> 147 (#653 pass 3, 2026-09-07): the cooldown's heater-off is its own step graded on the lamp. // 141 -> 146 (#653 pass 2, 2026-09-07): +4 Tavg ceilings on the ascension stages, +1 the shutdown's STEAM DUMP AUTO step (three accs replacing one acc). // 139 -> 141 (#654, 2026-09-07): 2l — the heatup's 918 ppm plant reaches the startup step's 719 and the channel still reports 719 (books-by-command landed ~788, re-anchored). // 138 -> 139 (#653, 2026-09-07): the cooldown's DUMP SETPOINT step gains a cmd-kind STEAM DUMP AUTO entry — the chain leaves the dump in 'tavg' mode after the shutdown leg's scram (measured: every setpoint stage inert until AUTO is pressed again with the turbine tripped), which the per-leg replay from hot_zero_power could never see.   // 135 -> 138 (#641, 2026-09-05): 2k — a 300-step burst from the baseline step with no plot secures the source range at t=356 s and every 1/M step must leave as `overtaken` within 60 s, the index landing on the criticality step, the instructor naming the source range. INJECTION, `overtaken` stripped from the pool at runtime: the index pins at the baseline step (3) with all six unmet — the soft lock the owner reported. // 132 -> 135 (2026-09-05, the merge: #629's +3 (develop) and #627's +3 (workbench, 2j the accumulator clock hold) both over the 129 base; measured 135 on the merged tree). Before: 129 -> 132 (#629, 2026-09-05): the heatup leg gains the STEAM DUMP AUTO step after the letdown-transfer confirmation (+1, graded on the SELECTION `steam_dump_auto` — the dumps carry only 0.4-2.9 % once they are holding the anchor, so the valve position cannot tell an in-service controller from a shut one), and the Mode 3 confirmation gains the two EFFECT accs (+2: `adv_valve_pct < 1` and `steam_pressure_mpa ~ 7.03 tol 0.15`). INJECTION, with the shell's mode selection reverted to the unconditional 'tavg': exactly those two redden — the atmospheric dump valve reads 8.60 % and the header 7.29 MPa (1042 psig) — while the new step's own `steam_dump_auto` tick STAYS GREEN, because the command is accepted and the plant does go into a mode. Only the effect accs tell the two plants apart, which is why they exist (#624 item 25's shape). // 127 -> 129 (#624/#619 item 14, 2026-09-04): the heatup's 'place pressure control in service' step, graded on both AUTO lamps. INJECTION, deleting it: three reds — the Pressure SP step reads 2.51 MPa (364 psia, the plant never leaves Mode 5 pressure), the final pressurization reads 7.28 MPa (1056 psia) on pump heat alone, and the leg ends with pzr_pressure_lolo and subcooling_lost standing. // 123 -> 127 (#624/#619 item 25, 2026-09-04): the heatup leg gains the letdown transfer — a step that lines up BOTH orifices before the pressurization, and a confirmation after the ride that reads the PLANT (RHR out AND letdown still flowing) rather than the button. INJECTION, because the write alone would pass on a plant whose cross-connect was still carrying everything: delete the step and two checks redden — step 7's `pressure_mpa > 4.7` reads 4.41 MPa (640 psia), under the 665 psia accumulator cover gas (#608 item 4, re-opened), and `letdown_flow_actual > 0` reads exactly 0. A+B and not A: orifice A alone parks step 7 at 4.33 MPa (628 psia), also under the cover gas.   // 119 -> 123 (#622/#619 wave 2, 2026-09-04): +1 in 2b (at least one step HELD for an acknowledgement -- the ack gate makes a tick-only driver unable to finish a checklist, so that half now drives acks and asserts it actually had to) and +3 in the new 2i (the accumulator window drops the clock at the cover gas, REFUSES to re-accelerate while the valve is shut, and releases once the tanks are armed). 2i is the one that earned its keep: it caught the drop sitting BELOW the attentionStops early return while the set_speed refusal sat above it, so the clock stayed at 600x while the button that would have set it there was blocked -- invisible to a scratch probe running with the toggle at its default.   // 117 -> 119 (#624/#619 item 28, 2026-09-04): +2, both on pwr_raise_power's final step, and they are the check this leg NEVER had. `control_bank_steps > 300` (the bank moved off its starting 227) and `< 600` (it is not pinned on its top stop). ONLY THE UPPER BOUND BITES: the leg used to start from `50_percent`, which boots the bank fully withdrawn, so it ended at 627 -- which passes `> 300` comfortably. Injection-verified by restoring `from: '50_percent'`: `< 600` goes red at 627 while every other check in the leg stays green, which is exactly the signature the original defect had. The leg now starts from the new `low_power` IC, the state pwr_startup actually hands over (bank 227/627), so its five rod withdrawals are real commands instead of no-ops against the stop.   // 113 -> 117 at the workbench merge (2026-09-03): MEASURED. #608's 2e/2f (the tick-is-permission cover-gas check and the live refusal text) and #607's 2g/2h (every step has a details paragraph; catch-up does not eat the fresh Mode 5 opening confirm) are all four independent and all four pass on the merged tree. The incoming pair was renumbered 2e/2f -> 2g/2h; both lanes had used the same labels.   // 110 -> 113 (#608, 2026-09-02, owner playtest of the Mode 5 -> Mode 3 heatup). +1 from the pwr_heatup replay itself: step 6 stopped commanding the steam-dump setpoint the Mode 5 initial condition already boots at (measured 7.03 MPa, the exact value it sent) and became an observation graded on steam_dump_valve_pct, so the ride gains an acceptance where it had a bare dwell. +2 NEW static checks, both discriminating (proven by reverting each defect): (2e) any step whose acceptance is a primary-pressure threshold AND is immediately followed by the accumulator step must accept ABOVE the cover gas, read from the engine's own ACC.p0_mpa rather than retyped -- step 7 accepted at 609 psia against a 665 psia cover gas, so a player who took the tick as permission for the next step opened the valve below it, which is ACCEPTED and measured to backfeed the tank 100 % -> 97.2 % with boron 918 -> 940 ppm. The replay could never see it because step 7's hold:2400 dominates its acceptance, which is why the check is static and about the ARTIFACT. (2f) the lock's refusal names the heaters/spray recovery: it used to end "Depressurize below 1600 psig first", an instruction the Pressure SP dial cannot carry out because its sourced floor is 1700 psig, 85 psi ABOVE the lock -- a refusal that sends the player to a control that refuses them again.   // 109 -> 110 (#605, 2026-09-02): pwr_heatup step 4 gained an `acc` — Mode 5 now boots with the TURBINE TRIPPED, so 'confirm the generator is off line' has something to verify instead of a bare dwell.   // NEW (#244/#254/#526, 2026-08-31): THE SHIPPED PLANT'S checklist gate — all six pwr2 chain checklists replayed END TO END through the full stack (the HR12 record that the chain's numbers are this plant's own; it caught the P-11/lo-steam-press SI heatup trap and the PARAM_INSTRUMENT.pwr2 map authored from the definition file instead of the live broadcast — boron_analyzer/rcs_flow, exactly what check 2a exists for), plus the live Path 3 runtime on a pwr2 service: instrument-first grading, per-entry accs latching with the blinded-entry injection, and the PRED_DISPLAY coverage seam. secs measured alone 2026-08-31 (521 s — the six rides are ~40 plant-hours at 10 ms/tick).   // 110 -> 114 (#607, 2026-09-03): +1 heatup feed_coupled acc, +1 every-step-has-why, +2 catch-up (fresh start stays on confirm; RCPs-already-running lands on shutdown-bank). Replay 111/111 still green.   // 109 -> 110 (#605, 2026-09-02): pwr_heatup step 4 gained an `acc` — Mode 5 now boots with the TURBINE TRIPPED, so 'confirm the generator is off line' has something to verify instead of a bare dwell.   // NEW (#244/#254/#526, 2026-08-31): THE SHIPPED PLANT'S checklist gate — all six pwr2 chain checklists replayed END TO END through the full stack (the HR12 record that the chain's numbers are this plant's own; it caught the P-11/lo-steam-press SI heatup trap and the PARAM_INSTRUMENT.pwr2 map authored from the definition file instead of the live broadcast — boron_analyzer/rcs_flow, exactly what check 2a exists for), plus the live Path 3 runtime on a pwr2 service: instrument-first grading, per-entry accs latching with the blinded-entry injection, and the PRED_DISPLAY coverage seam. secs measured alone 2026-08-31 (521 s — the six rides are ~40 plant-hours at 10 ms/tick).   // ALSO: // 187 -> 191 (#696, 2026-09-10): +4, section 2p. `pwr_shutdown`'s precondition gains an UPPER bound -- only `power_pct > 10` was checked, which the leg's own `from: 'hot_full_power'` satisfies, so a standalone 100 % entry passed silently into the measured +21.0 degF (+11.7 degC) / 601.3 degF Tavg spike (owner: "shouldn't we walk down the power instead of just putting load to zero? Coolant temp spikes hard."), sourced against Ginna's 50 % RTP loss-of-load trip threshold. Set at 30 %, NOT the issue's own suggested 20 %: measured here, the leg's actual chained handoff (`pwr_lower_power` run to completion) settles at 22-23 % power, not the ~15 % that leg's text promises (the already-tracked #508 rod-trim residue) -- a 20 % ceiling would have WARNED on the leg's own shipped, correct route, which is worse than the silent gap it replaces. `precond` WARNS, never blocks (OWNER RULING, 2026-08-06), so this asserts the BANNER: unmet standalone at 100 %, met on the intended chained entry. RED BY INJECTION: the upper-bound row removed, the 100 % entry shows no unmet row at all -- the exact silence the owner hit.   // 179 -> 187 (#697, 2026-09-10): +6 in two new sections. 2n is the static sweep -- every pool `accs` entry that is cmd-kind (has `.cmd`, no `.p`) with no step-level `overtaken`: the 3 with no sibling predicate at all are the documented judgement calls (pwr_raise_power step 4, pwr_cooldown step 3 x2), pinned so a new one cannot join unnoticed; the 9 WITH a sibling are booted at the leg's own IC and checked with nothing pressed -- all read clean now that the two confirmed instances are fixed. 2o is the live proof: `pwr_cooldown` step 4 and `pwr_shutdown` step 3 both complete with STEAM DUMP AUTO never pressed (the owner's reported instance -- "steam dump AUTO was already green but it still required a press to check off step"), and RED BY INJECTION -- the pre-fix pure-cmd-kind shape soft-locks each forever (`met:false, obs:null`) even though every sibling predicate is already true underneath it. +2 more from the REPLAY (section 1): the two fixed entries now carry `.p`, so `runProcedure`'s existing per-entry accs check asserts them where it previously had nothing to assert. THE PROVEN-WRONG PRECEDENT: `pwr_heatup` step 8's two-entry hidden-cmd-plus-predicate shape does NOT bypass the press -- `_gradeAccs` never grades a cmd-only entry, so it can only latch via `_accsCmdWatch` seeing the exact command; a predicate SIBLING cannot satisfy it. It never surfaced there because `heater_auto`/`spray_auto` are false at that leg's own `cold_shutdown` IC. The fix merges `p`/`op`/`v` onto the SAME entry as `cmd` instead: `_gradeAccs` grades the `p` half regardless of any command (a plant already there ticks the box), `_accsCmdWatch` still latches the SAME entry on the press for a plant that is not.   // 178 -> 179 (#670 operator pass 2, 2026-09-09): EVERY PWR2 REACTOR-TRIP CAUSE HAS A TRIP_CAUSE ENTRY. ui/app.js's TRIP_CAUSE names the cause on the Reactor Trip tile, and every key it shipped with was the RETIRED engine's "<instrument> <direction>" form -- PWR2's control_kernel takes the cause from engine.getTripCause(), a pwr2_protection table ID, so ALL TWELVE of this plant's causes fell through the title-case fallback and the board printed "Reactor Trip -- Ot Delta T" to a licensed operator, who filed it as a typo. Same inherited-table trap as #546/#557. A source scan is the right instrument and a replay is not: producing all twelve trips takes twelve casualties, while the defect is a map with a hole in it -- the same shape as the PRED_DISPLAY check beside it. Proven red by deleting the 'ot_delta_t' key. // 177 -> 178 (#670 Phase 3, 2026-09-09): AN INCIDENT LEG'S STORY CLOCK NEVER RUNS BACKWARDS. The TMI-2 leg went 04:10:37 -> 04:08:37 between steps 9 and 10 -- each clock correct for the event it names (the first reactor-coolant-pump vibration alarm at 10 minutes, the auxiliary-feed discovery at 8), and the pair inverted once the steps were sequenced for teaching. The layman playthrough noticed, assumed it had mis-read, and said it lost confidence in the clock for the rest of the run. NOTHING ELSE COULD CATCH IT: the clock is decoration to the replay, so all sixteen steps passed their acceptances with it inverted. Non-decreasing, not strictly increasing -- two verifications of the same instant may share a clock, which is what steps 8 and 9 now do. Injection-verified (swap the two clocks back: red, naming the pair). One check, per incident leg. // 154 -> 177 (#670 Phase 2, 2026-09-09): the TMI-2 incident walkthrough, replayed end to end -- 22 checks for its 16 steps plus 1 for the chain check splitting in two (the six cycle legs still chain through `next`; incident legs come after them and chain to nothing). `secs` 1180 -> 1600 is the plan's budget from the Phase 0 measurement: the leg is 16,000 ticks of authored hold, 267 plant-minutes, measured at 248-437 s of wall on its own. THE HINT IS SCHEDULING ONLY -- it is not the cost and is never maintained. // 147 -> 154 AT THE MERGE (#660 C +1 and #664 +6, both from 147; the arithmetic combine, confirmed by the gate below). #664: 147 -> 153 (#664, 2026-09-08): the PWR2 pool re-latched the turbine ONCE (the startup leg's 8 % step) and no leg told a recovering player to; a turbine trip at power then a climb scrammed again on P-9 at 49.5 % *(OWNER RULING, 2026-09-08 on #663: 'C - leave the logic as sourced; fix the checklist gap')*. pwr_raise_power gains a step-2 'turbine on line' with EFFECT acceptances (turbine_tripped < 1, mwe_output > 8 -- a cmd-kind acc would soft-lock a plant already on line, the #641 shape), a P-9 caution, and its opening confirm now grades plant_mode ~ 1 instead of mwe_output > 5 (an OBSERVATION step a tripped-turbine player could never satisfy -- the actual soft lock). +2 replay accs, +4 group 2m: the step exists ahead of the first rod pull; a 40 % turbine trip leaves the reactor up; the artifact's own command puts 40 MWe back; the climb crosses 50 % with no trip (peak 70.9 %). Step deleted OR cmd blanked: 3 red, 'turbine_trip at t=2903 s, peak 49.12 %'.   ||  #660 C: 147 -> 148 (#660 C, 2026-09-08): every step now waits for Continue, so 2c's cmd-kind entry splits in two — the entry latches and the step HOLDS with awaiting_ack at index 0, then an explicit `checklist_check` advances it. // 146 -> 147 (#653 pass 3, 2026-09-07): the cooldown's heater-off is its own step graded on the lamp. // 141 -> 146 (#653 pass 2, 2026-09-07): +4 Tavg ceilings on the ascension stages, +1 the shutdown's STEAM DUMP AUTO step (three accs replacing one acc). // 139 -> 141 (#654, 2026-09-07): 2l — the heatup's 918 ppm plant reaches the startup step's 719 and the channel still reports 719 (books-by-command landed ~788, re-anchored). // 138 -> 139 (#653, 2026-09-07): the cooldown's DUMP SETPOINT step gains a cmd-kind STEAM DUMP AUTO entry — the chain leaves the dump in 'tavg' mode after the shutdown leg's scram (measured: every setpoint stage inert until AUTO is pressed again with the turbine tripped), which the per-leg replay from hot_zero_power could never see.   // 135 -> 138 (#641, 2026-09-05): 2k — a 300-step burst from the baseline step with no plot secures the source range at t=356 s and every 1/M step must leave as `overtaken` within 60 s, the index landing on the criticality step, the instructor naming the source range. INJECTION, `overtaken` stripped from the pool at runtime: the index pins at the baseline step (3) with all six unmet — the soft lock the owner reported. // 132 -> 135 (2026-09-05, the merge: #629's +3 (develop) and #627's +3 (workbench, 2j the accumulator clock hold) both over the 129 base; measured 135 on the merged tree). Before: 129 -> 132 (#629, 2026-09-05): the heatup leg gains the STEAM DUMP AUTO step after the letdown-transfer confirmation (+1, graded on the SELECTION `steam_dump_auto` — the dumps carry only 0.4-2.9 % once they are holding the anchor, so the valve position cannot tell an in-service controller from a shut one), and the Mode 3 confirmation gains the two EFFECT accs (+2: `adv_valve_pct < 1` and `steam_pressure_mpa ~ 7.03 tol 0.15`). INJECTION, with the shell's mode selection reverted to the unconditional 'tavg': exactly those two redden — the atmospheric dump valve reads 8.60 % and the header 7.29 MPa (1042 psig) — while the new step's own `steam_dump_auto` tick STAYS GREEN, because the command is accepted and the plant does go into a mode. Only the effect accs tell the two plants apart, which is why they exist (#624 item 25's shape). // 127 -> 129 (#624/#619 item 14, 2026-09-04): the heatup's 'place pressure control in service' step, graded on both AUTO lamps. INJECTION, deleting it: three reds — the Pressure SP step reads 2.51 MPa (364 psia, the plant never leaves Mode 5 pressure), the final pressurization reads 7.28 MPa (1056 psia) on pump heat alone, and the leg ends with pzr_pressure_lolo and subcooling_lost standing. // 123 -> 127 (#624/#619 item 25, 2026-09-04): the heatup leg gains the letdown transfer — a step that lines up BOTH orifices before the pressurization, and a confirmation after the ride that reads the PLANT (RHR out AND letdown still flowing) rather than the button. INJECTION, because the write alone would pass on a plant whose cross-connect was still carrying everything: delete the step and two checks redden — step 7's `pressure_mpa > 4.7` reads 4.41 MPa (640 psia), under the 665 psia accumulator cover gas (#608 item 4, re-opened), and `letdown_flow_actual > 0` reads exactly 0. A+B and not A: orifice A alone parks step 7 at 4.33 MPa (628 psia), also under the cover gas.   // 119 -> 123 (#622/#619 wave 2, 2026-09-04): +1 in 2b (at least one step HELD for an acknowledgement -- the ack gate makes a tick-only driver unable to finish a checklist, so that half now drives acks and asserts it actually had to) and +3 in the new 2i (the accumulator window drops the clock at the cover gas, REFUSES to re-accelerate while the valve is shut, and releases once the tanks are armed). 2i is the one that earned its keep: it caught the drop sitting BELOW the attentionStops early return while the set_speed refusal sat above it, so the clock stayed at 600x while the button that would have set it there was blocked -- invisible to a scratch probe running with the toggle at its default.   // 117 -> 119 (#624/#619 item 28, 2026-09-04): +2, both on pwr_raise_power's final step, and they are the check this leg NEVER had. `control_bank_steps > 300` (the bank moved off its starting 227) and `< 600` (it is not pinned on its top stop). ONLY THE UPPER BOUND BITES: the leg used to start from `50_percent`, which boots the bank fully withdrawn, so it ended at 627 -- which passes `> 300` comfortably. Injection-verified by restoring `from: '50_percent'`: `< 600` goes red at 627 while every other check in the leg stays green, which is exactly the signature the original defect had. The leg now starts from the new `low_power` IC, the state pwr_startup actually hands over (bank 227/627), so its five rod withdrawals are real commands instead of no-ops against the stop.   // 113 -> 117 at the workbench merge (2026-09-03): MEASURED. #608's 2e/2f (the tick-is-permission cover-gas check and the live refusal text) and #607's 2g/2h (every step has a details paragraph; catch-up does not eat the fresh Mode 5 opening confirm) are all four independent and all four pass on the merged tree. The incoming pair was renumbered 2e/2f -> 2g/2h; both lanes had used the same labels.   // 110 -> 113 (#608, 2026-09-02, owner playtest of the Mode 5 -> Mode 3 heatup). +1 from the pwr_heatup replay itself: step 6 stopped commanding the steam-dump setpoint the Mode 5 initial condition already boots at (measured 7.03 MPa, the exact value it sent) and became an observation graded on steam_dump_valve_pct, so the ride gains an acceptance where it had a bare dwell. +2 NEW static checks, both discriminating (proven by reverting each defect): (2e) any step whose acceptance is a primary-pressure threshold AND is immediately followed by the accumulator step must accept ABOVE the cover gas, read from the engine's own ACC.p0_mpa rather than retyped -- step 7 accepted at 609 psia against a 665 psia cover gas, so a player who took the tick as permission for the next step opened the valve below it, which is ACCEPTED and measured to backfeed the tank 100 % -> 97.2 % with boron 918 -> 940 ppm. The replay could never see it because step 7's hold:2400 dominates its acceptance, which is why the check is static and about the ARTIFACT. (2f) the lock's refusal names the heaters/spray recovery: it used to end "Depressurize below 1600 psig first", an instruction the Pressure SP dial cannot carry out because its sourced floor is 1700 psig, 85 psi ABOVE the lock -- a refusal that sends the player to a control that refuses them again.   // 109 -> 110 (#605, 2026-09-02): pwr_heatup step 4 gained an `acc` — Mode 5 now boots with the TURBINE TRIPPED, so 'confirm the generator is off line' has something to verify instead of a bare dwell.   // NEW (#244/#254/#526, 2026-08-31): THE SHIPPED PLANT'S checklist gate — all six pwr2 chain checklists replayed END TO END through the full stack (the HR12 record that the chain's numbers are this plant's own; it caught the P-11/lo-steam-press SI heatup trap and the PARAM_INSTRUMENT.pwr2 map authored from the definition file instead of the live broadcast — boron_analyzer/rcs_flow, exactly what check 2a exists for), plus the live Path 3 runtime on a pwr2 service: instrument-first grading, per-entry accs latching with the blinded-entry injection, and the PRED_DISPLAY coverage seam. secs measured alone 2026-08-31 (521 s — the six rides are ~40 plant-hours at 10 ms/tick).   // 110 -> 114 (#607, 2026-09-03): +1 heatup feed_coupled acc, +1 every-step-has-why, +2 catch-up (fresh start stays on confirm; RCPs-already-running lands on shutdown-bank). Replay 111/111 still green.   // 109 -> 110 (#605, 2026-09-02): pwr_heatup step 4 gained an `acc` — Mode 5 now boots with the TURBINE TRIPPED, so 'confirm the generator is off line' has something to verify instead of a bare dwell.   // NEW (#244/#254/#526, 2026-08-31): THE SHIPPED PLANT'S checklist gate — all six pwr2 chain checklists replayed END TO END through the full stack (the HR12 record that the chain's numbers are this plant's own; it caught the P-11/lo-steam-press SI heatup trap and the PARAM_INSTRUMENT.pwr2 map authored from the definition file instead of the live broadcast — boron_analyzer/rcs_flow, exactly what check 2a exists for), plus the live Path 3 runtime on a pwr2 service: instrument-first grading, per-entry accs latching with the blinded-entry injection, and the PRED_DISPLAY coverage seam. secs measured alone 2026-08-31 (521 s — the six rides are ~40 plant-hours at 10 ms/tick).
  ...
  // 44 -> 47 (#628, owner 2026-09-04): the checklist card's element ORDER and its suggested speed rung. Three claims, each injection-proven: every `.ckl-body` leads with `.ckl-txt` (reverting the move fails with "step 1 leads with ckl-act"); the rung is the smallest on the LIVE ladder that clears the wait in 30 s (owner, 2026-09-05: "Change it to 30s"), recomputed in the test over every pool in RD.MANUAL_PROCEDURES with its OWN literal 30 -- a check that imports the constant it is checking asserts only that the constant equals itself, so moving the target is a deliberate two-file change (a hard-coded top rung fails on pwr_heatup); and the rendered card names it. THE THIRD ONE WAS HOLLOW WHEN FIRST WRITTEN -- it read whichever checklist the picker had opened, whose step 1 holds 2 s, so it asserted only the negative half and stayed green against a hard-coded `2x`. It now STARTS the first procedure whose own step 1 carries a hold >= 180 s (pwr_stuck_porv on the retired engine, pwr_cooldown on pwr2 -- data-driven, neither id in the test).
  'verify_flags_ui.js':      { code: 0, secs: 9, score: '52/52' },   // 50 -> 52 (#686, 2026-09-11): the consolidated status line under the speed bar (OWNER RULING 2026-09-10, option B) -- two checks, both on the SHIPPED pwr2 pool, each its own fresh build: a qualifying step's `#warpInfo` names the same rung as the card's `.ckl-wait`, and a non-qualifying step prints nothing under the speed bar ("nothing on other steps" is the ruling's own words). Both proven red by injection (disabling the qualifying branch, and dropping the `hold >= 180` gate to `>= 0`) before landing. // 48 -> 50 (#670 Phase 2, 2026-09-09): the TMI-2 incident walkthrough is preview-only (plan R4, ruled), asserted on the RENDERED list on both channels -- dev lists its Start button, public with the walkthroughs AREA forced on does not. BOTH halves run on `?engine=pwr2` and that is the check: `SHELL` boots the RETIRED engine, whose pool has no incident leg, so on it the dev half fails for the wrong reason AND the public half PASSES for the wrong reason. Measured that way at authoring before the URL was fixed. // 47 -> 48 (2026-09-08): PRE-EXISTING DRIFT, not this change -- #660 sections C/D (166ea25f) added the check and left the baseline behind; measured 48/48 green on develop with test/verify_flags_ui.js untouched. // 42 -> 44 (#598 item 13, owner ruling 2026-09-02): the active checklist step card must carry the CONTROL outside the Details fold, and the expander must read Details. Asserted in the RENDERED DOM by starting a real checklist -- #485's lesson, where a source scan for a rendered string passed green against `(false ? ' (partial)' : '')`. The first cut of item 13 folded control and target away with everything else; measured, only 41 % of the 46 pwr2 steps carrying a control name it in their instruction text, so 27 steps had no visible answer to "which knob". Injection-proven: suppressing the line reddens the first check with "NO .ckl-use line on the active step". Not pinned to any step's wording, so content edits do not touch it.
  ...
  'verify_e2e_ui.js':        { code: 0, secs: 58, score: '4screenshots', slow: true },   // score unchanged, +1 CHECK FUNCTION (#710, 2026-09-11): testHeldNotePauseResume -- resumeSim() (and two siblings: the speed-button click handler, the walkthrough rewind handler) nulled `warpNote` unconditionally on any player act, on the theory the act means the plant-declared hold is over; it can still be standing (`set_speed(1)`, what a resume always sends, always succeeds under a hold -- only `>1` is refused), so an ordinary pause/resume inside the accumulator arming window silently dropped the ONLY standing explanation on screen, re-creating the #619 item 13 trap. Fix (`retireWarpNote` in ui/app.js) reads the LIVE `latest.true_state.speed_hold` instead of assuming an act ends the hold. UNLIKE #686's one-shot plant, this test leaves the `assembleSnapshot` override INSTALLED (toggled via a flag, not restored) so a real pause/resume broadcast still reports the hold, then flips it off for the negative half. Both halves injection-proven: reverting `retireWarpNote` to `warpNote = null` fails the positive assertion; making it never clear (`if (reason==='hold') return;` unconditionally) fails the negative one. // score unchanged, +1 ASSERTION in testHeldSpeedClick (#686, 2026-09-11): the accumulator-hold message under the speed bar (OWNER RULING 2026-09-11, "option A" -- kept, not deleted, because the arming window is a genuine multi-minute refusal unlike the momentary rate-based ones). Plants ONE upstream fact -- `true_state.speed_hold` via a one-shot `assembleSnapshot` override -- and runs it through the real `_assembleWithInstructor` -> `_attentionStop` -> `speed_snap` -> `_broadcast` -> `syncWarpInfo` chain, not the DOM directly. Proven red by injection: disabling the `warpNote.reason === 'hold'` branch fails this exact assertion. // score unchanged, +1 CHECK FUNCTION (#685, 2026-09-11): testWatchGlowRendered -- the DARK-WIRE proof for the new `hl_watch` field. run_manual_controls checks the LABELS are in the board vocabulary and cannot check that anything ever applies the class; `applyCklWatchGlow` has one caller, in a render path no Node harness enters, which is the exact shape of the three dark wires #507 wave 6 shipped. Asserts the EFFECT on real content -- pwr_heatup's last step, one of the four that resolved to NO element at all before this, authors 3 watch labels and the board paints 3 `.ckl-watch-glow`. The NEGATIVE half is load-bearing: that step authors no `hl` and no `control`, so 0 pulsing `.ckl-step-glow` must be painted, or the check passes on a renderer that draws one treatment for both lists. Both halves injection-verified: deleting the applyCklWatchGlow call gives 3 authored -> 0 painted; making it add BOTH classes gives watch 3 / press 3 and reddens the distinctness clause. The score is a screenshot count, so a new check function does not move it -- read the runner's own PASS line, not this number. // score unchanged, +1 CHECK FUNCTION (#694, 2026-09-10): testWalkthroughEventPause -- the browser half of the walkthrough pause proof (the Node half is run_checklist.js section 11, above). Proves the setTimeout loop actually stops rescheduling and `.bd-frozen` actually paints, on a synthetic fixture (no shipped step authors `pause` yet). The printed score is a screenshot count and does not move; `secs` is a scheduling hint per this file's own convention and is not maintained. // 16 -> 4 (#514, 2026-08-25): PWR-only — the shell no longer loads RBMK/BWR, and app.js falls back to the PWR on an absent engine, so an rbmk/bwr row would silently screenshot the PWR and certify nothing. The #111 strict xfail (manual unit conversion) is PWR-side and stays.
  ...
  /* 236 -> 237 (#591 item 3, 2026-08-31): the PERIOD pair. #350 item 5's check pinned a POSITION ("top ~455") and a position moves the next time the owner asks; what he actually reported is permanent and measurable - the value was drawn 20x13 px ON TOP of the caption. One check becomes two: NO OVERLAP, and INSIDE a card that hugs them. Both injection-verified by restoring the old geometry. */
  'verify_ckl_relevance.js': { code: 0, secs: 55, score: '21passed 21checks', slow: true },   // 18 -> 21 (#653, 2026-09-11): the leg's CAUTIONS now reach the player running the leg, and the `why` block wears a label. Three checks, all proven red by injection on this tree: delete the caution block from renderChecklist -> 2 red (19/21); neuter only the [data-ckl-cautions] handler -> 1 red (20/21), because drawing it and being able to open it are different claims; drop the ckl-why-lbl span -> 1 red (20/21), and it pins the LABEL, not the paragraph, which still renders. They read the RENDERED panel rather than the pool -- the caution strings were always IN the pool and `pr.cautions` rendered in exactly one place, `mProcCard`, the Manual tab's browse card, so from the moment a walkthrough started every caution on it was on a different screen. A source scan could not have made either claim. // 17 -> 18 (#670 operator pass 2, 2026-09-09): NO SI IN THE LIVE CHECKLIST PANEL, ASSERTED ON THE RENDERED LINE. The owner ruled on 2026-09-06 that live checklists carry no SI, `run_style`'s `checklist_no_si` was written for that ruling, and the panel printed "> 240 degF (116 degC)" while it stayed green -- because it walks the pool's AUTHORED strings and the SI is COMPOSED by fmtPredValue in ui/app.js at render time. 18 sites across four walkthroughs, six of them precondition-banner lines; a scan of every string in the built pool returns ZERO. A source scan cannot see a string concatenated in another file, so the check went where the string exists. The fixture is the heatup started at FULL POWER on purpose: both its preconditions fail, so one render exercises the banner and the criteria line together. Proven red by injection: it reddens and quotes the offending line verbatim. The neighbouring detail-line check lost its SI pairs in the same change and gained a stricter form -- band and anchor now sit adjacent, so it does NOT pass on the pre-ruling build. // 16 -> 17 (#670 Phase 2, 2026-09-09): the incident walkthrough is listed LAST and is WHITE at the Hot Full Power boot (its only entry condition is REACTOR POWER above 90 %); at Mode 5 it is greyed, which the existing "exactly one white row" check already covers. The CYCLE literal gains its id -- read off the rendered DOM, never off the pool, for the reason that literal exists. // 14 -> 16 (#660 items 17-18, 2026-09-08): the walkthrough's Rewind button follows the rewind RING, and only a browser can see it. `rewind_ready` is gated in Node by run_checklist, but the checklist card is key-cached and the flag was not in the key -- measured here first: step 1 with the ring emptied reported rewind_ready false in the snapshot while the button on the board stayed ENABLED, because nothing else in the key had moved (#606's shape, one card over). Two injections, both reddening the second check: the pre-fix `step_index > 0` button condition, and removing the lit-state bits from the render key. // 13 -> 14 (#606 adjacent, 2026-09-05): the entry banner's detail line is player-facing — "wants Tavg within 14 °F (8 °C) of 547 °F (286 °C), reads 122 °F (50 °C)", no raw param; it read "wants tavg_c ≈ 286, reads 50.2", and through the criteria formatter the 8 °C band would have printed 46 °F (the temp converter's +32 on a difference). // 10 -> 13 (#614/#615, 2026-09-03, owner playtest). #614: the not-applicable banner was rendered from LIVE precondition verdicts every build, and preconditions are ENTRY conditions -- the heatup wants tavg < 95, which the heatup itself falsifies -- so the banner popped in and out as the plant crossed Mode 5 -> 4 -> 3, changing the panel height under the reader. THAT was the scroll bounce the owner reported at #612, and my three hypotheses there were all wrong. It is captured once, on the first build that actually carries verdicts, and held. TWO earlier versions failed first and are recorded in the code: latching on firstBuild caught an EMPTY banner (the verdicts are not in the first broadcast), and suppressing once the run was `underway` hid it before it could be read (a checklist whose first step is already satisfied advances on its own). #615: clicking a number tile OUTSIDE its 48x18 frame -- a ~10 px label band on a 48x28 tile -- focused nothing, left activeElement on BODY and fed the digits to the global shortcuts, where 2/3/5 are time acceleration. Injection-verified (BODY/lost before, INPUT/2235 after); the #614 check is labelled GUARD in its own title because it is vacuous on the defect it names.   // 8 -> 10 (#612, 2026-09-03, owner playtest): the running checklist did not reach the bottom of its column. #607 moved it into the CHECKLISTS pane, a plain content-sized .tabpane, so it inherited neither half of the height treatment .tabpane.instructor has had since the transcript needed it -- measured at 1600x950 the pane ended 189 px short of the viewport with the log capped at 456 px by .ckl-log max-height 48vh. The gap check is DISCRIMINATING (139 px red on the pre-fix CSS, 19 px green after) and bands rather than pins the number, because the residue is card chrome and pinning 19 would re-red on any spacing change. The second check is labelled a GUARD in its own name: it passes pre-fix too, since .tab-body happened to fit, and is kept only to catch a future layout that reintroduces a nested scroller -- which would silently void the log-only scroll restore.   // 6 -> 8 (#606 follow-up, same day, OWNER: "the checklists should be in a logical order. ie, starting in mode 5 it should start with mode 5 to mode 3 and end with mode 3 to mode 5"). The within-category tiebreak was title.localeCompare, and these titles begin with the mode they START FROM — "Mode 3, Hot Standby -> Mode 1" sorts above "Mode 5, Cold Shutdown -> Mode 3", so STARTUP listed its second leg first and POWER inverted the same way ("load rampdown" above "power ascension"); SHUTDOWN was right by luck. It is the POOL'S DECLARATION ORDER now, which is the authored operating cycle and is independently gated against the `next` chain by run_checklist_pwr2 — so the list and the finished-card handoff cannot disagree. The pair is deliberate: one asserts the cycle at Mode 5, one asserts the SAME order at Mode 1, because a sort that re-derived itself from plant state would satisfy the first alone and re-break the 2026-08-11 standard-order directive. Injection-verified: restoring the alphabetical tiebreak reds the cycle check.   // NEW (#606, 2026-09-02, owner playtest: "when I started up in mode 5 the mode 5 checklist was greyed out but some where white"). The Checklists tab's grey/white verdict was computed once and keyed on `engineKey | active procedure id` — neither of which changes when the player resets to a different initial condition — so the list built at the default Hot Full Power boot survived the switch to Cold Shutdown byte-identical: the Mode 5 heatup greyed with "Requires RCS temperature below 203 degF" on a 122 degF plant, the at-power legs white. THE RANK WAS NEVER WRONG (measured at Mode 5: only the heatup grades ready) — only WHEN it was taken, which is why it needed a browser and not a check in run_checklist_pwr2: the stale value was a closure variable Node cannot reach and both endpoints read correct in isolation. The discriminator is the FLIP, not the Mode 5 reading — booting straight to ?init=cold_shutdown builds the list once, correctly, and passes on the defect. Marked slow: it is Playwright, and an unmarked Playwright gate under --fast is what ran CI red for 32 consecutive runs. Injection-verified: 4 of the 6 red on the pre-fix build.
  'verify_board_check.js':   { code: 0, secs: 2, score: '256checks' },   // 246 -> 256 (#684, 2026-09-10): +10 for the HALO GEOMETRY sweep. The step highlight is a box-shadow on the element `revealControl` returns, and that was the TILE -- whose box is the item's AUTHORED rect. Measured on the shipped board: 27 of 218 tiles have visible art hanging more than 1 px outside it, 17 more than 2 px; the PORV's ring covered 44.5 % of the valve's width and 80 px of PRESSURIZER hung below its halo, on an item that is not rotated at all. The sweep asserts NO tile's visible art overflows its glow box, which pins all of them at once and cannot pass on a PORV-only fix; it computes "visible art" INDEPENDENTLY of the renderer's own rule, so the two must agree. Injections caught: no-overflow-recorded (3 red), border compensation dropped (1 red), revealControl back to the tile (2 red), the PORV status label deleted (2 red).   // 245 -> 246 (#670 operator pass 2, 2026-09-09): A TILE MUST NEVER PRINT NEGATIVE ZERO. `(-0.18).toFixed(0)` is the string "-0", and SUBCOOLING MARGIN genuinely sits in that band for minutes as the plant reaches saturation -- an operator read "-0 F" off the one gauge that says whether the coolant is still water. The board's own fmtNum (pwr_board_wiring.js) has used String(Math.round(v)) since it was written AND ITS COMMENT NAMES THIS EXACT TRAP; comp_indicator_panel.js was four files away doing the cheap thing and nothing compared the two. Proven red by injection: -0.18 and -0.49 both render "-0" on the old line. // 242 -> 245 (#670 Phase 3, 2026-09-09): the PORV block valve is a PLAIN TOGGLE and the board said otherwise — the TMI-2 walkthrough's step 14 read "click the block valve symbol … then confirm" and the symbol's own inspect card said "Two-press confirm on isolate.", so the layman playtest spent 11 of its 55 minutes pressing it and only ever got the valve shut on an ODD press count. Measured with the shell's handleCommand wrapped: ten clicks 0.4 s apart with the pointer never moving produce TEN commands and SHUT OPEN SHUT OPEN…; `comp_valve_vertical`'s hit circle emits onControl('toggle', openFrac < 0.5 ? 1 : 0) and nothing else. Three checks: the symbol carries a click target, ONE click shuts it, and a SECOND click OPENS it. BOTH directions, deliberately — a probe that only asserted "one click shuts it" passes on a confirm-armed valve too, which is the shape that let the wrong wording stand. The press is dispatched, not `.click()`ed: the hit target is an SVG <circle> and HTMLElement.click is not on it (measured, `bvEl.click is not a function`). The block is bracketed to hand the valve back the way it found it.   // 241 -> 242 (2026-09-05-develop-e): 8399d6cd (the NUC INSTR card closed up, #598 items 7/9/10 follow-up) added the row-rhythm check — its own log says "1 of 242" — and left this at 241; acknowledged here. // 238 -> 241 (#630, 2026-09-05): the ECCS card's MODE caption and its word were authored on the SAME line (both top 755), which works only while the word is short — the RETIRED engine's vocabulary is RHR/HPI/LPI/off, and PWR2's quiescent word is STANDBY, 55.7 px against RHR's 25.3, overprinting the caption by 28.0 px on a normal plant. The three rows now stack uniformly (38 px group pitch, 15 px caption -> word). The checks PLANT the widest word themselves rather than reading what the plant publishes, because this harness drives the retired engine and would have passed through the whole defect; and the claim is the DROP, calibrated against the FLOW pair beside it, not a no-overlap test — every pair on this card overlaps by ~4 px of line leading and renders clean, so no-overlap reds on the fixed layout too. All three injection-verified (restore top 755: drop 0.0; move the word onto DISCG: 2 red; push it past the card: containment red).   // 236 -> 238 (#624/#619 item 24, 2026-09-04): the LETDOWN card's new status word is MEASURED onto the board, not argued onto it — one overlap pin against every content item and one containment pin against the card DOC_PATCHES grows from 150 to 175 px to hold it. Verified by injection: moving the word 25 px up reddens the overlap pin against the A+B 7 % button.   // 237 -> 236 (#598 items 7/9/10/15, 2026-09-01): net -1 from three changes. OUT: ROD AUTO's three toggle checks, retired with the button — it armed `rods_tavg`, PWR2 carries no such channel and measured DISABLED in every initial condition, and rod control is owner-ruled MANUAL, so it was DELETED from the board rather than left drawn and dark; the channel claim still stands in run_autoctl, and what is genuinely gone is the board half of it, which is the right coverage for a button that does not exist. IN: one check that the channel still comes up disengaged (the physics-perturbing checks below it assume a hand-controlled plant), and two that the #598 item 15 release takes TWO presses — the first arms and releases nothing, the second confirms. That pair is the only place the whole path is exercised: run_pwr2_board checks the row model, this is the rendered popover, a real click and the kernel's block state read back. The block DIRECTION is unchanged and still one click, which is asserted beside it so "warn" cannot quietly become "warn on everything".   // 231 -> 236 (#473, 2026-08-28): the pressurizer's INSIDE was ungated — this harness pinned its ports and nothing drawn within them, which is how the heater bank came to be drawn at 15.6-24.6 % level, straddling and above the 17 % cutoff that exists because the bank uncovers. Five checks: the water surface placed BY VOLUME against an independently retyped cavity integral, the bank inside the band the plant publishes, rods inside the curved wall, the band below the cutoff, and the MOUNTED board carrying the prop. // 230 -> 232 (#572, 2026-08-28): the SUR readout's block band is drawn from the LIVE interlock list now, not from _PROT (which resolves to the pwr table whichever plant is running — the #557 class, and the reason a 1.5 DPM band was painted on a plant with no such interlock). One check per direction: a plant that HAS the interlock still marks it, and a plant that does not never paints the block colour.   // 225 -> 230 (#556, 2026-08-27): the pressurizer level tile's live-arming fixtures — armed at the engine's 87 %, collapsed low side, NO high band below the at-power permissive P-7, and the legacy path (no published setpoints) keeping the authored edges. The fifth pins the authored high edge at the retired plant's REAL 97 % scram: it was hard-coded to 100 under the comment "No high-level trip exists", which was false for that plant too.   // 224 -> 225 (#507 wave 7): the power tile arms at the ENGINE-carried setpoint — a trip_block_status entry with `setpoint: 35` overrides the static pwr1 row (TRIP 35% on a pwr2 startup, never the table's 25); entries without a setpoint stay bit-identical.   // 222 -> 224 (#506.7, 2026-08-22): the powerTile selfTest fixtures carry trip_block_status (presence is the new arming gate) + two cases — the legacy tbs-less shape keeps the old armed reading bit-identical, and the pwr2 shape (tbs empty) gets authored bands with no "TRIP 25%".   // 215 -> 222 (2026-08-08, #358): +2 functional pins (SBO corner reads NO FLOW pre-SAT, gpm demand box ambers) + 5 selfTest cases for the delivery predicate. Injection-verified: blanking feedNoFlow's flow term reds all of them on the old lying behavior.
...
/* --shard=i/N — the CI partition (#637, 2026-09-05).
 *
 * WHY SHARD AT ALL, MEASURED rather than assumed. From the green #630 run's own
 * per-runner timings: the 106 runners cost 5189 s of CPU; CI's ubuntu-latest has FOUR
 * cores, so the pool below runs 3-way (`min(10, cores - 1)`); 5189 / 3 = 1730 s =
 * 28.8 min against an observed gate step of 29.1 min. Throughput-bound to 1 %. So the
 * CI wall is TOTAL CPU ÷ LANES and NOT the longest runner — splitting one runner moves
 * nothing there, and every timeout raise this file's history records bought the same
 * 90 seconds of margin nobody notices shrinking. Only fewer CPU-seconds or more lanes
 * move it, and a GitHub matrix job gets its OWN 4-core runner: N shards ≈ 3N lanes, at
 * no cost on a public repo. (The longest runner becomes the floor again once lanes are
 * plentiful — 1388 s green / 2094.6 s red for run_pwr2_engine, a 1.51× runner-class
 * spread on the same suite — which is why splitting it is a separate phase of #637.)
 *
 * LPT GREEDY over the `secs` hint: heaviest first, each into the currently lightest
 * shard. Longest-processing-time is the standard makespan heuristic and it is what
 * keeps the one heavy runner from landing on top of an already-heavy shard.
 * Deterministic — ties break alphabetically, and members are re-sorted into discovery
 * order afterwards so --record output is not shuffled by scheduling.
 *
 * THE HINT DECIDES BALANCE AND NOTHING ELSE. It is the same standing rule as the
 * longest-first sort below: a stale, missing or floored `secs` costs a slightly worse
 * wall and can never change a score, a drift verdict or an exit code. What it
 * must NOT be allowed to change is membership-completeness, so main() self-checks the
 * whole partition (total and disjoint) on every sharded run, and computes the
 * unknown/missing guard from `found` — the FULL discovered list, before any --only,
 * --fast or shard filtering. A runner falling BETWEEN shards is the exact failure this
 * repo has already had twice with no sharding at all: verify_board_check sat at 1
 * FAILURE through a lane merge, a green CI run and a release, and audit_manual_controls
 * at 32 mismatches, both because nothing enumerated them.
 */
/* A RUNNER IS NEVER FREE, so the cost floor is 1 s and that is the load-bearing part.
 * 29 of the 107 runners carry no `secs` hint at all — the hints total 4831 s against the
 * 5189 s the suite measured on CI — and on the raw hint a hintless runner adds 0, so it
 * never changes which bin is lightest: EVERY one of them lands in the same bin and stays
 * there. Measured on this suite at N = 3, three ways:
 *
 *     raw hint, lightest bin           23 / 55 / 28 runners  (the 55 is the hintless tail)
 *     + fewest-files tie-break         23 / 42 / 41          (a tie is luck: --fast gave 22/27/55)
 *     + 1 s floor  <- what runs        32 / 37 / 38          (--fast 31/36/37, est 1620 s each)
 *
 * The middle row is the trap worth keeping: it looks fixed, and it is only fixed while the
 * hinted loads happen to land equal — the same code gave 22/27/55 the moment --fast removed
 * three runners. (First two rows measured over 106 runners, before this file's own
 * run_ci_shards entry existed; the third over 107.)
 *
 */
function shardCost(f) { return Math.max(1, (BASELINES[f] && BASELINES[f].secs) || 0); }
function shardLoad(files) {
  return files.reduce(function (s, f) { return s + shardCost(f); }, 0);
}
function partitionShards(files, n) {
  var bins = [];
  for (var b = 0; b < n; b++) bins.push({ load: 0, files: [] });
  files.slice().sort(function (a, z) {
    return shardCost(z) - shardCost(a) || (a < z ? -1 : 1);
  }).forEach(function (f) {
    // Lightest bin; ties broken by the fewest files, then by index.
    var pick = 0;
    for (var i = 1; i < n; i++) {
      if (bins[i].load < bins[pick].load ||
         (bins[i].load === bins[pick].load && bins[i].files.length < bins[pick].files.length)) pick = i;
    }
    bins[pick].files.push(f);
    bins[pick].load += shardCost(f);
  });
  bins.forEach(function (x) { x.files.sort(); });   // back into discovery order within a shard
  return bins;
}

function usage(msg) {
  if (msg) console.log(C.red + msg + C.off);
  console.log('usage: node test/run_all.js [--fast] [--only a,b] [--record] [--quiet]\n' +
              '                            [--jobs=N] [--shard=i/N] [--list]\n' +
              '  --shard=i/N   1-based, 1 <= i <= N <= 16');
  process.exit(2);
}
...
async function main() {
  ...
  var list = argv.indexOf('--list') >= 0;
  // Malformed --shard is a HARD exit 2, never a silent full run: the whole point of the
  // matrix is that shard 3 of 3 runs a THIRD of the suite, so a typo that quietly ran
  // everything three times would read as a slow-but-green CI and cost nothing visible.
  // Exit 2 also distinguishes "you called me wrong" from exit 1, "the gate drifted".
  var shardArg = argv.filter(function (a) { return /^--shard=/.test(a); })[0];
  var shard = null;
  if (shardArg) {
    var sm = /^--shard=(\d+)\/(\d+)$/.exec(shardArg);
    if (!sm) usage('bad --shard: ' + shardArg + '  (want --shard=i/N)');
    shard = { i: parseInt(sm[1], 10), n: parseInt(sm[2], 10) };
    if (shard.n < 1 || shard.n > 16 || shard.i < 1 || shard.i > shard.n) {
      usage('bad --shard: ' + shardArg + '  (need 1 <= i <= N <= 16)');
    }
  }
  ...
  // The shard is taken AFTER --only/--fast filtering and BEFORE the longest-first sort:
  // filtering decides what is in scope, the shard partitions that, and the sort is a
  // scheduling detail inside one process. Taking the shard first would make --only mean
  // something different on every shard.
  var filtered = scripts;
  if (shard) {
    var parts = partitionShards(filtered, shard.n);

    // THE SELF-CHECK, and it is not decoration. A shard runner is the one place where a
    // silently dropped runner is invisible: every shard is still green, the aggregate is
    // still green, and nothing anywhere says the runner did not run. So assert the two
    // properties that make a partition a partition — total and disjoint — every time,
    // over all N shards, not just the one this process is about to execute.
    var seen = Object.create(null), dupes = [];
    parts.forEach(function (p) {
      p.files.forEach(function (f) { if (seen[f]) dupes.push(f); seen[f] = 1; });
    });
    var lost = filtered.filter(function (f) { return !seen[f]; });
    var extra = Object.keys(seen).filter(function (f) { return filtered.indexOf(f) < 0; });
    if (dupes.length || lost.length || extra.length) {
      console.log(C.red + C.bold + 'SHARD PARTITION IS BROKEN — refusing to run' + C.off);
      if (lost.length)  console.log(C.red + '  in no shard: ' + lost.join(', ') + C.off);
      if (dupes.length) console.log(C.red + '  in two shards: ' + dupes.join(', ') + C.off);
      if (extra.length) console.log(C.red + '  invented: ' + extra.join(', ') + C.off);
      process.exit(2);
    }
    scripts = parts[shard.i - 1].files;
    // An empty shard is a misconfiguration (N above the runner count), not a pass — but
    // it is a WARNING here rather than an exit, because --only legitimately produces one
    // locally. run_ci_shards.js fails on it for the workflow's N, which is where it matters.
    if (!scripts.length) {
      console.log(C.yellow + 'shard ' + shard.i + '/' + shard.n +
        ' is EMPTY — N exceeds the ' + filtered.length + ' runners in scope' + C.off);
    }
  }

  // --list is the dry form: it prints membership and stops. run_ci_shards.js drives it
  // for i in 1..N and asserts the union is discovery, which is how the workflow's numbers
  // stay derived. It still honours the unknown/missing guard — a dry run that could not
  // go red would be a way around the one check this file exists to make impossible.
  if (list) {
    // `est` is the floored-hint sum, printed because it is the only visible sign that the
    // partition is BALANCED rather than merely total — three equal COUNTS can still be
    // 20 min and 2 min if the heavy runners land together. It is a hint sum, not a
    // prediction: 29 of the 107 runners contribute the 1 s floor rather than a measurement.
    console.log((shard ? 'shard ' + shard.i + '/' + shard.n + ': ' : 'no shard: ') +
      scripts.length + ' of ' + filtered.length + ' runners' +
      (filtered.length === found.length ? '' : ' (of ' + found.length + ' discovered)') +
      '  est ' + shardLoad(scripts) + 's of ' + shardLoad(filtered) + 's');
    scripts.forEach(function (f) { console.log('  ' + f); });
  ...
  console.log(C.bold + 'Aggregate gate — ' + scripts.length + ' runners' + C.off +
    ...
    (shard ? C.bold + '  shard ' + shard.i + '/' + shard.n + ': ' + scripts.length +
             ' of ' + filtered.length + ' runners' + C.off : '') +
