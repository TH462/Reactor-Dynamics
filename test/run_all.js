...
var BASELINES = {
  ...
  'run_pwr2_relief.js':    { code: 0, score: '43passed 0failed 43checks', secs: 1 },   // 42 -> 43 (#539, 2026-08-27): the rated-flow guard was `=== undefined`, so it refused to invent a MISSING plant and then silently accepted one of size NOUGHT — the same fabrication, differently spelled, and the case that actually shipped. Now `> 0`. Mutations 24 -> 25.  // 38 -> 42 (#511, 2026-08-24): THE MSIV — sourced placement (Ginna TS Bases B 3.7.2): a shut valve zeroes the DUMP (downstream) and touches neither the safeties nor the ADV (upstream); mid-stroke passes proportionally; absent msiv_frac means open (the pre-#511 caller). Mutations 23 -> 24 (the msiv gate severed).   // +8 (the ADV, 2026-08-19): Ginna TS Bases B 3.7.4 verbatim -- 329,000 lb/hr per-MWt scaled (one valve, one loop; the source's own ~4%-of-RTP cross-check lands at 4.1 %), auto setpoint [derived] 1040 psig (the WAT-05 45-psi margin below the 1085 pop), modulating not popping, operator cooldown lever, block valve, and NOT gated on the condenser -- which is function (b)'s whole point. Mutations 19 -> 23 (auto-dead, condenser-gated, rung-inverted, block-ignored), incl. two anchors the ADV's own arrival broke, re-pointed.
