...
var BASELINES = {
  ...
  'run_pwr2_board.js':     { code: 0, score: '11passed 0failed 11checks', secs: 30 },   // NEW (#506, 2026-08-22): the pwr board driver over a REAL pwr2 SimulationService, headless — the seam the playtest broke and no gate stood on. Pump props finite (the RCP froze on a NaN spin speed) · power tile authored bands (was "TRIP 25%" at 99.8 %) · the payload round trips (HPI STOP secures, heater MANUAL lands, letdown lamps latch, dump CLOSED reads, charging OFF reads, S/F rod speeds measurably differ) · THE NO-ORPHAN SWEEP: every button item handled+acknowledged, momentary, UI-local or disabled (DESIGN_CRITERIA Q4 as a check). 2 mutations (tile presence condition, set_hpi payload revert), both caught.
  ...
  'run_pwr2_sg.js':        { code: 0, score: '27passed 0failed 27checks', secs: 340 },   // 25 -> 27 (#507 wave 5): the THIRD inlet stream — a tube leak lands in the mass ledger (feed 60 + leak 40 − steam 100 holds level exactly) and lands HOT (40 kg/s at primary enthalpy pressurizes the secondary where the same cold AFW flow suppresses it). +2 mutations (leak dropped from mass / from energy).  // 90 -> 340 (2026-08-16): the 90 was never measured on a quiet machine. Timed alone, twice: 341 s. `secs` is only a longest-first scheduling nudge and cannot fail anything, which is exactly why a wrong one goes unnoticed -- it just schedules a 5.7-minute runner as if it were a 90-second one.
  'run_pwr2_sources.js':   { code: 0, score: '31passed 0failed 31checks', secs: 195 },   // FIXTURE RAMPED (#499 guards, 2026-08-19h): the affinity fixture's instant hand-pin teleported the sealed loop 2.589 MPa in ONE step and pwr2_core's root-tracking limit refused it -- correctly; the pin now ramps over 400 steps (max 0.673 MPa/step, never latches, P still reaches the 18.000 wall) and the identity holds exact at the settled state, 0.3005 = 0.3005.   // +1 (#492, 2026-08-18): the in-corpus nat-circ capability anchor audit #488 A3 found uncited -- Ginna TS Bases "as high as 3% RTP can be removed by natural circulation alone" -- asserted as a CAPABILITY (9 MW carried with the core subcooled, 116.5 kg/s), the flow magnitude still deliberately unasserted.   // +3 (#479, 2026-08-17): mergeSources(), the concatenation point break/ECCS/CVCS all need before stepPlant.   // +4 (#479, 2026-08-17): THE PUMP DENSITY COUPLING. pumpHead returned dP_rated*r*r -- pressure rise from shaft speed alone. A centrifugal pump develops HEAD (dP = rho*g*H), so the rise scales with impeller density, and friction for a given MASS flow runs as 1/rho. Measured before the fix, 5 cm2 break, no ECCS: mdot_loop held 1630 kg/s for the whole 840 s blowdown with the core dry at 470 degC and 2.4 % inventory left -- the plant circulating rated mass flow of STEAM, so clad heat-up was 1 degC, core damage was unreachable and natural circulation could never be seen. No new constant, exactly 1 at the design density; the equilibrium that falls out is the pump-affinity identity mdot proportional to rho, checked to 1e-3.
