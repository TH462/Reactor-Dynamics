...
var BASELINES = {
  ...
  'run_pwr2_loop.js':      { code: 0, score: '37passed 0failed 37checks', secs: 5 },
  ...
  // SPLIT BY PLANT (#513, owner-approved 2026-08-25): run_campaign.js (part A, the structural
  // suites + every pwr_* mission) + run_campaign_b.js (part B, rbmk_* + bwr_* — the on-hold
  // plants, measured 8 s). Together they are the same 51 suites; the split is scheduling only.
  'run_campaign_b.js':     { code: 0, secs: 8, score: '13/13 42passed' },
  'run_campaign.js':       { code: 0, secs: 257, score: '38/38 3007passed' },   // 3039 -> 3049 (#447): pwr_qualify's `challenge` arming cue re-keyed a FOURTH time, and this time off a bare level threshold onto the deception's two-parameter signature (level recovered past 45 % WHILE inventory is below 96 %). The shed removes heater energy, so less void forms and the lift crests 54.9 % instead of 78.5 % — a `> 58` cue could never arm. The new cue fires at 1322 s shed, 791 s on the pre-#447 plant, and NEVER uninjected; reaching `challenge` earlier is what adds the checks.
