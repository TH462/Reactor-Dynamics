...
'run_service_invariance.js': { code: 0, score: '8passed 0xfail 0failed 8checks', secs: 90 },
  ...
  'verify_e2e_ui.js':        { code: 0, secs: 58, score: '4screenshots', slow: true,
    note: 'CONTENTION-SENSITIVE AT ONE CHECK: testPauseResumeSpeed (#691) is a real-time ' +
          'Playwright timing assertion (does a speed/pause button light within a window). ' +
          'Measured 2026-09-11: it FAILED with RD_workbench concurrently running ' +
          'run_pwr2_engine_c/run_behavior_c/run_campaign_c on the same machine, no code changed, ' +
          'then PASSED run alone. RE-RUN THIS RUNNER ALONE before adjudicating a red on that ' +
          'check as real — same treatment as run_ci_shards.js\'s cross-lane races, never widen ' +
          'the assertion\'s window to paper over contention.' },   // score unchanged, +1 CHECK FUNCTION (2026-09-11, #707 follow-up): testRodLimitMarginIndicationRange -- commits the one-off Playwright probe #707 proved the chart-lane fix with but never checked in, on the SEPARATE Indications-tab scanner-detail rendering path (indicationFacts() in ui/app.js, not the chart lane). Reads the RENDERED "Indicating range 0 steps to N steps." string off `.num-line[data-ser="rod_margin"]`'s data-scanner-detail attribute, not source. Two checks: not the retired 912 literal, AND the string follows RD.pwr2.kinetics.RODS.max_steps moved under the running plant through a real Free Play reset (buildIndications() runs once per rebuild, not per broadcast, so the poke alone changes nothing until the plant rebuilds). Injection-verified: reverting to the generated ind.range reds check 1 (912); a captured literal 627 passes check 1 and reds check 2 (627 -> 1568 under the moved bank, string stays at 627). Screenshot count unmoved. // score unchanged, +1 CHECK FUNCTION (#707, 2026-09-11): testRodLaneBankScale -- the three rod chart lanes were declared to the RETIRED engine's 912-step bank against this plant's 627 (WTSM 8.1 s8.1.5.4, ML11223A252), so a bank ON ITS STOP drew at 69 % of its lane. The check reads the DRAWN lane chrome, not the literal, and types no bank number: a channel at full scale must reach its lane top, no lane top may exceed the published bank, and the scale must FOLLOW the bank moved under the running chart -- that last one is what a parse-time capture cannot do, and it is the only check the captured variant reds (measured: 912 literal -> top 700 vs bank 627; captured 627 -> passes 1/2/4, reds 3). Screenshot count unmoved. // score unchanged, +1 CHECK FUNCTION (#694, 2026-09-10): testWalkthroughEventPause -- the browser half of the walkthrough pause proof (the Node half is run_checklist.js section 11, above). Proves the setTimeout loop actually stops rescheduling and `.bd-frozen` actually paints, on a synthetic fixture (no shipped step authors `pause` yet). The printed score is a screenshot count and does not move; `secs` is a scheduling hint per this file's own convention and is not maintained. // 16 -> 4 (#514, 2026-08-25): PWR-only — the shell no longer loads RBMK/BWR, and app.js falls back to the PWR on an absent engine, so an rbmk/bwr row would silently screenshot the PWR and certify nothing. The #111 strict xfail (manual unit conversion) is PWR-side and stays.
