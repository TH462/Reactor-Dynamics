...
var CTL = {
  ...
  close_msiv:        { g: 'Secondary', c: 'MSIV — Close', u: 'Isolates main steam. The turbine trips, the bottled Steam Generator pressurizes to its code safety valves, and with feed gone it boils down toward the low-level reactor trip — close it deliberately. It is also the answer to a steam line break DOWNSTREAM of the valve: shutting it puts the valve between the generator and the break and the blowdown stops. A break UPSTREAM, between generator and valve, has no isolation on this single-generator plant.', cmd: 'close_msiv', p: '' },
  ...
  set_esf_auto:      { g: 'Safety', c: 'ESF Auto Re-arm (HPI/LPI, AFW, RHR)', u: 'Returns an engineered-safety system to AUTOMATIC: the system re-arms for its auto-actuation, and a still-standing start condition fires immediately. Any manual action on the system (on/off/throttle) puts it in MANUAL. RHR auto re-opens its suction valve once tripped and below the 400 psi interlock.', cmd: 'set_esf_auto', p: '{system, auto}' },
  set_rhr:           { g: 'Safety', c: 'RHR Suction Valve (doubles as LPI)', u: 'Opens/closes the Residual Heat Removal (RHR) hot-leg suction valve to align shutdown cooling. INTERLOCKED: it will only open below 400 psi (2.76 MPa) and AUTO-CLOSES if primary pressure climbs back above that — depressurize into the RHR band first. Aligned, RHR recirculates coolant hot leg → heat exchanger → cold leg through the low-pressure (LPI) pump. The same pump/line is the automatic Low-Pressure Injection path on a LOCA.', cmd: 'set_rhr', p: '{active}' },
  set_rhr_hx:        { g: 'Safety', c: 'RHR Cooldown Rate (HX Flow Split)', u: 'Sets how much of the RHR loop flow passes through the heat exchanger vs. the bypass (0–100 %). More through the exchanger = faster cooldown. Total flow is unchanged, so this throttles cooldown RATE without disturbing inventory — walk it up to hold the ~50 °C/h limit.', cmd: 'set_rhr_hx', p: '{fraction | pct}' },
  set_dhr:           { g: 'Safety', c: 'RHR (deprecated command alias)', u: 'Deprecated one-release alias for the RHR suction valve (set_rhr) — kept for save-file compatibility. The system is called RHR everywhere in the UI and manuals; only this command id still carries the old name.', cmd: 'set_dhr', p: '{active}' },
  ...
  set_charging_flow: { g: 'Reactivity', c: 'Charging Pump (CVCS)', u: 'Charging injects coolant into the cold leg — raises primary inventory and carries the boron. Manual setpoint (clipped to the pump run-out, #421; the board box reads gpm) or auto make-up.', cmd: 'set_charging_flow', p: '{normalized}' },
  ...
  set_letdown_flow:  { g: 'Reactivity', c: 'Letdown (deprecated command alias)', u: 'Deprecated alias — snaps a requested flow to the nearest letdown orifice lineup (off / A / B / A+B). The real control is set_letdown_orifices; letdown removes coolant from the Reactor Coolant System, lowering primary inventory.', cmd: 'set_letdown_flow', p: '{normalized}' },
...
var CONTROL_SETS = {
  pwr: ['rod_start', 'rod_nudge', 'rod_stop', 'rod_stop_all', 'scram', 'set_boron_adjust', 'set_charging_pump',
        ...
        'set_feed_pump_speed', 'feed_pump_nudge', 'set_afw', 'set_afw_flow', 'set_esf_auto', 'set_steam_demand', 'set_steam_dump', 'set_hpi', 'set_rhr', 'set_rhr_hx', 'set_sr_detector', 'set_trip_block', 'open_msiv', 'close_msiv', 'automate'],
...
var IND = {
  ...
  // Instruments added between the 2026-07-31 regeneration and #387 — they were live in
  // the engine but absent here, so the Failures-tab picker could not offer them (and a
  // bare regeneration would have printed raw ids as names, the I-12 failure re-run).
  pzr_spray_flow:   { n: 'Pressurizer Spray Flow', m: 'Flow through the pressurizer spray line, percent of the line\'s maximum — cold-leg water sprayed into the steam space to condense steam and lower pressure. Zero with the valve shut; pinned at zero when the RCPs that drive it are off.', u: '%' },
  containment_pressure: { n: 'Containment Pressure', m: 'Containment building pressure. Sits at atmospheric on a healthy plant; rises when a break or open relief valve discharges into the building — the direct evidence that primary inventory is leaving through a containment-side hole (a tube rupture bypasses it into the steam generator instead).', u: 'MPa' },
  containment_temp: { n: 'Containment Temperature', m: 'Containment atmosphere temperature. A steam release drives it with pressure (the atmosphere runs at the steam\'s saturation temperature); the channel is slow — a building RTD, not a pipe probe.', u: '°C' },
  containment_sump_level: { n: 'Containment Sump Level', m: 'Water level in the containment sump, from a float gauge. Break liquid and condensed spray drain here, so a rising sump with falling pressurizer level is the inventory-balance confirmation of a containment-side leak.', u: '%' },
  core_exit_temp:   { n: 'Core-Exit Temperature', m: 'Core-exit thermocouple reading — the post-TMI inadequate-core-cooling channel. Equals the bulk hot-leg reading on a covered core; climbs above it when the core uncovers and the exit reads superheated steam instead of water.', u: '°C' },
  pzr_level_dev:    { n: 'Pressurizer Level Deviation', m: 'Indicated pressurizer level MINUS the programmed level for the current Tavg. Load changes move level and program together, so this stays near zero through them — a sustained deviation is an inventory signal (leak or unmatched charging/letdown), which an absolute level alarm cannot separate from a load swing.', u: '%' },
  rod_limit_margin: { n: 'Rod Insertion Limit Margin', m: 'Control-bank steps remaining above the rod insertion limit — the shutdown-authority-remaining signal the ROD LIMIT LO annunciator reads. Reads full scale when the limit does not apply.', u: 'steps' },
  tavg_rate:        { n: 'Heatup/Cooldown Rate', m: 'Rate of change of indicated Tavg, damped to the hourly scale the Tech-Spec-class 100 °F/hr limit is written in. Quiet through a normal post-trip settle; a genuine overcooling transient crosses the alarm within a minute.', u: '°C/hr' },
  loop_delta_t:     { n: 'Loop ΔT', m: 'Indicated hot-leg minus cold-leg temperature, percent of rated ΔT — the power-as-the-coolant-sees-it signal the OTΔT/OPΔT protection compares against its computed setpoints.', u: '%' },
  otdt_setpoint:    { n: 'OTΔT Setpoint', m: 'The overtemperature-ΔT trip line, computed live from indicated Tavg and pressure — it moves as the plant moves, which is the point: it protects the DNB limit wherever the plant currently sits.', u: '%' },
  opdt_setpoint:    { n: 'OPΔT Setpoint', m: 'The overpower-ΔT trip line, computed live from indicated Tavg — the linear-power (fuel melt) companion to OTΔT.', u: '%' },
...
var ALARM_RESPONSE = {
  pwr: {
    reactor_trip:      { m: 'The reactor has shut down (Reactor Protection System actuation).', r: 'Confirm rods in and power dropping; establish decay-heat removal (AFW / RHR); verify inventory.' },
    ...
    high_tavg:         { m: 'Average coolant temperature is above its high alarm — the plant is running hotter than the temperature program.', r: 'Reduce power or raise heat removal (steam dump / feed); check the Tavg-vs-load balance.' },
    pzr_pressure_high: { m: 'Primary pressure is above its normal band (heading toward the spray / PORV region).' },
    pzr_pressure_low:  { m: 'Primary pressure is below its normal band — subcooling margin is shrinking.' },
    ...
    sg_level_low:      { m: 'Steam Generator level is below its normal band — the heat sink is shrinking.', r: 'Restore feedwater; Auxiliary Feedwater auto-starts at the low-low level.' },
    rcp_cavitation:    { m: 'A Reactor Coolant Pump is cavitating — its suction is near saturation and flow is degrading.', r: 'Restore subcooling at the pump suction (raise pressure / lower temperature); reduce reliance on the affected pump.' },
    hpi_active:        { m: 'Emergency injection (HPI/LPI) is running (status indication).' },
    turbine_trip:      { m: 'The turbine has tripped or steam demand has fallen low.' },
    msiv_closed:       { m: 'The Main Steam Isolation Valve is shut — the steam generator is bottled off from the turbine.' },
    sg_press_high:     { m: 'Steam Generator (secondary) pressure is high.' },
    cond_vac_low:      { m: 'Condenser vacuum is degrading — turbine backpressure is rising.' },
    cond_vac_trip:     { m: 'Condenser vacuum has fallen to the turbine-trip level.' },
  },
  rbmk: {
