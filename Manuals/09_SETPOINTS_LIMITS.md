...
| **SI backup (containment)** | containment_pressure | high | **18.1 psi (0.125 MPa)** absolute — the sourced **3.5 psig** | **Built (#784), and unblockable** — WTSM 12.3: *"This SI actuation signal cannot be blocked by the operator."* Unlike the other three SI paths, P-11 does not gate it. The high-energy-line-break backup: starts injection on building pressure when the primary has not yet fallen far enough on its own. Measured, large LOCA severity 1.0, full power: this signal is the one that actually latches SI, at **5.58 s** — ahead of the low-pressure path, which used to latch it at 8.88 s before this signal existed. **It trips the reactor** (#800): on a full-severity reactor coolant pump seal leak it latches SI with the plant at power, and the reactor now trips on the same step (measured 369 s) instead of 78 s later on low-low steam generator level |
