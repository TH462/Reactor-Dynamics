...
| **Atmospheric dump (ADV)** | steam_pressure | high | **1247 psi (8.60 MPa)** | **SHIPS IN AUTO.** Vents to atmosphere, upstream of the MSIV and independent of the condenser: this is the cooldown path when the condenser is gone. In AUTO it holds a bottled generator at the setpoint instead of on the 1350 psi (9.31 MPa) code safeties — but capping pressure is not a cooldown; lower the setpoint or open the valve for that. Full open at 1283 psi (8.85 MPa); capacity 10 % of rated steam flow. Setpoint box clamps to the same 29–1350 psi band as the Dump SP. Cools well past the 100 °F/hr limit at full open — see `12` §12.18 |
