...
|----|-----|---------|-------------------|--------|
| I-01 | — | **No cold plant state.** **RESOLVED (2026-07) FOR THE RETIRED ENGINE, AND REOPENED BY THE ENGINE SWAP (2026-08-30).** The `cold_shutdown` (Mode 5) initial condition shipped in `pwr_config.js`; the plant the site runs refuses it by name and has no Mode 5 at all (#524). Its cold end is Mode 4, Hot Shutdown. The full Mode 4↔1 heatup/cooldown runs on integrated physics with time-compressed rates. | **Mode 5, Cold Shutdown** / **Mode 4, Hot Shutdown** are now **[sim]** (rates compressed), driven by missions `pwr_mode5_to_mode3`/`pwr_return_to_mode1`/`pwr_mode3_to_mode5`. 04-N03/N15 and 05 updated. | **Resolved in product** (pre-ship review) |
