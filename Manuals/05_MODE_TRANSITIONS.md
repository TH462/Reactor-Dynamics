...
**NOTE:** The **full commercial story** Mode 5, Cold Shutdown → Mode 1, At Power → Mode 5, Cold Shutdown is now driveable **end to end on the board** from the `cold_shutdown` initial condition. Since #419 the heatup runs on **real rates end to end** — the pump-heat thermal ramp (measured, ~12.3 plant-hours to the settled no-load point at a steady ~30 °F/hr (16.7 °C/hr)), the sourced 0.23 psi/s pressurization slew, and real boron clocks — so what is compressed is the **wall clock** (time acceleration), never the evolution. See the honesty notes below.
