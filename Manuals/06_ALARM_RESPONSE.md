...
| **Means** | The plant has just isolated **every letdown path** — both orifices *and* the RHR-to-CVCS cross-connect, if you were on shutdown cooling — and cut the pressurizer heaters. Both are automatic and both are latched: the fire latch re-arms only above 20 %, and there is **no automatic restoration** — letdown stays shut until you re-open an orifice by hand. **Your orifice selection is not touched**: the A / B lamps read what you last selected, so a lit lamp here is not a flowing orifice (**03** §7.3). |
