...
Second, **the code safeties still lift on a fast transient.** Bottling the generator from full power spikes past the ADV faster than it can open — measured, an MSIV closure at power reaches 1314 psi (9.06 MPa) with the ADV already wide open, lifts the safeties, and they reseat about two minutes later as the ADV takes over. Coming *up* from a blown-down break there is no spike, and the ADV catches it at the setpoint with the safeties never lifting at all.
