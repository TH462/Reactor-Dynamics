...
**The same letdown-vs-heatup-rate mismatch carries level well past that programme on three of the shipped legs, and it is declared rather than fixed** (§12.23, #706): `pwr_heatup` runs **+20.4 points** above the 25.00 % program (peak **45.37 %**, held for 11.6 of the leg's ~13.4 plant-hours), `pwr_cooldown` **+43.07 points**, and `pwr_shutdown` **+21.19 points**. The RHR-to-CVCS cross-connect autocloses at the sourced 585 psig (4.03 MPa) (above) while the heatup rate is still near its ride-wide peak, and orifice letdown at that system pressure cannot keep pace — #679's CVCS-scale fix (above) makes an already-tight post-autoclose margin about 14.6 % tighter but did not create the shortfall. Non-damaging and self-correcting: the plant reaches its design point at the end of each leg regardless.
...
The isolation **cannot tell where the break is**, and neither can a real one: it fires identically on an upstream break, where shutting the valve changes nothing because the break is on the generator's side of it. **The containment-pressure leg IS built, auto-only and sourced (#784, 2026-09-21)** — WTSM 12.3, main steam isolation on *"(1) a high-high containment pressure signal or (2) high steam flow coincident with…"*, this being signal (1): a break inside containment that pressurizes the building past the sourced **30 psig (44.7 psia / 0.308 MPa)** high-high shuts the MSIV even if it never spikes steam flow, and the valve latches shut. Proved by injection — on a large LOCA the building passes that setpoint and `msli_ctmt` latches at **59.6 s**, shutting the valve (§12.4d). Two parts of the real function remain declared: the steam-flow setpoint is fixed where a real one is programmed on turbine load, which means this plant under-protects at low load (§12.19); and **the flow/pressure coincidence above is not reachable from the failure panel**, because this engine has no steam-line break to produce it (`engines/pwr2/pwr2_shell.js` refuses both `secondary_depressurize` verbs with *"no steam-line break model yet"*). ⚠ **The isolation timings in the FIRST paragraph above (the flow/pressure coincidence leg) were measured on the retired engine and have NOT been re-measured here** — and **09** §3.0, re-measured against this engine, still records that leg (and the plain low-steam-pressure leg) as NOT MODELLED. **The containment leg is the one exception**, re-measured and now agreeing between the two chapters. Treat the flow/pressure leg as unverified until that disagreement is settled; the containment leg and the valve itself are both real.
...
| 12.4d | **Containment spray, the fan coolers and the steam-line isolation that goes with them are BUILT — auto-only, sourced** *(row REWRITTEN 2026-09-21, #784, OWNER RULING: "Authorise it — auto-only (Recommended)", against the shipped engine. The previous revision, #672 on 2026-09-18, declared none of this built; that declaration is what this row replaces)* | Two anchors are still **sourced**, unchanged from before: Ginna UFSAR ch15 (ML20339A101) **1.0 × 10⁶ ft³ net free volume**, scaled to this plant on the CVCS volume ratio, and the pre-accident condition **125 °F (51.7 °C) at 1.0 psig** (measured 15.70 psia / 0.1082 MPa, 125.0 °F on every initial condition). **What is newly sourced, all Ginna documents**: WTSM 12.3 (ML11223A310) — safety injection backs up on **3.5 psig (18.1 psia / 0.125 MPa)**, *"cannot be blocked by the operator"*; the high-high at **30 psig (44.7 psia / 0.308 MPa)** starts containment spray AND shuts the main steam isolation valve, one bistable, two consumers. UFSAR ch15 Table 15.6-18a / 15.6-11H and TS Bases B 3.6.6 (ML20339A221) give the capacities and the licensed post-single-failure configuration this plant carries (HR9): **one** spray train (of two, 100 %-capacity each) at **1800 gpm (6.81 m³/min)** of **50 °F (10 °C)** refueling-water-storage-tank water, **28.5 s** response; **two** fan units (of four) realigning on **any safety injection** — not a containment setpoint of their own — **44 s** response. **Five declared simplifications, none of them hidden inside the sourced numbers:** (1) **spray runs indefinitely** — no refueling-water-storage-tank inventory node exists (the same omission `pwr2_eccs.js` already carries for injection); the anchor plant's 184,950 gal would feed one pump about 103 minutes before the switchover to sump recirculation this engine does not model; (2) **spray efficiency is 1.0** — drops are assumed to reach thermal equilibrium with the atmosphere, optimistic, and this does **not** cancel item (3); (3) **still no structural heat sink**, which real walls use to absorb a large fraction of the blowdown energy in the first minute, and which **OVERSTATES** peak pressure; (4) **the fans' sink temperature is the pre-accident 125 °F (51.7 °C), not the sourced 30 °F (−1.1 °C) service water** — deliberate: with no structural heat sink and no normal-mode ventilation model, a service-water-keyed term would drag a recovered containment about 95.4 °F (53 °C) below its documented resting point; (5) **exactly one fitted constant in the whole build**, the fan-cooler heat-transfer coefficient — no document in any lane's corpus gives a fan heat-removal rate (`find_source` for "fan cooler", "Btu/hr.*fan", "heat removal rate", "recirculation fan" returns nothing numeric), so it is bounded by two sourced facts instead: GEND-061 orders spray ahead of the fans (*"Heat transfer from the containment atmosphere to containment sprays is rapid compared to heat removal by containment coolers"*, measured **9.2 MWt vs 3.9 MWt** at the mitigated peak) and TS Bases B 3.6.6's **59.7 psig** design limit bounds it from above. Still absent, unchanged: the pressurizer relief tank (relief and safety discharge go straight to the atmosphere), sump recirculation and sump geometry (`containment_sump_pct` stays an honest display scale, not a level), and **recombiners**, the whole hydrogen half — §12.4e, and the ruling that authorised this row named spray, the fans and the isolation, nothing else. There is still no design or failure pressure beyond the sourced 59.7 psig limit — NUREG-1431's bracketed `[44.1] psig` template value is still declined | **The mitigated peak sits 1.9 psi under the 59.7 psig design limit — thin, and by declared choice**: the fan coefficient was **not** retuned to widen it, because doing so would hide the missing structural-heat-sink term (3) behind a fitted one (5). Measured full stack, large LOCA severity 1.0, full power, 600 s: **78.5 psig (0.643 MPa)** unmitigated and still climbing, **70.0 psig** fan coolers alone, **62.7 psig** spray + isolation alone, **57.8 psig** with both — the only case that **turns over**, peaking at **403 s**. Containment high-high latches spray/isolation at **59.6 s** (spray delivering at 88.2 s); the containment SI backup is still the first safety-injection path to latch, at **5.56 s** — after its 2.0 s confirmation hold (#800), and after the overtemperature ΔT trip at 4.76 s. **In a station blackout none of it delivers** — spray and the fan coolers are AC loads and stay dark, and the ride is bit-identical to the unbuilt plant. **The main steam isolation valve is NOT an AC load and still shuts** — the one piece that survives a blackout. There are still no containment controls on the board — auto-only was the ruling, Question 4 of `Blueprint/DESIGN_CRITERIA.md` — so there is nothing to operate from the panel; the annunciators (**06** A36–A39) and the Physics tab are the window. Do not read the psig values as a licensing calculation. |
...
| 12.17 | **RETIRED — this plant now reads containment pressure and acts on it, built #784** *(filed 2026-08-05 as #370, "nothing reads containment pressure"; revised 2026-08-06; declared CLOSED at #386 stage 2 on 2026-08-08 against the retired engine; **RE-OPENED 2026-09-18, #672**, because that closure never travelled to the engine the site runs; **CLOSED AGAIN, for real, 2026-09-21, #784** — safety injection, containment spray and the main steam isolation valve all now read `containment_pressure_mpa`, and proved by injection rather than by a source read: see §12.4d)* | Kept as a pointer rather than deleted. The row's own history — closed once against an engine that did not ship, reopened, closed again against the one that did — is the lesson: a claim about what a plant does not do has to be checked against the SHIPPED engine (the standing-procedure rule on inherited claims), not a sibling that used to carry the same behaviour or a ruling that has since been executed | Read §12.4d for the current behaviour and the numbers. Still declared alongside it: §12.19, the steam-flow setpoint's own fixed-vs-programmed gap. |
...
| 12.23 | **A pump-heat heatup carries the pressurizer above its level program — on three of the shipped legs, and it is not the retuned charging path §6.3 already declares (#680)** *(declared 2026-09-21, #706 — OWNER RULING, 2026-09-21: "Declare it but don't add a checklist caution.")* | The mechanism is letdown, not charging: the RHR-to-CVCS cross-connect autocloses at the sourced 585 psig (4.03 MPa) (§6.3) while a heatup rate still near its ride-wide peak keeps demanding inventory be removed, and orifice letdown at that system pressure cannot keep pace. Measured full stack: `pwr_heatup` runs **+20.4 points** above the 25.00 % program (peak **45.37 %**, held for 11.6 of the leg's ~13.4 plant-hours), `pwr_cooldown` **+43.07 points**, and `pwr_shutdown` **+21.19 points**. With charging deliberately secured the level reaches **82.6 %** and is still climbing after 4.2 plant-hours. #679's CVCS-scale fix (§6.3) makes an already-tight post-autoclose margin about 14.6 % tighter but did not create the shortfall. | **No — non-damaging and self-correcting.** The plant reaches its design point at the end of the leg regardless. Nothing is fixed for it: retuning the low-pressure letdown path is declined with this ruling (`normalLetdownKgs()`'s shared orifice/RHR-path figure stays its own open question), and so is a checklist caution on `pwr_heatup` / `pwr_cooldown` / `pwr_shutdown`. **State the gap rather than close it:** `pzr_level_high` (75 % absolute, caution) is live on PWR2 and *is* reached during these excursions, but there is no high-level annunciator the player can see below 10 % power — no lamp is authorised here. |
...
- The containment **building** is modelled — one lumped air/steam volume over a liquid sump, on Ginna's sourced free volume and pre-accident condition (§12.4d). **Engineered safeguards are auto-only and built (#784)**: safety injection backs up on the sourced **3.5 psig**, unblockable; the **30 psig** high-high starts containment spray (one credited train) and the fan-cooler safety realign (on any safety injection) and shuts the main steam isolation valve (§12.4d). **In a station blackout spray and the fan coolers stay dark** — both are AC loads — while the isolation valve, not a motor load, still shuts. **Recombiners are the one system NOT built**: no document in any lane's corpus carries a capacity for them, and the ruling that authorised spray/fans/isolation did not name them. Their contract fields (`ctmt_recomb_demand`, `ctmt_recomb_active`) are **registered statics**, permanently false, so the absence is machine-readable (`engines/pwr2/pwr2_true_state.js`). **Hydrogen is real and accumulates** — generated by the cladding oxidation itself and published as `ctmt_h2_pct` — but nothing removes it and **it never ignites**: `ctmt_h2_burned` is a registered static 0, so the one-time TMI-2-style burn is not modelled (§12.4e). Also still missing: a pressurizer relief tank, sump recirculation and sump geometry, a structural heat sink, an oxygen ledger, local hydrogen pocketing and igniters. **There are no containment controls on the board in this build** — auto-only was the ruling, Question 4 of `Blueprint/DESIGN_CRITERIA.md`; the annunciators (**06** A36–A39) and the Physics tab are the window. *(Rewritten 2026-09-21, #784 — the 2026-09-18 revision, #672, declared spray, the fan coolers and the isolation leg all absent, against an engine that had none of them; this bullet is what replaced that declaration once they were built.)*
