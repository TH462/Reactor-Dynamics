...
| Driver | Behaviour |
|---|---|
| **Thermal expansion** | The loop coolant expands and contracts and the difference transits the surge line — 1.39 % level per °F (2.5 % per °C), anchored at 55 % for full-power Tavg, floored at 28 % |
| **Loop mass** | ±776 % per inventory fraction, the same slope in both directions — a subcooled loop is incompressible, so every net mass change has nowhere to go but the surge line |
| **Void displacement** | **+375 % per void fraction**, credited as a **flow when the displacement happens** — saturated loop steam pushes liquid up the surge line, split between the surge line and any open break by their admittances |
...
of roughly +350. **Indicated level rises while inventory falls** — and nowhere outside the
saturated regime does it do that. The gauge is not lying. It is telling the truth about a
quantity that has stopped meaning what you think it means.

**The lift needs the surge line to be the discharge path, and since Rev 14 the split is
applied to each displacement as it happens.** Loop steam displacing liquid up the surge line
is what happens when the break is at or above the pressurizer steam space (the stuck-open
relief valve: TMI), or when there is no break at all and the loop is boiling (loss of heat
sink) — those paths keep the full calibrated lift, and on them the deception is the lesson.
With a hole in the **loop**, each displacement divides between the surge line and the hole
(WCAP-16009-NP-A §11-4-5, the two-phase surge-line discharge during blowdown) — at the
...
The isolation **cannot tell where the break is**, and neither can a real one: it fires identically on an upstream break, where shutting the valve changes nothing because the break is on the generator's side of it. Since #386 stage 2 there is a **third isolation path**: high-high containment pressure (the sourced 30 psig signal) also shuts the MSIV — so a break inside containment that never spikes steam flow still isolates, on the building's own evidence (§12.17, closed). One part of the real function remains declared: the steam-flow setpoint is fixed where a real one is programmed on turbine load, which means this plant under-protects at low load (§12.19).
...
- The containment **building** is modelled as of #386 stage 1 (2026-08-05); stage 2 (2026-08-08) gave it **active heat removal and protection**: spray on the sourced 30 psig high-high (auto-securing on recovery), fan-cooler safety realign on SI, the unblockable 3.5 psig SI backup, a steam-line-isolation leg, and an upstream steam-line-break source term (§12.4d). Stage 3 (2026-08-08) gave it **hydrogen**: inventory generated by the cladding oxidation itself, transport through whatever opening the primary is discharging from, auto-starting recombiners, and the ruled one-time TMI-2-style burn (§12.4e). **All automatic — no containment controls exist on the board in this build** (owner ruling, pending the board redesign); the annunciators A36–A42 and the Physics tab are the window. Still missing: a pressurizer relief tank, sump recirculation, an oxygen ledger (the burn latch stands in for it), local hydrogen pocketing and igniters (large-dry plant; the burn triggers on the bulk average)
