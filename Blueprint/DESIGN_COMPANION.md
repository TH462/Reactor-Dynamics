...
| 8.30 | **The load-rejection arm CLEARS ITSELF, where the real one latches until a control-room reset** (declared 2026-08-06, #374 evidence pass / audit #297) | The real fast-open arming *"remains armed until the loss-of-load signal is manually reset by a control room operator"* — a mode selector turned to RESET, spring return (WTSM §11.2, ML11223A294, §11.2.2.3). Ours stands down on its own when the power/load mismatch falls inside `dump_reject_clear_mwe` (10 MWe): there is no automatic clear in the real design to source a value FOR, so the value is UNVERIFIED by construction. | **Deliberate — the auto-clear stands in for a reset control the board does not carry.** And it is HALF OF ONE TRADE with §8.21's arm threshold, seen from the other side: the real arm is far more sensitive — *"a ramp load decrease at a rate greater than 5%/min, or a step load decrease of greater than 10%"*, a quarter of our 40 MWe — and it can AFFORD that sensitivity precisely because a human de-arms it; an arm that sensitive with our auto-clear would vent on every dispatch cut, and our blunt 40 MWe arm with a manual-latch reset would leave the dump armed forever on a plant whose operator was never given the reset. **The two constants are one design decision**: do not build the operator-latched reset without the sensitive real arm, or the sensitive arm without the reset — half of the pair re-opens the §8.21 cliff ruling (#219) from whichever side was built. **AND SINCE #460 THE AUTO-CLEAR CANNOT FIRE AT ALL IN THE SHIPPED LINEUP** *(measured 2026-08-17, #489; ruled ACCEPTED the same day — owner selected "Accept and document it" from options I wrote, a selection, not verbatim words)*. The clear tests `\|load_imbalance_mwe\| < 10 MWe` — *the reactor has come back to meet the load* — and what brings it back is the rod controller. #460 (2026-08-11) took `rods_tavg` out of the shipped free-play lineup, so nothing walks power down: measured, a 100 → 59 MWe rejection leaves the reactor at **88.4 %** against a 59 MWe target, an imbalance of ~29 MWe that never re-enters the 10 MWe window. **The dump therefore stays pegged at its 28 % cap indefinitely** — ridden 45 minutes, ~29 MWt to the condenser at steady state. This is the auto-clear's PREMISE aging out from under it, not a coding error; the same commit invalidated a Tier A curriculum row (#484) and five probes (#460's own reds). Accepted rather than fixed **because the fix is not available in isolation** — see the trade above: giving the board the RESET without the sensitive real arm is the half-pair this row already forbids. With rods in AUTO the clear fires normally and none of this appears. | The RESET control and the sourced sensitive arm (5 %/min ramp, 10 % step) as ONE indivisible piece of work, retiring both this row and §8.21's cliff numbers together — **and that work now also has to answer for the MANUAL-rod lineup, in which a ride-out has no end condition at all.** |
...
| 8.19 | ~~**AFW auto-start is offset 3 points above the SG lo-lo reactor trip (20 % vs 17 %)**~~ *(RETIRED 2026-08-08, #380 — the departure was CLOSED: AFW auto-start moved onto the same 17 % lo-lo signal as the reactor trip, the sourced single-signal design (OWNER RULING 2026-08-08: selected "Single-signal AFW"). Two findings closed it. **First, the trip value was never the departure**: the "~30–32 % NR" this row cited is the STS *template's* bracketed plant-specific placeholder (NUREG-1431 Vol 1 ML12100A222 Tables 3.3.1-1/3.3.2-1 print `[30.4]%`/`[32.3]%`; the Bases volume previously cited carries no numbers) — while Ginna, this plant's own #419 anchor, specifies **exactly 17 %** ("low-low level of 17%", UFSAR ch10 ML20339A040) and the W training plant 11.5 % (ML11223A293). **Second, the teaching window survives in its REAL ordering**: the 30 % warning → trip window is 11 s (TR-14, #135), and "AFW started, level still falling" is a POST-trip observation in Ginna's own LOFW analysis (Table 15.2-4: AFW delivers ~60 s after the trip; assumption I, ML20339A101) — and on a total feed loss the PI-4 feed-flow start (the real condition 3) has AFW running from ~3 s anyway, measured 2026-08-08. The lesson probes are TR-14/TR-2 — this row's TR-8 credit was a mis-cite; TR-8 trips on the P-9 turbine-trip backstop.)* — original text: **AFW auto-start is offset 3 points above the SG lo-lo reactor trip (20 % vs 17 %)** (sourced 2026-07-31, #220) | The real plant uses **one signal at one setpoint** for both: SG low-low level is the first of the five AFW auto-start conditions (WTSM §5.7, ML11223A229) and is the same low-low function that trips the reactor (NUREG-1431 Tables 3.3.1-1 / 3.3.2-1, ~30–32 % NR span). The offset is ours. | **Deliberate.** It buys a visible *"AFW started — level still falling"* window that a single-setpoint plant does not give a lone operator. The #380 evidence pass measured what moving the trip to the "sourced" ~30 % would take (2026-08-06): at 30.5 % the drain trips at 28.5 s, inside TR-14's Ginna band — the blocker was this row's own ladder (a 30.5 % trip sits above the 30 % LO warning). Both halves of that premise retired 2026-08-08. | ~~One signal at one setpoint~~ — executed 2026-08-08 (#380). |
...
| 8.35 | **The condenser-vacuum → MWe gain is MONOTONIC across the whole operator band — there is no LP-turbine low-backpressure saturation knee** *(ruled 2026-08-08-develop-g, follow-on to the `-f` CW re-anchor)* | A real LP turbine stops paying for vacuum: the last-stage exhaust annulus reaches maximum volumetric flow and chokes, so below roughly **1.0–1.5 inHgA** additional vacuum extracts no further work and invites last-stage flutter and moisture erosion — the output-vs-backpressure curve flattens and can turn over. **That knee VALUE is UNVERIFIED**: `tools/find_source.js` returns nothing in any lane for a turbine exhaust-pressure limit, so the figure is recall, not evidence. What IS sourced is where the anchor plant's condenser was designed to sit — *"The condensers are designed for a circulating water temperature of 50F with an approximate 24.5F temperature rise"* (Ginna UFSAR ch 10 §10.4.3, ML20339A040) — a 74.5 °F CW outlet, which with this model's own 5.4 °F (3 °C) `cw_ttd_c` puts condensing at ≈ 79.9 °F ≈ **1.03 inHgA**. Ours keeps paying past that. Measured full stack, `hot_full_power`, 30 min, reactor at 99.94 % throughout: 85 °F → 2.74 inHgA / **95.4 MWe**; 60 °F reference → 1.42 inHgA / **100.0 MWe**; 50 °F → **101.1**; **35 °F → 0.76 inHgA / 102.3 MWe** — *deeper than the anchor plant's own design backpressure*, and still scaling linearly in `condenser_vacuum_kpa`. `vacuum_max_kpa` (99.5 kPa ≈ 0.53 inHgA) is a runaway backstop, not a physical limit: it binds nowhere inside the 35–85 °F band. | **Deliberate, and the knee's cost is the LESSON — not compute** *(OWNER RULING, 2026-08-08: "is it worth the extra computer when running the sim to do the knee? probably not worth it i just want to show the relationship between this temperature and the plant.")*. **The compute premise does not survive contact**: the clip is already evaluated every step (`clip(vacuum_rated − dP, vacuum_lost, vacuum_max_kpa)` in `pwr_steam_generator.js` stepTurbine), so a knee is a **constant change, not new math** — and the ruling still holds at zero cost, which is why it is recorded as a design call rather than a performance one. Building it would flatten the 35–50 °F half of the operator's range: the player moves the knob and MWe stops responding, which reads as a **broken control**, not as a turbine limit. That is the same argument `pwr_config.js` already makes one level out for declining to cap the gain at `vacuum_rated`. **Q3 buys nothing** (the winter-uprate lesson *is* the monotonic relationship) and **Q4 vetoes** (an orphan region of a live control). Magnitude is defensible independently: −4.6 %/+2.3 % over the 50 °F band ≈ **0.14 %/°F**, inside the ~0.1–0.2 %/°F once-through rule of thumb — **that band is recall, UNVERIFIED**. Optimistic at the cold end only; the warm half is unaffected. **THE 102 % COINCIDENCE IS A TRAP, recorded here so it is not "fixed" the wrong way**: in PWR licensing 102 % of RTP is the Appendix K calorimetric-uncertainty margin the Chapter 15 analyses are *performed at* (Ginna UFSAR ch 15, ML20339A101 — *"core power of 1.811 MWt (102% of uprated power)"*, i.e. 1811 against RTP 1775 MWt), which is **analysis conservatism, not an operating allowance**. Electrical output above nameplate is licensed by nothing and is routine in winter; **reactor** power above 100 % RTP is a Tech Spec violation. So **102 % MWe here is prototypical and 102 % `power_pct` would not be** — and nothing in this row licenses the latter: the reactor holds ~100 % across the entire band, the gain is a secondary-cycle efficiency effect on unchanged MWt. | A sourced exhaust-pressure limit (turbine vendor curve, or a plant heat-balance diagram) applied as an output-vs-backpressure **curve** rather than the present linear `condenser_vacuum_kpa / vacuum_rated` ratio, so the cold end rolls off instead of clipping. Worth building only if a turbine-limits lesson is ever added — and it would need the CW floor raised, or the roll-off placed inside the band, to stay visible rather than becoming the dead control this row declines to ship. Pairs with §8.15 (turbine/condenser behavioral, not thermodynamic), which is the row this one refines. |
| 8.36 | **THERE IS NO AUTOMATIC ROD CONTROL, AND THAT IS THE POINT** (declared 2026-08-30, #528) | A real plant hands the control bank to a controller holding average coolant temperature on a reference programmed from turbine load, and the operator supervises it (WTSM 8.1, ML11223A252). The retired engine had exactly that (`rods_tavg`). PWR2 has no such channel and will not be given one. | **Deliberate, and EDUCATIONAL rather than a simplification — the DESIGN_CRITERIA Q3 route, taken openly** *(OWNER DIRECTIVE, 2026-08-30: “I want to keep rod control manual. This is a learning plant not an actual power plant and I think making the player move rods manually will help their learning.”)*. It costs prototypicality on Q2 and buys the Tier A coupling back: with a controller in the loop the player watches Tavg return to program and learns nothing from it; without one they feel that **the rods set temperature while the turbine sets power**, that a load cut leaves Tavg ~17 °F (9.4 °C) high on moderator feedback alone, and that reactivity per step varies several-fold across the bank. **The ROD AUTO button is KEPT and left permanently dark**, against the Q4 orphan-control test, because the contrast is the lesson — removing it would hide the very comparison the ruling is about; the Scanner card carries the reason so it does not read as broken. | **NOTHING** — this is not deferred work. Reversing it needs a new ruling. Two consequences live elsewhere: `§8.30`'s steam-dump auto-clear can never fire in a manual-rod lineup (already recorded there), and campaign mission `pwr_rod_auto` teaches the channel and is unreachable while missions are free-play-only (#525). |
