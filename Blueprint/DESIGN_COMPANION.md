...
| 8.34 | **The atmospheric dump valves' capacity and setpoint are unsourced** (built 2026-08-05, #371; NARROWED 2026-08-06 — the ship-SHUT half is retired, see below) | A real plant's ADVs modulate automatically to hold steam pressure, and their capacity and setpoints come from the plant's design. Ours modulate automatically too as of 2026-08-06, so only the numbers remain: `adv_max 0.10` / `adv_setpoint 8.60` are engineering choices, not citations. **No document in any lane's corpus contains "atmospheric" in a steam-relief sense**: WTSM §11.2 is dump-to-condenser throughout, §5.7's only hit is the turbine-driven AFW pump's own exhaust, §19.0 has none. The closest support is indirect — §5.7.6 defines AFW as maintaining inventory *"for removal of heat energy from the RCS by secondary side steam release"* and never names the release path — plus §11.2's C-9 condenser-available interlock, which is what justifies the ADV sitting outside it, and its statement that the condenser dump is *"not required for the safe shutdown of the reactor"*. | **Only the capacity is a departure now.** *Default shut — RETIRED 2026-08-06* *(OWNER, 2026-08-06: "Amos dump should start in auto" — ADV, dictated)*. The original justification was that shipping AUTO "would silently remove the code safeties from every bottled-SG evolution — TR-5, TR-8, the MSIV mission, TR-12b's safety lift". **Measured peak-tracked, that is false.** An MSIV closure at hot full power lifts the safeties either way and within three seconds of the same instant — peak 9.31 MPa (1350 psi) with the ADV in AUTO against 9.32 (1351) with it shut, lift at 9.08 vs 9.09 MPa. TR-5 and TR-16 both still pin the lift and both pass. What changed is the TAIL: shut, the safeties stay open for the whole event; in AUTO they reseat at 5 min and the ADV holds 8.60. A plant does not sit on its main steam safety valves for half an hour, which is the entire reason an ADV sits below them. One evolution genuinely lost its lift — TR-12b, whose generator re-pressurizes *up* from a blown-down break and so never spikes — and was re-authored rather than re-banded. §8.29's *"there is no controlled cooldown path"* is still closed by a lever: AUTO caps pressure but leaves Tavg at 301 °C indefinitely, so cooling still takes the operator. *Capacity*: 0.10 is **sized, not guessed** — it clears the decay-heat hold several times over so it can genuinely cool, and it reaches RHR-entry temperature on a timescale where the ~55 °C/hr limit is achievable **and exceedable**. That second half is the point: a fully-open ADV cools far faster than the limit, so #375's cooldown-rate annunciator comes in and holding the rate is a skill rather than a formality. Numbers RE-MEASURED 2026-08-06 after the #364 decay refit (×0.42) moved every figure this row first quoted: decay at 1 h is **1.44 %** (was ~2 %), so the authority ratio is **6.9×** (was ~5×); full-open initial rate **−352 °C/hr (−634 °F/hr), 6.4× the limit** (was −165 / 3×); RHR entry reached **43 min** after opening. The argument only strengthened — more margin, and throttling is more of a skill, not less. Pinned by TR-17, whose bands held through the refit unmoved. | A sourced capacity, if a document that names one is ever fetched. (The other item here — *"auto-modulating ADVs as the default lineup, which needs its own measurement campaign against every bottled-SG evolution"* — was done on 2026-08-06; the campaign found two probes to re-author, TR-12b and TR-17, and left TR-5/TR-16 untouched.) |
