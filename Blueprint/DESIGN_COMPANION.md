...
| 8.34 | **The atmospheric dump valves ship SHUT, and their capacity and setpoint are unsourced** (built 2026-08-05, #371) | A real plant's ADVs modulate automatically to hold steam pressure, and their capacity and setpoints come from the plant's design. Ours default to CLOSED — the operator opens them — and `adv_max 0.10` / `adv_setpoint 8.60` are engineering choices, not citations. **No document in any lane's corpus contains "atmospheric" in a steam-relief sense**: WTSM §11.2 is dump-to-condenser throughout, §5.7's only hit is the turbine-driven AFW pump's own exhaust, §19.0 has none. The closest support is indirect — §5.7.6 defines AFW as maintaining inventory *"for removal of heat energy from the RCS by secondary side steam release"* and never names the release path — plus §11.2's C-9 condenser-available interlock, which is what justifies the ADV sitting outside it, and its statement that the condenser dump is *"not required for the safe shutdown of the reactor"*. | **Deliberate on both counts.** *Default shut*: with `adv_override 0` the term is identically zero and the plant is byte-identical, so every bottled-SG evolution the sim teaches — TR-5, TR-8, the MSIV mission, TR-12b's safety lift — is untouched. Shipping it AUTO at 8.60 would silently remove the code safeties from all of them, which is a large unattributable re-baseline bundled into a feature. And the gap §8.29 named was *"there is no controlled cooldown path"*; a lever the operator reaches for is what closes it. *Capacity*: 0.10 is **sized, not guessed** — it clears the decay-heat hold several times over so it can genuinely cool, and it reaches RHR-entry temperature on a timescale where the ~55 °C/hr limit is achievable **and exceedable**. That second half is the point: a fully-open ADV cools far faster than the limit, so #375's cooldown-rate annunciator comes in and holding the rate is a skill rather than a formality. Numbers RE-MEASURED 2026-08-06 after the #364 decay refit (×0.42) moved every figure this row first quoted: decay at 1 h is **1.44 %** (was ~2 %), so the authority ratio is **6.9×** (was ~5×); full-open initial rate **−352 °C/hr (−634 °F/hr), 6.4× the limit** (was −165 / 3×); RHR entry reached **43 min** after opening. The argument only strengthened — more margin, and throttling is more of a skill, not less. Pinned by TR-17, whose bands held through the refit unmoved. | Auto-modulating ADVs as the default lineup, which needs its own measurement campaign against every bottled-SG evolution; and a sourced capacity if a document that names one is ever fetched. |
