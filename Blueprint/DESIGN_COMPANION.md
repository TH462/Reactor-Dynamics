...
| 8.34 | **The atmospheric dump valves' capacity and setpoint are unsourced** (built 2026-08-05, #371; NARROWED 2026-08-06 — the ship-SHUT half is retired, see below) | A real plant's ADVs modulate automatically to hold steam pressure, and their capacity and setpoints come from the plant's design. Ours modulate automatically too as of 2026-08-06, so only the numbers remain: `adv_max 0.10` / `adv_setpoint 8.60` are engineering choices, not citations. **No document in any lane's corpus contains "atmospheric" in a steam-relief sense**: WTSM §11.2 is dump-to-condenser throughout, §5.7's only hit is the turbine-driven AFW pump's own exhaust, §19.0 has none. The closest support is indirect — §5.7.6 defines AFW as maintaining inventory *"for removal of heat energy from the RCS by secondary side steam release"* and never names the release path — plus §11.2's C-9 condenser-available interlock, which is what justifies the ADV sitting outside it, and its statement that the condenser dump is *"not required for the safe shutdown of the reactor"*. | **Only the capacity is a departure now.** *Default shut — RETIRED 2026-08-06* *(OWNER, 2026-08-06: "Amos dump should start in auto" — ADV, dictated)*. The original justification was that shipping AUTO "would silently remove the code safeties from every bottled-SG evolution — TR-5, TR-8, the MSIV mission, TR-12b's safety lift". **Measured full stack, that is only half right, and the half it gets wrong is the one it rested on.** A fast transient still reaches the safeties: bottling from full power spikes to 9.06 MPa with the ADV already wide open, lifts them, and reseats ~2 min later as the ADV takes over — TR-5 and TR-16 both still pin the lift and both pass unchanged. What actually changed is the *steady* state, and it changed for the better: with the valve SHUT the plant parks on its code safeties at 9.00 MPa for a whole hour, which is not what a real plant does; in AUTO it settles at the 8.60 setpoint with the safeties in reserve, which is the entire reason an ADV sits below them. One evolution genuinely lost its lift and was re-authored rather than re-banded — TR-12b, whose generator re-pressurizes *up* from a blown-down break and so never spikes. And §8.29's *"there is no controlled cooldown path"* is still closed by a lever: AUTO caps pressure but leaves Tavg at 301 °C indefinitely, so cooling still takes the operator. *Capacity*: 0.10 is **sized, not guessed** — it clears the decay-heat hold (~2 % at an hour) five times over so it can genuinely cool, and it reaches RHR-entry temperature on a timescale where the ~55 °C/hr limit is achievable **and exceedable**. That second half is the point: measured, a fully-open ADV cools at **−165 °C/hr** initially, three times the limit, so #375's cooldown-rate annunciator comes in and holding the rate is a skill rather than a formality. Pinned by TR-17. | A sourced capacity, if a document that names one is ever fetched. (The other item here — *"auto-modulating ADVs as the default lineup, which needs its own measurement campaign against every bottled-SG evolution"* — was done on 2026-08-06; the campaign found two probes to re-author, TR-12b and TR-17, and left TR-5/TR-16 untouched.) |
| 8.31 | **The high-high containment pressure isolation path is not built** (declared 2026-08-05, #370) | The real design isolates the steam lines on **either** high-high containment pressure **or** the steam-flow coincidence (WTSM §12.3.5.1, ML11223A310). We build only the second. | **Not a choice — there is nothing to sense.** This plant has no containment model at all: no pressure, no temperature, no sump, no humidity. Synthesising a containment-pressure signal to actuate protection on would be inventing an instrument, which is the #220 defect class §8.24 already refuses for the RCP-bus loss-of-flow trips. The consequence is bounded and specific: a break whose signature is containment pressurization *without* the steam-flow coincidence — a small break inside containment — does not isolate here. | A containment volume with pressure and temperature, which would also make the RCP-bus trips of §8.24 buildable and give the LOCA somewhere to discharge to. |
