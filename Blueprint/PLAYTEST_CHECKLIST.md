...
| Check | What good feels like | Knob if wrong |
|---|---|---|
| **FULL load rejection @100 % (the ride-out signature):** NO scram. Dump opens to its **40 % stop and stays there**, so the core sheds the rest — self-parks ~46 % power with Tavg ~320 and pzr level ~95.6 % (PZR LVL HI blaring, 1.4 points under the 97 % going-solid trip); the **PORV lifts** as the backstop. Stable but loudly asking for trim. Walk the rods in at your own pace; it lands at 297 °C no-load | The signature FG-4 experience: drama then *your* recovery, no protection theatrics. **Two corrections, 2026-07-31:** the event is a LOAD REJECTION, not a turbine trip — since #216 a turbine trip above P-9 scrams (TR-1b) — and the numbers are for the **0.40** dump, which is what finally produces the drama this row describes. At 1.05 the same event was a non-event (Tavg 305.3, power 97.5 %) | `steam_dump_max` 0.40 — **SOURCED now** (WTSM §11.2), not a feel knob; `alpha_MTC` (stronger = parks lower/cooler) |
