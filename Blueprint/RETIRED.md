...
| 2026-07-29 | `ui/diagram/pwr_synoptic.js` + `.css`, `ui/test_panel/synoptic_check.html` | The **V1 PWR board** (~100 KB of JS + a stylesheet), superseded by the V2 board in `ui/diagram/board/`. It was still `<script>`-loaded on every control-room page and never mounted. Its dev harness went with it — `synoptic_check.html` mounts `RD.PwrSynoptic` and can only test the deleted module. **Two things it carried that did not die with it:** the four `.app.pwr-synoptic` shell hooks in its CSS were never V1 styling and now live in `ui/shell.css` (the V2 board needs them — the class name is unchanged); and `SYN_CONTROL_MAP`, which `run_campaign` used as the PWR highlight-label pool, is replaced there by the board driver's `CONTROL_LABEL_MAP` (a strict superset — see #246). | `930e0c9` |
