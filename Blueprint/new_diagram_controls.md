...
* **Replaces current UI**: This spec **supersedes** the existing plant-display layout — the per-view Primary/Secondary/Diagram split, the tabbed PD control lists, and `pwr_full_plant_diagram_v2.html` (and related partial-loop SVGs). The new UI is **one integrated schematic** as the sole plant-control surface: **margin cards** for major systems, **diagram-embedded panels** where controls belong on the equipment (CVCS box, accumulator tanks), plus vital-few strip, alarms, strip chart, instructor, and sim controls (M8).
...
* **“What matters now”** (optional shell mode, instructor step, or scenario hook): temporarily emphasize the **few readouts and components the operator should watch next** — soft pulse on margin card border, embedded panel chrome, diagram node, and any linked leader. Does **not** replace alarm colors or add truth the board lacks. **Example — TMI-class stuck PORV:** while `stuck_porv_open` + `porv_indicator_stuck_closed` are active, highlight **`pwSubcoolBar`** (eroding margin), **PZR Level card** + `pwPzrWater` (misleading rise), **Relief card** + `pwPorv` (lying indicator), and optionally **CVCS embedded panel** letdown/charging context — **not** the whole diagram. Instructor may drive the same set via `highlight` targets; alarms still read instruments only (HR1).
...
* **Contract alignment**: Indications and controls map to M1 (`pwr_instruments.js` §8.8, `CONTEXT.md` §6.7). Cards are the diagram-local home for parameters not given a dedicated gauge elsewhere. **Diagram build is blocked on** `Blueprint/pwr_synoptic_prerequisites.md` — engine + instruments ship first (Opus); Fable builds UI after that gate passes. No grey placeholder hardware on the schematic.
* **Two Operating Modes** (mode switch is the **only** control for showing/hiding physics-only teaching readouts — no separate True Physics toggle):

  * **Learning Mode**: Full color, rich teaching visuals, glows, strong failure call-outs. Shows **all board indications** plus **physics-only teaching fields** (§ Learning readouts). **Dual readouts** (*Indicated* / *Actual*) only on **deception-relevant** pairs (§ Deception duals) — not on every field.
...
* **HR1 — Plant Indications**: Alarms and annunciation always read `snapshot.instruments` (and status booleans) in **both** modes — a failed sensor means wrong or missing alarms, identical to a real board.
  ...
  * **Learning Mode**: Board indications always shown (same values as Realistic). **Additionally** show physics-only teaching fields (single readout). **Additionally** show *Indicated* / *Actual* pairs **only** on § Deception duals. Switching to Realistic is the only way to hide physics-only and dual *Actual* columns.
* **Animation HR1** (strict in Realistic): **Nothing** on the diagram or cards may read or animate from `true_state`, derived physics, or command setpoints pretending to be plant feedback — **only** `snapshot.instruments`, §8.8 status booleans, and `control_state` for **valve/pump commanded pose** where the plant has no position instrument (bow-tie rotation on command is OK; **flow dashes and spin speed are not**).
  ...
  * **Learning Mode**: May **additionally** animate or overlay from `true_state` where it teaches (relief when `porv_open` true while indicator lies; faint true subcooling ghost on bar — § Subcooling margin bar). Dual readouts name the gap.
...
* **Card placement** (margin cards — spatial, not side-rails):
  ...
  * **Adjacency first**: Place each card **as close as possible** to the component it controls or indicates. High-priority cards (Rod, Power & Reactivity, PZR cluster, Emergency) get the nearest clear margin.
  * **Adjacency is preference, not law**: Cards **must never overlap** vessels, valves, pipe runs, or flow animations. When tight, **slide along the margin** within a logical **anchor zone** (§ A.1b) or **cluster** related cards (PZR Pressure + Level + Relief; SG Level + Steam).
...
* **Subcooling margin bar** (Power & Reactivity card — primary TMI diagnostic):
  ...
  * **Learning Mode**: Same indicated bar; optional faint **ghost cursor** at true `subcooling_c` **behind** the indicated line when running deception / failed-sensor lessons (not shown in Realistic).
...
Three tiers — keeps card density manageable.
...
| `boron_analyzer` / `boron_ppm` | Sample lag and analyzer error vs core truth (once analyzer ships). |
| `instruments[id]` / `true_state[SOURCE[id]]` | **Only while** that instrument has an active `set_instrument_failure` (stuck/drift/dead) — teaches generic sensor deception without cluttering normal operation. |

**Not routine duals** (do **not** show *Indicated*/*Actual* every step): `subcooling_margin` vs `subcooling_c` (at TMI the instrument-derived subcooling **is** the operator truth — dual only if running a failed-P/T-sensor lesson); `sg_level` vs true level (shrink/swell is instrument physics, not malice — animate to **indicated** level on the board).

**Physics-only teaching fields** (single readout, Learning only — hidden in Realistic):

| Field | Teaches |
|-------|---------|
| `xenon_pct_eq` + ρ-from-xenon cues | Poison / reactivity lag after power changes. |
| `reactivity_pcm`, `startup_rate_dpm`, `reactor_period_s` | Reactivity computer — not a protection input (M1 §8.9). |
| `core_inventory_pct` | Inventory loss during relief/LOCA — **no board instrument** (not on real plants); Learning only. Realistic infers from PZR level (on PZR card), CVCS flows, subcooling. |
| `primary_void_fraction` | Two-phase / boil-off context — Learning only (no board instrument). |
| `fuel_temp_c`, `decay_heat_pct`, melt/damage flags | Core heat state beyond flux. |
| Primary–secondary ΔT (derived from true leg temps + `steam_pressure_mpa`) | SG heat-transfer intuition. |
| Cherenkov + amber fuel glow | Power and decay-heat visualization (not numeric). |
...
* Two cards (+ on-diagram temperature taps — no Temperature card):
  ...
  * **Power & Reactivity Card** (reactor-centric aggregates — leader to core)

    * **Reactor Power** (`power_range`, %) — primary neutron-flux-linked indication; Cherenkov intensity follows this in Learning Mode.
    * **T-avg** (`tavg`) and **ΔT** (derived `thot − tcold` from instruments) — numeric readouts.
    * **Subcooling margin** — § **Subcooling margin bar** (vertical zone bar + large °C); instrument-derived from P and Tavg (M1 §8.6); TMI truth diagnostic while PORV indicator may lie.
    * Fuel temperature + damage status (Learning Mode only — true state; no board instrument).
    * **Reactivity computer** (display/derived only, never protection): reactivity (pcm), Startup Rate (dpm), Reactor Period (s).
    * **Xenon poison (Learning Mode only)** — **hidden in Realistic Mode** (no xenon gauge on a real board). Uses engine true state `xenon_pct_eq` (% of xenon-135 equilibrium at current power context).

      * **Bar gauge**: vertical or horizontal fill bar, 0 → 200 % of equilibrium (headroom for post-power-drop xenon peak). **100 %** tick mark labeled *“equilibrium at this power.”*
      * **Intuitive cues** (Learning Mode):

        * **ρ from xenon** (pcm): derived (-)worth × (X/X_eq) — show as *“Xenon holding back **N** pcm”* so the user ties the bar to reactivity.
        * **Trend tag** from smoothed dX/dt: *Building* ↑ / *Peaking* ↔ / *Burning off* ↓ (plain language, not raw derivatives).
        * **Cause hint** (one line, contextual): e.g. *“Power was higher recently — iodine decay feeding xenon”* after a reduction; *“Xenon peak after shutdown — do not withdraw rods into it.”*
        * Optional link: when xenon bar > 100 % and SUR negative, subtle amber on the bar (poison outweighing rod withdrawal).
      * **Not** an HR1 instrument — teaching overlay only; never used for trips/alarms.
...
* Three cards (may **stack Pressure + Level** vertically above PZR; **Relief** beside or atop relief path — whatever fits without overlap):

  * **Pressure Control Card**: Primary pressure (`primary_pressure`) — **system reference pressure** (see § Primary pressure model); Heater AUTO | ON | OFF + manual % slider; Spray AUTO + manual % / open slider. Hover highlights heater coils and spray nozzle on vessel.
  * **Level Card**: Pressurizer level (`pzr_level`) — animated bar matching sim timing; shrink/swell not modeled on PZR (SG only).
  * **Relief Valves Card**
...
* One card (replaces prior “flagged for addition”):
  ...
  * **Primary Flow & Inventory Card** (no PZR level — **PZR level on Level card only**; no subcooling — **on Power & Reactivity** with T-avg/ΔT)

    * **RCP** running status (`rcp_running`) — loop flow animation keyed to this in Realistic.
    * **Learning only:** true `core_inventory_pct`, `primary_void_fraction` (§ Physics-only teaching). Charging/letdown detail lives in **CVCS embedded panel** (not duplicated here).
...
* **Emergency Cooling card** (margin card — **operator-actuated** HPI / AFW / RHR / LPI only):

  * **HPI**: AUTO | ON | OFF (`set_hpi`); `hpi_active` status.
  * **AFW**: START | STOP (`set_afw`).
  * **RHR**: AUTO | ON | OFF (`set_rhr`) — `pwr_synoptic_prerequisites.md`.
  * **LPI**: AUTO | ON | OFF — `pwr_synoptic_prerequisites.md`; `lpi_active` + `instruments.lpi_flow` on diagram line.
  * Hover HPI control → `pwHpiLine`, cold leg, Primary Flow card region.
...
* Two cards:
  ...
  * **SG Heat Transfer & Level Card**: SG level (`sg_level` — animate to **indicated** level); secondary steam pressure (`instruments.steam_pressure`); primary–secondary ΔT (Learning only, § Physics-only teaching).
...
**TMI teaching note**: During stuck-open PORV + stuck-closed indicator, PORV OPEN alarm stays clear, pressurizer level rises, inventory falls, and **subcooling margin** erodes — **`pwSubcoolBar`, PZR Level card, and Relief / `pwPorv`** must be visible together (§ “What matters now” drives the same trio). **Realistic Mode**: lying indicator, **no relief animation**, no `porv_open` readout. **Learning Mode**: § Deception dual on PORV **plus** optional relief animation from true `porv_open`.
...
* **Learning Mode** here = M8 **Education** mode (terminology alias). Mode switch replaces any prior “True Physics overlay” or per-tap visibility toggles — one control, two presentations.
* Click/hover `data-highlight-id` on every major component, margin card, and embedded panel for instructor highlight lookup.

### Modes Summary

* **Learning Mode**: Indications (same as Realistic) + § Physics-only teaching fields + § Deception duals only; glows; xenon bar; richer true-state-driven animation where it teaches.
* **Realistic Mode**: **Indications and status only**; Animation HR1; no physics-only fields; boron from **analyzer**; alarms and danger states always full red/amber.
...
**Teaching implication:** PZR pressure on the Pressure Control card is the **RCS reference**. Do **not** show leg or RCP pressure taps in v1. Leg **temperature** taps are the right spatial detail for now.
...
│                     │  [PZR Press][PZR Lvl][Relief]  [RCP card]           │
└─────────────────────┴───────────────────────────────────────────────────────┘
...
| **Clustering** | Group related **margin** cards (PZR ×3, SG ×2) in the same zone. CVCS and accumulators use **embedded panels** — never clustered as margin cards. |
...
| **What matters now** | Soft pulse on linked margin card + **embedded panel** + diagram node + leader (e.g. TMI → subcool bar, PZR level, PORV). |
...
| Emergency Cooling | margin card | `emergency-cooling` | left ~(2, 72) | `pwHpiLineAnchor` |
| Pressure Control (PZR) | margin card | `pzr-pressure` | top ~(36, 3) | `pwPzrAnchor` |
| Level (PZR) | margin card | `pzr-level` | top ~(36, 14) | `pwPzrAnchor` |
...
**Counts:** **13 margin cards** only. **`pwCvcsPanel`** + **`pwAccumulatorPanel`** are **embedded** (excluded from the 13). **`pwCvcsChargePump`** is diagram hardware, not a card.
...
|`reactor-power`|`pwCoreGlow`, `pwSubcoolBar`, `pwTapSubcool` (hover anchor)|`instruments.power_range`, `instruments.tavg`, ΔT, `instruments.subcooling_margin` on § Subcooling margin bar|Cherenkov Learning only; bar cursor from indication only in Realistic|Power & Reactivity|**live**|
...
|`primary-inventory`|`pwHotLeg`, `pwColdLeg` flow classes|`instruments.rcp_running`; void + true inventory Learning only|Loop dashes from `rcp_running`|Primary Flow & Inventory|**live**|
|`pzr-pressure`|`pwPzrHeater`, `pwPzrSprayValve`, `pwPzrSprayMist`|`instruments.primary_pressure`, `control_state.heater_power_pct`, `control_state.spray_valve_pct`|Heater glow; spray mist; valve rotation|Pressure Control|**live**|
|`pzr-level`|`pwPzrWater`, `pwPzrSurface`|`instruments.pzr_level`|Water rect + surface path match sim timing|Level|**live**|
...
|`emergency-cooling`|`pwHpiLine`, `pwHpiFlow`, `pwAfwLine`, `pwAfwFlow`, `pwRhrLoop`, `pwLpiLine`|HPI/AFW/RHR/LPI status + flow instruments|Margin **Emergency card** controls only|**live**|
...
|`sg-level`|`pwSgWater`, `pwSgSurface`, `pwSgUtubeRef`|`instruments.sg_level`, `instruments.steam_pressure`|Level to **indicated** SG level (shrink/swell)|SG Heat & Level|**live**|
...
**Power & Reactivity card** holds **T-avg**, **ΔT**, **§ Subcooling margin bar**, power, reactivity, xenon. **T-hot** / **T-cold** leg taps only. **`pwTapSubcool`** highlights hot leg and bar together on hover.
...
1. **Spatial layout:** At 1280×800, chart + alarms **bottom ~38–40 %**; diagram stage above. **13 margin cards**; **`pwCvcsPanel`** + **`pwAccumulatorPanel`** embedded on diagram (§A.1b); **`pwCvcsChargePump`** on pipe; no overlap; leaders prominent on hover.
2. **Geometry:** SG steam leaves **top-right**; turbine **over** square condenser; generator **beside** turbine on short shaft; cooling tower in open margin — matches § Diagram geometry v2.
3. **HR1 — Realistic Mode:** Every readout matches `snapshot.instruments` or status. **Zero** `true_state` in numbers or motion (Animation HR1 checklist) — spot-check: stuck PORV + closed indicator → **no relief animation**; accumulator idle → **no** discharge dashes or flow label.
4. **Subcooling bar:** Zone colors (green > 11 °C, yellow 11–0, red < 0); cursor tracks `instruments.subcooling_margin`; large numeric beside bar; Learning ghost at `subcooling_c` only in deception lessons.
5. **HR1 — Learning Mode:** § Deception duals only (PORV, boron, active instrument failures). § Physics-only fields visible. PORV dual + relief animation from `porv_open` during TMI sequence.
6. **Commands:** Every margin-card and **embedded-panel** control issues the correct `CONTEXT.md` §6.7 command and reflects in the next snapshot (`test/run_e2e_controls.js` PWR paths, or equivalent).
7. **SCRAM:** Two-step cover on Rod card (and header SCRAM if present) trips reactor; shutdown bank animates in; `rps_scrammed` true.
8. **CVCS:** **`pwCvcsPanel`** embedded on box (not margin cards); **`pwCvcsChargePump`** on charging leg outside box; animation from flow instruments only.
9. **CVCS isolate:** Letdown **Isolate** sets flow to 0; branch animation stops; inventory response unchanged vs slider to 0.
10. **Charging pump:** OFF stops charge flow animation and blocks borate/dilute effect on `boron_ppm`.
11. **PZR spray:** With RCP stopped, spray command produces **no** mist animation (spray scales with flow).
12. **TMI deception:** **Realistic** — closed indicator, silent alarm, **no relief animation**, **subcooling bar** erodes, PZR level may rise. **Learning** — PORV dual + optional relief from `porv_open`.
13. **Steam dump:** Bypass path animates from steam-dump **instrument/status** only in Realistic.
14. **Learning vs Realistic:** Learning adds § Physics-only fields + § Deception duals + xenon/glows + optional subcooling ghost; Realistic is indications/status only.
15. **Engine gate:** `pwr_synoptic_prerequisites.md` §1 complete — snapshot has all instruments/status listed there; diagram rows below are live.
16. **Accumulators:** **`pwAccumulatorPanel` embedded** on tanks (not margin card); discharge line + check valve + tee; flow UI **only while** `accumulators_discharging`.
17. **Hover & highlight:** Card/**embedded panel** hover → **prominent leader** + node; control hover → physical components + related indications; `pwPorv` ↔ Relief card; `pwTapSubcool` ↔ `pwSubcoolBar`.
18. **What matters now (TMI):** With stuck PORV failures active, soft pulse on **`pwSubcoolBar`**, **PZR Level** margin card + `pwPzrWater`, **Relief** margin card + `pwPorv` — not full-diagram flash.
19. **Regression:** `PWRScenarioTests` **11/11**; RBMK/BWR legacy PD unchanged.
20. **Save/restore:** Mid-transient save/restore within instrument lag tolerance.
