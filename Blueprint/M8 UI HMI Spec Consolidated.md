...
| **Fast-forward auto-dropout (drop triggers, waypoints, Instructor FF suppression)** | `08 §11` | Attention stops (scram / new failure / first alarm on a quiet board → 1×, 2026-07-17) and the **WARP tier's transient watch** (600×/3600× → 60× on a rate excursion, #625, 2026-09-04; `CONTEXT.md §4`). The **Instructor** drives acceleration with automatic drop-out at set points (beat `speed`; §3) and its speeds never warp. |
