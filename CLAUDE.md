...
| **Run an independent audit slice (#221)** | **`tools\audit.cmd <slice>`** (Git Bash: `sh tools/audit.sh <slice>`) — the ONE launch path, #382. It preflights and **refuses** if the session would not be independent, then starts a session on `Blueprint/AUDIT_CHARTER.md` with **this file** and the auto-memory index excluded (`.claude/settings.audit.json`) — #221 RoE 1's priming problem. Three skills: **`audit-prep`** (before — the parts only a primed session can do), **`audit-slice`** (the run), **`audit-close`** (after — triage + the convergence row #221 asks for and nothing tracked). **If you are reading THIS file, you are primed and cannot be the auditor** — prep, print the launch line, stop; do not read the slice's code "to help". **A NEW WINDOW, NOT `/clear`** — measured 2026-08-05, `/clear` fires `SessionStart` and the lane hook prints WIP issue *titles* into the fresh context. Preflight proves the config, not the session: the auditor's first turn must state on the slice issue whether CLAUDE.md was auto-loaded. |
