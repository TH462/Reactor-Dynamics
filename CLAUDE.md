...
- **The board was drawing the DEMAND where it should have drawn the FLOW — 27 items, three of them
  plant defects (2026-08-04, #350).** *(OWNER, 2026-08-04, "Diagram adjustments and issues")* Five things
  to know. **Every pump spun on its RUN COMMAND**: measured full stack with a station blackout at 120 s,
  `condensate_pump_running` reads TRUE for the whole event (correctly — nobody stopped it) against
  `condensate_flow` of 0, and the feed pump's COMMANDED speed winds **100 → 120 %** chasing a level it
  cannot reach while `fw_flow` is 1.5e-52. Three pumps drawn spinning on a dead bus, one FASTER than at
  power. Run lights stay the operator's demand (the #329/#332 split); an impeller is a rotor. **Item 10
  was #231's revert, at the right LEVEL.** Folding a per-element rate into dash velocity made every
  fitting run at a different speed from the pipe it joins — true, and the fix is to make the number a
  property of the SYSTEM: `lineFrac()` computes one fraction-of-rated per TRAIN (17), and everything drawn
  on that train reads it, so two elements that meet cannot disagree by construction. **Each entry is ONE
  LINE's own flow, and that is what lets the run-state REPLACE the port gate** rather than be ANDed with
  it — which item 18 needs, because with the RCPs stopped the pump art correctly reads STOPPED while
  `rcs_flow` still measures **4.47 %** of buoyancy-driven flow through those pipes, and the instrument is
  the better evidence. The first cut lumped the steam dump in with main steam and would have drawn a shut
  dump valve passing full flow at power; `board_check`'s #236 pins caught it. **A dash-speed change is a
  DISCONTINUITY that cannot be removed** — CSS progress is `((now − delay) / duration) mod 1` and `now`
  grows without bound, so retiming moves the dashes by an amount depending on how long the page has been
  open. Quantising onto a ladder with hysteresis is the trade: a line re-times only on a real band change,
  the hop is at most one dash period, and the whole system hops together. **The vital strip strobed at
  STEADY POWER, not during the failures it was reported under**: reactor power changed band **49 times in
  40 sim-seconds** at hot full power because 100.0 % sits on a boundary and the channel sigma is 0.21 % —
  #306's alarm chatter one layer up. A/B: **49 → 0**, with the LOCA crossings unchanged at 1/4/6/1/5/0
  both ways. **And the colour inversion is a CONVENTION change that propagates**: `phaseTempColor` now
  returns the full-strength fluid colour as `bore` and the darkened form as `flow`, which flips the meaning
  of a pair every board component consumes — eleven components had to be corrected (bodies take `bore`,
  anything that MOVES takes `flow`), while `comp_tee` and `comp_pressurizer` were already right because
  they write onto a `K.pipe` stroke stack in the kit's own order. **NOT FINISHED — item 8's third clause
  is a CONTROL-layer defect**: during a blackout `feed_sg` keeps integrating against a plant it cannot
  move and winds `feed_pump_speed_pct` to its 120 % rail, so the gpm box shows a demand nothing can
  deliver. That is integral windup, not a board bug, and patching the board would be the wrong layer.
