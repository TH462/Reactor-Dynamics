...
- **A tripped reactor had NO leg ΔT, and 44 green probes never asked (2026-08-03, #315 — CLOSED, §6 included).** The
  hot/cold leg split was `delta_T_rated × power_pct/100 / max(flow, floor)` — and `power_pct` is
  **fission power alone**. Heat leaving the core through the legs needs a temperature rise across
  them; a scrammed core is still rejecting ~7 % of rated. Four things to know. **Measured** full
  stack, three plant-minutes after a manual trip with the pumps running: core removing **6.61 %
  of rated**, computed split **0.0 °F** — and at thirty minutes still 0.0. Loss of forced flow
  after the trip: **3.8 °F against the 44.4 °F** the removed heat implies. **The BOARD half is
  worse than the physics half**: with the true signal at exactly zero, instrument noise is all
  that remains, and indicated `thot − tcold` over 25 minutes put the **COLD leg above the HOT leg
  in 724 of 1500 samples (48.3 %)** — after the fix, 0 of 1500. **Why it survived is the part
  worth carrying**: fission and total heat are **equal by construction in steady state**, and
  every probe here measures at or near equilibrium, so 44 of them agreed with a formula wrong
  everywhere else — *a term that is an identity in the regime you test in is a term nothing
  tests*. And **the rigorous option was measured and NOT taken**: driving the split from the
  instantaneous fuel→coolant flux carries the fuel node's stored-energy dump (closer to a real
  trip) but brings a full load rejection to **within 1.6 % of an OTΔT trip** and reddens three
  runners — #315 §6, deliberately left open. The fix is a CONSISTENCY fix: `stepFuel` and the
  Tavg balance already ran on total heat, and at rated `_Q_total` is exactly 1.0, so at-power
  behaviour is byte-identical. `run_behavior` **44 → 45** (TR-7b). The issue's second consumer,
  the condenser backpressure load fraction, was measured and is **not** a defect — 0.23 kPa worst
  case against a 3.39 kPa display digit. **§6 closed the same day** *(OWNER RULING, 2026-08-03:
  "Do as you recommend.")*: WTSM 12.2 shows the ONLY dynamic compensation in either real
  equation is on **Tavg**, nothing compensates the measured ΔT, and the document reads loop ΔT
  as *"a measure of reactor power"* — so a ΔT carrying a fuel lag is a worse measure of the one
  thing it is for. Cost of the alternative, measured: OTΔT margin **18.4 % → 1.8 %** on a full
  load rejection, and NOT rescuable by a faster fuel node (`h_fc` 0.05 → 0.10 with
  `heat_gen_coeff` doubled → `run_otdt` 21/39 and a scram at 1 s).
...
- **A reactor trip could be switched off at full power, and 34 green runners never asked
  ...
  permissive**, and manual blocks were exempt from auto-reinstate. Measured at 2235 psi
  (15.41 MPa) / 100 % power: `lo_press`, `si_trip` and `lo_flow` all ACCEPTED, and a 20 %-of-max
  cold-leg LOCA rode **64 s of unscrammed blowdown** — scram at 68.1 s on `pzr_level high` at
  130 psi (0.90 MPa), against a baseline **4.2 s** on `primary_pressure low` at 1782 psi
  (12.28 MPa). Four things to know. **The measurement settles it; the CONTENT was only the
  canary** — and this bullet's first draft said "the authored content was right and the kernel was
  wrong", which is an HR9 inversion worth not copying. The checklists diverging from the kernel
  (*"the plant will not let you block them down there"*) is what pointed at the defect and is
  blast-radius evidence that the fix breaks nothing authored; it is **not** why the plant changed.
  **The prototypicality half is UNVERIFIED**: the "P-11 enabled only below ~1970 psig" basis came
  from the audit, which flags its own NUREG refs as part recall, and there is **no sourced P-11
  citation in this repo** — that gap does not touch the finding but does touch the *shape* of the
  fix. **The two fixes each heal F1 alone**,
  so a single-sided injection leaves the LOCA probe green: with auto-reinstate corrected, a block
  set outside its permissive is deleted on the very next `evaluate`. Revert BOTH to reproduce.
  **TWO of `run_m4`'s own checks were pinning the defect**, and `run_behavior` PI-3's *fixture*
  was the defect — it blocked `lo_press` straight from hot full power and labelled it "P-10
  satisfied", which is the **wrong permissive** (`lo_press` carries its own P-11). Both
  re-authored, and PI-3 passes on the OLD kernel too, so it is a better test rather than a refit.
  And **the board's System Scanner copy described the removed rule in two places** — the #308
  class again: behaviour text is a third copy of the mechanism and it does not move itself.
- **PWR-N15 is executable now, and building it found a step the written procedure was missing
  (2026-08-02, #310).** The Mode 3 → Mode 5 cooldown had no runnable checklist, so #303's
  *measured* performance table had nothing reproducing it. `pwr_cooldown` is that table's
  source now — the board walkthrough AND a full-stack replay (`run_procedures_stack` 23/23,
  **204 checks**). Four things to know. **"Block SI" is THREE actions and the procedure named
  one**: HPI/LPI OFF disarms the ESF arm and stops the pumps, but **two** entries in
  `PWR_TRIPS` watch `primary_pressure` downward — `lo_press` (12.41 MPa) and **`si_trip`**
  (PI-3, reactor trip on safety injection, 12.4 MPa) — and neither auto-blocks on the way
  DOWN. Measured, the plant scrams ~5 plant-minutes into the first leg with *either* left
  armed, and the resulting turbine trip drives the dump into Tavg-error mode (−306 °C/hr).
  Both blocks need the P-11 permissive, so the Pressure SP has to come down first.
  **A discrete walk-down MEASURES BADLY on this plant** — the issue recommended it and the
  numbers said no: the dump's band is 0.25 MPa against a 40 % capacity and the primary trails
  by τ ≈ 37 s, so a 10 °C Dump SP step bursts at **−649 °C/hr**, a 26 °C leg at −1210, and
  holding −50 °C/hr with steps needs ~250 of them. Steps can `ramp` now (a setpoint walked
  along an authored polyline across the `hold`). **That schema change cost the UI nothing,
  and the estimate that said otherwise was wrong**: the live checklist NEVER issues `cmd` —
  `ui/app.js renderChecklist` draws text + highlights and the instructor grades off `acc`,
  watching for the player's own command — so `cmd`/`hold`/`ramp` are replay-side only, two
  gates, no browser work. And **the guard is the only check that can tell a ramp from a
