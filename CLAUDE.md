...
- **The pressurizer heaters ran through a station blackout, and free play found it before any
  gate did (2026-08-03, #329).** Owner report. The heaters were an UNCONDITIONAL heat source —
  `autoControl` resolved a demand and `stepPressure` applied it through `K_heater`, with nothing
  asking whether the plant had power. Measured full stack: **100.0 % heater power 17 min into an
  SBO** with every AC bus dead, and 100 % from the button press if the operator asks, walking
  pressure to 2352 psi (16.22 MPa) and causing a **spurious `pzr_level low` reactor trip at
  5m27s** off phantom heat. Sourced to **10 CFR 50.2**: SBO is *"the complete loss of ac electric
  power to the essential and nonessential switchgear buses"*, excluding only *"buses fed by
  station batteries through inverters"* — the vital instrument AC, which is why the board keeps
  reading and a megawatt of resistance heating does not. Three things to carry. **Why 36 green
  runners missed it is the #315 shape**: the heaters are only *demanded* below setpoint and an
  SBO **repressurizes**, so a Mode 3 blackout A/Bs byte-identical across the fix — the controller
  never asks for a percent in an hour. **The fix is NOT `heater_override = 0`**, which repeats
  #200: `set_heater` writes that field directly, so a blackout parked in the operator's demand is
  wiped by the next press of HEATER AUTO. It is a physical de-energization; the selector and the
  latched demand stay put and only delivered power goes to zero. **A plain LOOP must KEEP the
  heaters** — NUREG-0737 II.E.3.1 puts the minimum group on the diesels, and
  `loss_of_offsite_power` never sets the flag — which is what CA-7 leg C exists for: swap the
  guard for the plausible `!s.pump_running` and legs A/B stay GREEN with only leg C red. **The
  SBO outcome is unchanged** (damage at 30 min either way): it is terminal for #325's
  natural-circulation reason, and this is a correctness and indication fix, not a save.
