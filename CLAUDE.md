...
- **The pressurizer surge had ONE driver where it needs two, so losing coolant moved no gauge but the
  level one (2026-08-04, #337).** Measured full stack: an SGTR that took pzr level **55.0 → 15.7 %** and
  scrammed the reactor moved pressure **5 psi (0.034 MPa)** and subcooling **0.2 °F (0.1 °C)**. Five
  things to know. **The filed diagnosis had the order backwards.** #337 named three causes and led with
  "letdown has no path to pressure"; measured, that is a *symptom* of the second one — wiring the mass
  driver in at the shipped gains moved the case by **9 psi**, because the heater term simply rebalanced
  against it. **The fix is a CURRENCY, not a new term**: a surge is a volume displacement and WTSM 3.2
  (ML11223A213 p. 3.2-8) describes insurge/outsurge without reference to what caused it, so the law is
  stated per unit **pressurizer level rate** and both drivers convert into it through the geometry
  `stepLevel` already carries. `K_surge` (°C/s) → `K_surge_level` (%/s) at the identical value —
  1.0/`level_per_tavg` = 0.4 — so **the thermal channel is bit-identical** and only the mass driver is new.
  **The heater is 27× its own source and is DELIBERATELY still wrong**: WTSM 3.2 p. 3.2-9 rates 1794 kW
  as *"capable of raising the temperature of the pressurizer and its contents at approximately 55 °F/hr"*
  = 0.23 psi/s, ×12.6 for this plant's declared Mode 5↔1 time compression = **0.020 MPa/s — which lands
  exactly on `setpoint_pressurize_slew_mpa_s`**, i.e. the config states one physical quantity twice and
  the two disagreed by 27×. Not corrected here because it is ruled identity, measured: at 0.05 the plant
  can no longer ride out a full load rejection (TR-1h, OTΔT — the #311 trap from the other side) and
  below 0.20 a stuck-open spray valve runs it to 15 psi (TR-11). Flag **F14**, declared at `Manuals/12`
  §12.15. **It runs BOTH ways, and that is what broke PI-3**: its leg 2 walked pressure down through the
  SI setpoint only because injection could not push back; now unthrottled SI arrests the fall at
  12.47 MPa and takes the plant **solid**. And **the `range()` trap made a BRAND-NEW check hollow** —
  CA-9's added subcooling check first measured the loss against `range().max`, which spans the whole run
  on an event that *recovers above where it started* (86 °F on ECCS), scoring **−15.1 °F on the OLD
  engine** where the true loss is −2.05 °F. It passed against the very plant it exists to exclude.
  **BEFORE YOU ADD A GENERAL LAW, GREP FOR THE PER-PATH CONSTANTS THAT WERE STANDING IN FOR IT** —
  this cost most of the session. `K_surge_level` × `level_per_mass` = **310**, and
  `K_porv_relief`/`K_safety_relief` were **300**: those two constants *were* the mass→pressure
  coupling, fitted per path, so the new law carried a relief valve's authority twice and the TMI-2
  flagship blew down to **69 psi** where the canon recovers. Relief is excluded from the surge
  ...
  bulk coolant is doing. **The sourced criterion did NOT solve the gain** (WTSM 3.2 p. 3.2-11 sizes
  the PORVs to hold pressure under the high-pressure trip through a 50 % load decrease — satisfied
  at *every* value 300–1200, because the setpoint and the trip are 0.24 MPa apart), so it was
  solved against the relief ladder's own suites: `run_meltdown` and `run_scenarios` are red at
  300–450 and green at 500–600, and 600 reproduces the calibrated total.
  **NOT FINISHED — four runners red on the lane.** `run_behavior` **49/50** — TR-15 leg E, *"with
  the heat sink gone the plant is still lost"*, now fails at EVERY relief gain 400–600 (Tavg 482 /
