...
- **#386 stages 2–3** — containment ESF (spray, fan coolers, the sourced 3.5/30 psig actuations,
  ...
  receiving volume and nothing more. **#384 sits on top of all three** and is its own coupled plan.
- **#349** — pressurizer relief capacities are safeties:PORV **28.6×** against a sourced 3.0×.
  Latent: nothing reaches the code safeties in a real transient.
...
- **A declared simplification was load-bearing, and the scenario that found it had the accident's
  causal order backwards** (2026-08-04, #347). Spray's authority in a water-solid plant was
  declared an accepted simplification; measured on the one path #346 never exercised, it held
  pressure 164 psi BELOW the code-safety setpoint so nothing arrested the fill. **Declare a
  simplification only after measuring the regime it lives in.** And the TMI-2 beats armed
  `subcoolAlarm` ahead of `hpiAuto` — backwards: full injection beats one stuck-open valve, which
  is exactly why the 1979 crew securing it caused the accident.
...
`ops_cvcs_pzr_drain_rate` (53.7 s against `>= 300 s`) and it is a **RULED, ACCEPTED state, not a
regression** *(OWNER RULING, 2026-08-04: "A")*: #330 corrected `level_per_mass` 100 → 776 and the
acceptance is a direct product of that constant. **It must NOT be re-banded** — the probe exists
