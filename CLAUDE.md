...
- **A re-export changes PIPE ids.** Pipe ids are regenerated whenever a run is re-drawn, so
  ...
  board change** — it is not in `run_all`. Currently **149/149** (measured 2026-08-01 on `develop` after #289. **The line before this one claimed 143/143 and the harness was actually at 1 FAILURE / 143** — nobody had run it, which is the standing argument for running it rather than trusting this number. Both reds were HARNESS bugs, not plant defects: the TRIP BLOCKS check unblocked `ir_high` **at full power**, where the IR channel reads 2.0e-3 against a 1.67e-3 setpoint, so the trip condition is STANDING and the block is all that holds it off — the plant scrammed at once and every check below ran on a dead reactor; and `rps.scrammed` **LATCHES**, so re-blocking cannot undo it. The SCRAM two-step then ran on that already-scrammed plant, where its two clicks are the #75 RESET half, so it un-scrammed and asserted a scram. Two traps if you touch that area: `reset_rps` is refused **RODS_NOT_INSERTED** until the rods seat (measured, still coasting at 95 % power a few ticks after the scram), and the dual-mode SCRAM/RESET button reads which half it is off the **RENDERED** snapshot — re-render after a reset or the clicks land on the wrong half — this line said 95/95 once, which was already stale when
