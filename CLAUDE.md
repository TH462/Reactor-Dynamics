...
- **A re-export changes PIPE ids.** Pipe ids are regenerated whenever a run is re-drawn, so
  ...
  board change** — it is not in `run_all`. Currently **162 checks, 1 FAILURE** (measured 2026-08-01 on `workbench` after #238. **The one red is NOT this change**: "func: SCRAM two-step trips the reactor", a HARNESS bug that is already fixed in `develop`'s uncommitted #289 work — so expect a conflict on this line and on `board_check.html` at the merge, and re-measure rather than adding the two numbers. #238 took it 143 → 162: **+18 display-unit checks**, all injection-verified against seven faults, plus one on the band nesting that the other seventeen missed. Three traps live in that block, all of them mistakes made once: assembling a FOREIGN snapshot walks the driver's one-entry damping memo (`IN()` caches on snapshot identity), so a check that builds its own service must run LAST or it perturbs the tile values another check is about to compare; moving it last silently put it in **US mode**, where the thing it tests is correct, so for one run it passed while measuring nothing; and the harness's ctx passed `unit: 'si'`, a string nothing has ever read, so it claimed to test SI and rendered US for its whole life — the real accessor is `units()`. Earlier: this line said 95/95 once, which was already stale when
