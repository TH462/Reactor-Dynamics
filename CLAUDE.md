...
**Where the PWR is.** `run_all` is **39 runners, ALL AT BASELINE** (38 → 39 on
2026-08-06-workbench-g, #395/#396: `run_procedures_chain` — the CONTINUOUS operating day,
which both reloading procedure gates are blind to by construction. Heatup → the PWR-N02
step-15 dilution → startup on ONE plant, ending critical at Mode 1 with zero refusals;
procedures now carry machine-checkable `precond` rows the Instructor grades live on any
checklist, warn-never-block. The six Tier B evolutions are authored; `run_checklist`
24 → 38.) —
...
> `test/run_all.js`, not as prose here. Run it; it compares all 39 runners against that
...
(`.github/workflows/gates.yml`, ~8 min) — all 39 runners, browser gates included; it
...
10× it declares, so their steps got a tenth of their sim time (#245). 22/22 176 → 23/23 204 on 2026-08-02, #310 — the PWR-N15 `pwr_cooldown` checklist, 28 checks, and the first procedure to use **RAMP steps**: a setpoint walked along an authored polyline across the step's `hold` rather than typed once. That schema addition is REPLAY-SIDE ONLY because **the live checklist never issues `cmd`** — `ui/app.js renderChecklist` draws text + highlights and the instructor grades off `acc` — so it costs the UI and the browser gate nothing. It exists because a discrete walk-down MEASURES BADLY here: the dump's band is 36 psi (0.25 MPa) against a 40 % capacity and the primary trails by τ ≈ 37 s, so an 18 °F (10 °C) Dump SP step bursts at **−1168.2 °F/hr (−649 °C/hr)** and a whole 46.8 °F (26 °C) leg at −2178 °F/hr. Seven injections redden it; the one to remember is that flattening the four legs back into single steps leaves **27/28** — every acceptance still passes, because a staircase ARRIVES everywhere the procedure says, and the `tavg_rate_c_per_hr` guard is the only check that can tell the two apart)**, `run_procedures_chain` **50/50** (NEW 2026-08-06, #395/#396 — the continuous day on ONE service: heatup arrives Mode 3 at 856.8 ppm, the #395 seam probe flags exactly the startup's boron precondition row, the PWR-N02 step-15 dilution lands in 55.6 plant-min, and the startup then goes critical to Mode 1 at 10.75 % with both trip blocks ACCEPTED — the two refusals #396 measured are zero. Injection-verified both ways: dilution skipped reproduces #396 exactly, 15 red; neutered precondition evaluation, 5 red. Its replay machinery is `test/procedures_harness.js`, extracted from the stack runner — whose unchanged score is the neutrality assertion), `run_checklist` **38/38** (24 → 38 on 2026-08-06, #395 — the precondition mechanism on a synthetic procedure, 7 red under a neutered evaluation, plus the content section: `pwr_startup`'s #396 boron seam row reads UNMET at cold_shutdown's own 857 ppm while all 16 authored Tier B rows measured MET on their six `from:` ICs), `run_scenarios`
