...
- **On release (merge `develop` → `main`)** → **the NEXT release is the LAUNCH release:
  `Pre Alpha` → `Alpha 1.0.0`, and the update-tracking page comes back** *(OWNER DIRECTIVE,
  2026-08-04: "The next release will take the program out of pre-Alpha and into Alpha and
  bring back the update tracking page. Update tracking summaries/lists should be concise.")*.
  This **lifts** the 2026-07-31 no-version-bumps suspension. Procedure: *Website changelog &
  version numbers* below; the launch-only extras (manual set to **Rev 0**, tags) are **#282**
  — read that issue, it is the checklist. `run_release` goes **8 → 11**: setting the version
  arms three more checks by itself, so update `BASELINES` and *Project status* with it.
  **The bump and the first entry MUST land in ONE change.** `run_release` is in pre-release
  mode, where "no published entries" is the *correct* state — so a `changelog.html` entry
  added while `RD_RELEASE` still reads `Pre Alpha` is a **red gate**, and so is the reverse.
  Every release AFTER launch is the ordinary Platform.Feature.Refinement bump.
...
> **LIVE AGAIN — and the next release is the LAUNCH release** *(OWNER DIRECTIVE, 2026-08-04:
> "The next release will take the program out of pre-Alpha and into Alpha and bring back the
> update tracking page. Update tracking summaries/lists should be concise.")*, superseding the
> 2026-07-31 suspension of this subsection. That release is **`Alpha 1.0.0`** — one version for
> everything accumulated since `Pre Alpha`, not a replay of the skipped bumps — and its
> launch-only extras (manual set to Rev 0, `BASELINES`, tags) are **#282**. The digit rules
> below choose every version *after* it.
