...
> | `C:\grok_build\RD_workbench` | `workbench` | overflow lane 1, for when a second agent is already on `develop` — **also an AUDIT LANE** |
> | `C:\grok_build\RD_backshop` | `backshop` | overflow lane 2, same rules as workbench (third concurrent agent) — **also an AUDIT LANE** |
>
> **THE TWO OVERFLOW LANES DO NOT AUTO-LOAD THIS FILE** *(OWNER RULING, 2026-08-05, #383: "Let's do
> it with the files not the skills.")*. Each carries a gitignored `.claude/settings.local.json` with
> the #221 exclusions (`claudeMdExcludes` for all three trees, `autoMemoryEnabled: false`). It
> **layers by default and needs no flag**, so any session started in either tree is unprimed and a
> `/clear` is enough to begin an audit slice — which is the point. `develop` was reverted and loads
> normally.
>
> **The accepted cost: ordinary non-audit work in those two lanes also runs without this file** — no
> lane rules, no merge-conflict list, no gate baselines, no standing traps. Recorded here because the
> only prior record was a comment *inside* a gitignored file, in the one tree that had already been
> reverted. Working in workbench or backshop and need this file? **Read it explicitly** — it is on
> disk, only the auto-load is suppressed. To give a lane its orientation back, delete that tree's
