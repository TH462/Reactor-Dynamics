...
**Where the PWR is.** `run_all` is **38 runners**, and **six are below baseline on the `backshop`
lane** — one HR9 cascade running through two changes, not six unrelated defects. `run_behavior`
50/52, `run_procedures` 28/29 and `run_procedures_stack` 27/29 are the **#337** tail (the
pressurizer surge gained an inventory driver and the content authored against the old plant is
stale). `run_scenarios` 1/3 and `run_campaign` 42/51 are **#347**, the same shape one level deeper:
#346 gave the plant a water-solid regime, and the TMI-2 flagship's decision point turns out to have
been resting on the discard #346 removed. `run_hardrules` 177 is bookkeeping, not a defect — see
its entry below. Everything else is at baseline, `run_pwr` 36/36 and `run_meltdown` 12/12 included.
