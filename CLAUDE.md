...
- **The pressurizer surge had ONE driver where it needs two, so losing coolant moved no gauge but the
  ...
  **BEFORE YOU ADD A GENERAL LAW, GREP FOR THE PER-PATH CONSTANTS THAT WERE STANDING IN FOR IT** —
  ...
  `K_porv_relief`/`K_safety_relief` are **300**: those two constants *were* the mass→pressure
  coupling, fitted per path, so the new law carries a relief valve's authority TWICE. The TMI-2
  flagship blows down to **69 psi** where the canon recovers. **The obvious correction is measured
  WORSE** — excluding relief from the surge (the valves vent the pressurizer STEAM space, so that
  mass never crosses the surge line, exactly as `pwr_config`'s own comment has always said) reddens
  `run_meltdown` 12 → 11 and `run_scenarios` 3/3 → 1/3, i.e. it breaks PHYSICS acceptance where the
  double count breaks only authored CONTENT, and **HR9 ranks those the other way round**. All four
  combinations measured (table in the TUNING_LOG entry); the double count SHIPS, as flag **F15**.
  **NOT FINISHED — three runners red on the lane.** `run_procedures`/`run_procedures_stack`
