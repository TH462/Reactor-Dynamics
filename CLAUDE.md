...
- **The plant had no concept of being WATER-SOLID, so it threw the ECCS overfill away instead
  (2026-08-04, #346).** Measured full stack: a lost heat sink with injection running pinned inventory at
  exactly **120.00 %** — `primary.mass_max` — for 45 minutes, pressure **flat at 2232 psi (15.39 MPa)**,
  no PORV lift, cold RWST water quenching the plant 660 → 447 °F through a mass sink with no outlet.
  Six things to know. **RAISING THE CEILING IS NOT THE FIX, and measuring that first is what produced
  the real one**: at `mass_max` 3.0 the plant runs to **300 % inventory** with pressure still parked in
  the PORV band. The clip was hiding the gap, not causing it — the surplus level slope IS the
  **steam-space** slope (#249 derives it from the 720 ft³ bubble) and it was still in force with the
  bubble gone. **The bubble is the compressibility**, so when the level line reaches 100 % the gain
  steps to the **bulk modulus of water**: `solid_bulk_mpa` 1300 MPa/frac, a physical constant rather
  than a fit, converted into the shared level currency at the point of use so neither it nor
  `level_per_mass_surplus` has to be re-solved when the other moves. **The internal check beats the
  number**: the same argument on the STEAM side must reproduce the shipped `K_surge_level`, and
  isothermal bubble compression on #249's own geometry gives **206 MPa/frac** against the shipped
  **310** — so the ratio the plant feels, solid ≈ 4× stiffer, is not an artifact of picking one of the
  two. **F15's premise fails at solid and taking only that third of it is WORSE THAN TAKING NONE.**
  Relief is excluded from the surge because the valves "release steam from the steam space"; with no
  steam space they pass liquid. That variant was BUILT and MEASURED: it drops the relieving equilibrium
  ~145 psi, which un-deadheads the ECCS, injection out-runs the PORV, and inventory walks **back to the
  120.00 % clip** — the defect returning by another road. The same argument applies to **spray**
  (nothing to condense) and the **heaters** (no bubble to flash), so the honest version is a coupled
  three-term regime plus a re-solve of `K_porv_relief`/`K_safety_relief`; declared at `Manuals/12`
  **§12.4c**, not attempted. **"SOLID" IS NOT `pzr_level_pct >= 100`** — the void term pegs the SAME
  gauge at 100 % on a boiling, half-empty core, which is the TMI deception and the exact opposite of
  solid, and this plant transits it on the way here with the PORV at 55 % duty. CA-12's first draft
  gated on level alone and **every leg-A check passed against the very engine it exists to exclude**;
  it needs level-at-top AND overfilled AND no void. Its second draft then measured a window still
  inside the ECCS refill, which swings pressure 161 psi on **both** plants. **And the cost is the
  plant's signature scenario, #347**: `run_scenarios` 3/3 → **1/3**, `run_campaign` 48/51 → **42/51**,
  all TMI-2. The flagship's decision point was resting on this same discard — with `_mass` pinned,
  `_dmass_dt` is identically zero, so `K_porv_relief · porv_flow` ran unopposed to **52 psi (0.36 MPa)
  while the inventory gauge read 120 %**, two bugs agreeing. Unthrottled injection now matches a
  stuck-open PORV, the plant goes solid at pressure, `subcooling_low` never fires — **which is the
  TMI-2 counterfactual and is correct.** What the scenario never modelled is the crew's 1979
  throttle-back, and `pwr_tmi2_p3` was already 2 missions red from #337.
