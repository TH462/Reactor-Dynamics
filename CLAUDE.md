...
reds are 6 RBMK + 4 BWR.** The single PWR red is NEW at #330 and is an **OPEN OWNER
DECISION, not an accepted regression**: `ops_cvcs_pzr_drain_rate` reads **53.7 s** for its
15-point pressurizer drop against a `>= 300 s` acceptance, because that acceptance is a
direct product of `level_per_mass` (`0.030 · gain · level_per_mass`) and #330 corrected that
constant 100 → 776. It is **deliberately NOT re-banded** — the probe exists because of a
2026-07-22 owner request for a drain-rate feel target, and re-banding a target whenever the
plant moves retires it instead of reporting against it. Both options are measured in the
probe's own comment (`test/ops_pwr.js`): ship as-is and the drain is 7.76× faster than the
target with `run_e2e_controls` at 59/59, or scale `cvcs_inventory_gain` to 0.00154639 to
restore the rate exactly (loop τ and the droop equilibrium both preserved, implied RCS volume
1,389 → 10,779 gal against a real ~68,000) and `run_e2e_controls` falls to **52/59** as leaks
CVCS used to hold stop being held. A real plant takes ~79 min for this drop, so both sim
values are far from prototypical — the choice is between two game-feel numbers.
