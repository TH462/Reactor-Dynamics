...
so the planned `saturated` predicate change was not shipped — `run_behavior` 63. Remaining:
stage 4 (#384 floors — `K_break_vent` + path-scoped sat-pull, CA-15 re-author; the drain-phase
Psat slaving is now the whole of #384), stage 5 (#407 core-exit subcooling + the NUREG-0737
fetch, blocked all session on nrc.gov 403 + Wayback 503), stage 6 (#334 slider memo + the
low-Δp break-law question). Stage list: TUNING_LOG develop-f.
