...
| **Run an independent audit slice (#221)** | **`tools\audit.cmd <slice>`** (Git Bash: `sh tools/audit.sh <slice>`) — the ONE launch path, #382. It preflights and **refuses** if the session would not be independent, then starts a session on `Blueprint/AUDIT_CHARTER.md` with **this file** and the auto-memory index excluded (`.claude/settings.audit.json`) — #221 RoE 1's priming problem. Procedure saved as the `audit-slice` skill. **If you are reading THIS file, you are primed and cannot be the auditor** — print the launch line and stop; do not read the slice's code "to help". Preflight proves the config, not the session: the auditor's first turn must state on the slice issue whether CLAUDE.md was auto-loaded. |
